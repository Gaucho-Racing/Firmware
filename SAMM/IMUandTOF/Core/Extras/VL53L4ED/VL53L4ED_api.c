...
VL53L4ED_Error VL53L4ED_GetSWVersion(VL53L4ED_Version_t *p_Version)
{
...
VL53L4ED_Error VL53L4ED_SetI2CAddress(Dev_t dev, uint8_t new_address)
{
	VL53L4ED_Error status = VL53L4ED_ERROR_NONE;

	status |= VL53L4ED_WrByte(dev, VL53L4ED_I2C_SLAVE__DEVICE_ADDRESS, (uint8_t)(new_address >> (uint8_t)1));
	return status;
}

VL53L4ED_Error VL53L4ED_GetSensorId(Dev_t dev, uint16_t *p_id)
{
	VL53L4ED_Error status = VL53L4ED_ERROR_NONE;
...
VL53L4ED_Error VL53L4ED_SensorInit(Dev_t dev)
{
	VL53L4ED_Error status = VL53L4ED_ERROR_NONE;
	uint8_t Addr, tmp;
	...
	do {
		status |= VL53L4ED_RdByte(dev, VL53L4ED_FIRMWARE__SYSTEM_STATUS, &tmp);
	...
	for (Addr = (uint8_t)0x2D; Addr <= (uint8_t)0x87; Addr++) {
		status |= VL53L4ED_WrByte(dev, Addr, VL53L4ED_DEFAULT_CONFIGURATION[Addr - (uint8_t)0x2D]);
	}
	...
	status |= VL53L4ED_WrByte(dev, VL53L4ED_VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND, (uint8_t)0x09);
...
VL53L4ED_Error VL53L4ED_ClearInterrupt(Dev_t dev)
{
	VL53L4ED_Error status = VL53L4ED_ERROR_NONE;
...
VL53L4ED_Error VL53L4ED_StartRanging(Dev_t dev)
{
	VL53L4ED_Error status = VL53L4ED_ERROR_NONE;
	uint32_t tmp;
...
VL53L4ED_Error VL53L4ED_StopRanging(Dev_t dev)
{
	VL53L4ED_Error status = VL53L4ED_ERROR_NONE;
...
VL53L4ED_Error VL53L4ED_CheckForDataReady(Dev_t dev, uint8_t *p_is_data_ready)
{
	VL53L4ED_Error status = VL53L4ED_ERROR_NONE;
	uint8_t temp;
	uint8_t int_pol;
...
VL53L4ED_Error VL53L4ED_SetRangeTiming(Dev_t dev, uint32_t timing_budget_ms, uint32_t inter_measurement_ms)
{
	VL53L4ED_Error status = VL53L4ED_ERROR_NONE;
	...
	/* Sensor runs in autonomous low power mode */
		...
		status |= VL53L4ED_RdWord(dev, VL53L4ED_RESULT__OSC_CALIBRATE_VAL, &clock_pll);
		...
		status |= VL53L4ED_WrDWord(dev, VL53L4ED_INTERMEASUREMENT_MS, (uint32_t)inter_measurement_factor);
...
VL53L4ED_Error VL53L4ED_GetRangeTiming(Dev_t dev, uint32_t *p_timing_budget_ms, uint32_t *p_inter_measurement_ms)
{
	VL53L4ED_Error status = VL53L4ED_ERROR_NONE;
...
VL53L4ED_Error VL53L4ED_GetResult(Dev_t dev, VL53L4ED_ResultsData_t *p_result)
{
	VL53L4ED_Error status = VL53L4ED_ERROR_NONE;
	uint16_t temp_16;
	uint8_t temp_8;
	...
	status |= VL53L4ED_RdByte(dev, VL53L4ED_RESULT__RANGE_STATUS, &temp_8);
	...
	status |= VL53L4ED_RdWord(dev, VL53L4ED_RESULT__SPAD_NB, &temp_16);
	p_result->number_of_spad = temp_16 / (uint16_t)256;

	status |= VL53L4ED_RdWord(dev, VL53L4ED_RESULT__SIGNAL_RATE, &temp_16);
	p_result->signal_rate_kcps = temp_16 * (uint16_t)8;

	status |= VL53L4ED_RdWord(dev, VL53L4ED_RESULT__AMBIENT_RATE, &temp_16);
	p_result->ambient_rate_kcps = temp_16 * (uint16_t)8;

	status |= VL53L4ED_RdWord(dev, VL53L4ED_RESULT__SIGMA, &temp_16);
	p_result->sigma_mm = temp_16 / (uint16_t)4;

	status |= VL53L4ED_RdWord(dev, VL53L4ED_RESULT__DISTANCE, &temp_16);
...
VL53L4ED_Error VL53L4ED_GetOffset(Dev_t dev, int16_t *p_offset)
{
	VL53L4ED_Error status = VL53L4ED_ERROR_NONE;
	uint16_t temp;
...
VL53L4ED_Error VL53L4ED_SetXtalk(Dev_t dev, uint16_t XtalkValueKcps)
{
	...
	status |= VL53L4ED_WrWord(dev, VL53L4ED_XTALK_X_PLANE_GRADIENT_KCPS, 0x0000);
	status |= VL53L4ED_WrWord(dev, VL53L4ED_XTALK_Y_PLANE_GRADIENT_KCPS, 0x0000);
	status |= VL53L4ED_WrWord(dev, VL53L4ED_XTALK_PLANE_OFFSET_KCPS, (XtalkValueKcps << 9));
...
VL53L4ED_Error VL53L4ED_GetXtalk(Dev_t dev, uint16_t *p_xtalk_kcps)
{
	...
	status |= VL53L4ED_RdWord(dev, VL53L4ED_XTALK_PLANE_OFFSET_KCPS, p_xtalk_kcps);
...
VL53L4ED_Error VL53L4ED_SetDetectionThresholds(Dev_t dev, uint16_t distance_low_mm, uint16_t distance_high_mm, uint8_t window)
{
	VL53L4ED_Error status = VL53L4ED_ERROR_NONE;
...
VL53L4ED_Error VL53L4ED_GetDetectionThresholds(Dev_t dev, uint16_t *p_distance_low_mm, uint16_t *p_distance_high_mm, uint8_t *p_window)
{
	VL53L4ED_Error status = VL53L4ED_ERROR_NONE;
...
VL53L4ED_Error VL53L4ED_SetSignalThreshold(Dev_t dev, uint16_t signal_kcps)
{
	...
	status |= VL53L4ED_WrWord(dev, VL53L4ED_MIN_COUNT_RATE_RTN_LIMIT_MCPS, signal_kcps >> 3);
	return status;
}

VL53L4ED_Error VL53L4ED_GetSignalThreshold(Dev_t dev, uint16_t *p_signal_kcps)
{
	VL53L4ED_Error status = VL53L4ED_ERROR_NONE;
	uint16_t tmp = 0;

	status |= VL53L4ED_RdWord(dev, VL53L4ED_MIN_COUNT_RATE_RTN_LIMIT_MCPS, &tmp);
	*p_signal_kcps = tmp << 3;

	return status;
}

VL53L4ED_Error VL53L4ED_SetSigmaThreshold(Dev_t dev, uint16_t sigma_mm)
{
	VL53L4ED_Error status = VL53L4ED_ERROR_NONE;
	...
	if (sigma_mm > (uint16_t)((uint16_t)0xFFFF >> 2)) {
		...
		status |= VL53L4ED_WrWord(dev, VL53L4ED_RANGE_CONFIG__SIGMA_THRESH, sigma_mm << 2);
	}

	return status;
}

VL53L4ED_Error VL53L4ED_GetSigmaThreshold(Dev_t dev, uint16_t *p_sigma_mm)
{
	VL53L4ED_Error status = VL53L4ED_ERROR_NONE;

	status += VL53L4ED_RdWord(dev, VL53L4ED_RANGE_CONFIG__SIGMA_THRESH, p_sigma_mm);
...
VL53L4ED_Error VL53L4ED_StartTemperatureUpdate(Dev_t dev)
{
	...
	status |= VL53L4ED_WrByte(dev, VL53L4ED_VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND, (uint8_t)0x81);
	...
	status += VL53L4ED_WrByte(dev, VL53L4ED_VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND, 0x09);
