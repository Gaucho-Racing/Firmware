...
VL53L4ED_Error VL53L4ED_CalibrateOffset(Dev_t dev, int16_t TargetDistInMm, int16_t *p_measured_offset_mm, int16_t nb_samples)
{
	...
	if (((nb_samples < (int16_t)5) || (nb_samples > (int16_t)255)) || ((TargetDistInMm < (int16_t)10) || (TargetDistInMm > (int16_t)1000))) {
...
VL53L4ED_Error VL53L4ED_CalibrateXtalk(Dev_t dev, int16_t TargetDistInMm, uint16_t *p_measured_xtalk_kcps, int16_t nb_samples)
{
	...
	*p_measured_xtalk_kcps = 0;
	if (((nb_samples < (int16_t)5) || (nb_samples > (int16_t)255)) || ((TargetDistInMm < (int16_t)10) || (TargetDistInMm > (int16_t)5000))) {
		status |= (uint8_t)VL53L4ED_ERROR_INVALID_ARGUMENT;
	} else {
		/* Disable Xtalk compensation */
		status |= VL53L4ED_WrWord(dev, VL53L4ED_XTALK_PLANE_OFFSET_KCPS, *p_measured_xtalk_kcps);
		...
		if (CounterNbSamples == 0) {
			...
			if (tmp_xtalk > (uint16_t)127) {
				...
				*p_measured_xtalk_kcps = (uint16_t)(round(tmp_xtalk));
				...
				status |= VL53L4ED_WrWord(dev, VL53L4ED_XTALK_PLANE_OFFSET_KCPS, calXtalk);
			}
		}
	}
