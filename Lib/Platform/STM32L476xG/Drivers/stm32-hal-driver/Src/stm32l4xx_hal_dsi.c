			       ...
			       DSI_PLLInitTypeDef *PLLInit)
{
	...
	unitIntervalx4 =
	    (4000000U * tempIDF * ((1UL << (0x3U & PLLInit->PLLODF)))) /
...
HAL_StatusTypeDef HAL_DSI_EnterULPMData(DSI_HandleTypeDef *hdsi)
{
	...
	if ((hdsi->Instance->WRPCR & DSI_WRPCR_PLLEN) != DSI_WRPCR_PLLEN) {
		...
		__HAL_UNLOCK(hdsi);
		return HAL_ERROR;
	} else if ((hdsi->Instance->WRPCR & DSI_WRPCR_REGEN) !=
		   DSI_WRPCR_REGEN) {
		/* Process Unlocked */
		__HAL_UNLOCK(hdsi);
		return HAL_ERROR;
	...
	switch (Timing) {
		case DSI_TCLK_POST:
			...
			if (State != DISABLE) {
				...
				hdsi->Instance->WPCR[4U] |=
				    Value & DSI_WPCR4_TCLKPOST;
			}
		...
		case DSI_TLPX_CLK:
			...
			if (State != DISABLE) {
				...
				hdsi->Instance->WPCR[3U] |=
				    (Value << 24U) & DSI_WPCR3_TLPXC;
			}
		...
		case DSI_THS_TRAIL:
			...
			if (State != DISABLE) {
				...
				hdsi->Instance->WPCR[2U] |=
				    (Value << 24U) & DSI_WPCR2_THSTRAIL;
			}
