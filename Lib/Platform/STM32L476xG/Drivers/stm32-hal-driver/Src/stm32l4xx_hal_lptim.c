 ...
 * Callback ID (when available)
 *          @arg @ref HAL_LPTIM_REP_COUNTER_WRITE_CB_ID Repetition counter
 * register write complete Callback ID (when available)
...
void LPTIM_Disable(LPTIM_HandleTypeDef *hlptim)
{
...
#endif
	{
		...
		if (tmpCMP != 0UL) {
			 ...
			 * LPTIM_CMP register */
			if (LPTIM_WaitForFlag(hlptim, LPTIM_FLAG_CMPOK) ==
			    HAL_TIMEOUT) {
		...
		if (tmpARR != 0UL) {
			...
			/* Wait for the completion of the write operation to the
			 * LPTIM_ARR register */
			if (LPTIM_WaitForFlag(hlptim, LPTIM_FLAG_ARROK) ==
			    HAL_TIMEOUT) {
				hlptim->State = HAL_LPTIM_STATE_TIMEOUT;
			}
		...
		if (tmpRCR != 0UL) {
			...
			/* Wait for the completion of the write operation to the
			 * LPTIM_RCR register */
			if (LPTIM_WaitForFlag(hlptim, LPTIM_FLAG_REPOK) ==
			    HAL_TIMEOUT) {
				hlptim->State = HAL_LPTIM_STATE_TIMEOUT;
			}
			__HAL_LPTIM_CLEAR_FLAG(hlptim, LPTIM_FLAG_REPOK);
