/**
 ...
 complete interrupt on the DMA Tx/Rx channels.
 ...
 before calling HAL_SWPMI_DeInit or HAL_SWPMI_Init function.
    [..]
...
SWPMI_WaitOnFlagSetUntilTimeout(SWPMI_HandleTypeDef *hswpmi, uint32_t Flag,
...
HAL_StatusTypeDef HAL_SWPMI_Init(SWPMI_HandleTypeDef *hswpmi)
{
	...
	if (hswpmi == NULL) {
		...
		MODIFY_REG(hswpmi->Instance->OR, SWPMI_OR_CLASS,
		...
		if (hswpmi->Init.VoltageClass == SWPMI_VOLTAGE_CLASS_B) {
			...
			wait_loop_index =
			    (300U * (SystemCoreClock / (1000000U * 4U))) + 150U;
...
__weak void HAL_SWPMI_MspInit(SWPMI_HandleTypeDef *hswpmi)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SWPMI_MspInit can be implemented in the user file
	 */
}
...
__weak void HAL_SWPMI_MspDeInit(SWPMI_HandleTypeDef *hswpmi)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SWPMI_MspDeInit can be implemented in the user file
	 */
}
 ...
 */
HAL_StatusTypeDef
HAL_SWPMI_RegisterCallback(SWPMI_HandleTypeDef *hswpmi,
 ...
 */
HAL_StatusTypeDef
HAL_SWPMI_UnRegisterCallback(SWPMI_HandleTypeDef *hswpmi,
			     HAL_SWPMI_CallbackIDTypeDef CallbackID)
{
	HAL_StatusTypeDef status = HAL_OK;
...
Interrupt mode or the selected DMA channel interrupt handler when using DMA
mode. The HAL_SWPMI_TxCpltCallback(), HAL_SWPMI_RxCpltCallback() user callbacks
...
HAL_StatusTypeDef HAL_SWPMI_Transmit(SWPMI_HandleTypeDef *hswpmi,
				     const uint32_t *pData, uint16_t Size,
				     uint32_t Timeout)
{
		...
		if ((tmp_state == HAL_SWPMI_STATE_READY) ||
			...
			if (tmp_state == HAL_SWPMI_STATE_READY) {
				...
				__HAL_SWPMI_DISABLE_IT(
				    hswpmi, SWPMI_IT_TCIE | SWPMI_IT_TIE |
						SWPMI_IT_TXUNRIE |
						SWPMI_IT_TXBEIE);
			...
			do {
				...
				if (HAL_IS_BIT_SET(hswpmi->Instance->ISR,
						   SWPMI_FLAG_TXE)) {
			...
			if (SWPMI_WaitOnFlagSetUntilTimeout(
				hswpmi, SWPMI_FLAG_TXBEF, tickstart, Timeout) !=
			    HAL_OK) {
				/* Timeout occurred */
				hswpmi->ErrorCode |=
	...
	if ((pData == NULL) || (Size == 0U)) {
		...
		if ((tmp_state == HAL_SWPMI_STATE_READY) ||
			...
			if (HAL_DMA_Start_IT(hswpmi->hdmatx,
					     ...
					     Size) != HAL_OK) {
				hswpmi->State =
				    tmp_state; /* Back to previous state */
				...
				/* Enable the DMA transfer for transmit request
				   by setting the TXDMA bit in the SWPMI CR
				   register */
					...
					uint32_t *pData, uint16_t Size)
{
	...
	if ((pData == NULL) || (Size == 0U)) {
		...
		if ((tmp_state == HAL_SWPMI_STATE_READY) ||
			...
			/* Set the SWPMI DMA Half transfer complete callback */
			hswpmi->hdmarx->XferHalfCpltCallback =
			    SWPMI_DMARxHalfCplt;
			...
			if (HAL_DMA_Start_IT(hswpmi->hdmarx,
				...
				/* Enable the SWPMI receive CRC Error and
				...
				/* Enable the DMA transfer for the receiver
				   request by setting the RXDMA bit in the SWPMI
				   CR register */
...
HAL_StatusTypeDef HAL_SWPMI_DisableLoopback(SWPMI_HandleTypeDef *hswpmi)
{
	...
	/* Make sure the SWPMI interface is not enabled to reset the loopback
	 * mode */
	CLEAR_BIT(hswpmi->Instance->CR, SWPMI_CR_SWPACT);
...
void HAL_SWPMI_IRQHandler(SWPMI_HandleTypeDef *hswpmi)
{
	...
	if (((regisr & SWPMI_FLAG_RXBERF) != 0U) &&
		...
		CLEAR_BIT(hswpmi->Instance->IER,
			  SWPMI_IT_RXBERIE | SWPMI_IT_RXBFIE);
	...
	if (errcode != HAL_SWPMI_ERROR_NONE) {
		...
		if ((errcode & HAL_SWPMI_ERROR_UDR) != 0U) {
			...
			if (HAL_IS_BIT_SET(hswpmi->Instance->CR,
					   SWPMI_CR_TXDMA)) {
				...
				if (hswpmi->hdmatx != NULL) {
					...
					/* Abort DMA TX */
					if (HAL_DMA_Abort_IT(hswpmi->hdmatx) !=
					    HAL_OK) {
						/* Call Directly
						 * hswpmi->hdmatx->XferAbortCallback
			...
			if (HAL_IS_BIT_SET(hswpmi->Instance->CR,
				...
				if (hswpmi->hdmarx != NULL) {
					...
					hswpmi->hdmarx->XferAbortCallback =
					    SWPMI_DMAAbortOnError;
					/* Abort DMA RX */
					if (HAL_DMA_Abort_IT(hswpmi->hdmarx) !=
					    HAL_OK) {
						/* Call Directly
						 * hswpmi->hdmarx->XferAbortCallback
						 * function in case of error */
...
__weak void HAL_SWPMI_TxCpltCallback(SWPMI_HandleTypeDef *hswpmi)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SWPMI_TxCpltCallback is to be implemented in the user
	   file
	 */
}
...
__weak void HAL_SWPMI_TxHalfCpltCallback(SWPMI_HandleTypeDef *hswpmi)
{
	...
	/* NOTE: This function should not be modified, when the callback is
	   needed, the HAL_SWPMI_TxHalfCpltCallback is to be implemented in the
	   user file
	 */
}
...
__weak void HAL_SWPMI_RxCpltCallback(SWPMI_HandleTypeDef *hswpmi)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SWPMI_RxCpltCallback is to be implemented in the user
	   file
	 */
}
...
__weak void HAL_SWPMI_RxHalfCpltCallback(SWPMI_HandleTypeDef *hswpmi)
{
	...
	/* NOTE: This function should not be modified, when the callback is
	   needed, the HAL_SWPMI_RxHalfCpltCallback is to be implemented in the
	   user file
	 */
}
...
__weak void HAL_SWPMI_ErrorCallback(SWPMI_HandleTypeDef *hswpmi)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SWPMI_ErrorCallback is to be implemented in the user
	   file
	 */
}
...
static void SWPMI_EndReceive_IT(SWPMI_HandleTypeDef *hswpmi)
{
	...
	CLEAR_BIT(hswpmi->Instance->IER, SWPMI_IT_RIE | SWPMI_IT_RXBERIE |
					     SWPMI_IT_RXOVRIE |
					     SWPMI_IT_RXBFIE);
...
static void SWPMI_DMATransmitCplt(DMA_HandleTypeDef *hdma)
{
	SWPMI_HandleTypeDef *hswpmi =
	    (SWPMI_HandleTypeDef *)((DMA_HandleTypeDef *)hdma)->Parent;
	uint32_t tickstart;
	...
	if ((hdma->Instance->CCR & DMA_CCR_CIRC) == 0U) {
		...
		/* Wait the TXBEF */
		if (SWPMI_WaitOnFlagSetUntilTimeout(
			hswpmi, SWPMI_FLAG_TXBEF, tickstart,
