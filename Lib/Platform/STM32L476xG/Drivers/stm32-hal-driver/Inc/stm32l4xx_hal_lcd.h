...
typedef struct {
	uint32_t Prescaler; /*!< Configures the LCD Prescaler.
				 This parameter can be one value of @ref
			       LCD_Prescaler */
	uint32_t
	    Divider;		  /*!< Configures the LCD Divider.
...
#define HAL_LCD_ERROR_UDR                                                      \
	(0x00000002U) /*!< Update display request flag timeout error */
#define HAL_LCD_ERROR_UDD                                                      \
...
#define LCD_VOLTAGESOURCE_INTERNAL                                             \
	(0x00000000U) /*!< Internal voltage source for the LCD */
...
#define LCD_PULSEONDURATION_2                                                  \
	(LCD_FCR_PON_1) /*!< Pulse ON duration = 2/CK_PS  */
#define LCD_PULSEONDURATION_3                                                  \
	(LCD_FCR_PON_1 | LCD_FCR_PON_0) /*!< Pulse ON duration = 3/CK_PS  */
#define LCD_PULSEONDURATION_4                                                  \
	(LCD_FCR_PON_2) /*!< Pulse ON duration = 4/CK_PS  */
 ...
 * @{
 */
#define LCD_DEADTIME_0                                                         \
	(0x00000000U) /*!< No dead Time                                  */
#define LCD_DEADTIME_1                                                         \
	(LCD_FCR_DEAD_0) /*!< One Phase between different couple of Frame   */
#define LCD_DEADTIME_2                                                         \
	(LCD_FCR_DEAD_1) /*!< Two Phase between different couple of Frame   */
#define LCD_DEADTIME_3                                                         \
	(LCD_FCR_DEAD_1 |                                                      \
	 LCD_FCR_DEAD_0) /*!< Three Phase between different couple of Frame */
#define LCD_DEADTIME_4                                                         \
	(LCD_FCR_DEAD_2) /*!< Four Phase between different couple of Frame  */
#define LCD_DEADTIME_5                                                         \
	(LCD_FCR_DEAD_2 |                                                      \
	 LCD_FCR_DEAD_0) /*!< Five Phase between different couple of Frame  */
#define LCD_DEADTIME_6                                                         \
	(LCD_FCR_DEAD_2 |                                                      \
	 LCD_FCR_DEAD_1) /*!< Six Phase between different couple of Frame   */
#define LCD_DEADTIME_7                                                         \
	(LCD_FCR_DEAD) /*!< Seven Phase between different couple of Frame */
/**
 * @}
 */
...
#define LCD_BLINKFREQUENCY_DIV8                                                \
	(0x00000000U) /*!< The Blink frequency = fLCD/8    */
#define LCD_BLINKFREQUENCY_DIV16                                               \
	(LCD_FCR_BLINKF_0) /*!< The Blink frequency = fLCD/16   */
#define LCD_BLINKFREQUENCY_DIV32                                               \
	(LCD_FCR_BLINKF_1) /*!< The Blink frequency = fLCD/32   */
#define LCD_BLINKFREQUENCY_DIV64                                               \
	(LCD_FCR_BLINKF_1 |                                                    \
	 LCD_FCR_BLINKF_0) /*!< The Blink frequency = fLCD/64   */
#define LCD_BLINKFREQUENCY_DIV128                                              \
	(LCD_FCR_BLINKF_2) /*!< The Blink frequency = fLCD/128  */
#define LCD_BLINKFREQUENCY_DIV256                                              \
	(LCD_FCR_BLINKF_2 |                                                    \
	 LCD_FCR_BLINKF_0) /*!< The Blink frequency = fLCD/256  */
#define LCD_BLINKFREQUENCY_DIV512                                              \
	(LCD_FCR_BLINKF_2 |                                                    \
	 LCD_FCR_BLINKF_1) /*!< The Blink frequency = fLCD/512  */
#define LCD_BLINKFREQUENCY_DIV1024                                             \
	(LCD_FCR_BLINKF) /*!< The Blink frequency = fLCD/1024 */
/**
 * @}
 */
...
#define LCD_CONTRASTLEVEL_3                                                    \
	(LCD_FCR_CC_1 | LCD_FCR_CC_0)	   /*!< Maximum Voltage = 2.99V    */
#define LCD_CONTRASTLEVEL_4 (LCD_FCR_CC_2) /*!< Maximum Voltage = 3.12V    */
#define LCD_CONTRASTLEVEL_5                                                    \
	(LCD_FCR_CC_2 | LCD_FCR_CC_0) /*!< Maximum Voltage = 3.26V    */
#define LCD_CONTRASTLEVEL_6                                                    \
	(LCD_FCR_CC_2 | LCD_FCR_CC_1)	 /*!< Maximum Voltage = 3.40V    */
...
#define LCD_MUXSEGMENT_DISABLE                                                 \
	(0x00000000U) /*!< SEG pin multiplexing disabled                 */
#define LCD_MUXSEGMENT_ENABLE                                                  \
	(LCD_CR_MUX_SEG) /*!< SEG[31:28] are multiplexed with SEG[43:40]    */
/**
 * @}
 */
...
#define LCD_FLAG_FCRSF                                                         \
	LCD_SR_FCRSR /*!< LCD Frame Control Register Synchronization flag */
/**
 * @}
 */
 ...
 * @retval None
 */
#define __HAL_LCD_RESET_HANDLE_STATE(__HANDLE__)                               \
	((__HANDLE__)->State = HAL_LCD_STATE_RESET)

/** @brief  Enable the LCD peripheral.
 * @param __HANDLE__ specifies the LCD Handle.
 * @retval None
 */
#define __HAL_LCD_ENABLE(__HANDLE__)                                           \
	SET_BIT((__HANDLE__)->Instance->CR, LCD_CR_LCDEN)

/** @brief  Disable the LCD peripheral.
 * @param __HANDLE__ specifies the LCD Handle.
 * @retval None
 */
#define __HAL_LCD_DISABLE(__HANDLE__)                                          \
	CLEAR_BIT((__HANDLE__)->Instance->CR, LCD_CR_LCDEN)
 ...
 * @retval None
 */
#define __HAL_LCD_VOLTAGE_BUFFER_ENABLE(__HANDLE__)                            \
	SET_BIT((__HANDLE__)->Instance->CR, LCD_CR_BUFEN)

/** @brief  Disable the voltage output buffer for higher driving capability.
 * @param __HANDLE__ specifies the LCD Handle.
 * @retval None
 */
#define __HAL_LCD_VOLTAGE_BUFFER_DISABLE(__HANDLE__)                           \
	CLEAR_BIT((__HANDLE__)->Instance->CR, LCD_CR_BUFEN)
...
#define __HAL_LCD_PULSEONDURATION_CONFIG(__HANDLE__, __DURATION__)             \
	do {                                                                   \
		MODIFY_REG((__HANDLE__)->Instance->FCR, LCD_FCR_PON,           \
			   (__DURATION__));                                    \
		LCD_WaitForSynchro(__HANDLE__);                                \
	} while (0)
...
#define __HAL_LCD_DEADTIME_CONFIG(__HANDLE__, __DEADTIME__)                    \
	do {                                                                   \
		MODIFY_REG((__HANDLE__)->Instance->FCR, LCD_FCR_DEAD,          \
			   (__DEADTIME__));                                    \
		LCD_WaitForSynchro(__HANDLE__);                                \
	} while (0)
...
#define __HAL_LCD_CONTRAST_CONFIG(__HANDLE__, __CONTRAST__)                    \
	do {                                                                   \
		MODIFY_REG((__HANDLE__)->Instance->FCR, LCD_FCR_CC,            \
			   (__CONTRAST__));                                    \
		LCD_WaitForSynchro(__HANDLE__);                                \
	} while (0)
 ...
 */
#define __HAL_LCD_GET_IT_SOURCE(__HANDLE__, __IT__)                            \
 ...
 */
#define __HAL_LCD_GET_FLAG(__HANDLE__, __FLAG__)                               \
...
#define IS_LCD_BIAS(__BIAS__)                                                  \
	(((__BIAS__) == LCD_BIAS_1_4) || ((__BIAS__) == LCD_BIAS_1_2) ||       \
...
#define IS_LCD_DEAD_TIME(__TIME__)                                             \
	(((__TIME__) == LCD_DEADTIME_0) || ((__TIME__) == LCD_DEADTIME_1) ||   \
	 ((__TIME__) == LCD_DEADTIME_2) || ((__TIME__) == LCD_DEADTIME_3) ||   \
	 ((__TIME__) == LCD_DEADTIME_4) || ((__TIME__) == LCD_DEADTIME_5) ||   \
