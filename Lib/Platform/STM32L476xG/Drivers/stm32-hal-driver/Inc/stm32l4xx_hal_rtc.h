...
typedef struct {
	...
	uint32_t SubSeconds; /*!< Specifies the RTC_SSR RTC Sub Second register
				...
				running 32 bit counter. */
	uint32_t
	    BinaryAutoClr; /*!< Clear synchronously counter (RTC_SSR) on binary
...
#else
#define __HAL_RTC_IS_CALENDAR_INITIALIZED(__HANDLE__)                          \
	(((((__HANDLE__)->Instance->ISR) & (RTC_FLAG_INITS)) ==                \
	  RTC_FLAG_INITS)                                                      \
	     ? 1U                                                              \
	     : 0U)
#endif /* #if defined(STM32L412xx) || defined(STM32L422xx) || defined          \
...
#else
#define __HAL_RTC_ALARM_GET_IT(__HANDLE__, __INTERRUPT__)                      \
	(((((__HANDLE__)->Instance->ISR) & (__INTERRUPT__ >> 4)) != 0U) ? 1U   \
									: 0U)
#endif
/**
...
#define __HAL_RTC_ALARM_CLEAR_FLAG(__HANDLE__, __FLAG__)                       \
	(((__FLAG__) == RTC_FLAG_ALRAF)                                        \
	     ? (((__HANDLE__)->Instance->SCR = (RTC_CLEAR_ALRAF)))             \
