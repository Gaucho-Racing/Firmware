...
#define ADC_CALIBRATION_TIMEOUT                                                \
	(296960UL) /*!< ADC calibration time-out value (unit: CPU cycles) */
				...
				const ADC_InjectionConfTypeDef *pConfigInjected)
{
	...
	assert_param(
	    IS_FUNCTIONAL_STATE(pConfigInjected->QueueInjectedContext));
	...
	if (pConfigInjected->InjectedSingleDiff != ADC_DIFFERENTIAL_ENDED) {
		assert_param(
		    IS_ADC_CHANNEL(hadc, pConfigInjected->InjectedChannel));
	} else {
	...
	if ((tmp_adc_is_conversion_on_going_regular == 0UL) &&
		...
		if (pConfigInjected->InjectedOffsetNumber != ADC_OFFSET_NONE) {
			...
			if (__LL_ADC_CHANNEL_TO_DECIMAL_NB(
				...
				pConfigInjected->InjectedChannel)) {
				LL_ADC_SetOffsetState(hadc->Instance,
						      LL_ADC_OFFSET_1,
				 ...
				 const ADC_MultiModeTypeDef *pMultimode)
{
	...
	if (pMultimode->Mode != ADC_MODE_INDEPENDENT) {
		assert_param(
		    IS_ADC_DMA_ACCESS_MULTIMODE(pMultimode->DMAAccessMode));
		assert_param(
...
HAL_StatusTypeDef HAL_ADCEx_DisableInjectedQueue(ADC_HandleTypeDef *hadc)
{
	...
	if ((tmp_adc_is_conversion_on_going_regular == 0UL) &&
	    (tmp_adc_is_conversion_on_going_injected == 0UL)) {
		LL_ADC_INJ_SetQueueMode(hadc->Instance,
