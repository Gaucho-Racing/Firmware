...
typedef struct {
	...
	uint32_t
	    KeySize; /*!< 128 or 256-bit key length.
	...
	uint32_t
	    GCMCMACPhase; /*!< Indicates the processing phase of the Galois
...
typedef enum {
	HAL_CRYP_PHASE_READY =
	    0x01, /*!< CRYP peripheral is ready for initialization. */
	HAL_CRYP_PHASE_PROCESS =
	    0x02, /*!< CRYP peripheral is in processing phase */
	...
	HAL_CRYP_PHASE_INIT_OVER =
	    0x04, /*!< GCM/GMAC(/CMAC)(/CCM) init phase has been carried out */
	HAL_CRYP_PHASE_HEADER_OVER = 0x05, /*!< GCM/GMAC(/CMAC)(/CCM) header
					      phase has been carried out  */
	HAL_CRYP_PHASE_PAYLOAD_OVER =
	    0x06, /*!< GCM(/CCM) payload phase has been carried out */
	HAL_CRYP_PHASE_FINAL_OVER =
	    0x07, /*!< GCM/GMAC(/CMAC)(/CCM) final phase has been carried out */
	HAL_CRYP_PHASE_HEADER_SUSPENDED =
	    0x08, /*!< GCM/GMAC(/CMAC)(/CCM) header phase has been suspended */
	HAL_CRYP_PHASE_PAYLOAD_SUSPENDED =
	    0x09, /*!< GCM(/CCM) payload phase has been suspended */
	HAL_CRYP_PHASE_NOT_USED =
	    0x0a /*!< Phase is irrelevant to the current chaining mode */
} HAL_PhaseTypeDef;
...
typedef enum {
	HAL_CRYP_SUSPEND_NONE =
	    0x00,		/*!< CRYP peripheral suspension not requested */
...
typedef enum {
	HAL_CRYP_INPUTCPLT_CB_ID =
	    0x01U, /*!< CRYP input DMA transfer completion callback ID  */
	HAL_CRYP_OUTPUTCPLT_CB_ID =
	    0x02U, /*!< CRYP output DMA transfer completion callback ID */
	HAL_CRYP_COMPCPLT_CB_ID =
	    0x03U, /*!< CRYP computation completion callback ID         */
...
#endif /* USE_HAL_CRYP_REGISTER_CALLBACKS */
{
	...
	uint32_t
	    CrypInCount; /*!< Input data size in bytes or, after suspension, the
	...
	DMA_HandleTypeDef
	    *hdmain; /*!< CRYP peripheral Input DMA handle parameters */

	DMA_HandleTypeDef
	    *hdmaout; /*!< CRYP peripheral Output DMA handle parameters */
	...
	void (*InCpltCallback)(
		...
		*hcryp); /*!< CRYP input DMA transfer completion callback */

	void (*OutCpltCallback)(
	    struct __CRYP_HandleTypeDef
		*hcryp); /*!< CRYP output DMA transfer completion callback */

	void (*CompCpltCallback)(
	    struct __CRYP_HandleTypeDef
		*hcryp); /*!< CRYP computation completion callback */
...
#define CRYP_DATATYPE_32B                                                      \
	((uint32_t)0x00000000) /*!< 32-bit data type (no swapping)        */
 ...
 * @{
 */
#define CRYP_CHAINMODE_AES_ECB                                                 \
	((uint32_t)0x00000000) /*!< Electronic codebook chaining algorithm */
#define CRYP_CHAINMODE_AES_CBC                                                 \
	AES_CR_CHMOD_0 /*!< Cipher block chaining algorithm */
#define CRYP_CHAINMODE_AES_CTR                                                 \
 ...
 * @{
 */
#define CRYP_KEY_WRITE_ENABLE                                                  \
	((uint32_t)0x00000000) /*!< Enable decryption key writing  */
#define CRYP_KEY_WRITE_DISABLE                                                 \
	((uint32_t)0x00000001) /*!< Disable decryption key writing */
/**
 * @}
 */
 ...
 * @{
 */
#define CRYP_DMAIN_DISABLE                                                     \
	((uint32_t)0x00000000) /*!< Disable DMA Input phase management */
#define CRYP_DMAIN_ENABLE                                                      \
	AES_CR_DMAINEN /*!< Enable DMA Input phase management  */
/**
 * @}
 */
 ...
 * @{
 */
#define CRYP_DMAOUT_DISABLE                                                    \
	((uint32_t)0x00000000) /*!< Disable DMA Output phase management */
#define CRYP_DMAOUT_ENABLE                                                     \
	AES_CR_DMAOUTEN /*!< Enable DMA Output phase management  */
/**
 * @}
 */
 ...
 * @{
 */
#define CRYP_GCM_INIT_PHASE                                                    \
	((uint32_t)0x00000000) /*!< GCM/GMAC (or CCM) init phase   */
#define CRYP_GCMCMAC_HEADER_PHASE                                              \
	AES_CR_GCMPH_0 /*!< GCM/GMAC/CCM/CMAC header phase */
#define CRYP_GCM_PAYLOAD_PHASE                                                 \
	AES_CR_GCMPH_1 /*!< GCM/CCM payload phase          */
#define CRYP_GCMCMAC_FINAL_PHASE                                               \
	AES_CR_GCMPH /*!< GCM/GMAC/CCM/CMAC final phase  */
...
#define __HAL_CRYP_SET_OPERATINGMODE(__HANDLE__, __OPERATING_MODE__)           \
	MODIFY_REG((__HANDLE__)->Instance->CR, AES_CR_MODE,                    \
		   (__OPERATING_MODE__))
...
#define __HAL_CRYP_SET_CHAININGMODE(__HANDLE__, __CHAINING_MODE__)             \
	MODIFY_REG((__HANDLE__)->Instance->CR, AES_CR_CHMOD,                   \
		   (__CHAINING_MODE__))
 ...
 * @retval None
 */
#define __HAL_CRYP_CLEAR_IT(__HANDLE__, __INTERRUPT__)                         \
