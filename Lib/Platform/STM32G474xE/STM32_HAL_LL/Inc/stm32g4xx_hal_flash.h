...
typedef struct {
	...
	uint32_t BootEntryPoint; /*!< Set the Boot Lock (used for
				    OPTIONBYTE_BOOT_LOCK). This parameter can be
				    a value of @ref FLASH_OB_Boot_Lock */
	uint32_t
	    SecBank; /*!< Bank of securable memory area to be programmed (used
			...
			Securable memory area to be programmed in bank 2 (*)
			  @note (*) availability depends on devices */
	uint32_t
	    SecSize; /*!< Size of securable memory area to be programmed (used
			for OPTIONBYTE_SEC), in number of pages. Securable
			memory area is starting from first page of the bank.
			  Only one securable memory could be programmed at the
			same time. This parameter must be a value between 0 and
...
#define FLASH_FLAG_ECCR_ERRORS                                                 \
	(FLASH_FLAG_ECCC | FLASH_FLAG_ECCD | FLASH_FLAG_ECCC2 |                \
	 FLASH_FLAG_ECCD2)
#else
...
#define FLASH_SIZE                                                             \
	((((*((uint16_t *)FLASH_SIZE_DATA_REGISTER)) == 0xFFFFU))              \
	     ? (0x200UL << 10U)                                                \
	     : (((*((uint32_t *)FLASH_SIZE_DATA_REGISTER)) & 0xFFFFUL)         \
		<< 10U))
#define FLASH_BANK_SIZE (FLASH_SIZE >> 1)
#define FLASH_PAGE_NB                                                          \
	((FLASH_SIZE == 0x00080000U)                                           \
	     ? 128U                                                            \
...
#define IS_OB_USER_SRAM_PARITY(VALUE)                                          \
	(((VALUE) == OB_SRAM_PARITY_ENABLE) ||                                 \
	 ((VALUE) == OB_SRAM_PARITY_DISABLE))

#define IS_OB_USER_CCMSRAM_RST(VALUE)                                          \
	(((VALUE) == OB_CCMSRAM_RST_ERASE) ||                                  \
	 ((VALUE) == OB_CCMSRAM_RST_NOT_ERASE))
...
#define IS_OB_USER_NRST_MODE(VALUE)                                            \
	(((VALUE) == OB_NRST_MODE_GPIO) ||                                     \
	 ((VALUE) == OB_NRST_MODE_INPUT_ONLY) ||                               \
