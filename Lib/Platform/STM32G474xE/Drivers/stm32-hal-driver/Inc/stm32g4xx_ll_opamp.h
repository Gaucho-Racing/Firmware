...
#define LL_OPAMP_MODE_FOLLOWER                                                 \
	(OPAMP_CSR_VMSEL_1 |                                                   \
	 OPAMP_CSR_VMSEL_0) /*!< OPAMP functional mode, OPAMP operation in     \
			       follower */
 ...
 * @{
 */
#define LL_OPAMP_POWERMODE_NORMALSPEED                                         \
...
#define LL_OPAMP_TRIMMING_NMOS_VREF_50PC_VDDA                                  \
	(OPAMP_CSR_TRIMOFFSETN |                                               \
	 OPAMP_CSR_CALSEL_1) /*!< OPAMP trimming of transistors differential   \
				pair NMOS (internal reference voltage set to   \
				0.5*Vdda). */
...
{
	return (uint32_t)(READ_BIT(OPAMPx->CSR, OPAMP_CSR_PGGAIN_4 |
						    OPAMP_CSR_PGGAIN_3 |
						    OPAMP_CSR_VMSEL));
}
...
{
	return (uint32_t)(READ_BIT(OPAMPx->CSR, OPAMP_CSR_PGGAIN_2 |
						    OPAMP_CSR_PGGAIN_1 |
						    OPAMP_CSR_PGGAIN_0));
}
...
LL_OPAMP_SetInputNonInvertingSecondary(OPAMP_TypeDef *OPAMPx,
				       uint32_t InputNonInverting)
{
 ...
 */
__STATIC_INLINE void
LL_OPAMP_SetInputInvertingSecondary(OPAMP_TypeDef *OPAMPx,
				    uint32_t InputInverting)
{
...
{
	return (uint32_t)(READ_BIT(OPAMPx->TCMR, OPAMP_TCMR_T1CMEN |
						     OPAMP_TCMR_T8CMEN |
						     OPAMP_TCMR_T20CMEN));
}
