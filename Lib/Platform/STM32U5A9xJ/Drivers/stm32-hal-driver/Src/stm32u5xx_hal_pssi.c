	...
	if (hpssi->State == HAL_PSSI_STATE_READY) {
		...
		if (hpssi->XferSize > 0U) {
			if (hpssi->hdmatx != NULL) {
				...
				/* Enable the DMA  */
				if ((hpssi->hdmatx->Mode & DMA_LINKEDLIST) ==
				    DMA_LINKEDLIST) {
					if (hpssi->hdmatx->LinkedListQueue !=
					    NULL) {
						/* Enable the DMA channel */
						/* Set DMA data size */
						hpssi->hdmatx->LinkedListQueue
						    ->Head->LinkRegisters
	...
	if (hpssi->State == HAL_PSSI_STATE_READY) {
		...
		if (hpssi->XferSize > 0U) {
			if (hpssi->hdmarx != NULL) {
				/* Configure BusWidth */
				if (hpssi->hdmarx->Init.SrcDataWidth ==
				...
				/* Enable the DMA  */
				if ((hpssi->hdmarx->Mode & DMA_LINKEDLIST) ==
				    DMA_LINKEDLIST) {
					if (hpssi->hdmarx->LinkedListQueue !=
					    NULL) {
						/* Enable the DMA channel */
						/* Set DMA data size */
						hpssi->hdmarx->LinkedListQueue
						    ->Head->LinkRegisters
							[NODE_CBR1_DEFAULT_OFFSET] =
						    hpssi->XferSize;
						/* Set DMA source address */
