/**
 ...
 read and write protection mechanisms.
  ...
  [..] The Flash memory interface implements the TrustZone security features
 (TZ) supported by ARM Cortex-M33 core (CM33).
 ...
 FLASH memory of all STM32U5xx devices.
...
/**
 * @brief  Program a quad-word or a burst of 8 quad-words at a specified
 * address.
				    ...
				    uint32_t DataAddress)
{
	...
	if (status == HAL_OK) {
		...
		/* If the program operation is completed, disable the PG (and
		 * BWR Bit in Burst programming mode) */
...
__weak void HAL_FLASH_EndOfOperationCallback(uint32_t ReturnValue)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_FLASH_EndOfOperationCallback could be implemented in
	   the user file
	 */
}
...
__weak void HAL_FLASH_OperationErrorCallback(uint32_t ReturnValue)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_FLASH_OperationErrorCallback could be implemented in
	   the user file
	 */
}
...
HAL_StatusTypeDef HAL_FLASH_OB_Launch(void)
{
	...
	/* We should not reach here : Option byte launch generates Option byte
	   reset so return error */
	return HAL_ERROR;
}
...
{
	/* Wait for the FLASH operation to complete by polling on BUSY and WDW
	   flags to be reset. Even if the FLASH operation fails, the BUSY & WDW
	   flags will be reset, and an error flag will be set */
