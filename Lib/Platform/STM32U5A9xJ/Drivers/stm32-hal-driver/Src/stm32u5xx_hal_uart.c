					...
					const uint8_t *pData, uint16_t Size)
{
	...
	if (huart->gState == HAL_UART_STATE_READY) {
		...
		if (huart->hdmatx != NULL) {
			...
			/* In case of 9bits/No Parity transfer, pData buffer
			...
			if ((huart->hdmatx->Mode & DMA_LINKEDLIST) ==
			    DMA_LINKEDLIST) {
				...
				status = HAL_DMA_Start_IT(
				    huart->hdmatx, (uint32_t)huart->pTxBuffPtr,
...
HAL_StatusTypeDef HAL_UART_DMAPause(UART_HandleTypeDef *huart)
{
	...
	if ((HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAT)) &&
		...
		/* Suspend the UART DMA Tx channel : use blocking DMA Suspend
		 * API (no callback) */
		if (huart->hdmatx != NULL) {
	...
	if ((HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAR)) &&
		...
		/* Suspend the UART DMA Rx channel : use blocking DMA Suspend
		 * API (no callback) */
					 ...
					 uint8_t *pData, uint16_t Size)
{
	...
	if (huart->hdmarx != NULL) {
		...
		/* In case of 9bits/No Parity transfer, pData buffer provided as
		   input parameter should be aligned on a u16 frontier, so
		   nbByte should be equal to Size * 2 */
		if ((huart->Init.WordLength == UART_WORDLENGTH_9B) &&
		    (huart->Init.Parity == UART_PARITY_NONE)) {
			nbByte = Size * 2U;
		}
