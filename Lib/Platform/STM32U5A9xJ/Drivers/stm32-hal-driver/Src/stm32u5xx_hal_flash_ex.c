/**
 ...
 ==============================================================================
  [..] This driver provides functions to configure and program the FLASH memory
	...
	(++) Configure the watermark security for each area including the secure
 hide areas
 ...
 to:
	...
	(++) Get the configuration of a watermark security area including the
 secure hide areas
 ...
 Use HAL_FLASHEx_GetBBSec()
	(++) Return the configuration of the block-based security and privilege
 for all the pages

      (#) Activation of the secure hide area function: Use
 ...
 accesses
      ...
      (#) Security inversion configuration function: Use
 HAL_FLASHEx_ConfigSecInversion()
	(++) FLASH secure state can be override

      (#) Get the security inversion configuration function: Use
 HAL_FLASHEx_GetSecInversion()
	...
	(++) Enable low-power mode for Flash Bank 1 and/or Bank 2
      ...
      (#) Enable low-power read mode function: Use
 HAL_FLASHEx_ConfigLowPowerRead()
	(++) Enable low-power read mode for Flash memory

      (#) Get the low-power read mode configuration function: Use
 HAL_FLASHEx_GetLowPowerRead()
	...
	(++) Return information about the on-going Flash operation. After a
	     system reset, return information about the interrupted Flash
 operation, if any.
...
static void FLASH_OB_WMSECConfig(uint32_t WMSecConfig, uint32_t WMSecStartPage,
				 uint32_t WMSecEndPage, uint32_t WMHDPEndPage);
...
#if defined(__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U)
static void FLASH_OB_GetWMSEC(uint32_t *WMSecConfig, uint32_t *WMSecStartPage,
			      uint32_t *WMSecEndPage, uint32_t *WMHDPEndPage);
				    ...
				    uint32_t *PageError)
{
	...
	if (status == HAL_OK) {
		...
		/* If the erase operation is completed, disable the associated
		 * bits */
		CLEAR_BIT((*reg_cr),
...
HAL_StatusTypeDef HAL_FLASHEx_OBProgram(FLASH_OBProgramInitTypeDef *pOBInit)
{
	...
	if (status == HAL_OK) {
		...
		if ((pOBInit->OptionType & OPTIONBYTE_WRP) != 0U) {
			...
			FLASH_OB_WRPConfig(
			    pOBInit->WRPArea, pOBInit->WRPStartOffset,
		...
		if ((pOBInit->OptionType & OPTIONBYTE_RDPKEY) != 0U) {
			...
			FLASH_OB_RDPKeyConfig(pOBInit->RDPKeyType,
					      pOBInit->RDPKey1,
					      pOBInit->RDPKey2);
		}
		...
		if ((pOBInit->OptionType & OPTIONBYTE_USER) != 0U) {
			...
			FLASH_OB_UserConfig(pOBInit->USERType,
					    pOBInit->USERConfig);
		}
		...
		if ((pOBInit->OptionType & OPTIONBYTE_WMSEC) != 0U) {
			...
			FLASH_OB_WMSECConfig(
			    pOBInit->WMSecConfig, pOBInit->WMSecStartPage,
			    pOBInit->WMSecEndPage, pOBInit->WMHDPEndPage);
		}
		...
		if ((pOBInit->OptionType & OPTIONBYTE_BOOTADDR) != 0U) {
			...
			FLASH_OB_BootAddrConfig(pOBInit->BootAddrConfig,
						pOBInit->BootAddr);
		}
 ...
 * @note   The fields pOBInit->WRPArea, pOBInit->WMSecConfig and
 * pOBInit->BootAddrConfig should indicate which area/address is requested for
 * the WRP, WM Security or Boot Address, else no information will be returned
 *
 * @retval None
 */
void HAL_FLASHEx_OBGetConfig(FLASH_OBProgramInitTypeDef *pOBInit)
{
...
#if defined(__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U)
	/* Get the configuration of the watermark secure area for the selected
	 * area */
	if ((pOBInit->WMSecConfig == OB_WMSEC_AREA1) ||
	    (pOBInit->WMSecConfig == OB_WMSEC_AREA2)) {
...
#endif /* __ARM_FEATURE_CMSE */
	{
		...
		FLASH_OB_GetBootAddr(pOBInit->BootAddrConfig,
				     &(pOBInit->BootAddr));
	}
}
...
HAL_FLASHEx_ConfigBBAttributes(FLASH_BBAttributesTypeDef *pBBAttributes)
{
	...
	if (status == HAL_OK) {
		...
		/* Modify the register values and check that new attributes are
		 * taken in account */
		...
		/* ISB instruction is called to be sure next instructions are
		 * performed with correct attributes */
		__ISB();
	}
...
/** @addtogroup FLASHEx_Exported_Functions_Group2 FLASHEx Exported Functions
 * Group2
 * @{
 */
 ...
 *         This parameter can be one of the following values:
 *            @arg FLASH_SPRIV_GRANTED:  access to secure Flash registers is
 * granted to privileged or unprivileged access
 *            @arg FLASH_SPRIV_DENIED:   access to secure Flash registers is
 * denied to unprivileged access
 *            @arg FLASH_NSPRIV_GRANTED: access to non-secure Flash registers is
 * granted to privileged or unprivileged access
 *            @arg FLASH_NSPRIV_DENIED:  access to non-secure Flash registers is
 * denied to unprivilege access
 *
 * @retval None
 */
void HAL_FLASHEx_ConfigPrivMode(uint32_t PrivMode)
{
	...
	MODIFY_REG(FLASH->PRIVCFGR,
		   (FLASH_PRIVCFGR_SPRIV | FLASH_PRIVCFGR_NSPRIV), PrivMode);
}
...
{
	return (FLASH->PRIVCFGR &
		(FLASH_PRIVCFGR_SPRIV | FLASH_PRIVCFGR_NSPRIV));
}
...
HAL_StatusTypeDef HAL_FLASHEx_EnablePowerDown(uint32_t Banks)
{
	...
	if ((Banks & FLASH_BANK_1) != 0U) {
		/* Check PD1 and PDREQ1 bits (Bank 1 is not in power-down mode
		   and not being already under power-down request) */
		if ((FLASH->NSSR & FLASH_NSSR_PD1) != 0U) {
			...
			while (((FLASH->NSSR & FLASH_NSSR_PD1) !=
				FLASH_NSSR_PD1)) {
	...
	if ((Banks & FLASH_BANK_2) != 0U) {
		/* Check PD2 and PDREQ2 bits (Bank 2 is not in power-down mode
		   and not being already under power-down request) */
		if ((FLASH->NSSR & FLASH_NSSR_PD2) != 0U) {
			...
			/* Check PD2 bit */
			tickstart = HAL_GetTick();
			while (((FLASH->NSSR & FLASH_NSSR_PD2) !=
				FLASH_NSSR_PD2)) {
				if ((HAL_GetTick() - tickstart) >
				    FLASH_TIMEOUT_VALUE) {
					return HAL_TIMEOUT;
...
void HAL_FLASHEx_GetOperation(FLASH_OperationTypeDef *pFlashOperation)
{
	...
	pFlashOperation->FlashArea =
	    opsr_reg & (FLASH_OPSR_SYSF_OP | FLASH_OPSR_BK_OP);
...
void FLASH_PageErase(uint32_t Page, uint32_t Banks)
{
	...
	MODIFY_REG(
	    (*reg_cr), (FLASH_NSCR_PNB | FLASH_NSCR_PER | FLASH_NSCR_STRT),
			       ...
			       uint32_t WRPEndOffset, FunctionalState WRPLock)
{
	...
	if (WRPArea == OB_WRPAREA_BANK1_AREAA) {
		FLASH->WRP1AR =
		    (((uint32_t)(~WRPLock) << FLASH_WRP1AR_UNLOCK_Pos) |
		     (WRPEndOffset << FLASH_WRP1AR_WRP1A_PEND_Pos) |
		     WRPStartOffset);
	} else if (WRPArea == OB_WRPAREA_BANK1_AREAB) {
		FLASH->WRP1BR =
		    (((uint32_t)(~WRPLock) << FLASH_WRP1BR_UNLOCK_Pos) |
		     (WRPEndOffset << FLASH_WRP1BR_WRP1B_PEND_Pos) |
		     WRPStartOffset);
	} else if (WRPArea == OB_WRPAREA_BANK2_AREAA) {
		FLASH->WRP2AR =
		    (((uint32_t)(~WRPLock) << FLASH_WRP2AR_UNLOCK_Pos) |
		     (WRPEndOffset << FLASH_WRP2AR_WRP2A_PEND_Pos) |
		     WRPStartOffset);
	} else if (WRPArea == OB_WRPAREA_BANK2_AREAB) {
		FLASH->WRP2BR =
		    (((uint32_t)(~WRPLock) << FLASH_WRP2BR_UNLOCK_Pos) |
...
static void FLASH_OB_UserConfig(uint32_t UserType, uint32_t UserConfig)
{
	...
	if ((UserType & OB_USER_SRAM_RST) != 0U) {
		...
		assert_param(
		    IS_OB_USER_SRAM_RST(UserConfig & FLASH_OPTR_SRAM_RST));
	...
	if ((UserType & OB_USER_SWAP_BANK) != 0U) {
		...
		assert_param(
		    IS_OB_USER_SWAP_BANK(UserConfig & FLASH_OPTR_SWAP_BANK));
	...
	if ((UserType & OB_USER_BKPRAM_ECC) != 0U) {
		...
		assert_param(
		    IS_OB_USER_BKPRAM_ECC(UserConfig & FLASH_OPTR_BKPRAM_ECC));
	...
	if ((UserType & OB_USER_SRAM3_ECC) != 0U) {
		...
		assert_param(
		    IS_OB_USER_SRAM3_ECC(UserConfig & FLASH_OPTR_SRAM3_ECC));
	...
	if ((UserType & OB_USER_SRAM2_ECC) != 0U) {
		...
		assert_param(
		    IS_OB_USER_SRAM2_ECC(UserConfig & FLASH_OPTR_SRAM2_ECC));
	...
	if ((UserType & OB_USER_PA15_PUPEN) != 0U) {
		...
		assert_param(
		    IS_OB_USER_PA15_PUPEN(UserConfig & FLASH_OPTR_PA15_PUPEN));
	...
	if ((UserType & OB_USER_IO_VDD_HSLV) != 0U) {
		...
		assert_param(IS_OB_USER_IO_VDD_HSLV(UserConfig &
						    FLASH_OPTR_IO_VDD_HSLV));
	...
	if ((UserType & OB_USER_IO_VDDIO2_HSLV) != 0U) {
		...
		assert_param(IS_OB_USER_IO_VDDIO2_HSLV(
		    UserConfig & FLASH_OPTR_IO_VDDIO2_HSLV));
 ...
 *            @arg OB_WMSEC_AREA1 or @arg OB_WMSEC_AREA2: Select Flash Secure
 * Area 1 or Area 2
 *            @arg OB_WMSEC_SECURE_AREA_CONFIG: configure Flash Secure Area
 *            @arg OB_WMSEC_HDP_AREA_CONFIG: configure Flash secure hide Area
 *            @arg OB_WMSEC_HDP_AREA_ENABLE: enable secure hide Area in Secure
 * Area
 *            @arg OB_WMSEC_HDP_AREA_DISABLE: disable secure hide Area in Secure
 * Area
 *
 ...
 *          This parameter can be page number between WMSecStartPage and (max
 * number of pages in the bank - 1)
 *
 * @param  WMHDPEndPage specifies the end page of the secure hide area
 *         This parameter can be a page number between WMSecStartPage and
 * WMSecEndPage
 *
 * @retval None
 */
				 ...
				 uint32_t WMSecEndPage, uint32_t WMHDPEndPage)
{
	...
	if ((WMSecConfig & OB_WMSEC_SECURE_AREA_CONFIG) != 0U) {
		tmp_secwm1 = ((WMSecEndPage << FLASH_SECWM1R1_SECWM1_PEND_Pos) |
			      WMSecStartPage);
	}
...
static void FLASH_OB_BootLockConfig(uint32_t BootLockConfig)
{
	...
	/* Configure the option bytes register */
	MODIFY_REG(FLASH->SECBOOTADD0R, FLASH_SECBOOTADD0R_BOOT_LOCK,
		   BootLockConfig);
}
#endif /* __ARM_FEATURE_CMSE */
...
static void FLASH_OB_BootAddrConfig(uint32_t BootAddrConfig, uint32_t BootAddr)
{
	...
	if (BootAddrConfig == OB_BOOTADDR_NS0) {
		...
		MODIFY_REG(FLASH->SECBOOTADD0R, FLASH_SECBOOTADD0R_SECBOOTADD0,
			   BootAddr);
	}
#endif /* __ARM_FEATURE_CMSE */
	else {
 ...
 *
 * @param[out]  WRPEndOffset specifies the address where to copied the end page
			    ...
			    uint32_t *WRPEndOffset, FunctionalState *WRPLock)
{
	...
	if (WRPArea == OB_WRPAREA_BANK1_AREAA) {
		*WRPStartOffset =
		    READ_BIT(FLASH->WRP1AR, FLASH_WRP1AR_WRP1A_PSTRT);
		*WRPEndOffset =
		    (READ_BIT(FLASH->WRP1AR, FLASH_WRP1AR_WRP1A_PEND) >>
		     FLASH_WRP1AR_WRP1A_PEND_Pos);
		*WRPLock = (READ_BIT(FLASH->WRP1AR, FLASH_WRP1AR_UNLOCK) != 0U)
			       ? DISABLE
			       : ENABLE;
	} else if (WRPArea == OB_WRPAREA_BANK1_AREAB) {
		*WRPStartOffset =
		    READ_BIT(FLASH->WRP1BR, FLASH_WRP1BR_WRP1B_PSTRT);
		*WRPEndOffset =
		    (READ_BIT(FLASH->WRP1BR, FLASH_WRP1BR_WRP1B_PEND) >>
		     FLASH_WRP1BR_WRP1B_PEND_Pos);
		*WRPLock = (READ_BIT(FLASH->WRP1BR, FLASH_WRP1BR_UNLOCK) != 0U)
			       ? DISABLE
			       : ENABLE;
	} else if (WRPArea == OB_WRPAREA_BANK2_AREAA) {
		*WRPStartOffset =
		    READ_BIT(FLASH->WRP2AR, FLASH_WRP2AR_WRP2A_PSTRT);
		*WRPEndOffset =
		    (READ_BIT(FLASH->WRP2AR, FLASH_WRP2AR_WRP2A_PEND) >>
		     FLASH_WRP2AR_WRP2A_PEND_Pos);
		*WRPLock = (READ_BIT(FLASH->WRP2AR, FLASH_WRP2AR_UNLOCK) != 0U)
			       ? DISABLE
			       : ENABLE;
	} else if (WRPArea == OB_WRPAREA_BANK2_AREAB) {
		*WRPStartOffset =
		    READ_BIT(FLASH->WRP2BR, FLASH_WRP2BR_WRP2B_PSTRT);
		*WRPEndOffset =
		    (READ_BIT(FLASH->WRP2BR, FLASH_WRP2BR_WRP2B_PEND) >>
		     FLASH_WRP2BR_WRP2B_PEND_Pos);
		*WRPLock = (READ_BIT(FLASH->WRP2BR, FLASH_WRP2BR_UNLOCK) != 0U)
			       ? DISABLE
			       : ENABLE;
	} else {
		/* Empty statement (to be compliant MISRA 15.7) */
 ...
 *         The return value can be a combination of @ref
 ...
 *                  When return from the function, this parameter will be a
 * combinaison of the following values:
 *                   @arg OB_WMSEC_AREAA or @arg OB_WMSEC_AREAB: selected Flash
 * Secure Area A or Area B
 *                   @arg OB_WMSEC_HDP_AREA_ENABLE: Secure Hide Area in Secure
 * Area enabled
 *                   @arg OB_WMSEC_HDP_AREA_DISABLE: Secure Hide Area in Secure
 * Area disabled
 *
			      ...
			      uint32_t *WMSecEndPage, uint32_t *WMHDPEndPage)
{
	...
	*WMHDPEndPage = ((tmp_secwm2 & FLASH_SECWM1R2_HDP1_PEND) >>
			 FLASH_SECWM1R2_HDP1_PEND_Pos);
