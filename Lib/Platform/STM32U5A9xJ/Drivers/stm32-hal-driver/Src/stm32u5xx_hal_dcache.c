...
__weak void HAL_DCACHE_MspInit(DCACHE_HandleTypeDef *hdcache)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_DCACHE_MspInit can be implemented in the user file
	 */
}
...
__weak void HAL_DCACHE_MspDeInit(DCACHE_HandleTypeDef *hdcache)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_DCACHE_MspDeInit can be implemented in the user file
	 */
}
/**
 * @}
	...
	if (HAL_DCACHE_IsEnabled(hdcache) != 0U) {
		...
		while (READ_BIT(hdcache->Instance->SR,
			...
			if ((HAL_GetTick() - tickstart) >
			    DCACHE_DISABLE_TIMEOUT_VALUE) {
				if (READ_BIT(hdcache->Instance->SR,
					     (DCACHE_SR_BUSYF |
					      DCACHE_SR_BUSYCMDF)) != 0U) {
					/* Update error code */
					hdcache->ErrorCode =
					    HAL_DCACHE_ERROR_TIMEOUT;
...
{
	return ((READ_BIT(hdcache->Instance->CR, DCACHE_CR_EN) != 0U) ? 1UL
								      : 0UL);
}
					 ...
					 uint32_t dSize)
{
	...
	status = DCACHE_CommandByAddr(hdcache, DCACHE_COMMAND_CLEAN, pAddr,
				      dSize, DCACHE_POLLING_MODE);
...
HAL_StatusTypeDef HAL_DCACHE_Invalidate_IT(DCACHE_HandleTypeDef *hdcache)
{
	...
	if (READ_BIT(hdcache->Instance->SR,
		...
		/* Set no operation on address range for callback under
		 * interrupt */
		MODIFY_REG(hdcache->Instance->CR, DCACHE_CR_CACHECMD, 0U);
...
__weak void HAL_DCACHE_CleanByAddrCallback(DCACHE_HandleTypeDef *hdcache)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_DCACHE_CleanByAddrCallback() should be implemented in
	   the user file
	 */
}
...
__weak void HAL_DCACHE_InvalidateByAddrCallback(DCACHE_HandleTypeDef *hdcache)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_DCACHE_InvalidateByAddrCallback() should be
...
HAL_DCACHE_CleanAndInvalidateByAddrCallback(DCACHE_HandleTypeDef *hdcache)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_DCACHE_CleanAndInvalidateByAddrCallback() should be
	   implemented in the user file
	 */
}
...
__weak void HAL_DCACHE_InvalidateCompleteCallback(DCACHE_HandleTypeDef *hdcache)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_DCACHE_InvalidateCompleteCallback() should be
	   implemented in the user file
	 */
}
...
__weak void HAL_DCACHE_ErrorCallback(DCACHE_HandleTypeDef *hdcache)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_DCACHE_ErrorCallback() should be implemented in the
	   user file
	 */
}
					      ...
					      uint32_t dSize, uint32_t mode)
{
	...
	if (READ_BIT(hdcache->Instance->SR,
		...
		WRITE_REG(hdcache->Instance->CMDREADDRR,
			  (op_addr + dSize - 1U));
		...
		if (mode == DCACHE_IT_MODE) {
			...
			/* Make sure that end of cache command interrupt is
			 * disabled */
