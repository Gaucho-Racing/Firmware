...
HAL_StatusTypeDef HAL_SDIO_Init(SDIO_HandleTypeDef *hsdio)
{
	...
	assert_param(
	    IS_SDMMC_HARDWARE_FLOW_CONTROL(hsdio->Init.HardwareFlowControl));
    ...
    (USE_HAL_SDIO_REGISTER_CALLBACKS == 1U)
		...
		if (hsdio->Init.TranceiverPresent ==
	...
	if (SDIO_WriteDirect(hsdio, SDMMC_SDIO_CCCR4_SD_BYTE3,
					   ...
					   uint32_t ClockSpeed)
{
	...
	if (hsdio->State == HAL_SDIO_STATE_READY) {
		ClockDiv = (HAL_RCCEx_GetPeriphCLKFreq(RCC_PERIPHCLK_SDMMC)) /
			   (2U * ClockSpeed);
		MODIFY_REG(hsdio->Instance->CLKCR, SDMMC_CLKCR_CLKDIV,
			   ClockDiv);
	} else {
		return HAL_ERROR;
	}
				      ...
				      HAL_SDIO_CCCR_TypeDef *pCccr)
{
	...
	for (count = 0U; count <= SDIO_CCCR_REG_NUMBER; count++) {
		if (SDIO_ReadDirect(hsdio, SDMMC_SDIO_CCCR0 + count,
				    HAL_SDIO_WRITE_ONLY, SDIO_FUNCTION_0,
					      ...
					      HAL_SDIO_FBR_t *pFbr)
{
	...
	for (func_idx = 2U; func_idx <= SDIO_MAX_IO_NUMBER; func_idx++) {
		for (count = 0U; count <= SDIO_CCCR_REG_NUMBER; count++) {
			if (SDIO_ReadDirect(hsdio,
					    (((uint32_t)SDMMC_SDIO_F1BR0 *
					      (uint32_t)func_idx) +
					     count),
					    HAL_SDIO_WRITE_ONLY,
					    SDIO_FUNCTION_0,
		...
		pFbr[(uint32_t)func_idx - 1U].ioStdFunctionCode =
		    tempBuffer[0U] & 0x0FU;
		pFbr[(uint32_t)func_idx - 1U].ioExtFunctionCode =
		    tempBuffer[1U];
		...
		if ((tempBuffer[0U] & 0x40U) != 0U) {
			pFbr[(uint32_t)func_idx - 1U].flags |=
			    (uint8_t)HAL_SDIO_FBR_SUPPORT_CSA;
		}
	}
		    ...
		    const HAL_SDIO_DirectCmd_TypeDef *Argument, uint8_t *pData)
{
	...
	if (hsdio->State == HAL_SDIO_STATE_READY) {
		...
		cmd |= 0U;
		errorstate =
		    SDMMC_SDIO_CmdReadWriteDirect(hsdio->Instance, cmd, pData);
		     ...
		     const HAL_SDIO_DirectCmd_TypeDef *Argument, uint8_t Data)
{
	...
	if (hsdio->State == HAL_SDIO_STATE_READY) {
		...
		cmd |= Data;
		errorstate =
		    SDMMC_SDIO_CmdReadWriteDirect(hsdio->Instance, cmd, &Data);
		if (errorstate != HAL_SDIO_ERROR_NONE) {
			hsdio->ErrorCode |= errorstate;
			if (errorstate != (SDMMC_ERROR_ADDR_OUT_OF_RANGE |
					   SDMMC_ERROR_ILLEGAL_CMD |
		      ...
		      uint8_t *pData, uint32_t Size_byte, uint32_t Timeout_Ms)
{
	...
	if (hsdio->State == HAL_SDIO_STATE_READY) {
		...
		if (Argument->Block_Mode == HAL_SDIO_MODE_BLOCK) {
			 ...
			 * Block/Byte counts */
			config.DataLength =
			    (uint32_t)(nbr_of_block * hsdio->block_size);
			config.DataBlockSize =
			    SDIO_Convert_Block_Size(hsdio, hsdio->block_size);
		} else {
			/* (Argument & 0x1FFU) is to get the 9 bits of
			 * Block/Byte counts */
			config.DataLength =
			    (Size_byte > 0U) ? Size_byte
		 ...
		 * of the CMD argument */
		config.TransferMode =
		    (Argument->Block_Mode == HAL_SDIO_MODE_BLOCK)
		...
		errorstate =
		    SDMMC_SDIO_CmdReadWriteExtended(hsdio->Instance, cmd);
		if (errorstate != HAL_SDIO_ERROR_NONE) {
			hsdio->ErrorCode |= errorstate;
			if (errorstate != (SDMMC_ERROR_ADDR_OUT_OF_RANGE |
					   SDMMC_ERROR_ILLEGAL_CMD |
					   SDMMC_ERROR_COM_CRC_FAILED |
					   SDMMC_ERROR_GENERAL_UNKNOWN_ERR)) {
				MODIFY_REG(hsdio->Instance->DCTRL,
					   SDMMC_DCTRL_FIFORST,
					   SDMMC_DCTRL_FIFORST);
				__HAL_SDIO_CLEAR_FLAG(hsdio,
						      SDMMC_STATIC_FLAGS);
		...
		while (!__HAL_SDIO_GET_FLAG(
			...
			if (__HAL_SDIO_GET_FLAG(hsdio, SDMMC_FLAG_RXFIFOHF) &&
				...
				while (!(__HAL_SDIO_GET_FLAG(
					   hsdio, SDMMC_FLAG_RXFIFOE)) &&
				       (dataremaining > 0U)) {
					data = SDMMC_ReadFIFO(hsdio->Instance);
					for (byteCount = 0U; byteCount < 4U;
					     byteCount++) {
						if (dataremaining > 0U) {
							*tempbuff =
							    (uint8_t)((data >>
		       ...
		       uint8_t *pData, uint32_t Size_byte, uint32_t Timeout_Ms)
{
	...
	if (hsdio->State == HAL_SDIO_STATE_READY) {
		...
		dataremaining = config.DataLength;
		while (!__HAL_SDIO_GET_FLAG(
		    hsdio, SDMMC_FLAG_TXUNDERR | SDMMC_FLAG_DCRCFAIL |
			       SDMMC_FLAG_DTIMEOUT | SDMMC_FLAG_DATAEND)) {
			...
			if (__HAL_SDIO_GET_FLAG(hsdio, SDMMC_FLAG_TXFIFOHE) &&
				...
				dataremaining -= SDMMC_FIFO_SIZE;
			} else if ((__HAL_SDIO_GET_FLAG(
				       hsdio, SDMMC_FLAG_TXFIFOHE |
						  SDMMC_FLAG_TXFIFOE)) &&
				   (dataremaining < SDMMC_FIFO_SIZE)) {
			  ...
			  uint8_t *pData, uint32_t Size_byte)
{
	...
	if (hsdio->State == HAL_SDIO_STATE_READY) {
		...
		if (nbr_of_block != 0U) {
			...
			hsdio->next_reg_addr =
			    (Argument->Reg_Addr) |
			    ((((nbr_of_block * hsdio->block_size) >> 1U) &
			      0x3FFFU)
			     << 1U) |
			    ((hsdio->remaining_data <=
			      HAL_SDIO_DATA_BLOCK_SIZE_512BYTE)
		...
		/* Read */
		hsdio->Context =
		    (uint32_t)((Argument->Block_Mode == HAL_SDIO_MODE_BLOCK)
				   ? SDIO_CONTEXT_READ_MULTIPLE_BLOCK
				   : SDIO_CONTEXT_READ_SINGLE_BLOCK) |
		    SDIO_CONTEXT_DMA;
		...
		cmd |= (Argument->Reg_Addr & 0x1FFFFU) << 9U;
		cmd |=
		    ((nbr_of_block == 0U) ? Size_byte : nbr_of_block) & 0x1FFU;
		errorstate =
		    SDMMC_SDIO_CmdReadWriteExtended(hsdio->Instance, cmd);
		if (errorstate != HAL_SDIO_ERROR_NONE) {
			hsdio->ErrorCode |= errorstate;
...
void HAL_SDIO_IRQHandler(SDIO_HandleTypeDef *hsdio)
{
	...
	if (READ_BIT(flags, SDMMC_FLAG_DATAEND) != 0U) {
		...
		__HAL_SDIO_DISABLE_IT(
		    hsdio, SDMMC_IT_DATAEND | SDMMC_IT_DCRCFAIL |
		...
		if ((ctx & SDIO_CONTEXT_DMA) != 0U) {
			...
			if ((hsdio->Instance->DCTRL & SDMMC_DCTRL_SDIOEN) !=
			    0U) {
				hsdio->Instance->DCTRL = SDMMC_DCTRL_SDIOEN;
			} else {
				hsdio->Instance->DCTRL = 0U;
		...
		if (hsdio->remaining_data != 0U) {
			...
			if (((ctx & SDIO_CONTEXT_READ_SINGLE_BLOCK) != 0U) ||
				...
				hsdio->pRxBuffPtr =
				    (uint8_t *)hsdio->next_data_addr;
				errorstate = HAL_SDIO_ReadExtended_DMA(
				    hsdio, &CMD53_desc, hsdio->pRxBuffPtr,
				    hsdio->remaining_data);
			} else {
				hsdio->pTxBuffPtr =
				    (uint8_t *)hsdio->next_data_addr;
				errorstate = HAL_SDIO_WriteExtended_DMA(
				    hsdio, &CMD53_desc, hsdio->pTxBuffPtr,
				    hsdio->remaining_data);
			}
 ...
 */
HAL_StatusTypeDef
HAL_SDIO_RegisterTransceiverCallback(SDIO_HandleTypeDef *hsdio,
					    ...
					    uint32_t IOFunction)
{
	...
	if (SDIO_ReadDirect(hsdio, SDMMC_SDIO_CCCR0_SD_BYTE2,
			    HAL_SDIO_WRITE_ONLY, SDIO_FUNCTION_0,
			    &ioEn) != HAL_OK) {
		return HAL_ERROR;
	}
	...
	if (SDIO_ReadDirect(hsdio, SDMMC_SDIO_CCCR0_SD_BYTE3,
			    HAL_SDIO_WRITE_ONLY, SDIO_FUNCTION_0,
			    &ioReady) != HAL_OK) {
		return HAL_ERROR;
	}
...
HAL_StatusTypeDef HAL_SDIO_EnableIOAsynInterrupt(SDIO_HandleTypeDef *hsdio)
{
	...
	if (SDIO_ReadDirect(hsdio, SDMMC_SDIO_CCCR20_SD_BYTE2,
			    HAL_SDIO_WRITE_ONLY, SDIO_FUNCTION_0,
...
static HAL_StatusTypeDef SDIO_InitCard(SDIO_HandleTypeDef *hsdio)
{
	...
	/* Send CMD5 */
	errorstate =
	    SDMMC_CmdSendOperationcondition(hsdio->Instance, 0U, &Resp4);
	if (errorstate != HAL_SDIO_ERROR_NONE) {
		return HAL_ERROR;
	}
	...
	if (nbr_of_func > 0U) {
		...
		if (SDMMC_CmdSendOperationcondition(
			hsdio->Instance,
			(SDIO_OCR_VDD_32_33 | SDIO_OCR_SDIO_S18R),
					  ...
					  const uint8_t *pData)
{
	...
	errorstate =
	    SDMMC_SDIO_CmdReadWriteDirect(hsdio->Instance, cmd, &response);
		   ...
		   uint16_t Size_byte)
{
	...
	while (!__HAL_SDIO_GET_FLAG(
		...
		if (__HAL_SDIO_GET_FLAG(hsdio, SDMMC_FLAG_TXFIFOHE) &&
		    (dataremaining >= SDMMC_FIFO_SIZE)) {
			for (regCount = SDMMC_FIFO_SIZE / 4U; regCount > 0U;
			     regCount--) {
				SDMMCx->FIFO = *u32tempbuff;
				u32tempbuff++;
			}
			dataremaining -= SDMMC_FIFO_SIZE;
		} else if ((__HAL_SDIO_GET_FLAG(hsdio,
						SDMMC_FLAG_TXFIFOHE |
						    SDMMC_FLAG_TXFIFOE)) &&
			   (dataremaining < SDMMC_FIFO_SIZE)) {
			const uint8_t *u8buff = (uint8_t *)u32tempbuff;
...
static HAL_StatusTypeDef SDIO_IOFunction_IRQHandler(SDIO_HandleTypeDef *hsdio)
{
	...
	if (hsdio->IOInterruptNbr == 1U) {
		if ((hsdio->SDIO_IOFunction_Callback[hsdio->IOFunctionMask -
						     1U]) != NULL) {
			(hsdio->SDIO_IOFunction_Callback[hsdio->IOFunctionMask -
							 1U])(
			    hsdio, hsdio->IOFunctionMask - 1U);
		}
	} else if ((hsdio->IOInterruptNbr > 1U) &&
		   (hsdio->IOFunctionMask != 0U)) {
		/* Get pending int firstly */
		if (SDIO_ReadDirect(hsdio, SDMMC_SDIO_CCCR4_SD_BYTE1,
				    HAL_SDIO_WRITE_ONLY, SDIO_FUNCTION_0,
				    &pendingInt) != HAL_OK) {
			return HAL_ERROR;
		}
		...
		if ((pendingInt != 0U) && (hsdio->IOFunctionMask != 0U)) {
			for (count = 1; count <= SDIO_MAX_IO_NUMBER; count++) {
				if (((pendingInt & (1U << count)) != 0U) &&
				    (((1U << count) & hsdio->IOFunctionMask) !=
				     0U)) {
					if ((hsdio->SDIO_IOFunction_Callback
						 [count - 1U]) != NULL) {
						(hsdio->SDIO_IOFunction_Callback
						     [count - 1U])(hsdio,
								   count);
					}
				}
