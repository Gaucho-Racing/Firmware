  ...
  ******************************************************************************
  ...
  [..]
    *** Initialization and de-initialization ***
	...
	(++) Enable MDFz or ADFz clock interface with
  __HAL_RCC_MDFz_CLK_ENABLE() or __HAL_RCC_ADFz_CLK_ENABLE().
	(++) Enable the clocks for the used GPIOS with
  __HAL_RCC_GPIOx_CLK_ENABLE().
	...
	(++) If DMA mode is used, initialize and configure DMA.
      (#) Configure the common parameters (only for first MDF or ADF instance
  init), serial interface parameters and filter bitstream selection by calling
	  HAL_MDF_Init() function.

    [..]
      (#) User can de-initialize MDF or ADF instance with HAL_MDF_DeInit()
  function.

    *** Acquisition ***
    ===================
    [..]
      (#) Configure filter parameters and start acquisition using
  HAL_MDF_AcqStart(), HAL_MDF_AcqStart_IT() or HAL_MDF_AcqStart_DMA().
      (#) In polling mode :
	    (++) Use HAL_MDF_PollForAcq() to detect the end of acquisition.
		 Use HAL_MDF_GetAcqValue to get acquisition value.
	    (++) Only for MDF instance, use HAL_MDF_PollForSnapshotAcq() to
  detect the end of snapshot acquisition. Use HAL_MDF_GetSnapshotAcqValue to get
  snapshot acquisition value.
	    (++) Only for ADF instance, use HAL_MDF_PollForSndLvl() to detect
  and get new sound level value and ambient noise value.
	    (++) Only for ADF instance, use HAL_MDF_PollForSad() to detect sound
  activity.
      (#) In interrupt mode :
	    (++) HAL_MDF_AcqCpltCallback() will be called at the end of
  acquisition. Use HAL_MDF_GetAcqValue to get acquisition value or use
		 HAL_MDF_GetSnapshotAcqValue to get snapshot acquisition value.
	    (++) Only for ADF instance, HAL_MDF_SndLvlCallback() will be called
  when new sound level and ambient noise values are available.
	    (++) Only for ADF instance, HAL_MDF_SadCallback() will be called
  when sound activity detection occurs.
	    (++) HAL_MDF_ErrorCallback() will be called if overflow, filter
  overrun or saturation occurs. Use HAL_MDF_GetErrorCode() to get the
  corresponding error.
      (#) In DMA mode :
	    (++) HAL_MDF_AcqHalfCpltCallback() and HAL_MDF_AcqCpltCallback()
  will be called respectively at the half acquisition and at the acquisition
  complete.
	    (++) Only for ADF instance, HAL_MDF_SndLvlCallback() will be called
  when new sound level and ambient noise values are available.
	    (++) Only for ADF instance, HAL_MDF_SadCallback() will be called
  when sound activity detection occurs.
	    (++) HAL_MDF_ErrorCallback() will be called if overflow, filter
  overrun, saturation or DMA error occurs. Use HAL_MDF_GetErrorCode() to get the
  corresponding error.
      (#) Use HAL_MDF_GenerateTrgo() to generate pulse on TRGO signal.
      (#) During acquisition, use HAL_MDF_SetDelay() and HAL_MDF_GetDelay() to
  respectively set and get the delay on data source.
      (#) During acquisition, use HAL_MDF_SetGain() and HAL_MDF_GetGain() to
  respectively set and get the filter gain.
      (#) During acquisition, use HAL_MDF_SetOffset() and HAL_MDF_GetOffset() to
  respectively set and get the filter offset error compensation.
      (#) During acquisition, only for MDF instance, use HAL_MDF_SetOffset() and
  HAL_MDF_GetOffset() to respectively set and get the filter offset error
  compensation.
      (#) Stop acquisition using HAL_MDF_AcqStop(), HAL_MDF_AcqStop_IT() or
  HAL_MDF_AcqStop_DMA().

    *** Clock absence detection ***
    ===============================
    [..]
      (#) Clock absence detection is always enabled so no need to start clock
  absence detection in polling mode. Use HAL_MDF_CkabStart_IT() to start clock
  absence detection in interrupt mode.
      (#) In polling mode, use HAL_MDF_PollForCkab() to detect the clock
  absence.
      (#) In interrupt mode, HAL_MDF_ErrorCallback() will be called if clock
  absence detection occurs. Use HAL_MDF_GetErrorCode() to get the corresponding
  error.
      (#) Stop clock absence detection in interrupt mode using
  HAL_MDF_CkabStop_IT().

    *** Short circuit detection ***
    ===============================
    [..]
      (#) Only for MDF instance, start short circuit detection using
  HAL_MDF_ScdStart() or HAL_MDF_ScdStart_IT().
      (#) In polling mode, use HAL_MDF_PollForScd() to detect short circuit.
      (#) In interrupt mode, HAL_MDF_ErrorCallback() will be called if short
  circuit detection occurs. Use HAL_MDF_GetErrorCode() to get the corresponding
  error.
      (#) Stop short circuit detection using HAL_MDF_ScdStop() or
  HAL_MDF_ScdStop_IT().

    *** Out-off limit detection ***
    ===============================
    [..]
      (#) Only for MDF instance, start out-off limit detection using
  HAL_MDF_OldStart() or HAL_MDF_OldStart_IT().
      (#) In polling mode, use HAL_MDF_PollForOld() to detect out-off limit and
  to get threshold information.
      (#) In interrupt mode, HAL_MDF_OldCallback() will be called if out-off
  limit detection occurs.
      (#) Stop out-off limit detection using HAL_MDF_OldStop() or
  HAL_MDF_OldStop_IT().
    ...
    *** generic functions ***
    ...
    [..]
      ...
      (#) Use HAL_MDF_GetErrorCode() to get the current MDF or ADF instance
  error code.
    ...
    *** Callback registration ***
    ...
    Function HAL_MDF_RegisterCallback() allows to register following callbacks :
      ...
      (+) SadCallback         : Sound activity detection callback (only for ADF
  instance).
    ...
    [..]
    For MDF instance and for specific out-off limit detection callback use
  dedicated register callback : HAL_MDF_RegisterOldCallback().

    [..]
    For ADF instance and for specific sound level callback use dedicated
  register callback : HAL_MDF_RegisterSndLvlCallback().

    [..]
    Use function HAL_MDF_UnRegisterCallback() to reset a callback to the default
  weak function.

    [..]
    HAL_MDF_UnRegisterCallback() takes as parameters the HAL peripheral handle
  and the Callback ID.
    [..]
  ...
  instance).
    ...
    [..]
    For MDF instance and for specific out-off limit detection callback use
  dedicated unregister callback : HAL_MDF_UnRegisterOldCallback().

    [..]
    For ADF instance and for specific sound level callback use dedicated
  unregister callback : HAL_MDF_UnRegisterSndLvlCallback().
...
#define MDF_INSTANCE_NUMBER                                                    \
	3U /* 2 instances for MDF1 and 1 instance for ADF1 */
#else	   /* defined(STM32U535xx) || defined(STM32U545xx) */
#define MDF_INSTANCE_NUMBER                                                    \
	7U /* 6 instances for MDF1 and 1 instance for ADF1 */
 ...
 */
static uint32_t
MDF_GetHandleNumberFromInstance(const MDF_Filter_TypeDef *const pInstance);
static void MDF_AcqStart(MDF_HandleTypeDef *const hmdf,
			 const MDF_FilterConfigTypeDef *const pFilterConfig);
...
/** @defgroup MDF_Exported_Functions_Group1  Initialization and
 ...
 *         in the MDF_InitTypeDef structure and initialize the associated
 * handle.
...
HAL_StatusTypeDef HAL_MDF_Init(MDF_HandleTypeDef *hmdf)
{
	...
	if (hmdf == NULL) {
		...
		assert_param(
		    IS_MDF_FILTER_BITSTREAM(hmdf->Init.FilterBistream));
		assert_param(
		    IS_FUNCTIONAL_STATE(hmdf->Init.SerialInterface.Activation));

		/* Check that instance has not been already initialized */
		if (a_mdfHandle[MDF_GetHandleNumberFromInstance(
			hmdf->Instance)] != NULL) {
			status = HAL_ERROR;
		} else {
			 ...
			 * instance */
			if (((v_mdf1InstanceCounter == 0U) &&
			     IS_MDF_INSTANCE(hmdf->Instance)) ||
			    ((v_adf1InstanceCounter == 0U) &&
			     IS_ADF_INSTANCE(hmdf->Instance))) {
				MDF_TypeDef *mdfBase;
				/* Get MDF base according instance */
				mdfBase = (IS_ADF_INSTANCE(hmdf->Instance))
					      ? ADF1
					      : MDF1;

				/* Check clock generator status */
				if ((mdfBase->CKGCR & MDF_CKGCR_CCKACTIVE) !=
				    0U) {
					status = HAL_ERROR;
				} else {
					...
					if (IS_MDF_INSTANCE(hmdf->Instance)) {
						assert_param(
						    IS_MDF_INTERLEAVED_FILTERS(
							hmdf->Init.CommonParam
							    .InterleavedFilters));
						mdfBase->GCR &=
						    ~(MDF_GCR_ILVNB);
						mdfBase->GCR |=
					...
					/* Configure processing clock divider,
					   ...
					   trigger */
					assert_param(IS_MDF_PROC_CLOCK_DIVIDER(
					    hmdf->Init.CommonParam
						.ProcClockDivider));
					assert_param(IS_FUNCTIONAL_STATE(
					    hmdf->Init.CommonParam.OutputClock
						.Activation));
					mdfBase->CKGCR = 0U;
					mdfBase->CKGCR |=
					    ((hmdf->Init.CommonParam
						  .ProcClockDivider -
					      1U)
					     << MDF_CKGCR_PROCDIV_Pos);
					if (hmdf->Init.CommonParam.OutputClock
						.Activation == ENABLE) {
						assert_param(
						    IS_MDF_OUTPUT_CLOCK_PINS(
							hmdf->Init.CommonParam
							    .OutputClock.Pins));
						assert_param(
						    IS_MDF_OUTPUT_CLOCK_DIVIDER(
							hmdf->Init.CommonParam
							    .OutputClock
							    .Divider));
						assert_param(
						...
						if (hmdf->Init.CommonParam
							...
							assert_param(
							    IS_MDF_OUTPUT_CLOCK_TRIGGER_EDGE(
								hmdf->Init
								    .CommonParam
								    .OutputClock
								    .Trigger
			...
			if ((status == HAL_OK) &&
				...
				if ((hmdf->Instance->SITFCR &
				     MDF_SITFCR_SITFACTIVE) != 0U) {
					status = HAL_ERROR;
				} else {
					/* Configure mode, clock source and
					 * threshold */
					assert_param(IS_MDF_SITF_MODE(
					    hmdf->Init.SerialInterface.Mode));
					assert_param(IS_MDF_SITF_CLOCK_SOURCE(
					    hmdf->Init.SerialInterface
						.ClockSource));
					assert_param(IS_MDF_SITF_THRESHOLD(
					    hmdf->Init.SerialInterface
						.Threshold));
			...
			if (status == HAL_OK) {
				...
				hmdf->Instance->BSMXCR |=
				    hmdf->Init.FilterBistream;
				...
				a_mdfHandle[MDF_GetHandleNumberFromInstance(
				    hmdf->Instance)] = hmdf;
	...
	if (hmdf == NULL) {
		...
		if (a_mdfHandle[MDF_GetHandleNumberFromInstance(
			...
			if (IS_MDF_INSTANCE(hmdf->Instance)) {
				...
				if ((hmdf->Instance->SCDCR &
				     MDF_SCDCR_SCDACTIVE) != 0U) {
					hmdf->Instance->SCDCR &=
					    ~(MDF_SCDCR_SCDEN);
				}

				/* Disable out-off limit detector if needed */
				if ((hmdf->Instance->OLDCR &
				     MDF_OLDCR_OLDACTIVE) != 0U) {
					hmdf->Instance->OLDCR &=
					    ~(MDF_OLDCR_OLDEN);
				}
			}
			...
			if (IS_ADF_INSTANCE(hmdf->Instance)) {
				if ((hmdf->Instance->SADCR &
				     MDF_SADCR_SADACTIVE) != 0U) {
					hmdf->Instance->SADCR &=
					    ~(MDF_SADCR_SADEN);
				}
			}
			...
			if ((hmdf->Instance->SITFCR & MDF_SITFCR_SITFACTIVE) !=
			    0U) {
			...
			if (((v_mdf1InstanceCounter == 1U) &&
			     IS_MDF_INSTANCE(hmdf->Instance)) ||
			    ((v_adf1InstanceCounter == 1U) &&
			     IS_ADF_INSTANCE(hmdf->Instance))) {
				MDF_TypeDef *p_mdf_base;
				/* Get MDF base according instance */
				p_mdf_base = (IS_ADF_INSTANCE(hmdf->Instance))
						 ? ADF1
						 : MDF1;
			...
			a_mdfHandle[MDF_GetHandleNumberFromInstance(
			    hmdf->Instance)] = (MDF_HandleTypeDef *)NULL;
...
__weak void HAL_MDF_MspInit(MDF_HandleTypeDef *hmdf)
{
	...
	/* NOTE : This function should not be modified, when the function is
	   needed, the HAL_MDF_MspInit could be implemented in the user file */
}
...
__weak void HAL_MDF_MspDeInit(MDF_HandleTypeDef *hmdf)
{
	...
	/* NOTE : This function should not be modified, when the function is
	   needed, the HAL_MDF_MspDeInit could be implemented in the user file
	 */
}

#if (USE_HAL_MDF_REGISTER_CALLBACKS == 1)
/**
 * @brief  Register a user MDF callback to be used instead of the weak
 * predefined callback.
 * @param  hmdf MDF handle.
 * @param  CallbackID ID of the callback to be registered.
 *         This parameter can be one of the following values:
 *           @arg @ref HAL_MDF_ACQ_COMPLETE_CB_ID acquisition complete callback
 * ID.
 *           @arg @ref HAL_MDF_ACQ_HALFCOMPLETE_CB_ID acquisition half complete
 * callback ID.
 *           @arg @ref HAL_MDF_SAD_CB_ID sound activity detector callback ID
 * (only for ADF instance).
 ...
 */
HAL_StatusTypeDef
HAL_MDF_UnRegisterCallback(MDF_HandleTypeDef *hmdf,
			   HAL_MDF_CallbackIDTypeDef CallbackID)
{
	HAL_StatusTypeDef status = HAL_OK;
 ...
 *         MDF out-off limit detector callback is redirected to the weak
 * predefined callback.
 * @param  hmdf MDF handle.
 * @retval HAL status.
 * @note   This function must not be used with ADF instance.
 ...
 */
HAL_StatusTypeDef
HAL_MDF_RegisterSndLvlCallback(MDF_HandleTypeDef *hmdf,
 ...
 *         MDF sound level callback is redirected to the weak predefined
 * callback.
 * @param  hmdf MDF handle.
 * @retval HAL status.
 * @note   This function must not be used with MDF instance.
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_MDF_AcqStart(MDF_HandleTypeDef *hmdf,
				   const MDF_FilterConfigTypeDef *pFilterConfig)
{
	HAL_StatusTypeDef status = HAL_OK;
	...
	if (pFilterConfig == NULL) {
		...
		if (IS_ADF_INSTANCE(hmdf->Instance)) {
			assert_param(IS_ADF_ACQUISITION_MODE(
			    pFilterConfig->AcquisitionMode));
			assert_param(IS_FUNCTIONAL_STATE(
			    pFilterConfig->SoundActivity.Activation));
		} else {
			assert_param(IS_MDF_ACQUISITION_MODE(
			    pFilterConfig->AcquisitionMode));
		}
		if ((IS_ADF_INSTANCE(hmdf->Instance)) &&
		    (pFilterConfig->SoundActivity.Activation == ENABLE) &&
		    ((pFilterConfig->AcquisitionMode ==
		      MDF_MODE_ASYNC_SINGLE) ||
		     (pFilterConfig->AcquisitionMode == MDF_MODE_SYNC_SINGLE) ||
		     (pFilterConfig->AcquisitionMode ==
		      MDF_MODE_WINDOW_CONT))) {
			status = HAL_ERROR;
		}
		/* Check state */
		...
		/* Check filter status */
		else if ((hmdf->Instance->DFLTCR & MDF_DFLTCR_DFLTACTIVE) !=
			 0U) {
			status = HAL_ERROR;
		} else {
		...
		if (status == HAL_OK) {
			/* For MDF instance, check OLD status and main filter
			 * order */
			assert_param(IS_MDF_CIC_MODE(pFilterConfig->CicMode));
			if (IS_MDF_INSTANCE(hmdf->Instance)) {
				if (((hmdf->Instance->OLDCR &
				      MDF_OLDCR_OLDACTIVE) != 0U) &&
				    (pFilterConfig->CicMode >=
...
HAL_StatusTypeDef HAL_MDF_PollForAcq(MDF_HandleTypeDef *hmdf, uint32_t Timeout)
{
	...
	if (hmdf->State != HAL_MDF_STATE_ACQUISITION) {
		...
		while (((hmdf->Instance->DFLTISR & MDF_DFLTISR_RXNEF) !=
			MDF_DFLTISR_RXNEF) &&
		       (status == HAL_OK)) {
			/* Check the timeout */
		 ...
		 */
		uint32_t error_flags = (hmdf->Instance->DFLTISR &
					(MDF_DFLTISR_DOVRF | MDF_DFLTISR_SATF |
					 MDF_DFLTISR_RFOVRF));
		if (error_flags != 0U) {
			/* Update error code */
			if ((error_flags & MDF_DFLTISR_DOVRF) ==
			    MDF_DFLTISR_DOVRF) {
				hmdf->ErrorCode |=
				    MDF_ERROR_ACQUISITION_OVERFLOW;
			}
			if ((error_flags & MDF_DFLTISR_SATF) ==
			    MDF_DFLTISR_SATF) {
				hmdf->ErrorCode |= MDF_ERROR_SATURATION;
			}
			if ((error_flags & MDF_DFLTISR_RFOVRF) ==
			    MDF_DFLTISR_RFOVRF) {
		...
		if (status == HAL_OK) {
			...
			if ((hmdf->Instance->DFLTCR & MDF_DFLTCR_ACQMOD) ==
			    MDF_MODE_ASYNC_SINGLE) {
				hmdf->State = HAL_MDF_STATE_READY;
			}
		}
...
/**
 * @brief  This function allows to poll for available snapshot acquisition
 * value.
 * @param  hmdf MDF handle.
 * @param  Timeout Timeout value in milliseconds.
 * @retval HAL status.
 * @note   This function must not be used with ADF instance.
 */
HAL_StatusTypeDef HAL_MDF_PollForSnapshotAcq(MDF_HandleTypeDef *hmdf,
					     uint32_t Timeout)
{
	HAL_StatusTypeDef status = HAL_OK;
	...
	if (hmdf->State != HAL_MDF_STATE_ACQUISITION) {
		...
		while (((hmdf->Instance->DFLTISR & MDF_DFLTISR_SSDRF) !=
			MDF_DFLTISR_SSDRF) &&
		       (status == HAL_OK)) {
			/* Check the timeout */
			if (Timeout != HAL_MAX_DELAY) {
		 ...
		 * occurs */
		uint32_t error_flags = (hmdf->Instance->DFLTISR &
					(MDF_DFLTISR_SSOVRF | MDF_DFLTISR_SATF |
					 MDF_DFLTISR_RFOVRF));
		if (error_flags != 0U) {
			/* Update error code */
			if ((error_flags & MDF_DFLTISR_SSOVRF) ==
			    MDF_DFLTISR_SSOVRF) {
				hmdf->ErrorCode |=
				    MDF_ERROR_ACQUISITION_OVERFLOW;
			}
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_MDF_GetAcqValue(const MDF_HandleTypeDef *hmdf,
				      int32_t *pValue)
{
	HAL_StatusTypeDef status = HAL_OK;
 ...
 */
HAL_StatusTypeDef
HAL_MDF_GetSnapshotAcqValue(MDF_HandleTypeDef *hmdf,
			    MDF_SnapshotParamTypeDef *pSnapshotParam)
{
	HAL_StatusTypeDef status = HAL_OK;
	...
	/* Check state */
		...
		/* Store value of decimation counter in snapshot parameter
		 * structure */
		pSnapshotParam->DecimationCounter =
		    (snpsdr_value & MDF_SNPSDR_MCICDC);

		/* Check snapshot format */
		if ((hmdf->Instance->DFLTCR & MDF_SNAPSHOT_16BITS) ==
		    MDF_SNAPSHOT_16BITS) {
			/* Store value of integrator counter in snapshot
			 * parameter structure */
			pSnapshotParam->IntegratorCounter =
			    ((snpsdr_value & MDF_SNPSDR_EXTSDR) >>
			     MDF_SNPSDR_EXTSDR_Pos);
			...
			/* Store snapshot acquisition value (23MSB) in snapshot
			 * parameter structure */
...
HAL_StatusTypeDef HAL_MDF_AcqStop(MDF_HandleTypeDef *hmdf)
{
	...
	if (hmdf->State != HAL_MDF_STATE_ACQUISITION) {
		...
		if (IS_ADF_INSTANCE(hmdf->Instance)) {
			if ((hmdf->Instance->SADCR & MDF_SADCR_SADACTIVE) !=
			    0U) {
	...
	if (status == HAL_OK) {
		...
		if (IS_ADF_INSTANCE(hmdf->Instance)) {
			hmdf->Instance->DFLTISR |=
			    (MDF_DFLTISR_DOVRF | MDF_DFLTISR_SATF |
			     MDF_DFLTISR_RFOVRF | MDF_DFLTISR_SDDETF |
			     MDF_DFLTISR_SDLVLF);
		} else {
			hmdf->Instance->DFLTISR |=
			    (MDF_DFLTISR_DOVRF | MDF_DFLTISR_SSDRF |
			     MDF_DFLTISR_SSOVRF | MDF_DFLTISR_SATF |
			     MDF_DFLTISR_RFOVRF);
		}
 ...
 */
HAL_StatusTypeDef
HAL_MDF_AcqStart_IT(MDF_HandleTypeDef *hmdf,
		    const MDF_FilterConfigTypeDef *pFilterConfig)
{
	HAL_StatusTypeDef status = HAL_OK;
	...
	if (pFilterConfig == NULL) {
		...
		if (status == HAL_OK) {
			...
			if (status == HAL_OK) {
				if (pFilterConfig->AcquisitionMode ==
				    MDF_MODE_SYNC_SNAPSHOT) {
					/* Enable snapshot overrun and data
					 * ready interrupts */
				...
				if (pFilterConfig->ReshapeFilter.Activation ==
				    ENABLE) {
					/* Enable reshape filter overrun
					 * interrupt */
					hmdf->Instance->DFLTIER |=
					    MDF_DFLTIER_RFOVRIE;
				}
...
HAL_StatusTypeDef HAL_MDF_AcqStop_IT(MDF_HandleTypeDef *hmdf)
{
	...
	if (status == HAL_OK) {
		...
		if (IS_ADF_INSTANCE(hmdf->Instance)) {
			hmdf->Instance->DFLTIER &=
			    ~(MDF_DFLTIER_FTHIE | MDF_DFLTIER_DOVRIE |
			      MDF_DFLTIER_SATIE | MDF_DFLTIER_RFOVRIE |
			      MDF_DFLTIER_SDDETIE | MDF_DFLTIER_SDLVLIE);
			hmdf->Instance->DFLTISR |=
			    (MDF_DFLTISR_DOVRF | MDF_DFLTISR_SATF |
			     MDF_DFLTISR_RFOVRF | MDF_DFLTISR_SDDETF |
			     MDF_DFLTISR_SDLVLF);
			...
			hmdf->Instance->DFLTIER &=
			    ~(MDF_DFLTIER_FTHIE | MDF_DFLTIER_DOVRIE |
			      MDF_DFLTIER_SSDRIE | MDF_DFLTIER_SSOVRIE |
			      MDF_DFLTIER_SATIE | MDF_DFLTIER_RFOVRIE);
			hmdf->Instance->DFLTISR |=
			    (MDF_DFLTISR_DOVRF | MDF_DFLTISR_SSDRF |
			     MDF_DFLTISR_SSOVRF | MDF_DFLTISR_SATF |
 ...
 */
HAL_StatusTypeDef
HAL_MDF_AcqStart_DMA(MDF_HandleTypeDef *hmdf,
		     const MDF_FilterConfigTypeDef *pFilterConfig,
	...
	if ((pFilterConfig == NULL) || (pDmaConfig == NULL)) {
		...
		if ((IS_ADF_INSTANCE(hmdf->Instance)) &&
		    ...
		    ((pFilterConfig->AcquisitionMode ==
		     ...
		     (pFilterConfig->AcquisitionMode ==
		      MDF_MODE_WINDOW_CONT))) {
			status = HAL_ERROR;
		} else if (pFilterConfig->AcquisitionMode ==
			   MDF_MODE_SYNC_SNAPSHOT) {
			status = HAL_ERROR;
		}
		/* Check state */
		...
		if (status == HAL_OK) {
			...
			if (status == HAL_OK) {
				...
				/* Start DMA transfer */
				hmdf->hdma->XferCpltCallback =
				    MDF_DmaXferCpltCallback;
				hmdf->hdma->XferHalfCpltCallback =
				    MDF_DmaXferHalfCpltCallback;
				hmdf->hdma->XferErrorCallback =
				    MDF_DmaErrorCallback;
				SrcAddress =
				    (pDmaConfig->MsbOnly == ENABLE)
					? (((uint32_t)&hmdf->Instance->DFLTDR) +
					   2U)
					: (uint32_t)&hmdf->Instance->DFLTDR;
				if ((hmdf->hdma->Mode & DMA_LINKEDLIST) ==
				    DMA_LINKEDLIST) {
					if (hmdf->hdma->LinkedListQueue !=
					    NULL) {
						hmdf->hdma->LinkedListQueue
						    ->Head->LinkRegisters
...
/**
 * @brief  This function allows to set delay to apply on data source in number
 * of samples.
 * @param  hmdf MDF handle.
...
HAL_StatusTypeDef HAL_MDF_SetDelay(MDF_HandleTypeDef *hmdf, uint32_t Delay)
{
	...
	if (hmdf->State != HAL_MDF_STATE_ACQUISITION) {
		...
		if ((hmdf->Instance->DLYCR & MDF_DLYCR_SKPBF) ==
		    MDF_DLYCR_SKPBF) {
			status = HAL_ERROR;
		} else {
...
/**
 * @brief  This function allows to get current delay applied on data source in
 * number of samples.
 * @param  hmdf MDF handle.
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_MDF_GetDelay(const MDF_HandleTypeDef *hmdf,
				   uint32_t *pDelay)
{
	HAL_StatusTypeDef status = HAL_OK;
 ...
 *         This parameter must be a number between Min_Data = -16 and Max_Data
 * = 24.
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_MDF_SetGain(MDF_HandleTypeDef *hmdf, int32_t Gain)
{
	...
	if (hmdf->State != HAL_MDF_STATE_ACQUISITION) {
		...
		if (Gain < 0) {
			...
			/* adjust gain value to set on register for negative
			 * value (offset of -16) */
			adjust_gain = Gain - 16;
		...
		/* Set gain */
		tmp_register =
		    (hmdf->Instance->DFLTCICR & ~(MDF_DFLTCICR_SCALE));
 ...
 *         This parameter must be a number between Min_Data = -33554432 and
 * Max_Data = 33554431.
 * @retval HAL status.
 * @note   This function must not be used with ADF instance.
 */
 ...
 * @note   This function must not be used with ADF instance.
 */
HAL_StatusTypeDef HAL_MDF_GetOffset(const MDF_HandleTypeDef *hmdf,
				    int32_t *pOffset)
{
	HAL_StatusTypeDef status = HAL_OK;
...
/**
  ...
  * @note   This function must not be used with MDF instance.
  */
HAL_StatusTypeDef HAL_MDF_PollForSndLvl(MDF_HandleTypeDef *hmdf,
					uint32_t Timeout, uint32_t *pSoundLevel,
					uint32_t *pAmbientNoise)
{
	...
	/* Check SAD status */
		...
		while (((hmdf->Instance->DFLTISR & MDF_DFLTISR_SDLVLF) !=
			MDF_DFLTISR_SDLVLF) &&
		       (status == HAL_OK)) {
			/* Check the timeout */
			if (Timeout != HAL_MAX_DELAY) {
...
HAL_StatusTypeDef HAL_MDF_PollForSad(MDF_HandleTypeDef *hmdf, uint32_t Timeout)
{
	...
	/* Check SAD status */
		...
		while (((hmdf->Instance->DFLTISR & MDF_DFLTISR_SDDETF) !=
			MDF_DFLTISR_SDDETF) &&
		       (status == HAL_OK)) {
			/* Check the timeout */
			if (Timeout != HAL_MAX_DELAY) {
...
__weak void HAL_MDF_AcqCpltCallback(MDF_HandleTypeDef *hmdf)
{
	...
	/* NOTE : This function should not be modified, when the function is
	   needed, the HAL_MDF_AcqCpltCallback could be implemented in the user
	   file */
}
...
__weak void HAL_MDF_AcqHalfCpltCallback(MDF_HandleTypeDef *hmdf)
{
	...
	/* NOTE : This function should not be modified, when the function is
	   needed, the HAL_MDF_AcqHalfCpltCallback could be implemented in the
	   user file */
}
 ...
 * @retval None.
 */
__weak void HAL_MDF_SndLvlCallback(MDF_HandleTypeDef *hmdf, uint32_t SoundLevel,
				   uint32_t AmbientNoise)
{
	/* Prevent unused argument(s) compilation warning */
	UNUSED(hmdf);
	UNUSED(SoundLevel);
	...
	/* NOTE : This function should not be modified, when the function is
	   needed, the HAL_MDF_SndLvlCallback could be implemented in the user
	   file */
}
...
__weak void HAL_MDF_SadCallback(MDF_HandleTypeDef *hmdf)
{
	...
	/* NOTE : This function should not be modified, when the function is
	   needed, the HAL_MDF_SadCallback could be implemented in the user file
	 */
}
...
HAL_StatusTypeDef HAL_MDF_PollForCkab(MDF_HandleTypeDef *hmdf, uint32_t Timeout)
{
	...
	if (status == HAL_OK) {
		...
		if ((hmdf->Instance->SITFCR & MDF_SITFCR_SITFACTIVE) == 0U) {
			...
			if ((hmdf->Instance->SITFCR & MDF_SITFCR_SITFMOD) !=
			    MDF_SITF_NORMAL_SPI_MODE) {
				status = HAL_ERROR;
			}
		}
	}
	...
	if (status == HAL_OK) {
		...
		while (((hmdf->Instance->DFLTISR & MDF_DFLTISR_CKABF) !=
			MDF_DFLTISR_CKABF) &&
		       (status == HAL_OK)) {
			/* Check the timeout */
			if (Timeout != HAL_MAX_DELAY) {
...
HAL_StatusTypeDef HAL_MDF_CkabStart_IT(MDF_HandleTypeDef *hmdf)
{
	...
	if (status == HAL_OK) {
		...
		if ((hmdf->Instance->DFLTISR & MDF_DFLTISR_CKABF) ==
		    MDF_DFLTISR_CKABF) {
			status = HAL_ERROR;
		} else {
 ...
 * @note   This function must not be used with ADF instance.
 */
HAL_StatusTypeDef HAL_MDF_ScdStart(MDF_HandleTypeDef *hmdf,
				   const MDF_ScdConfigTypeDef *pScdConfig)
{
	HAL_StatusTypeDef status = HAL_OK;
	...
	if (pScdConfig == NULL) {
		...
		if (status == HAL_OK) {
			...
			if ((hmdf->Instance->SCDCR & MDF_SCDCR_SCDACTIVE) ==
			    MDF_SCDCR_SCDACTIVE) {
				status = HAL_ERROR;
			} else {
				/* Configure threshold and break signal */
				hmdf->Instance->SCDCR =
				    (((pScdConfig->Threshold - 1U)
				      << MDF_SCDCR_SCDT_Pos) |
				     (pScdConfig->BreakSignal
				      << MDF_SCDCR_BKSCD_Pos));
...
HAL_StatusTypeDef HAL_MDF_PollForScd(MDF_HandleTypeDef *hmdf, uint32_t Timeout)
{
	...
	if (status == HAL_OK) {
		...
		if ((hmdf->Instance->SCDCR & MDF_SCDCR_SCDACTIVE) !=
			...
			while (((hmdf->Instance->DFLTISR & MDF_DFLTISR_SCDF) !=
				MDF_DFLTISR_SCDF) &&
			       (status == HAL_OK)) {
				/* Check the timeout */
				if (Timeout != HAL_MAX_DELAY) {
 ...
 * @note   This function must not be used with ADF instance.
 */
HAL_StatusTypeDef HAL_MDF_ScdStart_IT(MDF_HandleTypeDef *hmdf,
				      const MDF_ScdConfigTypeDef *pScdConfig)
{
	HAL_StatusTypeDef status = HAL_OK;
...
/**
 * @brief  This function allows to start out-off limit detection in polling
 * mode.
 * @param  hmdf MDF handle.
 ...
 * @note   This function must not be used with ADF instance.
 */
HAL_StatusTypeDef HAL_MDF_OldStart(MDF_HandleTypeDef *hmdf,
				   const MDF_OldConfigTypeDef *pOldConfig)
{
	HAL_StatusTypeDef status = HAL_OK;
	...
	if (pOldConfig == NULL) {
		...
		assert_param(IS_MDF_OLD_DECIMATION_RATIO(
		    pOldConfig->OldDecimationRatio));
		assert_param(IS_MDF_OLD_THRESHOLD(pOldConfig->HighThreshold));
		assert_param(IS_MDF_OLD_THRESHOLD(pOldConfig->LowThreshold));
		assert_param(
		    IS_MDF_OLD_EVENT_CONFIG(pOldConfig->OldEventConfig));
		...
		if (status == HAL_OK) {
			...
			if ((hmdf->Instance->OLDCR & MDF_OLDCR_OLDACTIVE) ==
			    MDF_OLDCR_OLDACTIVE) {
				status = HAL_ERROR;
			} else {
				/* Check filter status */
				if ((hmdf->Instance->DFLTCR &
				     MDF_DFLTCR_DFLTACTIVE) ==
				    MDF_DFLTCR_DFLTACTIVE) {
					/* Check main filter order */
					if ((hmdf->Instance->DFLTCICR &
					     MDF_DFLTCICR_CICMOD) >=
					    MDF_ONE_FILTER_SINC4) {
						status = HAL_ERROR;
					}
				...
				if (status == HAL_OK) {
					...
					hmdf->Instance->OLDCR =
					    (pOldConfig->OldCicMode |
					     pOldConfig->OldEventConfig |
					     ((pOldConfig->OldDecimationRatio -
					       1U)
					      << MDF_OLDCR_ACICD_Pos) |
					     (pOldConfig->BreakSignal
					      << MDF_OLDCR_BKOLD_Pos));

					/* Configure low and high thresholds */
					hmdf->Instance->OLDTHLR =
					    (uint32_t)pOldConfig->LowThreshold;
					hmdf->Instance->OLDTHHR =
					    (uint32_t)pOldConfig->HighThreshold;
 ...
 * @note   This function must not be used with ADF instance.
 */
HAL_StatusTypeDef HAL_MDF_PollForOld(MDF_HandleTypeDef *hmdf, uint32_t Timeout,
				     uint32_t *pThresholdInfo)
{
	HAL_StatusTypeDef status = HAL_OK;
	...
	if (status == HAL_OK) {
		...
		if ((hmdf->Instance->OLDCR & MDF_OLDCR_OLDACTIVE) !=
			...
			while (((hmdf->Instance->DFLTISR & MDF_DFLTISR_OLDF) !=
				MDF_DFLTISR_OLDF) &&
			       (status == HAL_OK)) {
				/* Check the timeout */
				if (Timeout != HAL_MAX_DELAY) {
			...
			if (status == HAL_OK) {
				...
				if ((hmdf->Instance->DFLTISR &
				     (MDF_DFLTISR_THLF | MDF_DFLTISR_THHF)) ==
				    0U) {
					*pThresholdInfo = MDF_OLD_IN_THRESHOLDS;
				} else if ((hmdf->Instance->DFLTISR &
					    MDF_DFLTISR_THLF) ==
					   MDF_DFLTISR_THLF) {
					*pThresholdInfo = MDF_OLD_LOW_THRESHOLD;
				} else {
					*pThresholdInfo =
					    MDF_OLD_HIGH_THRESHOLD;
				}
...
/**
 * @brief  This function allows to start out-off limit detection in interrupt
 * mode.
 * @param  hmdf MDF handle.
 * @param  pOldConfig Out-off limit detector configuration parameters.
 * @retval HAL status.
 * @note   This function must not be used with ADF instance.
 */
HAL_StatusTypeDef HAL_MDF_OldStart_IT(MDF_HandleTypeDef *hmdf,
				      const MDF_OldConfigTypeDef *pOldConfig)
{
	HAL_StatusTypeDef status = HAL_OK;
...
/**
 * @brief  This function allows to stop out-off limit detection in interrupt
 * mode.
 * @param  hmdf MDF handle.
 * @retval HAL status.
 * @note   This function must not be used with ADF instance.
...
__weak void HAL_MDF_OldCallback(MDF_HandleTypeDef *hmdf, uint32_t ThresholdInfo)
{
	...
	/* NOTE : This function should not be modified, when the function is
	   needed, the HAL_MDF_OldCallback could be implemented in the user file
	 */
}
...
void HAL_MDF_IRQHandler(MDF_HandleTypeDef *hmdf)
{
	...
	/* Check if out-off limit detection occurs */
		...
		/* Get threshold information */
		if ((hmdf->Instance->DFLTISR &
		     (MDF_DFLTISR_THLF | MDF_DFLTISR_THHF)) == 0U) {
			threshold_info = MDF_OLD_IN_THRESHOLDS;
		} else if ((hmdf->Instance->DFLTISR & MDF_DFLTISR_THLF) ==
			   MDF_DFLTISR_THLF) {
...
#else  /* USE_HAL_MDF_REGISTER_CALLBACKS */
			HAL_MDF_SndLvlCallback(hmdf, sound_level,
					       ambient_noise);
#endif /* USE_HAL_MDF_REGISTER_CALLBACKS */
		}
	}
...
__weak void HAL_MDF_ErrorCallback(MDF_HandleTypeDef *hmdf)
{
	...
	/* NOTE : This function should not be modified, when the function is
	   needed, the HAL_MDF_ErrorCallback could be implemented in the user
	   file */
}
			 ...
			 const MDF_FilterConfigTypeDef *const pFilterConfig)
{
	...
	/* Configure acquisition mode, discard samples, trigger and fifo
	 * threshold */
	...
	if ((pFilterConfig->AcquisitionMode == MDF_MODE_ASYNC_CONT) ||
		...
		hmdf->Instance->DFLTCR |=
		    (pFilterConfig->AcquisitionMode |
		     pFilterConfig->FifoThreshold |
		     (pFilterConfig->DiscardSamples << MDF_DFLTCR_NBDIS_Pos));
	} else {
		/* Trigger parameters are used */
		if (IS_ADF_INSTANCE(hmdf->Instance)) {
			assert_param(IS_ADF_TRIGGER_SOURCE(
			    pFilterConfig->Trigger.Source));
		} else {
			assert_param(IS_MDF_TRIGGER_SOURCE(
			    pFilterConfig->Trigger.Source));
		}
	...
	if (IS_MDF_INSTANCE(hmdf->Instance) &&
	    (pFilterConfig->AcquisitionMode == MDF_MODE_SYNC_SNAPSHOT)) {
		assert_param(
		    IS_MDF_SNAPSHOT_FORMAT(pFilterConfig->SnapshotFormat));
	...
	if (pFilterConfig->Gain < 0) {
		...
		/* adjust gain value to set on register for negative value
		 * (offset of -16) */
	...
	hmdf->Instance->DFLTCICR =
	    (pFilterConfig->DataSource | pFilterConfig->CicMode |
	...
	assert_param(
	    IS_FUNCTIONAL_STATE(pFilterConfig->ReshapeFilter.Activation));
	hmdf->Instance->DFLTRSFR = 0U;
	if (pFilterConfig->ReshapeFilter.Activation == ENABLE) {
		/* Configure reshape filter decimation ratio */
		assert_param(IS_MDF_RSF_DECIMATION_RATIO(
		    pFilterConfig->ReshapeFilter.DecimationRatio));
		hmdf->Instance->DFLTRSFR |=
		    pFilterConfig->ReshapeFilter.DecimationRatio;
	} else {
	...
	assert_param(
	    IS_FUNCTIONAL_STATE(pFilterConfig->HighPassFilter.Activation));
	if (pFilterConfig->HighPassFilter.Activation == ENABLE) {
		/* Configure high-pass filter cut-off frequency */
		assert_param(IS_MDF_HPF_CUTOFF_FREQ(
		    pFilterConfig->HighPassFilter.CutOffFrequency));
		hmdf->Instance->DFLTRSFR |=
		    pFilterConfig->HighPassFilter.CutOffFrequency;
	} else {
	...
	if (IS_MDF_INSTANCE(hmdf->Instance)) {
		assert_param(
		    IS_FUNCTIONAL_STATE(pFilterConfig->Integrator.Activation));
		if (pFilterConfig->Integrator.Activation == ENABLE) {
			/* Configure integrator value and output division */
			assert_param(IS_MDF_INTEGRATOR_VALUE(
			    pFilterConfig->Integrator.Value));
			assert_param(IS_MDF_INTEGRATOR_OUTPUT_DIV(
			    pFilterConfig->Integrator.OutputDivision));
			hmdf->Instance->DFLTINTR =
			    (((pFilterConfig->Integrator.Value - 1U)
			      << MDF_DFLTINTR_INTVAL_Pos) |
			     pFilterConfig->Integrator.OutputDivision);
		} else {
			/* Bypass integrator */
	...
	if (IS_ADF_INSTANCE(hmdf->Instance)) {
		...
		if (pFilterConfig->SoundActivity.Activation == ENABLE) {
			/* Configure SAD mode, frame size, hysteresis, sound
			   trigger event and data memory transfer only for ADF
			   instance */
			assert_param(
			    IS_MDF_SAD_MODE(pFilterConfig->SoundActivity.Mode));
			assert_param(IS_MDF_SAD_FRAME_SIZE(
			    pFilterConfig->SoundActivity.FrameSize));
			if (pFilterConfig->SoundActivity.Mode !=
			    MDF_SAD_AMBIENT_NOISE_DETECTOR) {
				assert_param(IS_FUNCTIONAL_STATE(
				    pFilterConfig->SoundActivity.Hysteresis));
			}
			assert_param(IS_MDF_SAD_SOUND_TRIGGER(
			    pFilterConfig->SoundActivity.SoundTriggerEvent));
			assert_param(IS_MDF_SAD_DATA_MEMORY_TRANSFER(
			...
			if ((pFilterConfig->SoundActivity.Mode !=
			    ...
			    (pFilterConfig->SoundActivity.Hysteresis ==
			     ENABLE)) {
				hmdf->Instance->SADCR =
				    (pFilterConfig->SoundActivity.Mode |
				     pFilterConfig->SoundActivity.FrameSize |
				     MDF_SADCR_HYSTEN |
				     pFilterConfig->SoundActivity
					 .SoundTriggerEvent |
			...
			/* Configure SAD minimum noise level, hangover window,
			   learning frames, ambient noise slope control and
			   signal noise threshold only for ADF instance */
			assert_param(IS_MDF_SAD_MIN_NOISE_LEVEL(
			    pFilterConfig->SoundActivity.MinNoiseLevel));
			assert_param(IS_MDF_SAD_HANGOVER_WINDOW(
			    pFilterConfig->SoundActivity.HangoverWindow));
			assert_param(IS_MDF_SAD_LEARNING_FRAMES(
			    pFilterConfig->SoundActivity.LearningFrames));
			assert_param(IS_MDF_SAD_SIGNAL_NOISE_THRESHOLD(
			    pFilterConfig->SoundActivity.SignalNoiseThreshold));
			if (pFilterConfig->SoundActivity.Mode !=
			    MDF_SAD_SOUND_DETECTOR) {
				assert_param(IS_MDF_SAD_AMBIENT_NOISE_SLOPE(
				    pFilterConfig->SoundActivity
					.AmbientNoiseSlope));
				hmdf->Instance->SADCFGR =
				    ((pFilterConfig->SoundActivity.MinNoiseLevel
				      << MDF_SADCFGR_ANMIN_Pos) |
				     pFilterConfig->SoundActivity
					 .HangoverWindow |
				     pFilterConfig->SoundActivity
					 .LearningFrames |
				     (pFilterConfig->SoundActivity
					  .AmbientNoiseSlope
				      << MDF_SADCFGR_ANSLP_Pos) |
				     pFilterConfig->SoundActivity
					 .SignalNoiseThreshold);
			} else {
				hmdf->Instance->SADCFGR =
				    ((pFilterConfig->SoundActivity.MinNoiseLevel
				      << MDF_SADCFGR_ANMIN_Pos) |
...
static void MDF_DmaXferCpltCallback(DMA_HandleTypeDef *hdma)
{
	MDF_HandleTypeDef *hmdf =
	    (MDF_HandleTypeDef *)((DMA_HandleTypeDef *)hdma)->Parent;
