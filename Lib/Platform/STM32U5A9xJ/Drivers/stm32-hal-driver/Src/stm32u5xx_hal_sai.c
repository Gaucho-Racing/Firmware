/**
  ...
  ******************************************************************************
  ...
  EXTERNAL_SAI2_CLOCK_VALUE in the stm32u5xx_hal_conf.h file.
...
HAL_StatusTypeDef HAL_SAI_Init(SAI_HandleTypeDef *hsai)
{
	...
	if (hsai->Init.PdmInit.Activation == ENABLE) {
		assert_param(IS_SAI_PDM_MIC_PAIRS_NUMBER(
		    hsai->Init.PdmInit.MicPairsNbr));
		assert_param(
	...
	if (hsai->Init.AudioFrequency != SAI_AUDIO_FREQUENCY_MCKDIV) {
		...
		if (hsai->Init.NoDivider == SAI_MASTERDIVIDER_DISABLE) {
			...
			if (hsai->Init.Protocol == SAI_SPDIF_PROTOCOL) {
				/* For SPDIF protocol, frame length is set by
				 * hardware to 64 */
				tmpframelength = SAI_SPDIF_FRAME_LENGTH;
			} else if (hsai->Init.Protocol == SAI_AC97_PROTOCOL) {
				/* For AC97 protocol, frame length is set by
				 * hardware to 256 */
			...
			tmpval = (freq * 10U) /
				 (hsai->Init.AudioFrequency * tmpframelength);
		} else {
			/* NODIV = 0 */
			uint32_t tmposr;
		...
		/* For SPDIF protocol, SAI shall provide a bit clock twice
		 * faster the symbol-rate */
		if (hsai->Init.Protocol == SAI_SPDIF_PROTOCOL) {
	...
	hsai->Instance->CR1 &=
	    ~(SAI_xCR1_MODE | SAI_xCR1_PRTCFG | SAI_xCR1_DS |
	...
	hsai->Instance->CR2 |=
	    (hsai->Init.FIFOThreshold | hsai->Init.CompandingMode |
	     hsai->Init.TriState);

	/* SAI Frame Configuration -----------------------------------------*/
	hsai->Instance->FRCR &=
	    (~(SAI_xFRCR_FRL | SAI_xFRCR_FSALL | SAI_xFRCR_FSDEF |
...
__weak void HAL_SAI_MspInit(SAI_HandleTypeDef *hsai)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SAI_MspInit could be implemented in the user file
	 */
}
...
__weak void HAL_SAI_MspDeInit(SAI_HandleTypeDef *hsai)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SAI_MspDeInit could be implemented in the user file
	 */
}
				   ...
				   uint16_t Size, uint32_t Timeout)
{
	...
	if (hsai->State == HAL_SAI_STATE_READY) {
		...
		while (hsai->XferCount > 0U) {
			...
			if ((hsai->Instance->SR & SAI_xSR_FLVL) !=
			    SAI_FIFOSTATUS_FULL) {
				if ((hsai->Init.DataSize == SAI_DATASIZE_8) &&
					...
					hsai->pBuffPtr++;
				} else if (hsai->Init.DataSize <=
					   SAI_DATASIZE_16) {
					temp = (uint32_t)(*hsai->pBuffPtr);
					hsai->pBuffPtr++;
					temp |=
					    ((uint32_t)(*hsai->pBuffPtr) << 8);
					hsai->pBuffPtr++;
					...
					hsai->pBuffPtr++;
					temp |=
					    ((uint32_t)(*hsai->pBuffPtr) << 16);
					hsai->pBuffPtr++;
					temp |=
					    ((uint32_t)(*hsai->pBuffPtr) << 24);
					hsai->pBuffPtr++;
					hsai->Instance->DR = temp;
				}
				hsai->XferCount--;
				...
				if ((((HAL_GetTick() - tickstart) > Timeout) ||
					 ...
					 * state update, unlock and error return
...
HAL_StatusTypeDef HAL_SAI_DMAStop(SAI_HandleTypeDef *hsai)
{
	...
	if ((hsai->State == HAL_SAI_STATE_BUSY_TX) && (hsai->hdmatx != NULL)) {
		if (HAL_DMA_Abort(hsai->hdmatx) != HAL_OK) {
			/* If the DMA Tx errorCode is different from DMA No
			 * Transfer then return Error */
	...
	if ((hsai->State == HAL_SAI_STATE_BUSY_RX) && (hsai->hdmarx != NULL)) {
		if (HAL_DMA_Abort(hsai->hdmarx) != HAL_OK) {
			/* If the DMA Rx errorCode is different from DMA No
			 * Transfer then return Error */
...
HAL_StatusTypeDef HAL_SAI_Abort(SAI_HandleTypeDef *hsai)
{
	...
	if ((hsai->Instance->CR1 & SAI_xCR1_DMAEN) == SAI_xCR1_DMAEN) {
		...
		if ((hsai->State == HAL_SAI_STATE_BUSY_TX) &&
		    (hsai->hdmatx != NULL)) {
			if (HAL_DMA_Abort(hsai->hdmatx) != HAL_OK) {
				/* If the DMA Tx errorCode is different from DMA
				 * No Transfer then return Error */
				if (hsai->hdmatx->ErrorCode !=
		...
		if ((hsai->State == HAL_SAI_STATE_BUSY_RX) &&
		    (hsai->hdmarx != NULL)) {
			if (HAL_DMA_Abort(hsai->hdmarx) != HAL_OK) {
				/* If the DMA Rx errorCode is different from DMA
				 * No Transfer then return Error */
				if (hsai->hdmarx->ErrorCode !=
				    HAL_DMA_ERROR_NO_XFER) {
					status = HAL_ERROR;
					hsai->ErrorCode |= HAL_SAI_ERROR_DMA;
				}
				       ...
				       uint16_t Size)
{
	...
	if (hsai->State == HAL_SAI_STATE_READY) {
		...
		/* For transmission, the DMA source is data buffer.
		   We have to compute DMA size of a source block transfer in
		   bytes according SAI data size. */
		if ((hsai->Init.DataSize == SAI_DATASIZE_8) &&
		    (hsai->Init.CompandingMode == SAI_NOCOMPANDING)) {
		...
		if ((hsai->hdmatx->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
			...
			status = HAL_DMA_Start_IT(
			    hsai->hdmatx, (uint32_t)hsai->pBuffPtr,
		...
		__HAL_SAI_ENABLE_IT(hsai,
				    SAI_InterruptFlag(hsai, SAI_MODE_DMA));

		/* Enable SAI Tx DMA Request */
		hsai->Instance->CR1 |= SAI_xCR1_DMAEN;

		/* Wait until FIFO is not empty */
		while ((hsai->Instance->SR & SAI_xSR_FLVL) ==
		       SAI_FIFOSTATUS_EMPTY) {
			/* Check for the Timeout */
				      ...
				      uint16_t Size)
{
	...
	if (hsai->State == HAL_SAI_STATE_READY) {
		...
		if ((hsai->hdmarx->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
			...
			status = HAL_DMA_Start_IT(
			    hsai->hdmarx, (uint32_t)&hsai->Instance->DR,
					   ...
					   uint16_t val)
{
	...
	if (hsai->State != HAL_SAI_STATE_RESET) {
		CLEAR_BIT(hsai->Instance->CR2,
			  SAI_xCR2_MUTEVAL | SAI_xCR2_MUTE);
	...
	if (hsai->State != HAL_SAI_STATE_RESET) {
		...
		/* SAI Overrun error interrupt occurred
		   ----------------------------------*/
			...
			/* the transfer is not stopped, we will forward the
			information to the user and we let the user decide what
			needs to be done */
#if (USE_HAL_SAI_REGISTER_CALLBACKS == 1)
		...
		/* SAI mutedet interrupt occurred
		   ----------------------------------*/
			...
			if (hsai->mutecallback != NULL) {
				/* inform the user that an RX mute event has
				 * been detected */
				hsai->mutecallback();
			}
		}
		/* SAI AFSDET interrupt occurred
		   ----------------------------------*/
			...
			if ((cr1config & SAI_xCR1_DMAEN) == SAI_xCR1_DMAEN) {
				...
				if (hsai->hdmatx != NULL) {
					/* Set the DMA Tx abort callback */
					hsai->hdmatx->XferAbortCallback =
					    SAI_DMAAbort;

					/* Abort DMA in IT mode */
					if (HAL_DMA_Abort_IT(hsai->hdmatx) !=
					    HAL_OK) {
				...
				if (hsai->hdmarx != NULL) {
					/* Set the DMA Rx abort callback */
					hsai->hdmarx->XferAbortCallback =
					    SAI_DMAAbort;

					/* Abort DMA in IT mode */
					if (HAL_DMA_Abort_IT(hsai->hdmarx) !=
					    HAL_OK) {
						/* Update SAI error code */
						hsai->ErrorCode |=
						    HAL_SAI_ERROR_DMA;
		...
		/* SAI WCKCFG interrupt occurred
		   ----------------------------------*/
			...
			if ((cr1config & SAI_xCR1_DMAEN) == SAI_xCR1_DMAEN) {
				...
				/* If WCKCFG occurs, SAI audio block is
				 * automatically disabled */
				/* Disable all interrupts and clear all flags */
				hsai->Instance->IMR = 0U;
				hsai->Instance->CLRFR = 0xFFFFFFFFU;
				/* Set the SAI state to ready to be able to
...
__weak void HAL_SAI_TxCpltCallback(SAI_HandleTypeDef *hsai)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SAI_TxCpltCallback could be implemented in the user
	   file
	 */
}
...
__weak void HAL_SAI_TxHalfCpltCallback(SAI_HandleTypeDef *hsai)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SAI_TxHalfCpltCallback could be implemented in the
	   user file
	 */
}
...
__weak void HAL_SAI_RxCpltCallback(SAI_HandleTypeDef *hsai)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SAI_RxCpltCallback could be implemented in the user
	   file
	 */
}
...
__weak void HAL_SAI_RxHalfCpltCallback(SAI_HandleTypeDef *hsai)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SAI_RxHalfCpltCallback could be implemented in the
	   user file
	 */
}
...
__weak void HAL_SAI_ErrorCallback(SAI_HandleTypeDef *hsai)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SAI_ErrorCallback could be implemented in the user
	   file
	 */
}
...
static void SAI_Transmit_IT8Bit(SAI_HandleTypeDef *hsai)
{
	if (hsai->XferCount == 0U) {
		...
		__HAL_SAI_DISABLE_IT(hsai,
				     SAI_InterruptFlag(hsai, SAI_MODE_IT));
		hsai->State = HAL_SAI_STATE_READY;
#if (USE_HAL_SAI_REGISTER_CALLBACKS == 1)
...
static void SAI_DMAAbort(DMA_HandleTypeDef *hdma)
{
	...
	if (hsai->ErrorCode != HAL_SAI_ERROR_WCKCFG) {
		/* Disable SAI peripheral */
		/* No need to check return value because state will be updated
		 * and HAL_SAI_ErrorCallback will be called later */
		(void)SAI_Disable(hsai);
