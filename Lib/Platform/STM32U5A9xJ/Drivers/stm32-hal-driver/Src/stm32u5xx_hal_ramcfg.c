  ...
  ******************************************************************************
  ...
  ECC interrupt is enabled).
     ...
     *** Wait State feature ***
     ==========================
    [..]
	  (+) Call HAL_RAMCFG_ConfigWaitState() to configure the wait state
  value for a given SRAM according to the HCLK frequency and voltage range
	      scaling.
...
__weak void HAL_RAMCFG_MspInit(RAMCFG_HandleTypeDef *hramcfg)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_RAMCFG_MspInit can be implemented in the user file */
}
...
__weak void HAL_RAMCFG_MspDeInit(RAMCFG_HandleTypeDef *hramcfg)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_RAMCFG_MspDeInit can be implemented in the user file
	 */
}
/**
 * @}
...
HAL_StatusTypeDef HAL_RAMCFG_StartECC(RAMCFG_HandleTypeDef *hramcfg)
{
	...
	if (hramcfg->State == HAL_RAMCFG_STATE_READY) {
		...
		if ((hramcfg->Instance->CR & RAMCFG_CR_ECCE) !=
		    RAMCFG_CR_ECCE) {
			/* Start the SRAM ECC mechanism and latching the error
			 * address */
			hramcfg->Instance->CR |=
			    (RAMCFG_CR_ECCE | RAMCFG_CR_ALE);
		} else {
...
HAL_StatusTypeDef HAL_RAMCFG_StopECC(RAMCFG_HandleTypeDef *hramcfg)
{
	...
	if (hramcfg->State == HAL_RAMCFG_STATE_READY) {
		...
		if ((hramcfg->Instance->CR & RAMCFG_CR_ECCE) ==
			...
			/* Stop the SRAM ECC mechanism and latching the error
			 * address */
			hramcfg->Instance->CR &=
			    ~(RAMCFG_CR_ECCE | RAMCFG_CR_ALE);
		}
...
@verbatim
 ...
 ===============================================================================
    [..]
      This section provides functions allowing to get and configure the wait
state value.
    [..]
      The HAL_RAMCFG_ConfigWaitState() function allows configuring the wait
state value. The HAL_RAMCFG_GetWaitState() function allows getting the current
wait state value.
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_RAMCFG_ConfigWaitState(RAMCFG_HandleTypeDef *hramcfg,
					     uint32_t WaitState)
{
	HAL_StatusTypeDef status = HAL_OK;
	/* Check the parameters */
...
HAL_StatusTypeDef HAL_RAMCFG_Erase(RAMCFG_HandleTypeDef *hramcfg)
{
	...
	if (hramcfg->State == HAL_RAMCFG_STATE_READY) {
		...
		/*
		   Wait for the SRAM hardware erase operation to complete by
		   polling on SRAMBUSY flag to be reset.
		*/
		while (__HAL_RAMCFG_GET_FLAG(hramcfg, RAMCFG_FLAG_SRAMBUSY) !=
		       0U) {
			if ((HAL_GetTick() - tickstart) >
			    RAMCFG_TIMEOUT_VALUE) {
				/* Update the RAMCFG error code */
				hramcfg->ErrorCode = HAL_RAMCFG_ERROR_TIMEOUT;

				/* Update the RAMCFG state and return error
				 * status */
...
void HAL_RAMCFG_IRQHandler(RAMCFG_HandleTypeDef *hramcfg)
{
	...
	if (__HAL_RAMCFG_GET_IT_SOURCE(hramcfg, RAMCFG_IT_SINGLEERR) != 0U) {
		if (__HAL_RAMCFG_GET_FLAG(hramcfg, RAMCFG_FLAG_SINGLEERR) !=
		    0U) {
			/* Clear active flags */
	...
	if (__HAL_RAMCFG_GET_IT_SOURCE(hramcfg, RAMCFG_IT_DOUBLEERR) != 0U) {
		if (__HAL_RAMCFG_GET_FLAG(hramcfg, RAMCFG_FLAG_DOUBLEERR) !=
		    0U) {
			/* Clear active flags */
...
__weak void HAL_RAMCFG_DetectSingleErrorCallback(RAMCFG_HandleTypeDef *hramcfg)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_RAMCFG_DetectSingleErrorCallback can be implemented
	   in the user file. */
}
...
__weak void HAL_RAMCFG_DetectDoubleErrorCallback(RAMCFG_HandleTypeDef *hramcfg)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_RAMCFG_DetectDoubleErrorCallback can be implemented
	   in the user file. */
}
