/**
  ...
  ******************************************************************************
  ...
  ==============================================================================
    [..]
      (#) Peripheral control is exclusively done by the accompanying middleware
  library.

     *** Interrupt mode IO operation ***
     ===================================
     [..]
       (#) Configure the GPU2D hardware to perform graphics operation using the
  Third Party MW Library APIs.
       (#) Submit command List to the hardware.
       (#) Wait indefinitely for the completion of submitted Command List by
  GPU2D hardware.
       (#) Use HAL_GPU2D_IRQHandler() called under GPU2D_IRQHandler() interrupt
  subroutine.
       (#) At the end of Command List execution HAL_GPU2D_IRQHandler() function
  is executed and user can add his own function by customization of function
  pointer
       (#) CommandListCpltCallback (member of GPU2D handle structure) to notify
  the upper level about the completion of Command List execution.

     (#) Callback HAL_GPU2D_CommandListCpltCallback is invoked when the GPU2D
  hardware executes the programmed command list (Command List execution
  completion).

    (++) This callback is called when the compilation defines
  USE_HAL_GPU2D_REGISTER_CALLBACKS is set to 0 or not defined.

	(++) This callback should be implemented in the application side. It
  should notify the upper level that the programmed command list is completed.

      (#) To control the GPU2D state, use the following function:
  HAL_GPU2D_GetState().

      (#) To read the GPU2D error code, use the following function:
  HAL_GPU2D_GetError().
     ...
     *** GPU2D HAL driver macros list ***
     ...
     [..]
      ...
      (+) __HAL_GPU2D_GET_IT_SOURCE: Check whether the specified GPU2D interrupt
  is enabled or not.
     ...
     *** Callback registration ***
     ...
     [..]
	  ...
	  Use function @ref HAL_GPU2D_RegisterCallback() to register a user
  callback.

      (#) Function @ref HAL_GPU2D_RegisterCallback() allows to register
  following callbacks:
  ...
  Callback ID and a pointer to the user callback function.
      ...
      (#) Use function @ref HAL_GPU2D_UnRegisterCallback() to reset a callback
  to the default weak (surcharged) function.
	  @ref HAL_GPU2D_UnRegisterCallback() takes as parameters the HAL
  ...
  HAL_GPU2D_RegisterCallback before calling @ref HAL_GPU2D_DeInit or @ref
  HAL_GPU2D_Init function.

	  When The compilation define USE_HAL_GPU2D_REGISTER_CALLBACKS is set to
  0 or not defined, the callback registering feature is not available and weak
  (surcharged) callbacks are used.

     [..]
      (@) You can refer to the GPU2D HAL driver header file for more useful
  macros
...
/** @defgroup GPU2D_Exported_Functions_Group1 Initialization and
...
__weak void HAL_GPU2D_MspInit(GPU2D_HandleTypeDef *hgpu2d)
{
	...
	/* NOTE : This function should not be modified; when the callback is
	   needed, the HAL_GPU2D_MspInit can be implemented in the user file.
	 */
}
...
__weak void HAL_GPU2D_MspDeInit(GPU2D_HandleTypeDef *hgpu2d)
{
	...
	/* NOTE : This function should not be modified; when the callback is
	   needed, the HAL_GPU2D_MspDeInit can be implemented in the user file.
	 */
}
 ...
 * @note Weak predefined callback is defined for HAL_GPU2D_MSPINIT_CB_ID and
 ...
 *         GPU2D Callback is redirected to the weak (surcharged) predefined
 * callback
 * @param hgpu2d GPU2D handle
 ...
 *          @arg @ref HAL_GPU2D_MSPINIT_CB_ID GPU2D MspInit callback ID
 *          @arg @ref HAL_GPU2D_MSPDEINIT_CB_ID GPU2D MspDeInit callback ID
 * @note Callback pointers will be set to legacy weak predefined callbacks for
 ...
 *         GPU2D Command List Complete Callback is redirected to the weak
 * (surcharged) predefined callback
 * @param hgpu2d GPU2D handle
...
uint32_t HAL_GPU2D_ReadRegister(GPU2D_HandleTypeDef *hgpu2d, uint32_t offset)
{
	...
	/* No locking is required since reading a register is an atomic
	 * operation and doesn't incur a state change in hal_gpu2d. */
...
/**
 * @brief  Write a value to GPU2D Register. Helper function for the higher-level
 * library.
 * @param  hgpu2d    Pointer to a GPU2D_HandleTypeDef structure that contains
 *                   the configuration information for the GPU2D.
					  ...
					  uint32_t offset, uint32_t value)
{
	...
	/* No locking is required since writing a register is an atomic
	 * operation and doesn't incur a state change in hal_gpu2d. */
					      ...
					      uint32_t CmdListID)
{
	...
	/* NOTE : This function should not be modified; when the callback is
	   needed, the HAL_GPU2D_CommandListCpltCallback can be implemented in
	   the user file.
	 */
}
...
__weak void HAL_GPU2D_ErrorCallback(GPU2D_HandleTypeDef *hgpu2d)
{
	...
	/* NOTE : This function should not be modified; when the callback is
	   needed, the HAL_GPU2D_ErrorCallback can be implemented in the user
	   file. The default implementation stops the execution as an error is
...
/** @defgroup GPU2D_Exported_Functions_Group3 Peripheral State and Error
functions
