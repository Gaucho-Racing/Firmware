				   ...
				   uint32_t DeassertionTime)
{
	...
	/* Enable the Driver Enable mode by setting the DEM bit in the CR3
	 * register */
...
__weak void HAL_UARTEx_RxFifoFullCallback(UART_HandleTypeDef *huart)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_UARTEx_RxFifoFullCallback can be implemented in the
	   user file.
	 */
}
...
__weak void HAL_UARTEx_TxFifoEmptyCallback(UART_HandleTypeDef *huart)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_UARTEx_TxFifoEmptyCallback can be implemented in the
	   user file.
	 */
}
		...
		*RxLen = 0U;
		...
		while (huart->RxXferCount > 0U) {
			...
			if (__HAL_UART_GET_FLAG(huart, UART_FLAG_IDLE)) {
				...
				/* If Set, but no data ever received, clear flag
				 * without exiting loop */
				/* If Set, and data has already been received,
					      ...
					      uint8_t *pData, uint16_t Size)
{
	...
	if (huart->RxState == HAL_UART_STATE_READY) {
		...
		if (huart->ReceptionType == HAL_UART_RECEPTION_TOIDLE) {
			...
			/* In case of errors already pending when reception is
			   started, Interrupts may have already been raised and
			   lead to reception abortion. (Overrun error for
			   instance). In such case Reception Type has been reset
			   to HAL_UART_RECEPTION_STANDARD. */
			status = HAL_ERROR;
		}
		...
		if (status == HAL_OK) {
			if (huart->ReceptionType == HAL_UART_RECEPTION_TOIDLE) {
				...
				/* In case of errors already pending when
				   ...
				   already been raised and lead to reception
				   abortion. (Overrun error for instance). In
				   such case Reception Type has been reset to
    ...
    UART_HandleTypeDef *huart, const UART_AutonomousModeConfTypeDef *sConfig)
{
	...
	if (huart->gState == HAL_UART_STATE_READY) {
		...
		tmpreg =
		    ((sConfig->DataSize << USART_AUTOCR_TDN_Pos) |
				   ...
				   UART_AutonomousModeConfTypeDef *sConfig)
{
	...
	sConfig->TriggerSelection =
	    ((tmpreg & USART_AUTOCR_TRIGSEL) >> USART_AUTOCR_TRIGSEL_Pos);
