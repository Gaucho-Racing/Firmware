 ...
 * @param  hspi Pointer to a SPI_HandleTypeDef structure that contains
 *                the configuration information for the specified SPIx
 * peripheral.
 * @param  sConfig Pointer to a SPI_HandleTypeDef structure that contains
 *                the configuration information of the autonomous mode for the
...
HAL_StatusTypeDef HAL_SPIEx_ClearConfigAutonomousMode(SPI_HandleTypeDef *hspi)
{
	if (hspi->State == HAL_SPI_STATE_READY) {
		...
		/* Disable the selected SPI peripheral to be able to clear
		 * AUTOCR */
		__HAL_SPI_DISABLE(hspi);
