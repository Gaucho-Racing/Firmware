...
__weak void HAL_ICACHE_InvalidateCompleteCallback(void)
{
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_ICACHE_InvalidateCompleteCallback() should be
...
{
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_ICACHE_ErrorCallback() should be implemented in the
	   user file
	 */
}
    ...
    uint32_t Region, const ICACHE_RegionConfigTypeDef *const pRegionConfig)
{
	...
	if (READ_BIT(ICACHE->CR, ICACHE_CR_EN) != 0U) {
		...
		if ((*p_reg & ICACHE_CRRx_REN) != 0U) {
			 ...
			 * size 10 bits */
			/* Region 8MB:   BaseAddress size 6 bits, RemapAddress
			 * size 9 bits  */
			/* Region 16MB:  BaseAddress size 5 bits, RemapAddress
			 * size 8 bits  */
			/* Region 32MB:  BaseAddress size 4 bits, RemapAddress
			 * size 7 bits  */
			/* Region 64MB:  BaseAddress size 3 bits, RemapAddress
			 * size 6 bits  */
