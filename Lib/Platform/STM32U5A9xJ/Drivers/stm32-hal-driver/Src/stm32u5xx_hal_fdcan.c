...
#define SRAMCAN_TEFSA                                                          \
	((uint32_t)(SRAMCAN_RF1SA +                                            \
		    (SRAMCAN_RF1_NBR * SRAMCAN_RF1_SIZE))) /* Tx Event FIFO    \
							      Start Address */
#define SRAMCAN_TFQSA                                                          \
	((uint32_t)(SRAMCAN_TEFSA +                                            \
		    (SRAMCAN_TEF_NBR * SRAMCAN_TEF_SIZE))) /* Tx FIFO/Queue    \
							      Start Address */
#define SRAMCAN_SIZE                                                           \
...
HAL_StatusTypeDef HAL_FDCAN_Init(FDCAN_HandleTypeDef *hfdcan)
{
	...
	if (hfdcan->Init.FrameFormat == FDCAN_FRAME_FD_BRS) {
		assert_param(
		    IS_FDCAN_DATA_PRESCALER(hfdcan->Init.DataPrescaler));
	...
	*/
	...
	/* If FD operation with BRS is selected, set the data bit timing
	 * register */
	if (hfdcan->Init.FrameFormat == FDCAN_FRAME_FD_BRS) {
...
__weak void HAL_FDCAN_MspInit(FDCAN_HandleTypeDef *hfdcan)
{
	...
	/* NOTE: This function Should not be modified, when the callback is
	   needed, the HAL_FDCAN_MspInit could be implemented in the user file
	 */
}
...
__weak void HAL_FDCAN_MspDeInit(FDCAN_HandleTypeDef *hfdcan)
{
	...
	UNUSED(hfdcan);
	/* NOTE: This function Should not be modified, when the callback is
	   needed, the HAL_FDCAN_MspDeInit could be implemented in the user file
	 */
}
...
HAL_FDCAN_UnRegisterTxEventFifoCallback(FDCAN_HandleTypeDef *hfdcan)
{
	...
	if (hfdcan->State == HAL_FDCAN_STATE_READY) {
		hfdcan->TxEventFifoCallback =
		    HAL_FDCAN_TxEventFifoCallback; /* Legacy weak
						      TxEventFifoCallback  */
	} else {
		/* Update the error code */
		hfdcan->ErrorCode |= HAL_FDCAN_ERROR_INVALID_CALLBACK;
...
HAL_FDCAN_UnRegisterErrorStatusCallback(FDCAN_HandleTypeDef *hfdcan)
{
	...
	if (hfdcan->State == HAL_FDCAN_STATE_READY) {
		hfdcan->ErrorStatusCallback =
		    HAL_FDCAN_ErrorStatusCallback; /* Legacy weak
						      ErrorStatusCallback  */
	} else {
		/* Update the error code */
		hfdcan->ErrorCode |= HAL_FDCAN_ERROR_INVALID_CALLBACK;
		       ...
		       const FDCAN_FilterTypeDef *sFilterConfig)
{
	...
	if ((state == HAL_FDCAN_STATE_READY) ||
		...
		if (sFilterConfig->IdType == FDCAN_STANDARD_ID) {
			...
			assert_param(IS_FDCAN_MAX_VALUE(
			    sFilterConfig->FilterID1, 0x7FFU));
			assert_param(IS_FDCAN_MAX_VALUE(
			    sFilterConfig->FilterID2, 0x7FFU));
			assert_param(IS_FDCAN_STD_FILTER_TYPE(
		...
		} else /* sFilterConfig->IdType == FDCAN_EXTENDED_ID */
		{
			...
			assert_param(IS_FDCAN_MAX_VALUE(
			    sFilterConfig->FilterID1, 0x1FFFFFFFU));
			assert_param(IS_FDCAN_MAX_VALUE(
			    sFilterConfig->FilterID2, 0x1FFFFFFFU));
			assert_param(IS_FDCAN_EXT_FILTER_TYPE(
			    sFilterConfig->FilterType));

			/* Build first word of filter element */
			FilterElementW1 =
			    ((sFilterConfig->FilterConfig << 29U) |
					      ...
					      uint32_t CounterStartValue)
{
	...
	if (hfdcan->State == HAL_FDCAN_STATE_READY) {
		...
		MODIFY_REG(hfdcan->Instance->RWD, FDCAN_RWD_WDC,
			   CounterStartValue);
						   ...
						   uint32_t TimestampPrescaler)
{
	...
	if (hfdcan->State == HAL_FDCAN_STATE_READY) {
		/* Configure prescaler */
		MODIFY_REG(hfdcan->Instance->TSCC, FDCAN_TSCC_TCP,
			   TimestampPrescaler);
						   ...
						   uint32_t TimestampOperation)
{
	...
	if (hfdcan->State == HAL_FDCAN_STATE_READY) {
		...
		MODIFY_REG(hfdcan->Instance->TSCC, FDCAN_TSCC_TSS,
			   TimestampOperation);
						 ...
						 uint32_t TimeoutPeriod)
{
	...
	if (hfdcan->State == HAL_FDCAN_STATE_READY) {
		...
		MODIFY_REG(
		    hfdcan->Instance->TOCC, (FDCAN_TOCC_TOS | FDCAN_TOCC_TOP),
...
HAL_StatusTypeDef HAL_FDCAN_Stop(FDCAN_HandleTypeDef *hfdcan)
{
	...
	if (hfdcan->State == HAL_FDCAN_STATE_BUSY) {
		...
		/* Wait until FDCAN exits sleep mode */
		while ((hfdcan->Instance->CCCR & FDCAN_CCCR_CSA) ==
		       FDCAN_CCCR_CSA) {
			/* Check for the Timeout */
			if (Counter > FDCAN_TIMEOUT_VALUE) {
				/* Update error code */
					 ...
					 uint8_t *pRxData)
{
	...
	if (state == HAL_FDCAN_STATE_BUSY) {
		if (RxLocation == FDCAN_RX_FIFO0) /* Rx element is assigned to
						     the Rx FIFO 0 */
		{
			...
			if ((hfdcan->Instance->RXF0S & FDCAN_RXF0S_F0FL) ==
				...
				/* Check that the Rx FIFO 0 is full & overwrite
				 * mode is on */
				if (((hfdcan->Instance->RXF0S &
				      FDCAN_RXF0S_F0F) >>
				     FDCAN_RXF0S_F0F_Pos) == 1U) {
					if (((hfdcan->Instance->RXGFC &
					      FDCAN_RXGFC_F0OM) >>
					     FDCAN_RXGFC_F0OM_Pos) ==
					    FDCAN_RX_FIFO_OVERWRITE) {
						/* When overwrite status is on
						 * discard first message in FIFO
						 */
						GetIndex = 1U;
					}
				}
				...
				GetIndex += ((hfdcan->Instance->RXF0S &
					      FDCAN_RXF0S_F0GI) >>
		...
		} else /* Rx element is assigned to the Rx FIFO 1 */
		{
			...
			if ((hfdcan->Instance->RXF1S & FDCAN_RXF1S_F1FL) ==
			    0U) {
				/* Update error code */
				hfdcan->ErrorCode |= HAL_FDCAN_ERROR_FIFO_EMPTY;
				...
				/* Check that the Rx FIFO 1 is full & overwrite
				 * mode is on */
				if (((hfdcan->Instance->RXF1S &
				      FDCAN_RXF1S_F1F) >>
				...
				GetIndex += ((hfdcan->Instance->RXF1S &
					      FDCAN_RXF1S_F1GI) >>
		...
		/* Retrieve Identifier */
		if (pRxHeader->IdType ==
		    FDCAN_STANDARD_ID) /* Standard ID element */
		{
		...
		{
			pRxHeader->Identifier =
			    (*RxAddress & FDCAN_ELEMENT_MASK_EXTID);
		}
		...
		pRxHeader->ErrorStateIndicator =
		    (*RxAddress & FDCAN_ELEMENT_MASK_ESI);
		...
		/* Retrieve DataLength */
		pRxHeader->DataLength =
		    ((*RxAddress & FDCAN_ELEMENT_MASK_DLC) >> 16U);
		...
		for (ByteCounter = 0;
		     ByteCounter < DLCtoBytes[pRxHeader->DataLength];
		     ByteCounter++) {
		...
		if (RxLocation == FDCAN_RX_FIFO0) /* Rx element is assigned to
						     the Rx FIFO 0 */
		{
			/* Acknowledge the Rx FIFO 0 that the oldest element is
			 * read so that it increments the GetIndex */
			hfdcan->Instance->RXF0A = GetIndex;
		} else /* Rx element is assigned to the Rx FIFO 1 */
		{
			/* Acknowledge the Rx FIFO 1 that the oldest element is
			 * read so that it increments the GetIndex */
				       ...
				       FDCAN_TxEventFifoTypeDef *pTxEvent)
{
	...
	if (state == HAL_FDCAN_STATE_BUSY) {
		...
		{
			pTxEvent->Identifier =
			    ((*TxEventAddress & FDCAN_ELEMENT_MASK_STDID) >>
			     18U);
		} else /* Extended ID element */
		{
			pTxEvent->Identifier =
			    (*TxEventAddress & FDCAN_ELEMENT_MASK_EXTID);
		}

		/* Retrieve TxFrameType */
		pTxEvent->TxFrameType =
		    (*TxEventAddress & FDCAN_ELEMENT_MASK_RTR);

		/* Retrieve ErrorStateIndicator */
		pTxEvent->ErrorStateIndicator =
		    (*TxEventAddress & FDCAN_ELEMENT_MASK_ESI);

		/* Increment TxEventAddress pointer to second word of Tx Event
		 * FIFO element */
		TxEventAddress++;

		/* Retrieve TxTimestamp */
		pTxEvent->TxTimestamp =
		    (*TxEventAddress & FDCAN_ELEMENT_MASK_TS);
		...
		/* Retrieve BitRateSwitch */
		pTxEvent->BitRateSwitch =
		    (*TxEventAddress & FDCAN_ELEMENT_MASK_BRS);
		...
		/* Acknowledge the Tx Event FIFO that the oldest element is read
		 * so that it increments the GetIndex */
			    ...
			    FDCAN_ProtocolStatusTypeDef *ProtocolStatus)
{
	...
	ProtocolStatus->Warning =
	    ((StatusReg & FDCAN_PSR_EW) >> FDCAN_PSR_EW_Pos);
	ProtocolStatus->BusOff =
	    ((StatusReg & FDCAN_PSR_BO) >> FDCAN_PSR_BO_Pos);
	...
	if ((state == HAL_FDCAN_STATE_READY) ||
		...
		if ((((ActiveITs & FDCAN_IT_LIST_RX_FIFO0) != 0U) &&
		    ...
		    (((ActiveITs & FDCAN_IT_LIST_TX_FIFO_ERROR) != 0U) &&
		     (((ITs_lines_selection)&FDCAN_IT_GROUP_TX_FIFO_ERROR) ==
		      0U)) ||
		    (((ActiveITs & FDCAN_IT_LIST_MISC) != 0U) &&
		     (((ITs_lines_selection)&FDCAN_IT_GROUP_MISC) == 0U)) ||
		    (((ActiveITs & FDCAN_IT_LIST_BIT_LINE_ERROR) != 0U) &&
		     (((ITs_lines_selection)&FDCAN_IT_GROUP_BIT_LINE_ERROR) ==
		      0U)) ||
		    (((ActiveITs & FDCAN_IT_LIST_PROTOCOL_ERROR) != 0U) &&
		     (((ITs_lines_selection)&FDCAN_IT_GROUP_PROTOCOL_ERROR) ==
		      0U))) {
		...
		if ((ActiveITs & FDCAN_IT_TX_COMPLETE) != 0U) {
			/* Enable Tx Buffer Transmission Interrupt to set TC
			   flag in IR register, but interrupt will only occur if
			   TC is enabled in IE register */
			SET_BIT(hfdcan->Instance->TXBTIE, BufferIndexes);
		}

		if ((ActiveITs & FDCAN_IT_TX_ABORT_COMPLETE) != 0U) {
			/* Enable Tx Buffer Cancellation Finished Interrupt to
			   set TCF flag in IR register, but interrupt will only
			   occur if TCF is enabled in IE register */
						   ...
						   uint32_t InactiveITs)
{
	...
	if ((state == HAL_FDCAN_STATE_READY) ||
		...
		/* Check if some interrupts are still enabled on interrupt line
		 * 0 */
...
void HAL_FDCAN_IRQHandler(FDCAN_HandleTypeDef *hfdcan)
{
	 ...
	 * *******************************/
	if (FDCAN_CHECK_FLAG(itflag, FDCAN_FLAG_RX_HIGH_PRIORITY_MSG) !=
	    RESET) {
		if (FDCAN_CHECK_IT_SOURCE(
			itsource, FDCAN_IT_RX_HIGH_PRIORITY_MSG) != RESET) {
			/* Clear the High Priority Message flag */
			__HAL_FDCAN_CLEAR_FLAG(hfdcan,
	...
	if (FDCAN_CHECK_FLAG(itflag, FDCAN_FLAG_TX_ABORT_COMPLETE) != RESET) {
		if (FDCAN_CHECK_IT_SOURCE(
			itsource, FDCAN_IT_TX_ABORT_COMPLETE) != RESET) {
	...
	if (FDCAN_CHECK_FLAG(itflag, FDCAN_FLAG_TX_FIFO_EMPTY) != RESET) {
		if (FDCAN_CHECK_IT_SOURCE(itsource, FDCAN_IT_TX_FIFO_EMPTY) !=
		    RESET) {
	...
	if (FDCAN_CHECK_FLAG(itflag, FDCAN_FLAG_TX_COMPLETE) != RESET) {
		if (FDCAN_CHECK_IT_SOURCE(itsource, FDCAN_IT_TX_COMPLETE) !=
		    RESET) {
...
#if USE_HAL_FDCAN_REGISTER_CALLBACKS == 1
			/* Call registered callback*/
			hfdcan->TxBufferCompleteCallback(hfdcan,
							 TransmittedBuffers);
#else
			/* Transmission Complete Callback */
			HAL_FDCAN_TxBufferCompleteCallback(hfdcan,
							   TransmittedBuffers);
#endif /* USE_HAL_FDCAN_REGISTER_CALLBACKS */
		}
	}
	 ...
	 * ********************************/
	if (FDCAN_CHECK_FLAG(itflag, FDCAN_FLAG_TIMESTAMP_WRAPAROUND) !=
	    RESET) {
		if (FDCAN_CHECK_IT_SOURCE(
			itsource, FDCAN_IT_TIMESTAMP_WRAPAROUND) != RESET) {
	...
	if (FDCAN_CHECK_FLAG(itflag, FDCAN_FLAG_TIMEOUT_OCCURRED) != RESET) {
		if (FDCAN_CHECK_IT_SOURCE(itsource,
					  FDCAN_IT_TIMEOUT_OCCURRED) != RESET) {
	...
	if (FDCAN_CHECK_FLAG(itflag, FDCAN_FLAG_RAM_ACCESS_FAILURE) != RESET) {
		if (FDCAN_CHECK_IT_SOURCE(
			itsource, FDCAN_IT_RAM_ACCESS_FAILURE) != RESET) {
					  ...
					  uint32_t TxEventFifoITs)
{
	...
	/* NOTE: This function Should not be modified, when the callback is
	   needed, the HAL_FDCAN_TxEventFifoCallback could be implemented in the
	   user file
	 */
}
				      ...
				      uint32_t RxFifo0ITs)
{
	...
	/* NOTE: This function Should not be modified, when the callback is
	   needed, the HAL_FDCAN_RxFifo0Callback could be implemented in the
	   user file
	 */
}
				      ...
				      uint32_t RxFifo1ITs)
{
	...
	/* NOTE: This function Should not be modified, when the callback is
	   needed, the HAL_FDCAN_RxFifo1Callback could be implemented in the
	   user file
	 */
}
...
__weak void HAL_FDCAN_TxFifoEmptyCallback(FDCAN_HandleTypeDef *hfdcan)
{
	...
	/* NOTE: This function Should not be modified, when the callback is
	   needed, the HAL_FDCAN_TxFifoEmptyCallback could be implemented in the
	   user file
	 */
}
					       ...
					       uint32_t BufferIndexes)
{
	...
	/* NOTE: This function Should not be modified, when the callback is
	   needed, the HAL_FDCAN_TxBufferCompleteCallback could be implemented
	   in the user file
	 */
}
					    ...
					    uint32_t BufferIndexes)
{
	...
	/* NOTE: This function Should not be modified, when the callback is
	   needed, the HAL_FDCAN_TxBufferAbortCallback could be implemented in
	   the user file
	 */
}
...
__weak void HAL_FDCAN_TimestampWraparoundCallback(FDCAN_HandleTypeDef *hfdcan)
{
	...
	/* NOTE: This function Should not be modified, when the callback is
	   needed, the HAL_FDCAN_TimestampWraparoundCallback could be
...
__weak void HAL_FDCAN_TimeoutOccurredCallback(FDCAN_HandleTypeDef *hfdcan)
{
	...
	/* NOTE: This function Should not be modified, when the callback is
	   needed, the HAL_FDCAN_TimeoutOccurredCallback could be implemented in
	   the user file
	 */
}
...
__weak void HAL_FDCAN_HighPriorityMessageCallback(FDCAN_HandleTypeDef *hfdcan)
{
	...
	/* NOTE: This function Should not be modified, when the callback is
	   needed, the HAL_FDCAN_HighPriorityMessageCallback could be
	   implemented in the user file
	 */
}
...
__weak void HAL_FDCAN_ErrorCallback(FDCAN_HandleTypeDef *hfdcan)
{
	...
	/* NOTE: This function Should not be modified, when the callback is
	   needed, the HAL_FDCAN_ErrorCallback could be implemented in the user
	   file
	 */
}
					  ...
					  uint32_t ErrorStatusITs)
{
	...
	/* NOTE: This function Should not be modified, when the callback is
	   needed, the HAL_FDCAN_ErrorStatusCallback could be implemented in the
	   user file
	 */
}
				   ...
				   const uint8_t *pTxData, uint32_t BufferIndex)
{
	...
	if (pTxHeader->IdType == FDCAN_STANDARD_ID) {
		TxElementW1 =
		    (pTxHeader->ErrorStateIndicator | FDCAN_STANDARD_ID |
		     pTxHeader->TxFrameType | (pTxHeader->Identifier << 18U));
	} else /* pTxHeader->IdType == FDCAN_EXTENDED_ID */
	{
		TxElementW1 =
		    (pTxHeader->ErrorStateIndicator | FDCAN_EXTENDED_ID |
	...
	TxElementW2 =
	    ((pTxHeader->MessageMarker << 24U) | pTxHeader->TxEventFifoControl |
	     pTxHeader->FDFormat | pTxHeader->BitRateSwitch |
