  ...
  ******************************************************************************
  ...
  [..]
   ...
   (#) Call HAL_PWREx_EnableUSBHSTranceiverSupply() and
       ...
       (+) This feature is available only for STM32U59xxx, STM32U5Axxx,
  STM32U5Fxxx and STM32U5Gxxx devices

   (#) Call HAL_PWREx_EnableOTGHSPHYLowPowerRetention() and
       HAL_PWREx_DisableOTGHSPHYLowPowerRetention() to enable / disable OTG_HS
  PHY power during low power modes (Stop2, Stop3 and Standby).
       (+) This feature is available only for STM32U59xxx, STM32U5Axxx,
  STM32U5Fxxx and STM32U5Gxxx devices
  ...
  STM32U5Fxxx and STM32U5Gxxx devices
   ...
   (#) Call HAL_PWREx_EnableRAMsContentStopRetention() and
       ...
       (+) Retained RAM can be one of the following RAMs :
	     (++) SRAM1
	     (++) SRAM2
	     (++) SRAM3 (available only for STM32U575xx, STM32U585xx,
  STM32U59xxx, STM32U5Axxx, STM32U5Fxxx and STM32U5Gxxx devices)
	     (++) SRAM4
	     (++) SRAM5 (available only for STM32U59xxx, STM32U5Axxx,
		  STM32U5Fxxx and STM32U5Gxxx devices)
	     (++) SRAM6 (available only for STM32U5Fxxx and STM32U5Gxxx devices)
	     (++) ICACHE
	     (++) DMA2DRAM (available only for STM32U575xx, STM32U585xx,
  STM32U59xxx, STM32U5Axxx, STM32U5Fxxx and STM32U5Gxxx devices)
	     (++) PKA32RAM
	     (++) DCACHE1
	     (++) FMAC
...
HAL_StatusTypeDef HAL_PWREx_ControlVoltageScaling(uint32_t VoltageScaling)
{
	...
	/*
	 *  The Embedded power distribution (EPOD) must be enabled before
	 * switching to voltage scale 1 / 2 from voltage scale lower.
	 */
	if (VoltageScaling > PWR_REGULATOR_VOLTAGE_SCALE3) {
		MODIFY_REG(PWR->VOSR, (PWR_VOSR_VOS | PWR_VOSR_BOOSTEN),
			   (VoltageScaling | PWR_VOSR_BOOSTEN));
	} else {
		MODIFY_REG(PWR->VOSR, (PWR_VOSR_VOS | PWR_VOSR_BOOSTEN),
			   VoltageScaling);
	}
	...
	timeout =
	    ((PWR_FLAG_SETTING_DELAY * (SystemCoreClock / 1000U)) / 1000U) + 1U;
	...
	if (timeout != 0U) {
		...
		timeout =
		    ((PWR_FLAG_SETTING_DELAY * (SystemCoreClock / 1000U)) /
		     1000U) +
		    1U;
 ...
 *                        system startup.
 *                        This parameter can be one of the following values :
 *                        @arg PWR_LDO_SUPPLY  : The LDO regulator supplies the
 * Vcore Power Domains.
 *                        @arg PWR_SMPS_SUPPLY : The SMPS regulator supplies the
 * Vcore Power Domains.
 * @retval HAL Status.
 */
  ...
  *** Shutdown mode ***
   ====================
    [..]
      ...
      (+) Entry:
	  The Shutdown mode is entered by using the
HAL_PWREx_EnterSHUTDOWNMode() function.
...
__weak void HAL_PWREx_S3WUCallback(uint32_t WakeUpPin)
{
	...
	/* NOTE : This function should not be modified; when the callback is
	   needed, HAL_PWREx_S3WU_IRQHandler() API can be implemented in the
	   user file
	 */
}
/**
 ...
 ===============================================================================
    ...
    *** VBAT charging ***
    ...
    [..]
      ...
      The VBAT charging is done either through a 5 kOhm resistor or through
a 1.5 kOhm resistor depending on the VBRS bit value in the PWR_BDCR2 register.
    ...
    *** Backup domain battery ***
    ...
    [..]
      ...
      (+) After exiting reset, the USB Type-C (dead battery) behavior is
...
HAL_StatusTypeDef HAL_PWREx_ConfigPVM(PWR_PVMTypeDef *pConfigPVM)
{
	...
	switch (pConfigPVM->PVMType) {
		...
		case PWR_AVM1: /* VDDA Independent analog supply voltage monitor
				  1 (1.6V threshold) */
		...
		case PWR_AVM2: /* VDDA Independent analog supply voltage monitor
				  2 (1.8V threshold) */
...
/**
 * @brief  Enable OTG_HS PHY power during low power modes (Stop2, Stop3 and
 * Standby).
 * @retval None.
 */
...
/**
 * @brief  Disable OTG_HS PHY power during low power modes (Stop2, Stop3 and
 * Standby).
 * @retval None.
 */
...
{
	/* NOTE : This function should not be modified; when the callback is
	   needed, HAL_PWREx_UVMCallback() API can be implemented in the user
	   file
	 */
}
...
{
	/* NOTE : This function should not be modified; when the callback is
	   needed, HAL_PWREx_IO2VMCallback() API can be implemented in the user
	   file
	 */
}
...
{
	/* NOTE : This function should not be modified; when the callback is
	   needed, HAL_PWREx_AVM1Callback() API can be implemented in the user
	   file
	 */
}
...
{
	/* NOTE : This function should not be modified; when the callback is
	   needed, HAL_PWREx_AVM2Callback() API can be implemented in the user
	   file
	 */
}
/**
 ...
 *                      @arg PWR_SRAM2_FULL_STANDBY  : SRAM2 page 1 and page 2
 * retention.
 * @retval None.
 */
...
void HAL_PWREx_EnableRAMsContentStopRetention(uint32_t RAMSelection)
{
	...
	switch (RAMSelection & SRAM_ID_MASK) {
		...
		case SRAM2_ID: {
			...
			/* Calculate pages mask */
			dummy = (RAMSelection & PWR_SRAM2_FULL_STOP) &
				~SRAM_ID_MASK;
		...
		case SRAM3_ID: {
			...
			/* Calculate pages mask */
			dummy = (RAMSelection & ~SRAM_ID_MASK) &
				(PAGE01_ID | PAGE02_ID | PAGE03_ID | PAGE04_ID |
		...
		case ICACHERAM_ID: {
			/* Check the parameters */
			assert_param(
			    IS_PWR_ICACHE_STOP_RETENTION(RAMSelection));

			/* Calculate pages mask */
			dummy = (RAMSelection & PWR_ICACHE_FULL_STOP) &
				~SRAM_ID_MASK;
		...
		case DCACHE1RAM_ID: {
			/* Check the parameters */
			assert_param(
			    IS_PWR_DCACHE1_STOP_RETENTION(RAMSelection));

			/* Calculate pages mask */
			dummy = (RAMSelection & PWR_DCACHE1_FULL_STOP) &
				~SRAM_ID_MASK;
		...
		case DMA2DRAM_ID: {
			/* Check the parameters */
			assert_param(
			    IS_PWR_DMA2DRAM_STOP_RETENTION(RAMSelection));

			/* Calculate pages mask */
			dummy = (RAMSelection & PWR_DMA2DRAM_FULL_STOP) &
				~SRAM_ID_MASK;
		...
		case PERIPHRAM_ID: {
			/* Check the parameters */
			assert_param(
			    IS_PWR_PERIPHRAM_STOP_RETENTION(RAMSelection));

			/* Calculate pages mask */
			dummy = (RAMSelection & PWR_PERIPHRAM_FULL_STOP) &
				~SRAM_ID_MASK;
		...
		case PKARAM_ID: {
			/* Check the parameters */
			assert_param(
			    IS_PWR_PKA32RAM_STOP_RETENTION(RAMSelection));

			/* Calculate pages mask */
			dummy = (RAMSelection & PWR_PKA32RAM_FULL_STOP) &
				~SRAM_ID_MASK;
		...
		case DCACHE2RAM_ID: {
			/* Check the parameters */
			assert_param(
			    IS_PWR_DCACHE2_STOP_RETENTION(RAMSelection));

			/* Calculate pages mask */
			dummy = (RAMSelection & PWR_DCACHE2_FULL_STOP) &
				~SRAM_ID_MASK;
		...
		case GRAPHIPRAM_ID: {
			/* Check the parameters */
			assert_param(
			    IS_PWR_GRAPHICPRAM_STOP_RETENTION(RAMSelection));

			/* Calculate pages mask */
			dummy = (RAMSelection & PWR_GRAPHICPRAM_FULL_STOP) &
				~SRAM_ID_MASK;
		...
		case DSIRAM_ID: {
			/* Check the parameters */
			assert_param(
			    IS_PWR_DSIRAM_STOP_RETENTION(RAMSelection));

			/* Calculate pages mask */
			dummy = (RAMSelection & PWR_DSIRAM_FULL_STOP) &
				~SRAM_ID_MASK;
		...
		case JPEGRAM_ID: {
			/* Check the parameters */
			assert_param(
			    IS_PWR_JPEGRAM_STOP_RETENTION(RAMSelection));

			/* Calculate pages mask */
			dummy = (RAMSelection & PWR_JPEGRAM_FULL_STOP) &
				~SRAM_ID_MASK;
		...
		case SRAM5_ID: {
			...
			/* Calculate pages mask */
			dummy = ((RAMSelection & PWR_SRAM5_FULL_STOP) &
				 ~SRAM_ID_MASK);
		...
		case SRAM6_ID: {
			...
			/* Calculate pages mask */
			dummy = ((RAMSelection & PWR_SRAM6_FULL_STOP) &
				 ~SRAM_ID_MASK);
...
/**
  ...
  * @param  RAMSelection : Specifies the SRAM content to be retained in Run
  mode.
  *                        This parameter can be one or a combination of the
  following values :
  *                        @arg PWR_SRAM1_FULL_RUN : SRAM1 full content
  retention.
  *                        @arg PWR_SRAM2_FULL_RUN : SRAM2 full content
  retention.
  *                        @arg PWR_SRAM3_FULL_RUN : SRAM3 full content
  retention (available only for STM32U575xx,
  *                                                             STM32U585xx
  STM32U59xxx, STM32U5Axxx, STM32U5Fxxx and STM32U5Gxxx devices).
  *                        @arg PWR_SRAM4_FULL_RUN : SRAM4 full content
  retention.
  *                        @arg PWR_SRAM5_FULL_RUN : SRAM5 full content
  retention (available only for
  *                                                            STM32U59xxx,
  STM32U5Axxx, STM32U5Fxxx
  *                                                            and STM32U5Gxxx
  devices).
  *                        @arg PWR_SRAM6_FULL_RUN : SRAM6 full content
  retention (available only for
  *                                                            STM32U5Fxxx and
  STM32U5Gxxx devices).
  * @retval None.
...
/**
  ...
  *                        @arg PWR_SRAM3_FULL_RUN : SRAM3 full content lost
  (available only for STM32U575xx,
  *                                                             STM32U585xx
  STM32U59xxx, STM32U5Axxx, STM32U5Fxxx and STM32U5Gxxx devices).
 ...
 * modes.
 * @note   This bit is used to obtain the best trade-off between low-power
 * consumption and wakeup time. SRAM4 wakeup time increases the wakeup time when
 * exiting Stop 0, Stop 1 and Stop 2 modes, and also increases the LPDMA access
...
/** @defgroup PWREx_Exported_Functions_Group5 I/O Pull-Up Pull-Down
...
@verbatim
 ...
 ===============================================================================
    [..]
      ...
      PWR_PDCRx registers (x=A,B,C,D,E,F,G,H,I,J)), or can be kept in analog
state if none of the PWR_PUCRx or PWR_PDCRx register is set.
					     ...
					     uint32_t GPIO_Pin)
{
	...
	switch (GPIO_Port) {
		...
		case PWR_GPIO_I: /* Apply Pull Up to GPIO port I */
			SET_BIT(PWR->PUCRI,
				(GPIO_Pin & PWR_PORTI_AVAILABLE_PINS));
			CLEAR_BIT(PWR->PDCRI,
				  (GPIO_Pin & PWR_PORTI_AVAILABLE_PINS));
			break;
#endif /* PWR_PUCRI_PU0 */
		...
		case PWR_GPIO_J: /* Apply Pull Up to GPIO port J */
			SET_BIT(PWR->PUCRJ,
				(GPIO_Pin & PWR_PORTJ_AVAILABLE_PINS));
			CLEAR_BIT(PWR->PDCRJ,
				  (GPIO_Pin & PWR_PORTJ_AVAILABLE_PINS));
			break;
#endif /* defined (PWR_PUCRJ_PU0) */
					       ...
					       uint32_t GPIO_Pin)
{
	...
	switch (GPIO_Port) {
		case PWR_GPIO_A: /* Apply Pull Down to GPIO port A */
			SET_BIT(PWR->PDCRA, (GPIO_Pin & (~(PWR_GPIO_BIT_13 |
							   PWR_GPIO_BIT_15))));
			CLEAR_BIT(PWR->PUCRA,
				  (GPIO_Pin & (~(PWR_GPIO_BIT_14))));
			break;
