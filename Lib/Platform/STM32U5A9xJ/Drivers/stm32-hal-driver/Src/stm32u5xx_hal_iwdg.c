...
HAL_StatusTypeDef HAL_IWDG_Init(IWDG_HandleTypeDef *hiwdg)
{
	...
	/* Enable write access to IWDG_PR, IWDG_RLR, IWDG_WINR, IWDG_EWCR
	registers by writing 0x5555 in KR */
	...
	if (hiwdg->Init.EWI == IWDG_EWI_DISABLE) {
		/* EWI comparator value equal 0, disable the early wakeup
		 * interrupt acknowledge the early wakeup interrupt in any
		 * cases. it clears the EWIF flag in SR register Set Watchdog
		 * Early Wakeup Comparator to 0x00 */
		hiwdg->Instance->EWCR = IWDG_EWCR_EWIC;
	} else {
		/* EWI comparator value different from 0, enable the early
...
__weak void HAL_IWDG_MspInit(IWDG_HandleTypeDef *hiwdg)
{
	...
	/* NOTE: This function should not be modified, when the callback is
	   needed, the HAL_IWDG_MspInit could be implemented in the user file
	 */
}
 ...
 *         IWDG Callback is redirected to the weak (overridden) predefined
 * callback
 * @param  hiwdg IWDG handle
 * @param  CallbackID ID of the callback to be registered
 *         This parameter can be one of the following values:
...
__weak void HAL_IWDG_EarlyWakeupCallback(IWDG_HandleTypeDef *hiwdg)
{
	...
	/* NOTE: This function should not be modified, when the callback is
	   needed, the HAL_IWDG_EarlyWakeupCallback could be implemented in the
	   user file
	 */
}
