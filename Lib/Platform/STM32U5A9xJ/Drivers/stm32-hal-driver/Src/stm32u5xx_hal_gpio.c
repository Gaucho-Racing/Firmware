...
/**
 * @brief  Initialize the GPIOx/LPGPIOx peripheral according to the specified
 * parameters in the pGPIO_Init.
 * @note   If GPIOx/LPGPIOx peripheral pin is used in EXTI_MODE and the pin is
 * secure in case the system implements the security (TZEN=1), it is up to the
 * secure application to insure that the corresponding EXTI line is set secure.
 * @param  GPIOx or LPGPIOx: where x can be (A..I) for the GPIO and (1) for
 * LPGPIO to select the corresponding peripheral for STM32U5 family
	...
	while (((pGPIO_Init->Pin) >> position) != 0U) {
		...
		if (iocurrent != 0U) {
			...
			if (GPIOx == LPGPIO1) {
				...
				tmp |= ((pGPIO_Init->Mode & GPIO_MODE_OUTPUT_PP)
					<< position);
				GPIOx->MODER = tmp;
				...
				tmp &=
				    ~(GPIO_MODER_MODE0
				      << (pin_position * GPIO_MODER_MODE1_Pos));
				tmp |=
				    ((GPIO_MODE_AF_PP & 0x0FUL)
				     << (pin_position * GPIO_MODER_MODE1_Pos));
				p_gpio->MODER = tmp;
			...
			if ((pGPIO_Init->Mode == GPIO_MODE_OUTPUT_PP) ||
				...
				tmp = p_gpio->OSPEEDR;
				tmp &= ~(GPIO_OSPEEDR_OSPEED0
					 << (pin_position *
					     GPIO_OSPEEDR_OSPEED1_Pos));
				tmp |= (pGPIO_Init->Speed
					<< (pin_position *
					    GPIO_OSPEEDR_OSPEED1_Pos));
				p_gpio->OSPEEDR = tmp;
				...
				tmp |=
				    (((pGPIO_Init->Mode & GPIO_OUTPUT_TYPE) >>
				      4U)
				     << pin_position);
				p_gpio->OTYPER = tmp;
			}
			...
			if ((pGPIO_Init->Mode != GPIO_MODE_ANALOG) ||
			    ((pGPIO_Init->Mode == GPIO_MODE_ANALOG) &&
				...
				tmp &=
				    ~(GPIO_PUPDR_PUPD0
				      << (pin_position * GPIO_PUPDR_PUPD1_Pos));
				tmp |=
				    ((pGPIO_Init->Pull)
				     << (pin_position * GPIO_PUPDR_PUPD1_Pos));
				p_gpio->PUPDR = tmp;
			...
			if ((pGPIO_Init->Mode & EXTI_MODE) == EXTI_MODE) {
				...
				tmp &= ~((0x0FUL) << (EXTI_EXTICR1_EXTI1_Pos *
						      (position & 0x03U)));
				...
				if ((pGPIO_Init->Mode & RISING_EDGE) ==
				    RISING_EDGE) {
					tmp |= iocurrent;
				}
				EXTI->RTSR1 = tmp;
				...
				tmp = EXTI->FTSR1;
				tmp &= ~((uint32_t)iocurrent);
				if ((pGPIO_Init->Mode & FALLING_EDGE) ==
				    FALLING_EDGE) {
					tmp |= iocurrent;
				}
				EXTI->FTSR1 = tmp;
				...
				tmp = EXTI->EMR1;
				tmp &= ~((uint32_t)iocurrent);
				if ((pGPIO_Init->Mode & GPIO_MODE_EVT) ==
				    GPIO_MODE_EVT) {
					tmp |= iocurrent;
				}
				EXTI->EMR1 = tmp;
				...
				tmp = EXTI->IMR1;
				tmp &= ~((uint32_t)iocurrent);
				if ((pGPIO_Init->Mode & GPIO_MODE_IT) ==
				    GPIO_MODE_IT) {
					tmp |= iocurrent;
				}
				EXTI->IMR1 = tmp;
 ...
 * values.
 * @param  GPIOx or LPGPIOx: where x can be (A..I) for the GPIO and (1) for
 * LPGPIO to select the the corresponding peripheral for STM32U5 family
	...
	while ((GPIO_Pin >> position) != 0U) {
		...
		if (iocurrent != 0U) {
			...
			if (GPIOx == LPGPIO1) {
				...
				/*------------------------- EXTI Mode
				 * Configuration --------------------*/
				/* Clear the External Interrupt or Event for the
				 * current IO */
				tmp = EXTI->EXTICR[position >> 2U];
			 ...
			 * IO */
			p_gpio->AFR[pin_position >> 3U] &=
			    ~(0x0FUL << ((pin_position & 0x07U) *
					 GPIO_AFRL_AFSEL1_Pos));

			/* Configure the default value for IO Speed */
			p_gpio->OSPEEDR &=
			    ~(GPIO_OSPEEDR_OSPEED0
			      << (pin_position * GPIO_OSPEEDR_OSPEED1_Pos));
 ...
 * @note   This function uses GPIOx_BSRR/LPGPIOx_BSRR and GPIOx_BRR/LPGPIOx_BRR
 * registers to allow atomic read/modify accesses. In this way, there is no risk
				    ...
				    uint16_t PinSet)
{
	...
	/* Check the parameters */
	/* Make sure at least one parameter is different from zero and that
	 * there is no common pin */
	...
	while ((GPIO_Pin >> position) != 0U) {
		...
		if (iocurrent != 0U) {
			...
			if (GPIOx == LPGPIO1) {
				...
				p_gpio = LPGPIO_Map[position].GPIO_PORT;
				pin_position =
				    (1UL << (LPGPIO_Map[position].Pin_Pos));
			...
			/* Set LCKx bit(s): LCKK='1' + LCK[15-0] */
			p_gpio->LCKR = tmp;
			/* Read LCKK register. This read is mandatory to
			 * complete key lock sequence */
			tmp = p_gpio->LCKR;
...
__weak void HAL_GPIO_EXTI_Rising_Callback(uint16_t GPIO_Pin)
{
	...
	/* NOTE: This function should not be modified, when the callback is
	   needed, the HAL_GPIO_EXTI_Rising_Callback could be implemented in the
	   user file
	 */
}
...
__weak void HAL_GPIO_EXTI_Falling_Callback(uint16_t GPIO_Pin)
{
	...
	/* NOTE: This function should not be modified, when the callback is
	   needed, the HAL_GPIO_EXTI_Falling_Callback could be implemented in
	   the user file
	 */
}
				  ...
				  uint32_t PinAttributes)
{
	...
	while ((GPIO_Pin >> position) != 0U) {
		...
		if (iocurrent != 0U) {
			...
			MODIFY_REG(p_gpio->SECCFGR,
				   (GPIO_SECCFGR_SEC0 << pin_position),
						  ...
						  uint32_t *pPinAttributes)
{
	...
	while ((GPIO_Pin >> position) != 0U) {
		...
		if (iocurrent != 0U) {
			...
			if ((p_gpio->SECCFGR &
			     (GPIO_SECCFGR_SEC0 << pin_position)) != 0U) {
