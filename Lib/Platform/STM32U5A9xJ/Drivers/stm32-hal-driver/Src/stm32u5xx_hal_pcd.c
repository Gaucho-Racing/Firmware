...
__weak void HAL_PCD_MspInit(PCD_HandleTypeDef *hpcd)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_PCD_MspInit could be implemented in the user file
	 */
}
...
__weak void HAL_PCD_MspDeInit(PCD_HandleTypeDef *hpcd)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_PCD_MspDeInit could be implemented in the user file
	 */
}
...
HAL_PCD_UnRegisterDataOutStageCallback(PCD_HandleTypeDef *hpcd)
{
	...
	if (hpcd->State == HAL_PCD_STATE_READY) {
		hpcd->DataOutStageCallback =
		    HAL_PCD_DataOutStageCallback; /* Legacy weak
...
HAL_StatusTypeDef HAL_PCD_UnRegisterDataInStageCallback(PCD_HandleTypeDef *hpcd)
{
	...
	if (hpcd->State == HAL_PCD_STATE_READY) {
		hpcd->DataInStageCallback =
		    HAL_PCD_DataInStageCallback; /* Legacy weak
						    DataInStageCallback  */
	} else {
		/* Update the error code */
		hpcd->ErrorCode |= HAL_PCD_ERROR_INVALID_CALLBACK;
...
HAL_StatusTypeDef HAL_PCD_UnRegisterBcdCallback(PCD_HandleTypeDef *hpcd)
{
	...
	if (hpcd->State == HAL_PCD_STATE_READY) {
		hpcd->BCDCallback =
		    HAL_PCDEx_BCD_Callback; /* Legacy weak
...
HAL_StatusTypeDef HAL_PCD_UnRegisterLpmCallback(PCD_HandleTypeDef *hpcd)
{
	...
	if (hpcd->State == HAL_PCD_STATE_READY) {
		hpcd->LPMCallback =
		    HAL_PCDEx_LPM_Callback; /* Legacy weak
...
void HAL_PCD_IRQHandler(PCD_HandleTypeDef *hpcd)
{
	...
	if (USB_GetMode(hpcd->Instance) == USB_OTG_MODE_DEVICE) {
		...
		hpcd->FrameNumber =
		    (USBx_DEVICE->DSTS & USB_OTG_DSTS_FNSOF_Msk) >>
		...
		if (__HAL_PCD_GET_FLAG(hpcd, USB_OTG_GINTSTS_RXFLVL)) {
			...
			if (((RegVal & USB_OTG_GRXSTSP_PKTSTS) >> 17) ==
			    STS_DATA_UPDT) {
				...
				if ((ep_intr & 0x1U) != 0U) {
					...
					if ((epint & USB_OTG_DOEPINT_XFRC) ==
						...
						CLEAR_OUT_EP_INTR(
						    epnum,
						    USB_OTG_DOEPINT_XFRC);
						(void)PCD_EP_OutXfrComplete_int(
						    hpcd, epnum);
					}
					...
					if ((epint & USB_OTG_DOEPINT_STUP) ==
						...
						CLEAR_OUT_EP_INTR(
						    epnum,
						    USB_OTG_DOEPINT_STUP);
						/* Class B setup phase done for
						 * previous decoded setup */
						(void)PCD_EP_OutSetupPacket_int(
						    hpcd, epnum);
					}
		...
		if (__HAL_PCD_GET_FLAG(hpcd, USB_OTG_GINTSTS_BOUTNAKEFF)) {
			...
			for (epnum = 1U; epnum < hpcd->Init.dev_endpoints;
			     epnum++) {
				if (hpcd->OUT_ep[epnum].is_iso_incomplete ==
				    1U) {
					/* disable the EP */
		...
		if (__HAL_PCD_GET_FLAG(hpcd, USB_OTG_GINTSTS_IISOIXFR)) {
			for (epnum = 1U; epnum < hpcd->Init.dev_endpoints;
				...
				if ((hpcd->IN_ep[epnum].type == EP_TYPE_ISOC) &&
					...
					hpcd->IN_ep[epnum].is_iso_incomplete =
					    1U;

					/* Abort current transaction and disable
					 * the EP */
		...
		if (__HAL_PCD_GET_FLAG(hpcd, USB_OTG_GINTSTS_OTGINT)) {
			...
			if ((RegVal & USB_OTG_GOTGINT_SEDET) ==
...
void HAL_PCD_IRQHandler(PCD_HandleTypeDef *hpcd)
{
	...
	if ((wIstr & USB_ISTR_L1REQ) == USB_ISTR_L1REQ) {
		...
		if (hpcd->LPM_State == LPM_L0) {
			/* Force suspend and low-power mode before going to L1
			 * state*/
			hpcd->Instance->CNTR |= USB_CNTR_SUSPRDY;
			hpcd->Instance->CNTR |= USB_CNTR_SUSPEN;
...
__weak void HAL_PCD_DataOutStageCallback(PCD_HandleTypeDef *hpcd, uint8_t epnum)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_PCD_DataOutStageCallback could be implemented in the
	   user file
	 */
}
...
__weak void HAL_PCD_DataInStageCallback(PCD_HandleTypeDef *hpcd, uint8_t epnum)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_PCD_DataInStageCallback could be implemented in the
	   user file
	 */
}
/**
...
__weak void HAL_PCD_SetupStageCallback(PCD_HandleTypeDef *hpcd)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_PCD_SetupStageCallback could be implemented in the
	   user file
	 */
}
...
__weak void HAL_PCD_SOFCallback(PCD_HandleTypeDef *hpcd)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_PCD_SOFCallback could be implemented in the user file
	 */
}
...
__weak void HAL_PCD_ResetCallback(PCD_HandleTypeDef *hpcd)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_PCD_ResetCallback could be implemented in the user
	   file
	 */
}
...
__weak void HAL_PCD_SuspendCallback(PCD_HandleTypeDef *hpcd)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_PCD_SuspendCallback could be implemented in the user
	   file
	 */
}
...
__weak void HAL_PCD_ResumeCallback(PCD_HandleTypeDef *hpcd)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_PCD_ResumeCallback could be implemented in the user
	   file
	 */
}
					     ...
					     uint8_t epnum)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_PCD_ISOOUTIncompleteCallback could be implemented in
	   the user file
	 */
}
					    ...
					    uint8_t epnum)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_PCD_ISOINIncompleteCallback could be implemented in
	   the user file
	 */
}
...
__weak void HAL_PCD_ConnectCallback(PCD_HandleTypeDef *hpcd)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_PCD_ConnectCallback could be implemented in the user
	   file
	 */
}
						   ...
						   uint32_t epnum)
{
	...
	if (hpcd->Init.dma_enable == 1U) {
		...
		} else if ((DoepintReg & (USB_OTG_DOEPINT_STUP |
					  USB_OTG_DOEPINT_OTEPSPR)) == 0U) {
			/* StupPktRcvd = 1 this is a setup packet */
			if ((gSNPSiD > USB_OTG_CORE_ID_300A) &&
			    ((DoepintReg & USB_OTG_DOEPINT_STPKTRX) ==
			     USB_OTG_DOEPINT_STPKTRX)) {
				...
				ep->xfer_count = ep->xfer_size -
						 (USBx_OUTEP(epnum)->DOEPTSIZ &
						  USB_OTG_DOEPTSIZ_XFRSIZ);

				if (epnum == 0U) {
					if (ep->xfer_len == 0U) {
						/* this is ZLP, so prepare EP0
						 * for next setup */
						(void)USB_EP0_OutStart(
						    hpcd->Instance, 1U,
						    (uint8_t *)hpcd->Setup);
...
#if (USE_HAL_PCD_REGISTER_CALLBACKS == 1U)
				hpcd->DataOutStageCallback(hpcd,
							   (uint8_t)epnum);
#else
	...
	while ((hpcd->Instance->ISTR & USB_ISTR_CTR) != 0U) {
		...
		if (epindex == 0U) {
			...
			if ((wIstr & USB_ISTR_DIR) == 0U) {
				 ...
				 */
				ep = &hpcd->OUT_ep[0];
				wEPVal = (uint16_t)PCD_GET_ENDPOINT(
				    hpcd->Instance, PCD_ENDP0);

				if ((wEPVal & USB_EP_SETUP) != 0U) {
					/* Get SETUP Packet */
					ep->xfer_count = PCD_GET_EP_RX_CNT(
					    hpcd->Instance, ep->num);

					if (ep->xfer_count != 8U) {
						/* Set Stall condition for EP0
						 * IN/OUT */
			...
			wEPVal =
			    (uint16_t)PCD_GET_ENDPOINT(hpcd->Instance, epindex);
			...
			if ((wEPVal & USB_EP_VTRX) != 0U) {
				...
				if (ep->doublebuffer == 0U) {
					count = (uint16_t)PCD_GET_EP_RX_CNT(
					    hpcd->Instance, ep->num);
			...
			if ((wEPVal & USB_EP_VTTX) != 0U) {
				...
				if (ep->type == EP_TYPE_ISOC) {
					...
					if ((wEPVal & USB_EP_KIND) == 0U) {
						/* Multi-packet on the NON
						 * control IN endpoint */
						TxPctSize =
						    (uint16_t)PCD_GET_EP_TX_CNT(
							hpcd->Instance,
							ep->num);
...
#if (USE_USB_DOUBLE_BUFFER == 1U)
					/* Double Buffer bulk IN (bulk transfer
					   Len > Ep_Mps) */
					else {
						(void)HAL_PCD_EP_DB_Transmit(
						    hpcd, ep, wEPVal);
					}
#endif /* (USE_USB_DOUBLE_BUFFER == 1U) */
				}
				      ...
				      PCD_EPTypeDef *ep, uint16_t wEPVal)
{
	...
	if ((wEPVal & USB_EP_DTOG_RX) != 0U) {
		...
		if (ep->xfer_len == 0U) {
			/* Set NAK to OUT endpoint since double buffer is
			 * enabled */
		...
		if (count != 0U) {
			USB_ReadPMA(hpcd->Instance, ep->xfer_buff, ep->pmaaddr0,
				    count);
		}
	}
	/* Manage Buffer 1 DTOG_RX=0 */
		...
		if (count != 0U) {
			USB_ReadPMA(hpcd->Instance, ep->xfer_buff, ep->pmaaddr1,
				    count);
		}
	}
						...
						uint16_t wEPVal)
{
	...
	if ((wEPVal & USB_EP_DTOG_TX) != 0U) {
		...
		TxPctSize =
		    (uint16_t)PCD_GET_EP_DBUF0_CNT(hpcd->Instance, ep->num);
		...
		if (ep->xfer_len == 0U) {
			PCD_SET_EP_DBUF0_CNT(hpcd->Instance, ep->num, ep->is_in,
					     0U);
			PCD_SET_EP_DBUF1_CNT(hpcd->Instance, ep->num, ep->is_in,
					     0U);
			...
			if ((wEPVal & USB_EP_DTOG_RX) != 0U) {
				PCD_FREE_USER_BUFFER(hpcd->Instance, ep->num,
						     1U);
			}
		...
		} else /* Transfer is not yet Done */
		{
			...
			if (ep->xfer_fill_db == 1U) {
				...
				USB_WritePMA(hpcd->Instance, ep->xfer_buff,
					     ep->pmaaddr0, (uint16_t)len);
			}
		}
	} else /* Data Buffer1 ACK received */
	{
		/* multi-packet on the NON control IN endpoint */
		TxPctSize =
		    (uint16_t)PCD_GET_EP_DBUF1_CNT(hpcd->Instance, ep->num);
