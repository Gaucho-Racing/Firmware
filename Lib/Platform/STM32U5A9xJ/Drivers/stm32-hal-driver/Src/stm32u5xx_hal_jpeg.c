...
#define JPEG_DC_HUFF_TABLE_SIZE                                                \
	((uint32_t)12) /* Huffman DC table size : 12 codes*/

#define JPEG_QUANTVAL_MAX                                                      \
	((uint32_t)255) /* Quantization values are 8-bit numbers*/
...
#define JPEG_FIFO_TH_SIZE                                                      \
	((uint32_t)4U) /* JPEG Input/Output HW FIFO Threshold in words*/

#define JPEG_DMA_MASK ((uint32_t)0x00001800) /* JPEG DMA request Mask*/
#define JPEG_DMA_IDMA                                                          \
	((uint32_t)JPEG_CR_IDMAEN) /* DMA request for the input FIFO */
#define JPEG_DMA_ODMA                                                          \
	((uint32_t)JPEG_CR_ODMAEN) /* DMA request for the output FIFO */

#define JPEG_INTERRUPT_MASK ((uint32_t)0x0000007EU) /* JPEG Interrupt Mask*/

#define JPEG_CONTEXT_ENCODE                                                    \
	((uint32_t)0x00000001) /* JPEG context : operation is encoding*/
#define JPEG_CONTEXT_DECODE                                                    \
	((uint32_t)0x00000002) /* JPEG context : operation is decoding*/
...
#define JPEG_CONTEXT_IT                                                        \
	((uint32_t)0x00000008) /* JPEG context : Transfer use Interrupt */
...
#define JPEG_GET_DMA_REMAIN_DATA(__HANDLE__)                                   \
...
typedef struct {
	 ...
	 */
	uint8_t Bits[16]; /*!< bits[k] = # of symbols with codes of length k
			    bits, this parameter corresponds to BITS list in the
			    Annex C */
...
/*note  : To disable a DMA request we must use MODIFY_REG macro to avoid writing
   "1" to the FIFO flush bits located in the same DMA request enable register
   (CR register). */
...
/* The following Huffman tables, are based on the JPEG standard as defined by
 * the ITU-T Recommendation T.81. */
...
static HAL_StatusTypeDef JPEG_Bits_To_SizeCodes(const uint8_t *Bits,
						uint8_t *Huffsize,
						uint32_t *Huffcode,
						uint32_t *LastK);
...
JPEG_Set_HuffDC_Mem(const JPEG_HandleTypeDef *hjpeg,
		    JPEG_DCHuffTableTypeDef *HuffTableDC,
		    const __IO uint32_t *DCTableAddress);
static HAL_StatusTypeDef
JPEG_Set_HuffAC_Mem(const JPEG_HandleTypeDef *hjpeg,
		    JPEG_ACHuffTableTypeDef *HuffTableAC,
		    const __IO uint32_t *ACTableAddress);
static HAL_StatusTypeDef JPEG_Set_HuffEnc_Mem(JPEG_HandleTypeDef *hjpeg);
static void JPEG_Set_Huff_DHTMem(const JPEG_HandleTypeDef *hjpeg);
static uint32_t JPEG_Set_Quantization_Mem(const JPEG_HandleTypeDef *hjpeg,
...
HAL_StatusTypeDef HAL_JPEG_Init(JPEG_HandleTypeDef *hjpeg)
{
	...
	if (hjpeg->State == HAL_JPEG_STATE_RESET) {
		...
		hjpeg->InfoReadyCallback =
		    HAL_JPEG_InfoReadyCallback; /* Legacy weak InfoReadyCallback
						 */
		...
		hjpeg->GetDataCallback =
		    HAL_JPEG_GetDataCallback; /* Legacy weak GetDataCallback */
		hjpeg->DataReadyCallback =
		    HAL_JPEG_DataReadyCallback; /* Legacy weak DataReadyCallback
						 */
...
__weak void HAL_JPEG_MspInit(JPEG_HandleTypeDef *hjpeg)
{
	...
	/* NOTE : This function Should not be modified, when the callback is
	   needed, the HAL_JPEG_MspInit could be implemented in the user file
	 */
}
...
__weak void HAL_JPEG_MspDeInit(JPEG_HandleTypeDef *hjpeg)
{
	...
	/* NOTE : This function Should not be modified, when the callback is
	   needed, the HAL_JPEG_MspDeInit could be implemented in the user file
	 */
}
 ...
 */
HAL_StatusTypeDef
HAL_JPEG_RegisterCallback(JPEG_HandleTypeDef *hjpeg,
 ...
 */
HAL_StatusTypeDef
HAL_JPEG_UnRegisterCallback(JPEG_HandleTypeDef *hjpeg,
			    HAL_JPEG_CallbackIDTypeDef CallbackID)
{
	HAL_StatusTypeDef status = HAL_OK;
 ...
 */
HAL_StatusTypeDef
HAL_JPEG_RegisterInfoReadyCallback(JPEG_HandleTypeDef *hjpeg,
 ...
 */
HAL_StatusTypeDef
HAL_JPEG_UnRegisterInfoReadyCallback(JPEG_HandleTypeDef *hjpeg)
{
	HAL_StatusTypeDef status = HAL_OK;
 ...
 */
HAL_StatusTypeDef
HAL_JPEG_RegisterGetDataCallback(JPEG_HandleTypeDef *hjpeg,
 ...
 */
HAL_StatusTypeDef
HAL_JPEG_RegisterDataReadyCallback(JPEG_HandleTypeDef *hjpeg,
 ...
 */
HAL_StatusTypeDef
HAL_JPEG_UnRegisterDataReadyCallback(JPEG_HandleTypeDef *hjpeg)
{
	HAL_StatusTypeDef status = HAL_OK;
					  ...
					  const JPEG_ConfTypeDef *pConf)
{
	...
	if ((hjpeg == NULL) || (pConf == NULL)) {
		...
		assert_param(
		    IS_JPEG_CHROMASUBSAMPLING(pConf->ChromaSubsampling));
		...
		if (hjpeg->State == HAL_JPEG_STATE_READY) {
			...
			if (hjpeg->Conf.ColorSpace ==
				...
				error = JPEG_Set_Quantization_Mem(
				    hjpeg, hjpeg->QuantTable0,
				    (hjpeg->Instance->QMEM0));
			} else if (hjpeg->Conf.ColorSpace ==
				   JPEG_YCBCR_COLORSPACE) {
				/*
				   Set the Color Space for YCbCr : 2
				   quantization tables are used one for
				   Luminance(Y) and one for both Chrominances
				   (Cb & Cr)
				  */
				hjpeg->Instance->CONFR1 |=
				...
				error = JPEG_Set_Quantization_Mem(
				    hjpeg, hjpeg->QuantTable0,
				    (hjpeg->Instance->QMEM0));
				/*By default quantization table 0 for component
				 * 0 and quantization table 1 for both
				 * components 1 and 2*/
				(void)JPEG_Set_Quantization_Mem(
				    hjpeg, hjpeg->QuantTable1,
				    (hjpeg->Instance->QMEM1));

				if ((hjpeg->Context &
				     JPEG_CONTEXT_CUSTOM_TABLES) != 0UL) {
					/*Use user customized quantization
					 * tables , 1 table per component*/
					/* use 3 quantization tables , one for
					 * each component*/
					hjpeg->Instance->CONFR1 &=
					...
					/*Use Quantization 2 table for component
					 * 2*/
			...
			} else /* ColorSpace == JPEG_CMYK_COLORSPACE */
			{
				...
				error = JPEG_Set_Quantization_Mem(
				    hjpeg, hjpeg->QuantTable0,
				    (hjpeg->Instance->QMEM0));
				/*By default quantization table 0 for All
				 * components*/
				...
				if ((hjpeg->Context &
					...
					/*Use user customized quantization
					 * tables , 1 table per component*/
					/* use 4 quantization tables , one for
					 * each component*/
					hjpeg->Instance->CONFR1 |=
					    JPEG_CONFR1_COLORSPACE;
					...
					/*Use Quantization 3 table for component
					 * 3*/
					hjpeg->Instance->CONFR7 |=
					    JPEG_CONFR7_QT;
				}
			}
			...
			MODIFY_REG(
			    hjpeg->Instance->CONFR1, JPEG_CONFR1_YSIZE,
			    ((hjpeg->Conf.ImageHeight & 0x0000FFFFUL) << 16));
			/* set the number of pixels per line*/
			MODIFY_REG(
			    hjpeg->Instance->CONFR3, JPEG_CONFR3_XSIZE,
			...
			numberMCU =
			    (hMCU * vMCU) - 1UL; /* Bit Field JPEG_CONFR2_NMCU
						    shall be set to NB_MCU - 1*/
			/* Set the number of MCU*/
			hjpeg->Instance->CONFR2 =
			    (numberMCU & JPEG_CONFR2_NMCU);
				   ...
				   JPEG_ConfTypeDef *pInfo)
{
	...
	} else if ((hjpeg->Instance->CONFR1 & JPEG_CONFR1_NF) ==
		   JPEG_CONFR1_NF) {
	...
	if ((pInfo->ColorSpace == JPEG_YCBCR_COLORSPACE) ||
		...
		if ((yblockNb == 1UL) && (cBblockNb == 0UL) &&
			...
			pInfo->ChromaSubsampling =
			    JPEG_422_SUBSAMPLING; /*16x8 block*/
...
HAL_StatusTypeDef HAL_JPEG_Encode(JPEG_HandleTypeDef *hjpeg,
				  uint8_t *pDataInMCU, uint32_t InDataLength,
				  ...
				  uint32_t Timeout)
{
	...
	if (hjpeg->State == HAL_JPEG_STATE_READY) {
		if ((hjpeg->Context & JPEG_CONTEXT_CONF_ENCODING) ==
			...
			hjpeg->Context |=
			    (JPEG_CONTEXT_ENCODE | JPEG_CONTEXT_POLLING);
			...
			hjpeg->InDataLength =
			    InDataLength -
			    (InDataLength %
			     4UL); /* In Data length must be multiple
				      of 4 Bytes (1 word)*/
			hjpeg->OutDataLength =
			    OutDataLength -
			    (OutDataLength %
				  ...
				  uint32_t OutDataLength, uint32_t Timeout)
{
	...
	if (hjpeg->State == HAL_JPEG_STATE_READY) {
		...
		/*Set the Context to Encode with Polling*/
		hjpeg->Context &=
		    ~(JPEG_CONTEXT_OPERATION_MASK | JPEG_CONTEXT_METHOD_MASK);
		...
		hjpeg->InDataLength =
		    InDataLength -
		    (InDataLength % 4UL); /*In Data length must be multiple
					    of 4 Bytes (1 word)*/
		hjpeg->OutDataLength =
		    OutDataLength -
		    (OutDataLength % 4UL); /*Out Data length must be multiple
					     of 4 Bytes (1 word)*/
	...
	if (hjpeg->State != HAL_JPEG_STATE_READY) {
		...
		if ((hjpeg->Context & JPEG_CONTEXT_CONF_ENCODING) ==
			...
			hjpeg->Context &= ~(JPEG_CONTEXT_OPERATION_MASK |
					    JPEG_CONTEXT_METHOD_MASK);
			hjpeg->Context |=
			    (JPEG_CONTEXT_ENCODE | JPEG_CONTEXT_IT);
	...
	if (hjpeg->State != HAL_JPEG_STATE_READY) {
		...
		if ((hjpeg->Context & JPEG_CONTEXT_CONF_ENCODING) ==
			...
			hjpeg->Context &= ~(JPEG_CONTEXT_OPERATION_MASK |
					    JPEG_CONTEXT_METHOD_MASK);
			hjpeg->Context |=
			    (JPEG_CONTEXT_ENCODE | JPEG_CONTEXT_DMA);
				  ...
				  uint32_t XferSelection)
{
	...
	if ((hjpeg->Context & JPEG_CONTEXT_METHOD_MASK) == JPEG_CONTEXT_DMA) {
		...
		if ((XferSelection & JPEG_PAUSE_RESUME_INPUT) ==
			...
			/*JPEG Input DMA transfer data number must be multiple
			  of DMA buffer size as the destination is a 32 bits
			  register */
			hjpeg->InDataLength =
			    hjpeg->InDataLength - (hjpeg->InDataLength % 4UL);
...
void HAL_JPEG_ConfigInputBuffer(JPEG_HandleTypeDef *hjpeg,
				uint8_t *pNewInputBuffer, uint32_t InDataLength)
{
...
HAL_StatusTypeDef HAL_JPEG_Abort(JPEG_HandleTypeDef *hjpeg)
{
	...
	hjpeg->Context &=
	    ~(JPEG_CONTEXT_PAUSE_INPUT | JPEG_CONTEXT_PAUSE_OUTPUT);
				       ...
				       JPEG_ConfTypeDef *pInfo)
{
	...
	/* NOTE : This function Should not be modified, when the callback is
	   needed, the HAL_JPEG_HeaderParsingCpltCallback could be implemented
	   in the user file
	 */
}
...
__weak void HAL_JPEG_EncodeCpltCallback(JPEG_HandleTypeDef *hjpeg)
{
	...
	/* NOTE : This function Should not be modified, when the callback is
	   needed, the HAL_JPEG_EncodeCpltCallback could be implemented in the
	   user file
	 */
}
...
__weak void HAL_JPEG_ErrorCallback(JPEG_HandleTypeDef *hjpeg)
{
	...
	/* NOTE : This function Should not be modified, when the callback is
	   needed, the HAL_JPEG_ErrorCallback could be implemented in the user
	   file
	 */
}
				     ...
				     uint32_t NbDecodedData)
{
	...
	/* NOTE : This function Should not be modified, when the callback is
	   needed, the HAL_JPEG_GetDataCallback could be implemented in the user
	   file
	 */
}
				       ...
				       uint32_t OutDataLength)
{
	...
	/* NOTE : This function Should not be modified, when the callback is
	   needed, the HAL_JPEG_DataReadyCallback could be implemented in the
	   user file
	 */
}
						...
						uint32_t *LastK)
{
	...
	*LastK = j;
	...
	while (Huffsize[j] != 0U) {
		...
		/* code must fit in "size" bits (si), no code is allowed to be
		 * all ones*/
		if (si > 31UL) {
			return HAL_ERROR;
		}
    ...
    JPEG_AC_HuffCodeTableTypeDef *AC_SizeCodesTable)
{
	...
	error = JPEG_Bits_To_SizeCodes(AC_BitsValsTable->Bits, huffsize,
				       huffcode, &lastK);
	if (error != HAL_OK) {
		return error;
	}
    ...
    JPEG_DC_HuffCodeTableTypeDef *DC_SizeCodesTable)
{
	...
	uint8_t huffsize[257];
	uint32_t huffcode[257];
	error = JPEG_Bits_To_SizeCodes(DC_BitsValsTable->Bits, huffsize,
				       huffcode, &lastK);
	if (error != HAL_OK) {
		return error;
	}
...
static HAL_StatusTypeDef JPEG_Set_HuffEnc_Mem(JPEG_HandleTypeDef *hjpeg)
{
	...
	error = JPEG_Set_HuffAC_Mem(
	    hjpeg, (JPEG_ACHuffTableTypeDef *)(uint32_t)&JPEG_ACLUM_HuffTable,
	...
	error = JPEG_Set_HuffAC_Mem(
	    hjpeg, (JPEG_ACHuffTableTypeDef *)(uint32_t)&JPEG_ACCHROM_HuffTable,
	...
	error = JPEG_Set_HuffDC_Mem(
	    hjpeg, (JPEG_DCHuffTableTypeDef *)(uint32_t)&JPEG_DCLUM_HuffTable,
	...
	error = JPEG_Set_HuffDC_Mem(
	    hjpeg, (JPEG_DCHuffTableTypeDef *)(uint32_t)&JPEG_DCCHROM_HuffTable,
...
static void JPEG_Set_Huff_DHTMem(const JPEG_HandleTypeDef *hjpeg)
{
	const JPEG_ACHuffTableTypeDef *HuffTableAC0 =
	    (JPEG_ACHuffTableTypeDef *)(uint32_t)&JPEG_ACLUM_HuffTable;
	const JPEG_ACHuffTableTypeDef *HuffTableAC1 =
	    (JPEG_ACHuffTableTypeDef *)(uint32_t)&JPEG_ACCHROM_HuffTable;
	const JPEG_DCHuffTableTypeDef *HuffTableDC0 =
	    (JPEG_DCHuffTableTypeDef *)(uint32_t)&JPEG_DCLUM_HuffTable;
	const JPEG_DCHuffTableTypeDef *HuffTableDC1 =
	...
	/* DC0 BITS is a 16 Bytes table i.e 4x32bits words from DHTMEM base
	 * address to DHTMEM + 3*/
	...
	/* DC0 VALS is a 12 Bytes table i.e 3x32bits words from DHTMEM base
	 * address +4 to DHTMEM + 6 */
	address = (hjpeg->Instance->DHTMEM + 6);
	index = 12;
	while (index > 3UL) {
		*address =
		    (((uint32_t)HuffTableDC0->HuffVal[index - 1UL] & 0xFFUL)
		     << 24) |
		    (((uint32_t)HuffTableDC0->HuffVal[index - 2UL] & 0xFFUL)
		     << 16) |
		    (((uint32_t)HuffTableDC0->HuffVal[index - 3UL] & 0xFFUL)
		     << 8) |
	...
	/* AC0 BITS is a 16 Bytes table i.e 4x32bits words from DHTMEM base
	 * address + 7 to DHTMEM + 10*/
	...
	*address = value;
	...
	while (index > 3UL) {
		*address =
		    (((uint32_t)HuffTableAC0->HuffVal[index - 1UL] & 0xFFUL)
		     << 24) |
		    (((uint32_t)HuffTableAC0->HuffVal[index - 2UL] & 0xFFUL)
		     << 16) |
		    (((uint32_t)HuffTableAC0->HuffVal[index - 3UL] & 0xFFUL)
		     << 8) |
	...
	/* only Byte 2 and Byte 3 of the first word (@ DHTMEM + 51) belong to
	 * DC1 Bits table */
	address = (hjpeg->Instance->DHTMEM + 51);
	...
	*address = value;
	...
	/* DC1 VALS is a 12 Bytes table i.e 3x32bits words from DHTMEM base
	 * address +55 to DHTMEM + 58 */
	/* only Byte 2 and Byte 3 of the first word (@ DHTMEM + 55) belong to
	 * DC1 Val table */
	address = (hjpeg->Instance->DHTMEM + 55);
	...
	*address = value;
	...
	while (index > 3UL) {
		*address =
		    (((uint32_t)HuffTableDC1->HuffVal[index + 1UL] & 0xFFUL)
		     << 24) |
	...
	/* AC1 BITS is a 16 Bytes table i.e 4x32bits words from DHTMEM base
	 * address + 58 to DHTMEM + 62*/
	/* only Byte 2 and Byte 3 of the first word (@ DHTMEM + 58) belong to
	 * AC1 Bits table */
	address = (hjpeg->Instance->DHTMEM + 58);
	value = *address & 0x0000FFFFU;
	...
	*address = value;
	...
	/* only Byte 2 and Byte 3 of the first word (@ DHTMEM + 62) belong to
	 * AC1 VALS table */
	address = (hjpeg->Instance->DHTMEM + 62);
	value = *address & 0x0000FFFFUL;
	...
	*address = value;
	...
	while (index > 3UL) {
		*address =
		    (((uint32_t)HuffTableAC1->HuffVal[index + 1UL] & 0xFFUL)
		     << 24) |
					  ...
					  __IO uint32_t *QTableAddress)
{
	...
	if ((hjpeg->Conf.ImageQuality >= 50UL) &&
		...
		ScaleFactor = JPEG_HIGH_QUALITY_REFERENCE -
			      (hjpeg->Conf.ImageQuality * 2UL);
	} else if (hjpeg->Conf.ImageQuality > 0UL) {
		ScaleFactor = JPEG_LOW_QUALITY_REFERENCE /
	...
	while (i < (JPEG_QUANT_TABLE_SIZE - 3UL)) {
		quantRow = 0;
		for (j = 0; j < 4UL; j++) {
			/* Note that the quantization coefficients must be
			 * specified in the table in zigzag order */
			quantVal =
			    ((((uint32_t)QTable[JPEG_ZIGZAG_ORDER[i + j]]) *
			      ScaleFactor) +
			     50UL) /
			    100UL;
...
static void JPEG_SetColorYCBCR(JPEG_HandleTypeDef *hjpeg)
{
	...
	hjpeg->Instance->CONFR4 |=
	    (ySamplingH | ySamplingV | (yblockNb & JPEG_CONFR4_NB));
	...
	hjpeg->Instance->CONFR5 |=
	    (JPEG_CONFR5_HSF_0 | JPEG_CONFR5_VSF_0 | JPEG_CONFR5_QT_0 |
	...
	hjpeg->Instance->CONFR6 |=
	    (JPEG_CONFR6_HSF_0 | JPEG_CONFR6_VSF_0 | JPEG_CONFR6_QT_0 |
...
static void JPEG_Init_Process(JPEG_HandleTypeDef *hjpeg)
{
	...
	if ((hjpeg->Context & JPEG_CONTEXT_METHOD_MASK) == JPEG_CONTEXT_IT) {
		 ...
		 * interruptions*/
		__HAL_JPEG_ENABLE_IT(hjpeg, JPEG_IT_IFT | JPEG_IT_IFNF |
						JPEG_IT_OFT | JPEG_IT_OFNE |
...
static uint32_t JPEG_Process(JPEG_HandleTypeDef *hjpeg)
{
	...
	if ((hjpeg->Context & JPEG_CONTEXT_OPERATION_MASK) ==
		...
		if ((itflag & JPEG_FLAG_HPDF) != 0UL) {
			...
			/* Note : the image quality is only available at the end
			 * of the decoding operation */
			/* at the current stage the calculated image quality is
			 * not correct so reset it */
	...
	/*End of Conversion handling :i.e EOC flag is high and OFTF low and
	 * OFNEF low*/
	if ((itflag & (JPEG_FLAG_EOCF | JPEG_FLAG_OFTF | JPEG_FLAG_OFNEF)) ==
	    JPEG_FLAG_EOCF) {
		...
		if (hjpeg->JpegOutCount > 0UL) {
			/*Output Buffer is not empty, call
			 * DecodedDataReadyCallback*/
#if (USE_HAL_JPEG_REGISTER_CALLBACKS == 1)
			hjpeg->DataReadyCallback(hjpeg, hjpeg->pJpegOutBuffPtr,
						 hjpeg->JpegOutCount);
#else
			HAL_JPEG_DataReadyCallback(
			    hjpeg, hjpeg->pJpegOutBuffPtr, hjpeg->JpegOutCount);
#endif /* USE_HAL_JPEG_REGISTER_CALLBACKS */
		...
		hjpeg->Context &=
		    (JPEG_CONTEXT_CONF_ENCODING | JPEG_CONTEXT_CUSTOM_TABLES);
			       ...
			       uint32_t nbRequestWords)
{
	...
	if ((hjpeg->InDataLength == 0UL) || (nbRequestWords == 0UL)) {
		...
		if (hjpeg->InDataLength > 4UL) {
			hjpeg->InDataLength =
			    ((hjpeg->InDataLength + 3UL) / 4UL) * 4UL;
		}
		hjpeg->JpegInCount = 0;
 ...
 *         interrupt to prevent the JPEG interrupt from preempting the DMA
 * interrupt before the DMA state is updated to ready.
 * @param  hjpeg pointer to a JPEG_HandleTypeDef structure that contains
 *         the configuration information for JPEG module
...
static HAL_StatusTypeDef JPEG_DMA_StartProcess(JPEG_HandleTypeDef *hjpeg)
{
	...
	/*DMA transfer size must be a multiple of 4 bytes i.e multiple of 32bits
	 * words*/
	...
	if ((hjpeg->hdmain->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
		...
		/* Start DMA FIFO In transfer */
		if (HAL_DMA_Start_IT(hjpeg->hdmain,
				     (uint32_t)hjpeg->pJpegInBuffPtr,
				     (uint32_t)&hjpeg->Instance->DIR,
				     hjpeg->InDataLength) != HAL_OK) {
			hjpeg->ErrorCode |= HAL_JPEG_ERROR_DMA;
	...
	if ((hjpeg->hdmaout->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
		if (hjpeg->hdmaout->LinkedListQueue != NULL) {
			/* Set DMA data size */
			hjpeg->hdmaout->LinkedListQueue->Head
			    ->LinkRegisters[NODE_CBR1_DEFAULT_OFFSET] =
			    hjpeg->OutDataLength;
		...
		/* Start DMA FIFO Out transfer */
		if (HAL_DMA_Start_IT(hjpeg->hdmaout,
				     (uint32_t)&hjpeg->Instance->DOR,
				     (uint32_t)hjpeg->pJpegOutBuffPtr,
				     hjpeg->OutDataLength) != HAL_OK) {
			hjpeg->ErrorCode |= HAL_JPEG_ERROR_DMA;
...
static void JPEG_DMA_EndProcess(JPEG_HandleTypeDef *hjpeg)
{
	uint32_t tmpContext;
	hjpeg->JpegOutCount =
	    hjpeg->OutDataLength - JPEG_GET_DMA_REMAIN_DATA(hjpeg->hdmaout);
	...
	if (hjpeg->JpegOutCount == hjpeg->OutDataLength) {
		...
		hjpeg->DataReadyCallback(hjpeg, hjpeg->pJpegOutBuffPtr,
					 hjpeg->JpegOutCount);
#else
		HAL_JPEG_DataReadyCallback(hjpeg, hjpeg->pJpegOutBuffPtr,
					   hjpeg->JpegOutCount);
#endif /* USE_HAL_JPEG_REGISTER_CALLBACKS */
...
static void JPEG_DMA_PollResidualData(JPEG_HandleTypeDef *hjpeg)
{
	...
	for (count = JPEG_FIFO_SIZE; count > 0UL; count--) {
		if ((hjpeg->Context & JPEG_CONTEXT_PAUSE_OUTPUT) == 0UL) {
			if (__HAL_JPEG_GET_FLAG(hjpeg, JPEG_FLAG_OFNEF) !=
			    0UL) {
				dataOut = hjpeg->Instance->DOR;
				hjpeg->pJpegOutBuffPtr[hjpeg->JpegOutCount] =
				    (uint8_t)(dataOut & 0x000000FFUL);
				hjpeg->pJpegOutBuffPtr[hjpeg->JpegOutCount +
						       1UL] =
				...
				if (hjpeg->JpegOutCount ==
				    hjpeg->OutDataLength) {
					/*Output Buffer is full, call
					 * HAL_JPEG_DataReadyCallback*/
#if (USE_HAL_JPEG_REGISTER_CALLBACKS == 1)
					hjpeg->DataReadyCallback(
					    hjpeg, hjpeg->pJpegOutBuffPtr,
					    hjpeg->JpegOutCount);
#else
					HAL_JPEG_DataReadyCallback(
					    hjpeg, hjpeg->pJpegOutBuffPtr,
...
static void JPEG_DMAInCpltCallback(DMA_HandleTypeDef *hdma)
{
	JPEG_HandleTypeDef *hjpeg =
	    (JPEG_HandleTypeDef *)((DMA_HandleTypeDef *)hdma)->Parent;

	/* Disable The JPEG IT so the DMA Input Callback can not be interrupted
				 ...
				 in ending DMA stage */
	{
		...
		hjpeg->JpegInCount =
		    hjpeg->InDataLength - JPEG_GET_DMA_REMAIN_DATA(hdma);
		...
		if (hjpeg->InDataLength >= 4UL) {
			/*JPEG Input DMA transfer data number must be multiple
			  of 32 bits word as the destination is a 32 bits (4
			  bytes) register */
			hjpeg->InDataLength =
			    ((hjpeg->InDataLength + 3UL) / 4UL) * 4UL;
		} else {
...
static void JPEG_DMAOutCpltCallback(DMA_HandleTypeDef *hdma)
{
	...
	/* Disable The JPEG IT so the DMA Output Callback can not be interrupted
	 * by the JPEG EOC IT or JPEG HPD IT */
	__HAL_JPEG_DISABLE_IT(hjpeg, JPEG_INTERRUPT_MASK);
				 ...
				 in ending DMA stage */
	{
		if (__HAL_JPEG_GET_FLAG(hjpeg, JPEG_FLAG_EOCF) == 0UL) {
			...
			if ((hjpeg->Context & JPEG_CONTEXT_PAUSE_OUTPUT) ==
			    0UL) {
				/* Start DMA FIFO Out transfer */
				if (HAL_DMA_Start_IT(
					hjpeg->hdmaout,
...
static uint32_t JPEG_GetQuality(const JPEG_HandleTypeDef *hjpeg)
{
	...
	while (i < (JPEG_QUANT_TABLE_SIZE - 3UL)) {
		...
		for (j = 0; j < 4UL; j++) {
			...
			if (quantVal == 1UL) {
				/* if Quantization value = 1 then quality is
				 * 100%*/
				quality += 100UL;
			} else {
				/* Note that the quantization coefficients must
				 * be specified in the table in zigzag order */
				scale = (quantVal * 100UL) /
					((uint32_t)hjpeg->QuantTable0
					     [JPEG_ZIGZAG_ORDER[i + j]]);
