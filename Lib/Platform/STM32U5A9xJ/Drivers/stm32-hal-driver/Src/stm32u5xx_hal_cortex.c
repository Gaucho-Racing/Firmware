...
macro is defined inside the stm32u5xx_hal_cortex.h file.
...
Protection Unit (MPU_NS) from the secure context and the same sequence as above
applies to configure the non-secure MPU.
 ...
 * the appropriate CMSIS device file (stm32u5xxxx.h))
...
{
	__DMB(); /* Data Memory Barrier operation to force any outstanding
		    writes to memory before enabling the MPU */
	...
	/* Data Synchronization and Instruction Synchronization Barriers to
	 * ensure MPU configuration */
	__DSB(); /* Ensure that the subsequent instruction is executed only
		    after the write to memory */
	__ISB(); /* Flush and refill pipeline with updated MPU configuration
		    settings */
}
...
{
	__DMB(); /* Force any outstanding transfers to complete before disabling
		    MPU */
