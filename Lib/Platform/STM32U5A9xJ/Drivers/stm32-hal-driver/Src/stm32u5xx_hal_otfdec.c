...
__weak void HAL_OTFDEC_MspInit(OTFDEC_HandleTypeDef *hotfdec)
{
	...
	/* NOTE : This function should not be modified; when the callback is
	   needed, the HAL_OTFDEC_MspInit can be implemented in the user file.
	 */
}
...
__weak void HAL_OTFDEC_MspDeInit(OTFDEC_HandleTypeDef *hotfdec)
{
	...
	/* NOTE : This function should not be modified; when the callback is
	   needed, the HAL_OTFDEC_MspDeInit can be implemented in the user file.
	 */
}
...
__weak void HAL_OTFDEC_ErrorCallback(OTFDEC_HandleTypeDef *hotfdec)
{
	...
	/* NOTE : This function should not be modified; when the callback is
	   needed, the HAL_OTFDEC_ErrorCallback can be implemented in the user
	   file.
	 */
}
					  ...
					  uint32_t RegionIndex, uint32_t *pKey)
{
	...
	if (pKey == NULL) {
		...
		/* Compute theoretically expected CRC and compare it with that
		 * reported by the peripheral */
			...
			const OTFDEC_RegionConfigTypeDef *Config, uint32_t lock)
{
	...
	if (Config == NULL) {
		...
		/* Lock the region configuration according to lock parameter
		 * value */
		if (lock == OTFDEC_REG_CONFIGR_LOCK_ENABLE) {
			SET_BIT(region->REG_CONFIGR,
					      ...
					      uint32_t Attributes)
{
	...
	MODIFY_REG(hotfdec->Instance->PRIVCFGR, OTFDEC_PRIVCFGR_PRIV,
		   Attributes);
...
uint32_t HAL_OTFDEC_KeyCRCComputation(const uint32_t *pKey)
{
	...
	for (j = 0U; j < 4U; j++) {
		...
		if (j == 0U) {
			...
			keyval ^= (key_strobe[j] << 24) |
				  ((uint32_t)crc << 16) | (key_strobe[j] << 8) |
				  crc;
		}
						 ...
						 uint32_t *Attributes)
{
	...
	*Attributes =
	    READ_BIT(hotfdec->Instance->PRIVCFGR, OTFDEC_PRIVCFGR_PRIV);
