				     ...
				     uint32_t Timeout)
{
	...
	if (hmmc->State == HAL_MMC_STATE_READY) {
		...
		/* Check the case of 4kB blocks (field DATA SECTOR SIZE of
		...
		if (((hmmc->Ext_CSD[(MMC_EXT_CSD_DATA_SEC_SIZE_INDEX / 4)] >>
			...
			if ((NumberOfBlocks % 8U) != 0U) {
				 ...
				 * 8 sectors of 512 bytes = 4 KBytes */
				hmmc->ErrorCode |= HAL_MMC_ERROR_BLOCK_LEN_ERR;
				return HAL_ERROR;
			}

			if ((BlockAdd % 8U) != 0U) {
				/* The address should be aligned to 8
				 * (corresponding to 4 KBytes blocks) */
				hmmc->ErrorCode |=
		...
		while (!__HAL_MMC_GET_FLAG(
			...
			if (__HAL_MMC_GET_FLAG(hmmc, SDMMC_FLAG_RXFIFOHF) &&
				...
				for (count = 0U; count < (SDMMC_FIFO_SIZE / 4U);
				     count++) {
...
HAL_StatusTypeDef HAL_MMC_Abort(MMC_HandleTypeDef *hmmc)
{
	...
	if (hmmc->State == HAL_MMC_STATE_BUSY) {
		 ...
		 * transfers*/
		/*In case the data transfer just finished, the external memory
		  will not respond and will return
		  HAL_MMC_ERROR_CMD_RSP_TIMEOUT*/
		/*In case the data transfer aborted , the external memory will
		 * respond and will return HAL_MMC_ERROR_NONE*/
		...
		if ((hmmc->Instance->DCTRL & SDMMC_DCTRL_DTDIR) ==
			...
			while (!__HAL_MMC_GET_FLAG(
			    hmmc, SDMMC_FLAG_DABORT | SDMMC_FLAG_DATAEND)) {
				if ((HAL_GetTick() - tickstart) >=
				    SDMMC_DATATIMEOUT) {
					hmmc->ErrorCode = HAL_MMC_ERROR_TIMEOUT;
					hmmc->State = HAL_MMC_STATE_READY;
		...
		/*The reason of all these while conditions previously is that we
		  need to wait the SDMMC and clear the appropriate flags that
		  will be set depending of the abort/non abort of the memory */
		/*Not waiting the SDMMC flags will cause the next
		  SDMMC_DISABLE_IDMA to not get cleared and will result in next
					...
					uint32_t BlockEndAdd)
{
	...
	if (((hmmc->Ext_CSD[(MMC_EXT_CSD_DATA_SEC_SIZE_INDEX / 4)] >>
		...
		if (((start_add % 8U) != 0U) || ((end_add % 8U) != 0U)) {
			/* The address should be aligned to 8 (corresponding to
			 * 4 KBytes blocks) */
							...
							uint32_t Timeout)
{
	...
	if (hmmc->State == HAL_MMC_STATE_READY) {
		...
		/* Write Blocks in Polling mode */
		{
			...
			/* Write Multi Block command */
			errorstate =
			    SDMMC_CmdWriteMultiBlock(hmmc->Instance, 0);
		}
...
__weak void HAL_MMCEx_Read_DMALnkLstBufCpltCallback(MMC_HandleTypeDef *hmmc)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_MMCEx_Read_DMALnkLstBufCpltCallback can be
...
__weak void HAL_MMCEx_Write_DMALnkLstBufCpltCallback(MMC_HandleTypeDef *hmmc)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_MMCEx_Write_DMALnkLstBufCpltCallback can be
	   implemented in the user file
	 */
}
