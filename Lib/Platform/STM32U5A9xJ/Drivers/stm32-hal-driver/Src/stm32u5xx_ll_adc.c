...
#if defined(ADC_MULTIMODE_SUPPORT)
		if (pADC_CommonInitStruct->Multimode !=
		    LL_ADC_MULTI_INDEPENDENT) {
			MODIFY_REG(
			    pADCxy_COMMON->CCR,
			    ADC_CCR_PRESC | ADC_CCR_DUAL | ADC_CCR_DAMDF |
				ADC_CCR_DELAY,
			...
			MODIFY_REG(pADCxy_COMMON->CCR,
				   ADC_CCR_PRESC | ADC_CCR_DUAL |
				       ADC_CCR_DAMDF | ADC_CCR_DELAY,
				   pADC_CommonInitStruct->CommonClock |
				       LL_ADC_MULTI_INDEPENDENT);
		}
#else
		LL_ADC_SetCommonClock(pADCxy_COMMON,
				      pADC_CommonInitStruct->CommonClock);
#endif /* ADC_MULTIMODE_SUPPORT */
	} else {
...
ErrorStatus LL_ADC_DeInit(ADC_TypeDef *pADCx)
{
	...
	if (LL_ADC_IsEnabled(pADCx) == 1UL) {
		...
		if (pADCx != ADC4) /* ADC1 or ADC2 */
		{
			while ((LL_ADC_REG_IsStopConversionOngoing(pADCx) |
				LL_ADC_INJ_IsStopConversionOngoing(pADCx)) ==
			       1UL) {
				timeout_cpu_cycles--;
	...
	{
		CLEAR_BIT(pADCx->IER,
			  (LL_ADC_IT_AWD3 | LL_ADC_IT_AWD2 | LL_ADC_IT_AWD1 |
			   LL_ADC_IT_OVR | LL_ADC_IT_JEOS | LL_ADC_IT_JEOC |
			   LL_ADC_IT_EOS | LL_ADC_IT_EOC | LL_ADC_IT_EOSMP |
			   LL_ADC_IT_ADRDY));

		/* Reset register ISR */
		SET_BIT(pADCx->ISR,
			(LL_ADC_FLAG_AWD3 | LL_ADC_FLAG_AWD2 |
			 LL_ADC_FLAG_AWD1 | LL_ADC_FLAG_OVR | LL_ADC_FLAG_JEOS |
			 LL_ADC_FLAG_JEOC | LL_ADC_FLAG_EOS | LL_ADC_FLAG_EOC |
		...
		CLEAR_BIT(pADCx->CFGR2, ADC_CFGR2_ROVSM | ADC_CFGR2_TROVS |
		...
		CLEAR_BIT(pADCx->SMPR2, ADC_SMPR2_SMP18 | ADC_SMPR2_SMP17 |
					    ADC_SMPR2_SMP16 | ADC_SMPR2_SMP15 |
					    ADC_SMPR2_SMP14 | ADC_SMPR2_SMP13 |
					    ADC_SMPR2_SMP12 | ADC_SMPR2_SMP11 |
					    ADC_SMPR2_SMP10);
		...
		/* Reset register SQR1 */
		CLEAR_BIT(pADCx->SQR1, ADC_SQR1_SQ4 | ADC_SQR1_SQ3 |
					   ADC_SQR1_SQ2 | ADC_SQR1_SQ1 |
					   ADC_SQR1_L);

		/* Reset register SQR2 */
		CLEAR_BIT(pADCx->SQR2, ADC_SQR2_SQ9 | ADC_SQR2_SQ8 |
					   ADC_SQR2_SQ7 | ADC_SQR2_SQ6 |
					   ADC_SQR2_SQ5);

		/* Reset register SQR3 */
		CLEAR_BIT(pADCx->SQR3, ADC_SQR3_SQ14 | ADC_SQR3_SQ13 |
		...
		/* Reset register OFR1 */
		CLEAR_BIT(pADCx->OFR1, ADC_OFR1_SSAT | ADC_OFR1_OFFSET1_CH |
					   ADC_OFR1_OFFSET1);
		/* Reset register OFR2 */
		CLEAR_BIT(pADCx->OFR2, ADC_OFR2_SSAT | ADC_OFR2_OFFSET2_CH |
					   ADC_OFR2_OFFSET2);
		/* Reset register OFR3 */
		CLEAR_BIT(pADCx->OFR3, ADC_OFR3_SSAT | ADC_OFR3_OFFSET3_CH |
					   ADC_OFR3_OFFSET3);
		/* Reset register OFR4 */
		CLEAR_BIT(pADCx->OFR4, ADC_OFR4_SSAT | ADC_OFR4_OFFSET4_CH |
					   ADC_OFR4_OFFSET4);
		...
		CLEAR_BIT(pADCx->CALFACT,
			  ADC_CALFACT_CAPTURE_COEF | ADC_CALFACT_LATCH_COEF);
	} else {
		/* Check whether ADC state is compliant with expected state */
		if (READ_BIT(pADCx->CR, (ADC_CR_ADSTP | ADC_CR_ADSTART |
					 ADC_CR_ADDIS | ADC_CR_ADEN)) == 0UL) {
			/* ========== Reset ADC registers ========== */
			/* Reset register IER */
			CLEAR_BIT(pADCx->IER,
				  (LL_ADC_IT_ADRDY | LL_ADC_IT_EOC |
				   LL_ADC_IT_EOS | LL_ADC_IT_OVR |
				   LL_ADC_IT_EOSMP | LL_ADC_IT_AWD1 |
				   LL_ADC_IT_AWD2 | LL_ADC_IT_AWD3 |
				   LL_ADC_IT_EOCAL | LL_ADC_IT_LDORDY));

			/* Reset register ISR */
			SET_BIT(pADCx->ISR,
				(LL_ADC_FLAG_ADRDY | LL_ADC_FLAG_EOC |
				 LL_ADC_FLAG_EOS | LL_ADC_FLAG_OVR |
				 LL_ADC_FLAG_EOSMP | LL_ADC_FLAG_AWD1 |
			...
			CLEAR_BIT(pADCx->CFGR1,
				  (ADC_CFGR1_AWD1CH | ADC_CFGR1_AWD1EN |
				   ...
				   ADC_CFGR1_OVRMOD | ADC_CFGR1_EXTEN |
				   ADC_CFGR1_EXTSEL | ADC4_CFGR1_ALIGN |
				   ADC_CFGR1_RES | ADC4_CFGR1_SCANDIR |
				   ADC4_CFGR1_DMACFG | ADC4_CFGR1_DMAEN));
			...
			CLEAR_BIT(pADCx->CFGR2,
				  (ADC_CFGR2_TROVS | ADC_CFGR2_OVSS |
				   ADC4_CFGR2_OVSR | ADC_CFGR2_ROVSE |
				   ADC4_CFGR2_LFTRIG));

			/* Reset register SMPR */
			CLEAR_BIT(pADCx->SMPR1, ADC4_SMPR_SMP1 |
						    ADC4_SMPR_SMP2 |
						    ADC4_SMPR_SMPSEL);

			/* Reset register TR1 */
			MODIFY_REG(pADCx->AWD1TR,
				   ADC_AWD1TR_HT1 | ADC_AWD1TR_LT1,
				   ADC_AWD1TR_HT1);

			/* Reset register TR2 */
			MODIFY_REG(pADCx->AWD2TR,
				   ADC_AWD2TR_HT2 | ADC_AWD2TR_LT2,
				   ADC_AWD2TR_HT2);

			/* Reset register TR3 */
			MODIFY_REG(pADCx->AWD3TR,
				   ADC_AWD3TR_HT3 | ADC_AWD3TR_LT3,
				   ADC_AWD3TR_HT3);

			/* Reset register CHSELR */
			CLEAR_BIT(pADCx->CHSELR,
				  (ADC_CHSELR_CHSEL23 | ADC_CHSELR_CHSEL22 |
				   ADC_CHSELR_CHSEL21 | ADC_CHSELR_CHSEL20 |
				   ADC_CHSELR_CHSEL19 | ADC_CHSELR_CHSEL18 |
				   ADC_CHSELR_CHSEL17 | ADC_CHSELR_CHSEL16 |
				   ADC_CHSELR_CHSEL15 | ADC_CHSELR_CHSEL14 |
				   ADC_CHSELR_CHSEL13 | ADC_CHSELR_CHSEL12 |
				   ADC_CHSELR_CHSEL11 | ADC_CHSELR_CHSEL10 |
			...
			/* Reset register CCR */
			CLEAR_BIT(ADC4_COMMON->CCR,
				  ADC_CCR_VBATEN | ADC_CCR_VSENSEEN |
...
ErrorStatus LL_ADC_Init(ADC_TypeDef *pADCx, LL_ADC_InitTypeDef *pADC_InitStruct)
{
	...
	{
		assert_param(
		    IS_LL_ADC_LEFT_BIT_SHIFT(pADC_InitStruct->LeftBitShift));
	} else {
		assert_param(
		    IS_LL_ADC_DATA_ALIGN(pADC_InitStruct->DataAlignment));
	}
	...
	if (LL_ADC_IsEnabled(pADCx) == 0UL) {
		...
		if (pADCx != ADC4) /* ADC1 or ADC2 */
		{
			...
			MODIFY_REG(pADCx->CFGR2, ADC_CFGR2_LSHIFT,
			    ...
			    LL_ADC_REG_InitTypeDef *pADC_RegInitStruct)
{
	...
	assert_param(IS_ADC_ALL_INSTANCE(pADCx));
	if (pADCx == ADC4) {
		assert_param(IS_LL_ADC4_REG_TRIG_SOURCE(
		    pADC_RegInitStruct->TriggerSource));
		assert_param(IS_LL_ADC4_REG_SEQ_SCAN_LENGTH(
		    pADC_RegInitStruct->SequencerLength));
		assert_param(IS_LL_ADC_REG_DMA_TRANSFER(
		    pADC_RegInitStruct->DMATransfer));
		if ((LL_ADC_REG_GetSequencerConfigurable(pADCx) ==
		     LL_ADC_REG_SEQ_FIXED) ||
		    (pADC_RegInitStruct->SequencerLength !=
			...
			assert_param((pADC_RegInitStruct->ContinuousMode ==
	...
	{
		assert_param(IS_LL_ADC_REG_TRIG_SOURCE(
		    pADC_RegInitStruct->TriggerSource));
		assert_param(IS_LL_ADC_REG_SEQ_SCAN_LENGTH(
		    pADC_RegInitStruct->SequencerLength));
	...
	assert_param(
	    IS_LL_ADC_REG_OVR_DATA_BEHAVIOR(pADC_RegInitStruct->Overrun));
		...
		if (pADCx != ADC4) /* ADC1 or ADC2 */
		{
			if (pADC_RegInitStruct->SequencerLength !=
				...
				MODIFY_REG(
					...
					ADC_CFGR1_CONT | ADC_CFGR1_DMNGT |
					ADC_CFGR1_OVRMOD,
				    pADC_RegInitStruct->TriggerSource |
					pADC_RegInitStruct->SequencerDiscont |
			...
			if ((LL_ADC_REG_GetSequencerConfigurable(pADCx) ==
				...
				MODIFY_REG(
				    pADCx->CFGR1,
				    ADC_CFGR1_EXTSEL | ADC_CFGR1_EXTEN |
					ADC_CFGR1_DISCEN | ADC_CFGR1_CONT |
			    ...
			    LL_ADC_INJ_InitTypeDef *pADC_InjInitStruct)
{
	...
	assert_param(IS_ADC_ALL_INSTANCE(pADCx));
	assert_param(
	    IS_LL_ADC_INJ_TRIG_SOURCE(pADC_InjInitStruct->TriggerSource));
	assert_param(
	    IS_LL_ADC_INJ_SEQ_SCAN_LENGTH(pADC_InjInitStruct->SequencerLength));
	if (pADC_InjInitStruct->SequencerLength !=
