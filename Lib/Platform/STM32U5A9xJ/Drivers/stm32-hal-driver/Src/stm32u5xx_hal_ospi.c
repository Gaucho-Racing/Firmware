/**
 ...
 is complete.
     (+) In interrupt mode, HAL_OSPI_CmdCpltCallback() will be called when the
 ...
 configuration :
     (+) In polling mode, the output of the function is done when the transfer
 is complete.
     (+) In interrupt mode, HAL_OSPI_FifoThresholdCallback() will be called when
 the fifo threshold is reached and HAL_OSPI_TxCpltCallback() will be called when
 the transfer is complete.
     (+) In DMA mode, HAL_OSPI_TxHalfCpltCallback() will be called at the half
 transfer and HAL_OSPI_TxCpltCallback() will be called when the transfer is
 complete.
    [..]
     For the indirect read mode, use HAL_OSPI_Receive(), HAL_OSPI_Receive_DMA()
 or HAL_OSPI_Receive_IT() after the command configuration :
     (+) In polling mode, the output of the function is done when the transfer
 is complete.
     (+) In interrupt mode, HAL_OSPI_FifoThresholdCallback() will be called when
 the fifo threshold is reached and HAL_OSPI_RxCpltCallback() will be called when
 the transfer is complete.
     (+) In DMA mode, HAL_OSPI_RxHalfCpltCallback() will be called at the half
 transfer and HAL_OSPI_RxCpltCallback() will be called when the transfer is
 complete.
 ...
 match is reached. The automatic stop is activated to avoid an infinite loop.
     (+) In interrupt mode, HAL_OSPI_StatusMatchCallback() will be called each
 ...
 the AHB is done on the address range. HAL_OSPI_TimeOutCallback() will be called
 ...
 operation and flushes the fifo :
    ...
    *** Control functions ***
    =========================
    [..]
     HAL_OSPI_GetState() function gives the current state of the HAL OctoSPI
 driver.
    [..]
     HAL_OSPI_SetTimeout() function configures the timeout value used in the
 driver.
    [..]
     HAL_OSPI_SetFifoThreshold() function configures the threshold on the Fifo
 of the OSPI Peripheral.
    [..]
 ...
 ID and a pointer to the user callback function.
    ...
    [..]
     Use function HAL_OSPI_UnRegisterCallback() to reset a callback to the
 ...
 Callback ID.
    ...
    [..]
     By default, after the HAL_OSPI_Init() and if the state is
 ...
 HAL_OSPI_Init() and HAL_OSPI_DeInit() only when these callbacks are null (not
 registered beforehand). If not, MspInit or MspDeInit are not null, the
 HAL_OSPI_Init() and HAL_OSPI_DeInit() keep and use the user MspInit/MspDeInit
 ...
 register the MspInit/MspDeInit user callbacks using HAL_OSPI_RegisterCallback()
 before calling HAL_OSPI_DeInit() or HAL_OSPI_Init() function.
...
HAL_StatusTypeDef HAL_OSPI_Init(OSPI_HandleTypeDef *hospi)
{
	...
	if (hospi == NULL) {
		...
		assert_param(
		    IS_OSPI_CS_HIGH_TIME(hospi->Init.ChipSelectHighTime));
		assert_param(
		...
		assert_param(
		    IS_OSPI_SAMPLE_SHIFTING(hospi->Init.SampleShifting));
		assert_param(IS_OSPI_DHQC(hospi->Init.DelayHoldQuarterCycle));
		assert_param(
		    IS_OSPI_CS_BOUNDARY(hospi->Init.ChipSelectBoundary));
...
#endif /* defined (USE_HAL_OSPI_REGISTER_CALLBACKS) &&                         \
	  (USE_HAL_OSPI_REGISTER_CALLBACKS == 1U) */

			/* Configure the default timeout for the OSPI memory
			 * access */
			(void)HAL_OSPI_SetTimeout(
			    hospi, HAL_OSPI_TIMEOUT_DEFAULT_VALUE);
			...
			MODIFY_REG(hospi->Instance->DCR1,
				   (OCTOSPI_DCR1_MTYP | OCTOSPI_DCR1_DEVSIZE |
				    OCTOSPI_DCR1_CSHT | OCTOSPI_DCR1_DLYBYP |
			...
			/* Configure wrap size */
			MODIFY_REG(hospi->Instance->DCR2, OCTOSPI_DCR2_WRAPSIZE,
				   hospi->Init.WrapSize);

			/* Configure chip select boundary and maximum transfer
			 */
			hospi->Instance->DCR3 =
			    ((hospi->Init.ChipSelectBoundary
			...
			status = OSPI_WaitFlagStateUntilTimeout(
			    hospi, HAL_OSPI_FLAG_BUSY, RESET, tickstart,
			    hospi->Timeout);
			...
			if (status == HAL_OK) {
				...
				MODIFY_REG(hospi->Instance->CR, OCTOSPI_CR_DMM,
					   hospi->Init.DualQuad);
				 ...
				 * done after OSPI enable */
				if (hospi->Init.FreeRunningClock ==
				    HAL_OSPI_FREERUNCLK_ENABLE) {
					SET_BIT(hospi->Instance->DCR1,
						OCTOSPI_DCR1_FRCK);
				}

				/* Initialize the OSPI state */
				if (hospi->Init.MemoryType ==
...
__weak void HAL_OSPI_MspInit(OSPI_HandleTypeDef *hospi)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_OSPI_MspInit can be implemented in the user file
	 */
}
...
__weak void HAL_OSPI_MspDeInit(OSPI_HandleTypeDef *hospi)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_OSPI_MspDeInit can be implemented in the user file
	 */
}
...
void HAL_OSPI_IRQHandler(OSPI_HandleTypeDef *hospi)
{
    ...
    (USE_HAL_OSPI_REGISTER_CALLBACKS == 1U)
	  ...
	  (USE_HAL_OSPI_REGISTER_CALLBACKS == 1U) */
			...
			/* Disable the interrupts */
			__HAL_OSPI_DISABLE_IT(hospi, HAL_OSPI_IT_TC |
							 HAL_OSPI_IT_FT |
							 HAL_OSPI_IT_TE);
	...
	/* OctoSPI transfer error interrupt occurred
	   -------------------------------*/
		...
		__HAL_OSPI_DISABLE_IT(hospi, (HAL_OSPI_IT_TO | HAL_OSPI_IT_SM |
				   ...
				   uint32_t Timeout)
{
	...
	if (cmd->InstructionMode != HAL_OSPI_INSTRUCTION_NONE) {
		...
		assert_param(
		    IS_OSPI_INSTRUCTION_DTR_MODE(cmd->InstructionDtrMode));
	}
	...
	if (((state == HAL_OSPI_STATE_READY) &&
		...
		/* Wait till busy flag is reset */
		status = OSPI_WaitFlagStateUntilTimeout(
		    hospi, HAL_OSPI_FLAG_BUSY, RESET, tickstart, Timeout);
			...
			if (status == HAL_OK) {
				if (cmd->DataMode == HAL_OSPI_DATA_NONE) {
					/* When there is no data phase, the
					   ...
					   TC flag is set to go back in idle
					   state */
					status = OSPI_WaitFlagStateUntilTimeout(
					    hospi, HAL_OSPI_FLAG_TC, SET,
					    tickstart, Timeout);
				      ...
				      OSPI_RegularCmdTypeDef *cmd)
{
	...
	if ((hospi->State == HAL_OSPI_STATE_READY) &&
		...
		if (status == HAL_OK) {
			...
			__HAL_OSPI_CLEAR_FLAG(hospi, HAL_OSPI_FLAG_TE |
							 HAL_OSPI_FLAG_TC);
				       ...
				       uint32_t Timeout)
{
	...
	if ((hospi->State == HAL_OSPI_STATE_READY) &&
		...
		if (status == HAL_OK) {
			...
			MODIFY_REG(hospi->Instance->DCR1, OCTOSPI_DCR1_MTYP_0,
				   cmd->AddressSpace);
			...
			WRITE_REG(hospi->Instance->CCR,
				  (cmd->DQSMode | OCTOSPI_CCR_DDTR |
				   OCTOSPI_CCR_DMODE_2 | cmd->AddressSize |
				    ...
				    uint32_t Timeout)
{
	...
	if (pData == NULL) {
		...
		if (hospi->State == HAL_OSPI_STATE_CMD_CFG) {
			 ...
			 * indirect write */
			MODIFY_REG(hospi->Instance->CR, OCTOSPI_CR_FMODE,
			...
			do {
				 ...
				 * data */
				status = OSPI_WaitFlagStateUntilTimeout(
				    hospi, HAL_OSPI_FLAG_FT, SET, tickstart,
				    Timeout);
			...
			if (status == HAL_OK) {
				 ...
				 * back in idle state */
				status = OSPI_WaitFlagStateUntilTimeout(
				    hospi, HAL_OSPI_FLAG_TC, SET, tickstart,
				    Timeout);
	...
	if (pData == NULL) {
		...
		if (hospi->State == HAL_OSPI_STATE_CMD_CFG) {
			...
			do {
				...
				status = OSPI_WaitFlagStateUntilTimeout(
				    hospi,
				    (HAL_OSPI_FLAG_FT | HAL_OSPI_FLAG_TC), SET,
				    tickstart, Timeout);
					...
					uint8_t *pData)
{
	...
	if (pData == NULL) {
		...
		/* Check the state */
		if (hospi->State == HAL_OSPI_STATE_CMD_CFG) {
			if ((hospi->hdma->Mode & DMA_LINKEDLIST) ==
			    DMA_LINKEDLIST) {
			...
			if (data_width == DMA_DEST_DATAWIDTH_BYTE) {
				...
				if (((data_size % 2U) != 0U) ||
				    ((hospi->Init.FifoThreshold % 2U) != 0U)) {
				...
				if (((data_size % 4U) != 0U) ||
				    ((hospi->Init.FifoThreshold % 4U) != 0U)) {
					/* The number of data or the fifo
			...
			if (status == HAL_OK) {
				 ...
				 */
				hospi->hdma->XferHalfCpltCallback =
				    OSPI_DMAHalfCplt;
				...
				if ((hospi->hdma->Mode & DMA_LINKEDLIST) ==
					...
					MODIFY_REG(
					    hospi->hdma->Instance->CTR1,
					    (DMA_CTR1_SINC | DMA_CTR1_DINC),
					    (DMA_SINC_INCREMENTED |
					     DMA_DINC_FIXED));
					MODIFY_REG(hospi->hdma->Instance->CTR2,
						   DMA_CTR2_DREQ,
						   DMA_MEMORY_TO_PERIPH);

					status = HAL_DMA_Start_IT(
					    hospi->hdma, (uint32_t)pData,
				       ...
				       uint32_t Timeout)
{
	...
	if ((hospi->State == HAL_OSPI_STATE_CMD_CFG) &&
		...
		if (status == HAL_OK) {
			...
			MODIFY_REG(hospi->Instance->CR,
				   (OCTOSPI_CR_PMM | OCTOSPI_CR_APMS |
				    OCTOSPI_CR_FMODE),
			 ...
			 * state */
			status = OSPI_WaitFlagStateUntilTimeout(
			    hospi, HAL_OSPI_FLAG_SM, SET, tickstart, Timeout);
					...
					OSPI_MemoryMappedTypeDef *cfg)
{
	...
	if (hospi->State == HAL_OSPI_STATE_CMD_CFG) {
		...
		if (status == HAL_OK) {
			...
			if (cfg->TimeOutActivation ==
			    HAL_OSPI_TIMEOUT_COUNTER_ENABLE) {
				assert_param(
				    IS_OSPI_TIMEOUT_PERIOD(cfg->TimeOutPeriod));

				/* Configure register */
				WRITE_REG(hospi->Instance->LPTR,
					  cfg->TimeOutPeriod);
...
__weak void HAL_OSPI_ErrorCallback(OSPI_HandleTypeDef *hospi)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_OSPI_ErrorCallback could be implemented in the user
	   file
	 */
}
...
__weak void HAL_OSPI_AbortCpltCallback(OSPI_HandleTypeDef *hospi)
{
	...
	/* NOTE: This function should not be modified, when the callback is
	   needed, the HAL_OSPI_AbortCpltCallback could be implemented in the
	   user file
	 */
}
...
__weak void HAL_OSPI_FifoThresholdCallback(OSPI_HandleTypeDef *hospi)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_OSPI_FIFOThresholdCallback could be implemented in
	   the user file
	 */
}
...
__weak void HAL_OSPI_CmdCpltCallback(OSPI_HandleTypeDef *hospi)
{
	...
	/* NOTE: This function should not be modified, when the callback is
	   needed, the HAL_OSPI_CmdCpltCallback could be implemented in the user
	   file
	 */
}
...
__weak void HAL_OSPI_RxCpltCallback(OSPI_HandleTypeDef *hospi)
{
	...
	/* NOTE: This function should not be modified, when the callback is
	   needed, the HAL_OSPI_RxCpltCallback could be implemented in the user
	   file
	 */
}
...
__weak void HAL_OSPI_TxCpltCallback(OSPI_HandleTypeDef *hospi)
{
	...
	/* NOTE: This function should not be modified, when the callback is
	   needed, the HAL_OSPI_TxCpltCallback could be implemented in the user
	   file
	 */
}
...
__weak void HAL_OSPI_RxHalfCpltCallback(OSPI_HandleTypeDef *hospi)
{
	...
	/* NOTE: This function should not be modified, when the callback is
	   needed, the HAL_OSPI_RxHalfCpltCallback could be implemented in the
	   user file
	 */
}
...
__weak void HAL_OSPI_TxHalfCpltCallback(OSPI_HandleTypeDef *hospi)
{
	...
	/* NOTE: This function should not be modified, when the callback is
	   needed, the HAL_OSPI_TxHalfCpltCallback could be implemented in the
	   user file
	 */
}
...
__weak void HAL_OSPI_StatusMatchCallback(OSPI_HandleTypeDef *hospi)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_OSPI_StatusMatchCallback could be implemented in the
	   user file
	 */
}
...
__weak void HAL_OSPI_TimeOutCallback(OSPI_HandleTypeDef *hospi)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_OSPI_TimeOutCallback could be implemented in the user
	   file
	 */
}
 ...
 */
HAL_StatusTypeDef
HAL_OSPI_RegisterCallback(OSPI_HandleTypeDef *hospi,
 ...
 */
HAL_StatusTypeDef
HAL_OSPI_UnRegisterCallback(OSPI_HandleTypeDef *hospi,
			    HAL_OSPI_CallbackIDTypeDef CallbackID)
{
	HAL_StatusTypeDef status = HAL_OK;
...
HAL_StatusTypeDef HAL_OSPIM_Config(OSPI_HandleTypeDef *hospi,
				   OSPIM_CfgTypeDef *cfg, uint32_t Timeout)
{
	HAL_StatusTypeDef status = HAL_OK;
	uint32_t instance;
	uint8_t index;
	...
	/* Check the parameters of the OctoSPI IO Manager configuration
	 * structure */
	...
	if (status == HAL_OK) {
		...
		if ((OCTOSPIM->CR & OCTOSPIM_CR_MUXEN) != 0U) {
			...
			if (other_instance == 1U) {
				SET_BIT(
				    OCTOSPIM->PCR[(
					IOM_cfg[other_instance].ClkPort - 1U)],
				    OCTOSPIM_PCR_CLKSRC);
				if (IOM_cfg[other_instance].DQSPort != 0U) {
					SET_BIT(
					    OCTOSPIM
						->PCR[(IOM_cfg[other_instance]
							   .DQSPort -
						       1U)],
					    OCTOSPIM_PCR_DQSSRC);
				}
			...
			if (IOM_cfg[instance].ClkPort != 0U) {
				CLEAR_BIT(
				    OCTOSPIM
					->PCR[(IOM_cfg[instance].ClkPort - 1U)],
				    OCTOSPIM_PCR_CLKEN);
				if (IOM_cfg[instance].DQSPort != 0U) {
					CLEAR_BIT(
					    OCTOSPIM->PCR[(
						IOM_cfg[instance].DQSPort -
						1U)],
					    OCTOSPIM_PCR_DQSEN);
				}
		...
		if ((cfg->ClkPort == IOM_cfg[other_instance].ClkPort) ||
			...
			if ((cfg->ClkPort == IOM_cfg[other_instance].ClkPort) &&
				...
				CLEAR_BIT(
				    OCTOSPIM->PCR[(
					IOM_cfg[other_instance].NCSPort - 1U)],
				    OCTOSPIM_PCR_NCSEN);
				if (IOM_cfg[other_instance].IOLowPort !=
				    HAL_OSPIM_IOPORT_NONE) {
					CLEAR_BIT(
		...
		if (((cfg->Req2AckTime) >= 1U) &&
		    ((cfg->Req2AckTime) <= 256U)) {
			if ((cfg->Req2AckTime - 1U) >
		...
		if ((OCTOSPIM->CR & OCTOSPIM_CR_MUXEN) != 0U) {
			...
			if ((cfg->IOHighPort & OCTOSPIM_PCR_IOLEN) != 0U) {
				MODIFY_REG(
				    OCTOSPIM->PCR[((cfg->IOHighPort - 1U) &
						   OSPI_IOM_PORT_MASK)],
				    (OCTOSPIM_PCR_IOLEN | OCTOSPIM_PCR_IOLSRC),
				    (OCTOSPIM_PCR_IOLEN |
		...
		/******* Re-enable both OctoSPI after configure OctoSPI IO
		 * Manager ********/
...
static void OSPI_DMAError(DMA_HandleTypeDef *hdma)
{
	...
	if (HAL_OSPI_Abort_IT(hospi) != HAL_OK) {
		/* Disable the interrupts */
		__HAL_OSPI_DISABLE_IT(hospi, HAL_OSPI_IT_TC | HAL_OSPI_IT_FT |
						 HAL_OSPI_IT_TE);
					 ...
					 OSPIM_CfgTypeDef *cfg)
{
	...
	if ((instance_nb == 0U) || (instance_nb > OSPI_NB_INSTANCE) ||
		...
		if (instance_nb == 2U) {
			if ((OCTOSPIM->CR & OCTOSPIM_CR_MUXEN) == 0U) {
				value =
				    (OCTOSPIM_PCR_CLKSRC | OCTOSPIM_PCR_DQSSRC |
				     OCTOSPIM_PCR_NCSSRC |
				     OCTOSPIM_PCR_IOLSRC_1 |
				     OCTOSPIM_PCR_IOHSRC_1);
			} else {
		...
		for (index = 0U; index < OSPI_IOM_NB_PORTS; index++) {
			...
			if ((reg & OCTOSPIM_PCR_CLKEN) != 0U) {
				...
				if ((reg & OCTOSPIM_PCR_CLKSRC) ==
					...
					/* The clock correspond to the instance
					 * passed as parameter */
			...
			if ((reg & OCTOSPIM_PCR_DQSEN) != 0U) {
				...
				if ((reg & OCTOSPIM_PCR_DQSSRC) ==
					...
					/* The DQS correspond to the instance
					 * passed as parameter */
			...
			if ((reg & OCTOSPIM_PCR_NCSEN) != 0U) {
				...
				if ((reg & OCTOSPIM_PCR_NCSSRC) ==
				    (value & OCTOSPIM_PCR_NCSSRC)) {
					/* The nCS correspond to the instance
					 * passed as parameter */
			...
			if ((reg & OCTOSPIM_PCR_IOLEN) != 0U) {
				...
				if ((reg & OCTOSPIM_PCR_IOLSRC_1) ==
					...
					/* The IO Low correspond to the instance
					 * passed as parameter */
					if ((reg & OCTOSPIM_PCR_IOLSRC_0) ==
					    0U) {
						cfg->IOLowPort =
						    (OCTOSPIM_PCR_IOLEN |
						     (index + 1U));
			...
			if ((reg & OCTOSPIM_PCR_IOHEN) != 0U) {
				...
				if ((reg & OCTOSPIM_PCR_IOHSRC_1) ==
					...
					/* The IO High correspond to the
					 * instance passed as parameter */
					if ((reg & OCTOSPIM_PCR_IOHSRC_0) ==
					    0U) {
						cfg->IOHighPort =
						    (OCTOSPIM_PCR_IOLEN |
						     (index + 1U));
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_OSPI_DLYB_SetConfig(OSPI_HandleTypeDef *hospi,
					  HAL_OSPI_DLYB_CfgTypeDef *pdlyb_cfg)
{
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_OSPI_DLYB_GetConfig(const OSPI_HandleTypeDef *hospi,
					  HAL_OSPI_DLYB_CfgTypeDef *pdlyb_cfg)
{
	HAL_StatusTypeDef status = HAL_ERROR;
 ...
 */
HAL_StatusTypeDef
HAL_OSPI_DLYB_GetClockPeriod(OSPI_HandleTypeDef *hospi,
			     HAL_OSPI_DLYB_CfgTypeDef *pdlyb_cfg)
{
	HAL_StatusTypeDef status = HAL_ERROR;
