/**
  ...
  ******************************************************************************
  ...
  using the functions :
    ...
    (#) Set or get or clear the autonomous mode configuration using these
  functions :
 ...
 *                the configuration information for the specified I2Cx
 * peripheral.
 * @param  sConfig Pointer to a I2C_AutonomousModeConfTypeDef structure that
 * contains the configuration information of the autonomous mode for the
				  ...
				  const I2C_AutonomousModeConfTypeDef *sConfig)
{
	if (hi2c->State == HAL_I2C_STATE_READY) {
		...
		assert_param(IS_I2C_TRIG_SOURCE(hi2c->Instance,
						sConfig->TriggerSelection));
		assert_param(
		    IS_I2C_AUTO_MODE_TRG_POL(sConfig->TriggerPolarity));

		/* Disable the selected I2C peripheral to be able to configure
		 * AUTOCR */
		__HAL_I2C_DISABLE(hi2c);

		/* I2Cx AUTOCR Configuration */
		WRITE_REG(
		    hi2c->Instance->AUTOCR,
				  ...
				  I2C_AutonomousModeConfTypeDef *sConfig)
{
	...
	if (IS_I2C_GRP2_INSTANCE(hi2c->Instance)) {
		sConfig->TriggerSelection =
		    ((autocr_tmp & I2C_AUTOCR_TRIGSEL) | I2C_TRIG_GRP2);
	} else {
		sConfig->TriggerSelection =
		    ((autocr_tmp & I2C_AUTOCR_TRIGSEL) | I2C_TRIG_GRP1);
	}
...
HAL_StatusTypeDef HAL_I2CEx_ClearConfigAutonomousMode(I2C_HandleTypeDef *hi2c)
{
	if (hi2c->State == HAL_I2C_STATE_READY) {
		...
		/* Disable the selected I2C peripheral to be able to clear
		 * AUTOCR */
		__HAL_I2C_DISABLE(hi2c);
