/**
 ...
 simultaneously in Channel 1 and Channel 2.
     ...
     *** Autonomous Mode operation ***
     ===================================
     [..]
      (+) Use HAL_DACx_SetConfigAutonomousMode() to configure the autonomous
 mode
      (+) Use HAL_DACx_GetConfigAutonomousMode() to get the current
 configuration of the autonomous mode
      (+) Use HAL_DACx_ClearConfigAutonomousMode() to clear the configuration of
 the autonomous mode
					  ...
					  uint32_t Channel)
{
	...
	if ((hdac == NULL) || (sConfig == NULL)) {
		...
		MODIFY_REG(hdac->Instance->MCR,
			   (DAC_MCR_MODE1 << (Channel & 0x10UL)), 0U);
 ...
 * @note  The autonomous mode applies to the 2 channels of a DAC block (same for
 * both channels)
 * @param hdac pointer to a DAC_HandleTypeDef structure that contains
 *        the configuration information for the specified DAC.
 ...
 */
HAL_StatusTypeDef
HAL_DACEx_SetConfigAutonomousMode(DAC_HandleTypeDef *hdac,
 ...
 */
HAL_StatusTypeDef
HAL_DACEx_GetConfigAutonomousMode(const DAC_HandleTypeDef *hdac,
				  DAC_AutonomousModeConfTypeDef *sConfig)
{
	...
	sConfig->AutonomousModeState =
	    READ_BIT(hdac->Instance->AUTOCR, DAC_AUTOCR_AUTOMODE);
