					       ...
					       uint32_t XferOptions)
{
	...
	if (hsmbus->State == HAL_SMBUS_STATE_READY) {
		...
		if ((hsmbus->XferSize < hsmbus->XferCount) &&
			...
			if ((hsmbus->Instance->AUTOCR & I2C_AUTOCR_TRIGEN) ==
			    I2C_AUTOCR_TRIGEN) {
			...
			if ((hsmbus->PreviousState ==
			    ...
			    (IS_SMBUS_TRANSFER_OTHER_OPTIONS_REQUEST(tmp) ==
			     0)) {
					  ...
					  uint32_t Timeout)
{
	...
	if (hsmbus->State == HAL_SMBUS_STATE_READY) {
		...
		do {
			/* Generate Start */
			hsmbus->Instance->CR2 = SMBUS_GENERATE_START(
			    hsmbus->Init.AddressingMode, DevAddress);
			...
			while ((tmp1 == RESET) && (tmp2 == RESET)) {
				...
				tmp1 = __HAL_SMBUS_GET_FLAG(hsmbus,
							    SMBUS_FLAG_STOPF);
				tmp2 =
					  ...
					  uint32_t StatusFlags)
{
	...
	if (SMBUS_CHECK_FLAG(StatusFlags, SMBUS_FLAG_AF) != RESET) {
		...
		if (hsmbus->State == HAL_SMBUS_STATE_MASTER_BUSY_TX) {
			...
			if (SMBUS_CHECK_FLAG(StatusFlags, SMBUS_FLAG_RXNE) !=
			    RESET) {
				/* Read data from RXDR */
				*hsmbus->pBuffPtr =
				    (uint8_t)(hsmbus->Instance->RXDR);
		...
		*hsmbus->pBuffPtr = (uint8_t)(hsmbus->Instance->RXDR);
		...
		if ((hsmbus->XferCount != 0U) && (hsmbus->XferSize == 0U)) {
			DevAddress =
			    (uint16_t)(hsmbus->Instance->CR2 & I2C_CR2_SADD);
		...
		if (hsmbus->XferCount == 0U) {
			...
			/* Call TxCpltCallback() if no stop mode is set */
			else if (SMBUS_GET_STOP_MODE(hsmbus) !=
				 SMBUS_AUTOEND_MODE) {
					 ...
					 uint32_t StatusFlags)
{
	...
	if (SMBUS_CHECK_FLAG(StatusFlags, SMBUS_FLAG_STOPF) != RESET) {
		if ((hsmbus->State & HAL_SMBUS_STATE_LISTEN) ==
		    HAL_SMBUS_STATE_LISTEN) {
			/* Store Last receive data if any */
			if (__HAL_SMBUS_GET_FLAG(hsmbus, SMBUS_FLAG_RXNE) !=
			    RESET) {
				/* Read data from RXDR */
				*hsmbus->pBuffPtr =
				    (uint8_t)(hsmbus->Instance->RXDR);
			      ...
			      uint32_t InterruptRequest)
{
	...
	if ((InterruptRequest & SMBUS_IT_TX) == SMBUS_IT_TX) {
		...
		if ((tmpstate & HAL_SMBUS_STATE_LISTEN) !=
		    HAL_SMBUS_STATE_LISTEN) {
