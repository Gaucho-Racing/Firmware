...
typedef struct {
	uint32_t
	    Mode; /*!< Specifies the PKA operation mode.
...
{
	return ((READ_BIT(PKAx->CR, PKA_CR_OPERRIE) == (PKA_CR_OPERRIE)) ? 1UL
									 : 0UL);
}
...
{
	return ((READ_BIT(PKAx->SR, PKA_SR_RAMERRF) == (PKA_SR_RAMERRF)) ? 1UL
									 : 0UL);
}
...
{
	return ((READ_BIT(PKAx->SR, PKA_SR_OPERRF) == (PKA_SR_OPERRF)) ? 1UL
								       : 0UL);
}
