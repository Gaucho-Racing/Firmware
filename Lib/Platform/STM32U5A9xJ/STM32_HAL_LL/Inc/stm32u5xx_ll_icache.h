...
typedef struct {
	...
	uint32_t
	    RemapAddress; /*!< Configures the remap address to be remapped */
...
{
	return ((READ_BIT(ICACHE->CR, ICACHE_CR_EN) == (ICACHE_CR_EN)) ? 1UL
								       : 0UL);
}
...
{
	return ((READ_BIT(ICACHE->IER, ICACHE_IER_BSYENDIE) ==
		 (ICACHE_IER_BSYENDIE))
		    ? 1UL
		    : 0UL);
}
