...
HAL_StatusTypeDef HAL_SPI_Init(SPI_HandleTypeDef *hspi)
{
	...
	if (IS_SPI_LIMITED_INSTANCE(hspi->Instance)) {
		...
		assert_param(
		    IS_SPI_LIMITED_FIFOTHRESHOLD(hspi->Init.FifoThreshold));
	} else {
	...
	if (hspi->Init.CRCCalculation == SPI_CRCCALCULATION_ENABLE) {
		if (IS_SPI_LIMITED_INSTANCE(hspi->Instance)) {
			assert_param(
			    IS_SPI_LIMITED_CRC_LENGTH(hspi->Init.CRCLength));
		} else {
	...
	assert_param(
	    IS_SPI_MASTER_RX_AUTOSUSP(hspi->Init.MasterReceiverAutoSusp));
	...
	if (hspi->Init.CRCCalculation == SPI_CRCCALCULATION_ENABLE) {
		...
		assert_param(IS_SPI_CRC_POLYNOMIAL_SIZE(
		    hspi->Init.CRCPolynomial, crc_length));
	...
	if (hspi->Init.CRCCalculation == SPI_CRCCALCULATION_ENABLE) {
		...
		if (((IS_SPI_LIMITED_INSTANCE(hspi->Instance)) &&
			...
			crc_poly_msb_mask =
			    (0x1UL
			     << ((crc_length >> SPI_CFG1_CRCSIZE_Pos) + 0x1U));
			WRITE_REG(hspi->Instance->CRCPOLY,
				  (hspi->Init.CRCPolynomial) |
				      crc_poly_msb_mask);
		}
			   ...
			   HAL_SPI_CallbackIDTypeDef CallbackID)
{
	...
	if (HAL_SPI_STATE_READY == hspi->State) {
		switch (CallbackID) {
			...
			case HAL_SPI_SUSPEND_CB_ID:
				hspi->SuspendCallback =
				    HAL_SPI_SuspendCallback; /* Legacy weak
								SuspendCallback
							      */
				break;
				   ...
				   uint32_t Timeout)
{
	...
	if (((hspi->Instance->AUTOCR & SPI_AUTOCR_TRIGEN) == 0U) &&
	    (hspi->Init.Mode == SPI_MODE_MASTER)) {
				  ...
				  uint16_t Size, uint32_t Timeout)
{
	...
	if ((hspi->Init.DataSize > SPI_DATASIZE_16BIT) &&
		...
		while (hspi->RxXferCount > 0UL) {
			...
			/* Check if transfer is locked because of a suspend */
				 ...
				 * not software */
				if (HAL_IS_BIT_SET(hspi->Instance->CR1,
						   SPI_CR1_CSTART)) {
	...
	if (hspi->Init.CRCCalculation == SPI_CRCCALCULATION_ENABLE) {
		...
		if (SPI_WaitOnFlagUntilTimeout(hspi, SPI_FLAG_EOT, RESET,
	...
	if ((hspi->Init.DataSize > SPI_DATASIZE_16BIT) &&
		...
		while ((initial_TxXferCount > 0UL) ||
			...
			if (initial_RxXferCount > 0UL) {
				...
				/* Check RXWNE flag if RXP cannot be reached */
				else if ((initial_RxXferCount <
					  init_max_data_in_fifo) &&
					 ((temp_sr_reg & SPI_SR_RXWNE_Msk) !=
					  0UL)) {
					*((uint32_t *)hspi->pRxBuffPtr) =
					    *((__IO uint32_t *)&hspi->Instance
						  ->RXDR);
				       ...
				       const uint8_t *pData, uint16_t Size)
{
	...
	if (hspi->Init.DataSize <= SPI_DATASIZE_8BIT) {
		if (hspi->hdmatx->Init.SrcDataWidth ==
		    DMA_SRC_DATAWIDTH_HALFWORD) {
			hspi->TxXferCount =
			    (hspi->TxXferCount + (uint16_t)1UL) >> 1UL;
		}
		if (hspi->hdmatx->Init.SrcDataWidth == DMA_SRC_DATAWIDTH_WORD) {
			hspi->TxXferCount =
			    (hspi->TxXferCount + (uint16_t)3UL) >> 2UL;
		}
	...
	if ((hspi->hdmatx->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
		...
		status = HAL_DMA_Start_IT(
		    hspi->hdmatx, (uint32_t)hspi->pTxBuffPtr,
				      ...
				      uint16_t Size)
{
	...
	/* Adjust XferCount according to DMA alignment / Data size */
	if (hspi->Init.DataSize <= SPI_DATASIZE_8BIT) {
		if (hspi->hdmarx->Init.DestDataWidth ==
		    DMA_DEST_DATAWIDTH_HALFWORD) {
			hspi->RxXferCount =
			    (hspi->RxXferCount + (uint16_t)1UL) >> 1UL;
		}
		if (hspi->hdmarx->Init.DestDataWidth ==
		    DMA_DEST_DATAWIDTH_WORD) {
			hspi->RxXferCount =
			    (hspi->RxXferCount + (uint16_t)3UL) >> 2UL;
		}
	} else if (hspi->Init.DataSize <= SPI_DATASIZE_16BIT) {
		if (hspi->hdmarx->Init.DestDataWidth ==
		    DMA_DEST_DATAWIDTH_WORD) {
	...
	if ((hspi->hdmarx->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
		...
		status = HAL_DMA_Start_IT(
		    hspi->hdmarx, (uint32_t)&hspi->Instance->RXDR,
					      ...
					      uint8_t *pRxData, uint16_t Size)
{
	...
	/* Enable the SPI Error Interrupt Bit */
	__HAL_SPI_ENABLE_IT(
	    hspi, (SPI_IT_OVR | SPI_IT_UDR | SPI_IT_FRE | SPI_IT_MODF));
	...
	if (HAL_IS_BIT_SET(hspi->Instance->CFG1, SPI_CFG1_TXDMAEN)) {
		if (hspi->hdmatx != NULL) {
			...
			if (HAL_DMA_Abort(hspi->hdmatx) != HAL_OK) {
				if (HAL_DMA_GetError(hspi->hdmatx) ==
	...
	if (HAL_IS_BIT_SET(hspi->Instance->CFG1, SPI_CFG1_RXDMAEN)) {
		if (hspi->hdmarx != NULL) {
			...
			if (HAL_DMA_Abort(hspi->hdmarx) != HAL_OK) {
				if (HAL_DMA_GetError(hspi->hdmarx) ==
				    HAL_DMA_ERROR_TIMEOUT) {
					hspi->ErrorCode = HAL_SPI_ERROR_ABORT;
				}
			}
...
void HAL_SPI_IRQHandler(SPI_HandleTypeDef *hspi)
{
	...
	if ((trigger & (SPI_FLAG_MODF | SPI_FLAG_OVR | SPI_FLAG_FRE |
		...
		if (hspi->ErrorCode != HAL_SPI_ERROR_NONE) {
			...
			__HAL_SPI_DISABLE_IT(hspi, (SPI_IT_EOT | SPI_IT_RXP |
						    SPI_IT_TXP | SPI_IT_MODF |
						    SPI_IT_OVR | SPI_IT_FRE |
						    SPI_IT_UDR));

			/* Disable the SPI DMA requests if enabled */
			if (HAL_IS_BIT_SET(cfg1, SPI_CFG1_TXDMAEN |
						     SPI_CFG1_RXDMAEN)) {
...
static void SPI_AbortTransfer(SPI_HandleTypeDef *hspi)
{
	...
	/* Disable ITs */
	__HAL_SPI_DISABLE_IT(hspi, (SPI_IT_EOT | SPI_IT_TXP | SPI_IT_RXP |
