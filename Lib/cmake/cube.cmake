...
function(download_git)
    ...
    set(one_value_args NAME GIT_REPOSITORY GIT_TAG OUTPUT_PATH_VAR)
    ...
    cmake_parse_arguments("DOWNLOAD_GIT_ARG" "" "${one_value_args}" "${multi_value_args}" ${ARGN})

    message(STATUS "This may take a moment, fetching content for ${DOWNLOAD_GIT_ARG_NAME} from ${DOWNLOAD_GIT_ARG_GIT_REPOSITORY} at tag ${DOWNLOAD_GIT_ARG_GIT_TAG}")
    ...
    if(NOT ${NAME_LOWER}_POPULATED)
        ...
        separate_arguments(TARGET_FOLDER_LIST NATIVE_COMMAND "${DOWNLOAD_GIT_ARG_TARGET_FOLDER}")
        execute_process(
            COMMAND git sparse-checkout set ${TARGET_FOLDER_LIST}
    ...
    message(STATUS "Successfully fetched content for ${DOWNLOAD_GIT_ARG_NAME} from ${DOWNLOAD_GIT_ARG_GIT_REPOSITORY} at tag ${DOWNLOAD_GIT_ARG_GIT_TAG} to ${${DOWNLOAD_GIT_ARG_NAME}_SOURCE_DIR}")

    set("${DOWNLOAD_GIT_ARG_OUTPUT_PATH_VAR}" "${${DOWNLOAD_GIT_ARG_NAME}_SOURCE_DIR}")
...
function(add_arm_cmsis_core)
    ...
    set(one_value_args MAJOR GIT_TAG)
    ...
    if(NOT ARM_CMSIS_CORE_ARG_MAJOR MATCHES "^[0-9]$")
        message(FATAL_ERROR "MAJOR '${ARM_CMSIS_CORE_ARG_MAJOR}' is not a valid version number (ie '5')")
    endif()
    ...
    message(STATUS "Successfully fetched content for ARM CMSIS core version ${ARM_CMSIS_CORE_ARG_MAJOR} from ${ARM_CMSIS_CORE_ARG_GIT_TAG} to ${ARM_CMSIS_CORE_PATH}")
...
function(add_arm_cmsis_interface)
    ...
    set(one_value_args SERIES SPECIFIER GIT_TAG)
    unset(ARM_CMSIS_INTERFACE_ARG_SERIES)
    unset(ARM_CMSIS_INTERFACE_ARG_SPECIFIER)
    unset(ARM_CMSIS_INTERFACE_ARG_GIT_TAG)
    cmake_parse_arguments("ARM_CMSIS_INTERFACE_ARG" "" "${one_value_args}" "" ${ARGN})

    message(VERBOSE "Adding ARM CMSIS device interface for SERIES '${ARM_CMSIS_INTERFACE_ARG_SERIES}', SPECIFIER '${ARM_CMSIS_INTERFACE_ARG_SPECIFIER}', and GIT_TAG '${ARM_CMSIS_INTERFACE_ARG_GIT_TAG}'")

    string(LENGTH "${ARM_CMSIS_INTERFACE_ARG_SERIES}" SERIES_LENGTH)
    if(NOT SERIES_LENGTH EQUAL 2)
        message(FATAL_ERROR "SERIES '${ARM_CMSIS_INTERFACE_ARG_SERIES}' must be 2 characters long (ie 'g4')")
    endif()
    ...
    if(SPECIFIER_LENGTH LESS 6)
        message(FATAL_ERROR "SPECIFIER '${ARM_CMSIS_INTERFACE_ARG_SPECIFIER}' must be at least 6 characters long (ie 'g474re' or 'g474ret6')")
    endif()
    ...
    string(FIND "${ARM_CMSIS_INTERFACE_ARG_SPECIFIER}" "${ARM_CMSIS_INTERFACE_ARG_SERIES}" SERIES_POSITION_IN_SPECIFIER)
    if(NOT SERIES_POSITION_IN_SPECIFIER EQUAL 0)
        message(FATAL_ERROR "SPECIFIER '${ARM_CMSIS_INTERFACE_ARG_SPECIFIER}' must start with SERIES '${ARM_CMSIS_INTERFACE_ARG_SERIES}' at the beginning (eg 'g4' and 'g474ret6')")
    endif()
...
function(add_stm32_hal_interface)
    ...
    if(NOT SERIES_LENGTH EQUAL 2)
        message(FATAL_ERROR "SERIES '${STM32_HAL_ARG_SERIES}' must be 2 characters long (ie 'g4')")
    endif()
    ...
    message(STATUS "Successfully fetched content for STM32 HAL driver ${SERIES_UPPER} from ${STM32_HAL_ARG_GIT_TAG} to ${STM32_HAL_DRIVER_PATH}")
