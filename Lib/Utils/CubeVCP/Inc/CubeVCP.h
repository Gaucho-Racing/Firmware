 ...
 * @note This function should be called after the UART handle has been properly initialized and configured by STM32CubeMX.
 */
 ...
 * @note The data buffer does not have to remain valdid after the function returns, as the data will be copied into an internal buffer for transmission.
 ...
 * sent.
 */
HAL_StatusTypeDef VCP_SendString(const char *const data, uint16_t length);
