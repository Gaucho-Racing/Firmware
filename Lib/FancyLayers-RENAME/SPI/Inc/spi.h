...
typedef struct {
	SPI_TypeDef *SPIx; // Pointer to SPI register wrapper (e.g. SPI1, SPI2,
			   // SPI3 macro defines)
	// COPI, CIPO, SCLK, CS
	GPIO_TypeDef **GPIOx; // Pointer to GPIO port register wrapper (e.g.
			      // GPIOA, GPIOB, GPIOC, etc. macro defines)
	// COPI, CIPO, SCLK, CS
	uint32_t *pin_nums; // SPI pin numbers (e.g. LL_GPIO_PIN_0,
...
/**
 * @brief Read from the SPI buffer; primarily intended for polling. Rely on the
 * passed in handler when setup() is called
 *
 * @param handle
 * @return SPI_Message
 */
