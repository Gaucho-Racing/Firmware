...
void GR_SPI_Initialize(GR_SPI_Handler* handle, LL_SPI_InitTypeDef* config, GR_SPI_Pins* pin_config) {    
    ...
    *handle->spi_config = *config;
    ...
    else handle->transfer_size = GR_SPI_TRANSFER_SIZE_16;
...
uint32_t GR_SPI_Get_IRQn(SPI_TypeDef* SPIx) {
    switch((uint32_t) SPIx) {
        case (uint32_t) SPI1: return SPI1_IRQn; //35
        case (uint32_t) SPI2: return SPI2_IRQn; //36
        case (uint32_t) SPI3: return SPI3_IRQn; //51
