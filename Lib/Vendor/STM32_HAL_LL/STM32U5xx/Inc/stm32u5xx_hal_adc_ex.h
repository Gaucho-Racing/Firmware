...
typedef struct {
	...
	FunctionalState
	    InjectedOffsetSignedSaturation; /*!< Specify whether the Signed
...
#define ADC_IS_INDEPENDENT(__HANDLE__)                                         \
	(((((__HANDLE__)->Instance) == ADC4)) ? SET : RESET)
...
#define ADC_CFGR_REG_DISCONTINUOUS(__REG_DISCONTINUOUS_MODE__)                 \
	((__REG_DISCONTINUOUS_MODE__) << ADC_CFGR1_DISCEN_Pos)
...
#define ADC_CFGR_CONTINUOUS(__CONTINUOUS_MODE__)                               \
	((__CONTINUOUS_MODE__) << ADC_CFGR1_CONT_Pos)
...
#else
#define ADC_CFGR_DMACONTREQ(__HANDLE__, __DMACONTREQ_MODE__)                   \
	((((__HANDLE__)->Instance == ADC1)) ? ((__DMACONTREQ_MODE__))          \
					    : ((__DMACONTREQ_MODE__) << 1U))
#endif /*ADC2 */
...
#define ADC_NONMULTIMODE_OR_MULTIMODEMASTER(__HANDLE__)                        \
	((((__HANDLE__)->Instance == ADC1) ||                                  \
	  ((__HANDLE__)->Instance == ADC4))                                    \
	     ? SET                                                             \
	     : (((__LL_ADC_COMMON_INSTANCE(__HANDLE__))->CCR &                 \
		 ADC_CCR_DUAL) == RESET))
 ...
 * @note  Offset has to be left-aligned on bit 15, the LSB (right bits) are set
 * to 0 If resolution 14 bits, no shift. If resolution 12 bits, shift of 2 ranks
 * on the left. If resolution 10 bits, shift of 4 ranks on the left. If
 * resolution 8 bits, shift of 6 ranks on the left. If resolution 6 bits, shift
 * of 8 ranks on the left. therefore, shift = (14 - resolution) = 14 - (14 -
 * (((RES[2:0]) >> 2)*2))
 * @param __HANDLE__: ADC handle
...
#define ADC_OFFSET_SHIFT_RESOLUTION(__HANDLE__, __OFFSET__)                    \
	(((__OFFSET__)                                                         \
	  << (((((__HANDLE__)->Instance->CFGR1) & ADC_CFGR1_RES) >> 2UL) *     \
	      2UL)))

/**
 * @brief Shift the AWD1 threshold in function of the selected ADC resolution.
 * @note  Thresholds have to be left-aligned on bit 15, the LSB (right bits) are
 * set to 0. If resolution 14 bits, no shift. If resolution 12 bits, shift of 2
 * ranks on the left. If resolution 10 bits, shift of 4 ranks on the left. If
 * resolution 8 bits, shift of 6 ranks on the left. If resolution 6 bits, shift
 * of 8 ranks on the left. therefore, shift = (14 - resolution) = 14 - (14-
 * (((RES[2:0]) >> 2)*2))
 * @param __HANDLE__: ADC handle
 ...
 * resolution.
 * @note  Thresholds have to be left-aligned on bit 15, the LSB (right bits) are
 * set to 0. If resolution 16 bits, no shift. If resolution 14 bits, shift of 2
 * ranks on the left. If resolution 12 bits, shift of 4 ranks on the left. If
 * resolution 10 bits, shift of 6 ranks on the left. If resolution 8 bits, shift
 * of 8 ranks on the left. therefore, shift = (16 - resolution) = 16 - (16-
 * (((RES[2:0]) >> 2)*2))
 * @param __HANDLE__: ADC handle
 * @param __THRESHOLD__: Value to be shifted
 * @retval None
...
#define ADC_CLEAR_COMMON_CONTROL_REGISTER(__HANDLE__)                          \
	CLEAR_BIT(__LL_ADC_COMMON_INSTANCE((__HANDLE__)->Instance)->CCR,       \
		  ADC_CCR_PRESC | ADC_CCR_VBATEN | ADC_CCR_VSENSEEN |          \
		      ADC_CCR_VREFEN)
...
#define IS_ADC_CALFACT(__CALIBRATION_FACTOR__)                                 \
	((__CALIBRATION_FACTOR__) <= (0xFFFFU))
...
#define IS_ADC_OVERSAMPLING_RATIO(__RATIO__)                                   \
	((((__RATIO__) & ADC4_OVERSAMPLING_RATIO_PARAMETER) !=                 \
