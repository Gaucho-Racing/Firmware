...
typedef struct {
	...
	uint32_t NSS; /*!< Specifies whether the NSS signal is managed by
			 ...
			 bit. This parameter can be a value of
	...
	uint32_t
	    CRCPolynomial; /*!< Specifies the polynomial used for the CRC
			      calculation. This parameter must be an odd number
			      between Min_Data = 0 and Max_Data = 65535 */

	uint32_t CRCLength; /*!< Specifies the CRC Length used for the CRC
			       calculation. This parameter can be a value of
			       @ref SPI_CRC_length */

	uint32_t NSSPMode; /*!< Specifies whether the NSSP signal is enabled or
			      not . This parameter can be a value of @ref
			      SPI_NSSP_Mode This mode is activated by the SSOM
			      bit in the SPIx_CR2 register and it takes effect
			      only if the SPI interface is configured as
			      Motorola SPI master (FRF=0). */

	uint32_t NSSPolarity; /*!< Specifies which level of SS input/output
				 external signal (present on SS pin) is
				 considered as active one. This parameter can be
				 a value of @ref SPI_NSS_Polarity */
	...
	uint32_t MasterSSIdleness; /*!< Specifies an extra delay, expressed in
				      ...
				      inserted additionally between active edge
				      of SS and first data transaction start in
				      master mode. This parameter can be a value
				      of @ref SPI_Master_SS_Idleness */

	uint32_t
	    MasterInterDataIdleness; /*!< Specifies minimum time delay
					(expressed in SPI clock cycles periods)
					    inserted between two consecutive
					data frames in master mode. This
					parameter can be a value of
					    @ref SPI_Master_InterData_Idleness
				      */

	uint32_t
	    MasterReceiverAutoSusp; /*!< Control continuous SPI transfer in
	...
	uint32_t MasterKeepIOState; /*!< Control of Alternate function GPIOs
				       ...
				       @ref SPI_Master_Keep_IO_State */

	uint32_t
	    IOSwap; /*!< Invert MISO/MOSI alternate functions
			   This parameter can be a value of @ref SPI_IO_Swap */

	uint32_t
	    ReadyMasterManagement; /*!< Specifies if RDY Signal is managed
	...
	uint32_t ReadyPolarity; /*!< Specifies which level of RDY Signal input
				   (present on RDY pin) is considered as active
