/**
  ...
  ******************************************************************************
  ...
  [..]
      ...
      one instance: COMP1 (in this case, all comments related to pair of
  comparators are not applicable)
  ...
  instances of STM32U5xx devices.
...
HAL_StatusTypeDef HAL_COMP_Init(COMP_HandleTypeDef *hcomp)
{
	...
	if (hcomp == NULL) {
...
#if defined(COMP2)
		assert_param(IS_COMP_WINDOWMODE(hcomp->Instance,
						hcomp->Init.WindowMode));
#endif /* COMP2 */
		if (hcomp->Init.WindowMode != COMP_WINDOWMODE_DISABLE) {
			assert_param(
			    IS_COMP_WINDOWOUTPUT(hcomp->Init.WindowOutput));
		}
		...
		MODIFY_REG(hcomp->Instance->CSR,
			   COMP_CSR_PWRMODE | COMP_CSR_INMSEL |
			       COMP_CSR_INPSEL | COMP_CSR_WINMODE |
...
#if defined(COMP_WINDOW_MODE_SUPPORT)
		/* Set window mode */
		/* Note: Window mode bit is located into 1 out of the 2 pairs of
		 * COMP     */
		/*       instances. Therefore, this function can update another
		 * COMP      */
		/*       instance that the one currently selected. */

		if (hcomp->Init.WindowMode ==
		    COMP_WINDOWMODE_COMP1_INPUT_PLUS_COMMON) {
			CLEAR_BIT(COMP12_COMMON->CSR_ODD, COMP_CSR_WINMODE);
			SET_BIT(COMP12_COMMON->CSR_EVEN, COMP_CSR_WINMODE);
		} else if (hcomp->Init.WindowMode ==
			   COMP_WINDOWMODE_COMP2_INPUT_PLUS_COMMON) {
		...
		/* Note: Window mode mode output can also be used when window
		 * mode        */
		...
		switch (hcomp->Init.WindowOutput) {
			case COMP_WINDOWOUTPUT_COMP1:
				SET_BIT(COMP12_COMMON->CSR_ODD,
					COMP_CSR_WINOUT);
				CLEAR_BIT(COMP12_COMMON->CSR_EVEN,
					  COMP_CSR_WINOUT);
				break;

			case COMP_WINDOWOUTPUT_COMP2:
				CLEAR_BIT(COMP12_COMMON->CSR_ODD,
					  COMP_CSR_WINOUT);
				SET_BIT(COMP12_COMMON->CSR_EVEN,
					COMP_CSR_WINOUT);
				break;
...
__weak void HAL_COMP_MspInit(COMP_HandleTypeDef *hcomp)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_COMP_MspInit could be implemented in the user file
	 */
}
...
__weak void HAL_COMP_MspDeInit(COMP_HandleTypeDef *hcomp)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_COMP_MspDeInit could be implemented in the user file
	 */
}
...
HAL_StatusTypeDef HAL_COMP_Stop(COMP_HandleTypeDef *hcomp)
{
	...
	if (hcomp == NULL) {
		...
		/* Check compliant states: HAL_COMP_STATE_READY or
		 * HAL_COMP_STATE_BUSY    */
...
void HAL_COMP_IRQHandler(COMP_HandleTypeDef *hcomp)
{
	...
	uint32_t comparator_window_mode =
	    READ_BIT(hcomp->Instance->CSR, COMP_CSR_WINMODE);
#endif /* COMP_WINDOW_MODE_SUPPORT */
	...
	if (READ_BIT(EXTI->RPR1, exti_line) != 0UL) {
		...
		if (comparator_window_mode != 0UL) {
			 ...
			 * comparators          */
			/* in window mode. */
			/* Note: Pair of comparators in window mode can both
			 * trig IRQ when      */
			/*       input voltage is changing from "out of window"
			 * area            */
			/*       (low or high ) to the other "out of window"
			 * area (high or low).*/
			/*       Both flags must be cleared to call comparator
			 * trigger          */
...
__weak void HAL_COMP_TriggerCallback(COMP_HandleTypeDef *hcomp)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_COMP_TriggerCallback should be implemented in the
	   user file
	 */
}
