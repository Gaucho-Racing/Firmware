				...
				if (tmp_hal_status == HAL_OK) {
					...
					if (CalibrationMode == ADC_CALIB_OFFSET_LINEARITY) {
						MODIFY_REG(hadc->Instance->CR,
							   ADC_CR_ADCALLIN | ADC_CR_BITS_PROPERTY_RS, ADC_CR_ADCALLIN);
					}
		...
		if (tmp_hal_status == HAL_OK) {
			...
			for (cnt = 0UL; cnt <= 8UL; cnt++) {
				...
				pLinearCalib_Buffer[cnt] =
				    (uint32_t)(READ_BIT(hadc->Instance->CALFACT2, ADC_CALFACT2_CALFACT_Msk));
			}
		}
		...
		if (tmp_hal_status == HAL_OK) {
			...
			for (cnt = 0UL; cnt <= 7UL; cnt++) {
				...
				if (cnt == 7UL) {
					 ...
					 * concatenated */
					MODIFY_REG(hadc->Instance->CALFACT2, ADC_CALFACT2_CALFACT,
						   pLinearCalib_Buffer[cnt] |
						       ((pLinearCalib_Buffer[cnt + 1UL] & 0xFF000000UL) >> 8UL));
				} else {
					MODIFY_REG(hadc->Instance->CALFACT2, ADC_CALFACT2_CALFACT,
						   pLinearCalib_Buffer[cnt]);
				}
...
HAL_StatusTypeDef HAL_ADCEx_InjectedPollForConversion(ADC_HandleTypeDef *hadc, uint32_t Timeout)
{
	...
	if ((tmp_adc_inj_is_trigger_source_sw_start != 0UL) ||
	    ((READ_BIT(tmp_cfgr, ADC_CFGR1_JAUTO) == 0UL) &&
	     ((tmp_adc_reg_is_trigger_source_sw_start != 0UL) && (READ_BIT(tmp_cfgr, ADC_CFGR1_CONT) == 0UL)))) {
			...
			if ((hadc->DMA_Handle->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
				if ((hadc->DMA_Handle->LinkedListQueue != NULL) &&
					...
					if (HAL_DMAEx_List_GetNodeConfig(
						&node_conf, hadc->DMA_Handle->LinkedListQueue->Head) != HAL_OK) {
						return HAL_ERROR;
					}
					...
					hadc->DMA_Handle->LinkedListQueue->Head
					    ->LinkRegisters[NODE_CSAR_DEFAULT_OFFSET] = (uint32_t)&tmp_adc_common->CDR;
					hadc->DMA_Handle->LinkedListQueue->Head
				...
				tmp_hal_status = HAL_DMA_Start_IT(hadc->DMA_Handle, (uint32_t)&tmp_adc_common->CDR,
		...
		if (tmp_hal_status == HAL_OK) {
			...
			if ((hadc->DMA_Handle->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
				if ((hadc->DMA_Handle->LinkedListQueue != NULL) &&
				    (hadc->DMA_Handle->LinkedListQueue->Head != NULL)) {
					hadc->DMA_Handle->LinkedListQueue->Head
					    ->LinkRegisters[NODE_CBR1_DEFAULT_OFFSET] = (LengthInBytes * 2U);
					hadc->DMA_Handle->LinkedListQueue->Head
					    ->LinkRegisters[NODE_CSAR_DEFAULT_OFFSET] = (uint32_t)&tmp_adc_common->CDR2;
					hadc->DMA_Handle->LinkedListQueue->Head
					    ->LinkRegisters[NODE_CDAR_DEFAULT_OFFSET] = (uint32_t)pData;
					tmp_hal_status = HAL_DMAEx_List_Start_IT(hadc->DMA_Handle);
				} else {
				...
				tmp_hal_status = HAL_DMA_Start_IT(hadc->DMA_Handle, (uint32_t)&tmp_adc_common->CDR2,
...
HAL_StatusTypeDef HAL_ADCEx_InjectedConfigChannel(ADC_HandleTypeDef *hadc, ADC_InjectionConfTypeDef *pConfigInjected)
{
	...
	if (LL_ADC_INJ_IsConversionOngoing(hadc->Instance) == 0UL) {
		...
		hadc->Instance->PCSEL |=
		    (1UL << (__LL_ADC_CHANNEL_TO_DECIMAL_NB(pConfigInjected->InjectedChannel) & 0x1FUL));

		/* If auto-injected mode is disabled: no constraint */
		if (pConfigInjected->AutoInjectedConv == DISABLE) {
			MODIFY_REG(
			    hadc->Instance->CFGR1, ADC_CFGR1_JDISCEN,
	...
	if ((tmp_adc_is_conversion_on_going_regular == 0UL) && (tmp_adc_is_conversion_on_going_injected == 0UL)) {
		...
		if (pConfigInjected->InjectedOffsetNumber != ADC_OFFSET_NONE) {
			...
			if (__LL_ADC_CHANNEL_TO_DECIMAL_NB(LL_ADC_GetOffsetChannel(hadc->Instance, LL_ADC_OFFSET_3)) ==
			    __LL_ADC_CHANNEL_TO_DECIMAL_NB(pConfigInjected->InjectedChannel)) {
				LL_ADC_SetOffset(hadc->Instance, LL_ADC_OFFSET_3, pConfigInjected->InjectedChannel,
						 0x0);
			}
	...
	if (LL_ADC_IsEnabled(hadc->Instance) == 0UL) {
		...
		if (pConfigInjected->InjectedSingleDiff == ADC_DIFFERENTIAL_ENDED) {
			...
			tmp_channel = __LL_ADC_DECIMAL_NB_TO_CHANNEL(
			    (__LL_ADC_CHANNEL_TO_DECIMAL_NB(pConfigInjected->InjectedChannel) + 1UL) & 0x1FUL);
