	...
	if (hsai->State == HAL_SAI_STATE_READY) {
		...
		if ((hsai->hdmatx->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
			if (hsai->hdmatx->LinkedListQueue != NULL) {
				/* Set DMA data size */
				hsai->hdmatx->LinkedListQueue->Head->LinkRegisters[NODE_CBR1_DEFAULT_OFFSET] =
				    dmaSrcSize;

				/* Set DMA source address */
				hsai->hdmatx->LinkedListQueue->Head->LinkRegisters[NODE_CSAR_DEFAULT_OFFSET] =
			...
			status = HAL_DMA_Start_IT(hsai->hdmatx, (uint32_t)hsai->pBuffPtr, (uint32_t)&hsai->Instance->DR,
						  dmaSrcSize);
		}
	...
	if (hsai->State == HAL_SAI_STATE_READY) {
		...
		if ((hsai->hdmarx->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
			if (hsai->hdmarx->LinkedListQueue != NULL) {
				/* Set DMA data size */
				hsai->hdmarx->LinkedListQueue->Head->LinkRegisters[NODE_CBR1_DEFAULT_OFFSET] =
				    dmaSrcSize;
				...
				/* Set DMA destination address */
				hsai->hdmarx->LinkedListQueue->Head->LinkRegisters[NODE_CDAR_DEFAULT_OFFSET] =
				    (uint32_t)hsai->pBuffPtr;
			...
			status = HAL_DMA_Start_IT(hsai->hdmarx, (uint32_t)&hsai->Instance->DR, (uint32_t)hsai->pBuffPtr,
						  dmaSrcSize);
		}
