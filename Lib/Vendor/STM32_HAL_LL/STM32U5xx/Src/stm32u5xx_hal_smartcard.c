	...
	if (hsmartcard->gState == HAL_SMARTCARD_STATE_READY) {
		...
		if ((hsmartcard->hdmatx->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
			if ((hsmartcard->hdmatx->LinkedListQueue != NULL) &&
				...
				/* Set DMA data size */
				hsmartcard->hdmatx->LinkedListQueue->Head->LinkRegisters[NODE_CBR1_DEFAULT_OFFSET] =
				    Size;
	...
	if (hsmartcard->RxState == HAL_SMARTCARD_STATE_READY) {
		...
		if ((hsmartcard->hdmarx->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
			if ((hsmartcard->hdmarx->LinkedListQueue != NULL) &&
				...
				/* Set DMA data size */
				hsmartcard->hdmarx->LinkedListQueue->Head->LinkRegisters[NODE_CBR1_DEFAULT_OFFSET] =
				    Size;
