...
static HAL_StatusTypeDef SPI_WaitOnFlagUntilTimeout(const SPI_HandleTypeDef *hspi, uint32_t Flag, FlagStatus FlagStatus, uint32_t Timeout, uint32_t Tickstart);
...
HAL_StatusTypeDef HAL_SPI_Init(SPI_HandleTypeDef *hspi)
{
	...
	if (((IS_SPI_LIMITED_INSTANCE(hspi->Instance)) && (packet_length > SPI_LOWEND_FIFO_SIZE)) || ((IS_SPI_FULL_INSTANCE(hspi->Instance)) && (packet_length > SPI_HIGHEND_FIFO_SIZE)))
	{
		return HAL_ERROR;
	}
#if (USE_SPI_CRC != 0UL)
	if (hspi->Init.CRCCalculation == SPI_CRCCALCULATION_ENABLE)
	{
		/* Verify that the SPI instance supports CRC Length higher than
		 * 16bits */
	...
	NSS management, Communication speed, First bit, CRC calculation state,
	CRC Length */

	/* SPIx NSS Software Management Configuration */
	if ((hspi->Init.NSS == SPI_NSS_SOFT) &&
	    (((hspi->Init.Mode == SPI_MODE_MASTER) && (hspi->Init.NSSPolarity == SPI_NSS_POLARITY_LOW)) || ((hspi->Init.Mode == SPI_MODE_SLAVE) && (hspi->Init.NSSPolarity == SPI_NSS_POLARITY_HIGH))))
	{
	...
	WRITE_REG(hspi->Instance->CFG2,
		  (hspi->Init.NSSPMode | hspi->Init.TIMode | hspi->Init.NSSPolarity | hspi->Init.NSS | hspi->Init.CLKPolarity | hspi->Init.CLKPhase | hspi->Init.FirstBit | hspi->Init.Mode |
		   hspi->Init.MasterInterDataIdleness | hspi->Init.Direction | hspi->Init.MasterSSIdleness | hspi->Init.IOSwap | hspi->Init.ReadyMasterManagement | hspi->Init.ReadyPolarity));
	...
	if (hspi->Init.CRCCalculation == SPI_CRCCALCULATION_ENABLE)
	{
		...
		if (((IS_SPI_LIMITED_INSTANCE(hspi->Instance)) && (crc_length == SPI_CRC_LENGTH_16BIT)) || ((IS_SPI_FULL_INSTANCE(hspi->Instance)) && (crc_length == SPI_CRC_LENGTH_32BIT)))
		{
		...
		else
		{
			...
			/* Write CRC polynomial and set MSB bit at 1 in SPI
			 * Register */
...
__weak void HAL_SPI_MspInit(SPI_HandleTypeDef *hspi)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SPI_MspInit should be implemented in the user file
	 */
}
...
__weak void HAL_SPI_MspDeInit(SPI_HandleTypeDef *hspi)
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SPI_MspDeInit should be implemented in the user file
	 */
}
...
HAL_StatusTypeDef HAL_SPI_UnRegisterCallback(SPI_HandleTypeDef *hspi, HAL_SPI_CallbackIDTypeDef CallbackID)
{
	...
	if (HAL_SPI_STATE_READY == hspi->State)
	{
		switch (CallbackID)
		{
			...
			case HAL_SPI_SUSPEND_CB_ID:
				hspi->SuspendCallback = HAL_SPI_SuspendCallback; /* Legacy weak
										    SuspendCallback
										  */
				break;
	...
	if ((hspi->Init.DataSize > SPI_DATASIZE_16BIT) && (IS_SPI_FULL_INSTANCE(hspi->Instance)))
	{
		...
		while (hspi->TxXferCount > 0UL)
		{
			...
			else
			{
				...
				{
					/* Call standard close procedure with
					 * error check */
...
HAL_StatusTypeDef HAL_SPI_Receive(SPI_HandleTypeDef *hspi, uint8_t *pData, uint16_t Size, uint32_t Timeout)
{
	...
	if ((hspi->Init.DataSize > SPI_DATASIZE_16BIT) && (IS_SPI_FULL_INSTANCE(hspi->Instance)))
	{
		...
		while (hspi->RxXferCount > 0UL)
		{
			...
			{
				/* Verify suspend is triggered by hardware and
				 * not software */
...
HAL_StatusTypeDef HAL_SPI_TransmitReceive(SPI_HandleTypeDef *hspi, const uint8_t *pTxData, uint8_t *pRxData, uint16_t Size, uint32_t Timeout)
{
	...
	if ((hspi->Init.DataSize > SPI_DATASIZE_16BIT) && (IS_SPI_FULL_INSTANCE(hspi->Instance)))
	{
		...
		{
			/* Check TXP flag */
			if ((__HAL_SPI_GET_FLAG(hspi, SPI_FLAG_TXP)) && (initial_TxXferCount > 0UL) && (initial_RxXferCount < (initial_TxXferCount + fifo_length)))
			{
				*((__IO uint32_t *)&hspi->Instance->TXDR) = *((const uint32_t *)hspi->pTxBuffPtr);
				hspi->pTxBuffPtr += sizeof(uint32_t);
				hspi->TxXferCount--;
	...
	else if (hspi->Init.DataSize > SPI_DATASIZE_8BIT)
	{
		...
		while ((initial_TxXferCount > 0UL) || (initial_RxXferCount > 0UL))
		{
			...
			if (initial_RxXferCount > 0UL)
			{
				...
				/* Check RXPLVL flags when RXWNE cannot be
				   reached */
...
HAL_StatusTypeDef HAL_SPI_Transmit_DMA(SPI_HandleTypeDef *hspi, const uint8_t *pData, uint16_t Size)
{
	...
	{
		status = HAL_DMA_Start_IT(hspi->hdmatx, (uint32_t)hspi->pTxBuffPtr, (uint32_t)&hspi->Instance->TXDR, hspi->TxXferCount);
	}
...
HAL_StatusTypeDef HAL_SPI_Receive_DMA(SPI_HandleTypeDef *hspi, uint8_t *pData, uint16_t Size)
{
	...
	{
		status = HAL_DMA_Start_IT(hspi->hdmarx, (uint32_t)&hspi->Instance->RXDR, (uint32_t)hspi->pRxBuffPtr, hspi->RxXferCount);
	}
...
HAL_StatusTypeDef HAL_SPI_TransmitReceive_DMA(SPI_HandleTypeDef *hspi, const uint8_t *pTxData, uint8_t *pRxData, uint16_t Size)
{
	...
	/* Packing mode management is enabled by the DMA settings */
	if (((hspi->Init.DataSize > SPI_DATASIZE_16BIT) && ((hspi->hdmarx->Init.DestDataWidth != DMA_DEST_DATAWIDTH_WORD) || (hspi->hdmatx->Init.SrcDataWidth != DMA_SRC_DATAWIDTH_WORD)) &&
	     (IS_SPI_FULL_INSTANCE(hspi->Instance))) ||
	    ((hspi->Init.DataSize > SPI_DATASIZE_8BIT) && ((hspi->hdmarx->Init.DestDataWidth == DMA_DEST_DATAWIDTH_BYTE) || (hspi->hdmatx->Init.SrcDataWidth == DMA_SRC_DATAWIDTH_BYTE))))
	{
		/* Restriction the DMA data received is not allowed in this mode
		 */
		/* Unlock the process */
...
HAL_StatusTypeDef HAL_SPI_Abort(SPI_HandleTypeDef *hspi)
{
	...
	/* If master communication on going, make sure current frame is done
	...
	if (HAL_IS_BIT_SET(hspi->Instance->CFG1, SPI_CFG1_TXDMAEN))
	{
		if (hspi->hdmatx != NULL)
		{
			/* Abort the SPI DMA Tx Stream/Channel : use blocking
			 * DMA Abort API (no callback) */
			hspi->hdmatx->XferAbortCallback = NULL;
	...
	if (HAL_IS_BIT_SET(hspi->Instance->CFG1, SPI_CFG1_RXDMAEN))
	{
		if (hspi->hdmarx != NULL)
		{
			/* Abort the SPI DMA Rx Stream/Channel : use blocking
			 * DMA Abort API (no callback) */
			hspi->hdmarx->XferAbortCallback = NULL;
...
HAL_StatusTypeDef HAL_SPI_Abort_IT(SPI_HandleTypeDef *hspi)
{
	...
	/* If DMA Tx and/or DMA Rx Handles are associated to SPI Handle, DMA
	   Abort complete callbacks should be initialized before any call to DMA
	   Abort functions */
	...
	if (hspi->hdmarx != NULL)
	{
		if (HAL_IS_BIT_SET(hspi->Instance->CFG1, SPI_CFG1_RXDMAEN))
		{
			/* Set DMA Abort Complete callback if SPI DMA Rx request
			 * if enabled */
	...
	if (hspi->hdmatx != NULL)
	{
		if (HAL_IS_BIT_SET(hspi->Instance->CFG1, SPI_CFG1_TXDMAEN))
		{
			/* Set DMA Abort Complete callback if SPI DMA Tx request
			 * if enabled */
	...
	if ((dma_tx_abort_done == 1UL) && (dma_rx_abort_done == 1UL))
	{
		...
		if (HAL_IS_BIT_SET(hspi->ErrorCode, HAL_SPI_ERROR_ABORT))
		{
			/* return HAL_Error in case of error during Abort
			 * procedure */
			errorcode = HAL_ERROR;
		}
		else
...
void HAL_SPI_IRQHandler(SPI_HandleTypeDef *hspi)
{
	...
	if (HAL_IS_BIT_SET(trigger, SPI_FLAG_EOT))
	{
		...
		/* For the IT based receive extra polling maybe required for
		 * last packet */
		...
		if (hspi->ErrorCode != HAL_SPI_ERROR_NONE)
		{
			...
			if (HAL_IS_BIT_SET(cfg1, SPI_CFG1_TXDMAEN | SPI_CFG1_RXDMAEN))
			{
				...
				if (hspi->hdmarx != NULL)
				{
					...
					will lead to call
					HAL_SPI_ErrorCallback() at end of DMA
					abort procedure */
...
__weak void HAL_SPI_TxCpltCallback(SPI_HandleTypeDef *hspi) /* Derogation MISRAC2012-Rule-8.13 */
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SPI_TxCpltCallback should be implemented in the user
	   file
	 */
}
...
__weak void HAL_SPI_RxCpltCallback(SPI_HandleTypeDef *hspi) /* Derogation MISRAC2012-Rule-8.13 */
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SPI_RxCpltCallback should be implemented in the user
	   file
	 */
}
...
__weak void HAL_SPI_TxRxCpltCallback(SPI_HandleTypeDef *hspi) /* Derogation MISRAC2012-Rule-8.13 */
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SPI_TxRxCpltCallback should be implemented in the
	   user file
	 */
}
...
__weak void HAL_SPI_TxHalfCpltCallback(SPI_HandleTypeDef *hspi) /* Derogation MISRAC2012-Rule-8.13 */
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SPI_TxHalfCpltCallback should be implemented in the
	   user file
	 */
}
...
__weak void HAL_SPI_RxHalfCpltCallback(SPI_HandleTypeDef *hspi) /* Derogation MISRAC2012-Rule-8.13 */
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SPI_RxHalfCpltCallback() should be implemented in the
	   user file
	 */
}
...
__weak void HAL_SPI_TxRxHalfCpltCallback(SPI_HandleTypeDef *hspi) /* Derogation MISRAC2012-Rule-8.13 */
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SPI_TxRxHalfCpltCallback() should be implemented in
	   the user file
	 */
}
...
__weak void HAL_SPI_ErrorCallback(SPI_HandleTypeDef *hspi) /* Derogation MISRAC2012-Rule-8.13 */
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SPI_ErrorCallback should be implemented in the user
	   file
	 */
	/* NOTE : The ErrorCode parameter in the hspi handle is updated by the
	   SPI processes and user can use HAL_SPI_GetError() API to check the
...
__weak void HAL_SPI_AbortCpltCallback(SPI_HandleTypeDef *hspi) /* Derogation MISRAC2012-Rule-8.13 */
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SPI_AbortCpltCallback can be implemented in the user
	   file.
	 */
}
...
__weak void HAL_SPI_SuspendCallback(SPI_HandleTypeDef *hspi) /* Derogation MISRAC2012-Rule-8.13 */
{
	...
	/* NOTE : This function should not be modified, when the callback is
	   needed, the HAL_SPI_SuspendCallback can be implemented in the user
	   file.
	 */
}
 ...
 * @retval HAL status
 */
static HAL_StatusTypeDef SPI_WaitOnFlagUntilTimeout(const SPI_HandleTypeDef *hspi, uint32_t Flag, FlagStatus Status, uint32_t Timeout, uint32_t Tickstart)
{
