...
typedef struct {
	...
	uint32_t InputInverting; /*!< Set OPAMP inverting input connection.
				      ...
				      @note OPAMP inverting input is used with OPAMP in mode standalone or PGA with external capacitors for filtering circuit. Otherwise (OPAMP in mode follower), OPAMP
...
#define LL_OPAMP_MODE_PGA_IO0_BIAS                                                                                                                                                                     \
	(OPAMP_CSR_PGGAIN_3 | OPAMP_CSR_VMSEL_1) /*!< In PGA mode, the inverting input is connected to VINM0                                                                                           \
...
#define LL_OPAMP_MODE_PGA_IO0_IO1_BIAS                                                                                                                                                                 \
	(OPAMP_CSR_PGGAIN_4 | OPAMP_CSR_PGGAIN_3 | OPAMP_CSR_VMSEL_1) /*!< In PGA mode, the inverting input is connected to VINM0                                                                      \
									  - Input signal on VINM0, bias on VINPx: negative gain                                                                        \
									  - Bias on VINM0, input signal on VINPx: positive gain                                                                        \
...
#define LL_OPAMP_INPUT_INVERT_IO0_SEC                                                                                                                                                                  \
	(0x00000000UL) /*!< OPAMP secondary mode is standalone mode - Only applicable if @ref LL_OPAMP_MODE_STANDALONE                                                                                 \
...
#define LL_OPAMP_INPUT_INVERT_PGA_SEC                                                                                                                                                                  \
	(0x00000000UL) /*!< OPAMP secondary mode is PGA mode - Only applicable if configured mode through call to @ref LL_OPAMP_Init()                                                                 \
...
#define LL_OPAMP_INTERNAL_OUTPUT_ENABLED                                                                                                                                                               \
...
#define LL_OPAMP_TRIMMING_NMOS_VREF_90PC_VDDA                                                                                                                                                          \
	(OPAMP_CSR_TRIMOFFSETN | OPAMP_CSR_CALSEL_1 | OPAMP_CSR_CALSEL_0) /*!< OPAMP trimming of transistors differential pair NMOS (internal reference voltage set to 0.9*Vdda). Default parameters   \
									     to be used for calibration using two trimming steps (one with each transistors differential pair NMOS and PMOS). */
...
#define LL_OPAMP_TRIMMING_PMOS_VREF_10PC_VDDA                                                                                                                                                          \
	(OPAMP_CSR_TRIMOFFSETP | OPAMP_CSR_CALSEL_0) /*!< OPAMP trimming of transistors differential pair PMOS (internal reference voltage set to 0.1*Vdda). Default parameters to be used for         \
							calibration using two trimming steps (one with each transistors differential pair NMOS and PMOS). */
...
#define LL_OPAMP_TRIMMING_NMOS                                                                                                                                                                         \
	(LL_OPAMP_TRIMMING_NMOS_VREF_90PC_VDDA) /*!< OPAMP trimming of transistors differential pair NMOS (internal reference voltage set to 0.9*Vdda). Default parameters to be used for calibration  \
						   using two trimming steps (one with each transistors differential pair NMOS and PMOS). */
#define LL_OPAMP_TRIMMING_PMOS                                                                                                                                                                         \
	(LL_OPAMP_TRIMMING_PMOS_VREF_10PC_VDDA) /*!< OPAMP trimming of transistors differential pair PMOS (internal reference voltage set to 0.1*Vdda). Default parameters to be used for calibration  \
						   using two trimming steps (one with each transistors differential pair NMOS and PMOS). */
/**
 * @}
 */
