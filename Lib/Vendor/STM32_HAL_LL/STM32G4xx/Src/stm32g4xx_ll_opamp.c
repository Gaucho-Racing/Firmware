...
#define IS_LL_OPAMP_POWER_MODE(__POWER_MODE__)                                                                         \
	(((__POWER_MODE__) == LL_OPAMP_POWERMODE_NORMALSPEED) || ((__POWER_MODE__) == LL_OPAMP_POWERMODE_HIGHSPEED))
	...
	if (LL_OPAMP_IsLocked(OPAMPx) == 0U) {
		...
		if (OPAMP_InitStruct->FunctionalMode != LL_OPAMP_MODE_FOLLOWER) {
			MODIFY_REG(OPAMPx->CSR,
				   OPAMP_CSR_HIGHSPEEDEN | OPAMP_CSR_CALON | OPAMP_CSR_VMSEL | OPAMP_CSR_VPSEL |
