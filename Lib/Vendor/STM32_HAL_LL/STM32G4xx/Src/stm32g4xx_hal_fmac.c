...
static HAL_StatusTypeDef FMAC_FilterConfig(FMAC_HandleTypeDef *hfmac, FMAC_FilterConfigTypeDef *pConfig, uint8_t PreloadAccess)
{
	...
	if ((pConfig->pCoeffB != NULL) && (pConfig->CoeffBSize != 0U)) {
		...
		if (PreloadAccess == PRELOAD_ACCESS_POLLING) {
			 ...
			 * write */
			return (HAL_DMA_Start_IT(hfmac->hdmaPreload, (uint32_t)pConfig->pCoeffB, (uint32_t)&hfmac->Instance->WDATA, pConfig->CoeffBSize));
		}
	} else {
		/* Change the FMAC state */
		hfmac->State = HAL_FMAC_STATE_READY;
...
static void FMAC_DMAFilterConfig(DMA_HandleTypeDef *hdma)
{
	...
	if (hfmac->pInput != NULL) {
		...
		if (HAL_DMA_Start_IT(hfmac->hdmaPreload, (uint32_t)hfmac->pInput, (uint32_t)&hfmac->Instance->WDATA, hfmac->InputCurrentSize) == HAL_OK) {
			hfmac->pInput = NULL;
			hfmac->InputCurrentSize = 0U;
			return;
		}
