...
HAL_StatusTypeDef HAL_XSPI_Init(XSPI_HandleTypeDef *hxspi)
{
	...
	if (hxspi == NULL) {
		...
		if (hxspi->State == HAL_XSPI_STATE_RESET) {
			...
			MODIFY_REG(hxspi->Instance->DCR3, XSPI_DCR3_MAXTRAN, (hxspi->Init.MaxTran << XSPI_DCR3_MAXTRAN_Pos));
			...
			if (status == HAL_OK) {
				...
				MODIFY_REG(hxspi->Instance->CR, (XSPI_CR_DMM | XSPI_CR_CSSEL), (hxspi->Init.MemoryMode | hxspi->Init.MemorySelect));
#else
