 ...
 */
FLASH_ProcessTypeDef pFlash = {.Lock = HAL_UNLOCKED, .ErrorCode = HAL_FLASH_ERROR_NONE, .ProcedureOnGoing = 0U, .Address = 0U, .Bank = FLASH_BANK_1, .Sector = 0U, .NbSectorsToErase = 0U};
/**
 * @}
 */
...
#if defined(FLASH_SR_OBKERR)
		(*reg_cr) |= (FLASH_IT_EOP | FLASH_IT_WRPERR | FLASH_IT_PGSERR | FLASH_IT_STRBERR | FLASH_IT_INCERR | FLASH_IT_OBKERR | FLASH_IT_OBKWERR);
#else
...
#if defined(FLASH_SR_OBKERR)
		else if (((TypeProgram & (~FLASH_NON_SECURE_MASK)) == FLASH_TYPEPROGRAM_QUADWORD_OBK) || ((TypeProgram & (~FLASH_NON_SECURE_MASK)) == FLASH_TYPEPROGRAM_QUADWORD_OBK_ALT)) {
			/* Check the parameters */
			assert_param(IS_FLASH_OBK_ADDRESS(FlashAddress));
...
#if defined(FLASH_SR_OBKERR)
		(*reg_cr) &= ~(FLASH_IT_EOP | FLASH_IT_WRPERR | FLASH_IT_PGSERR | FLASH_IT_STRBERR | FLASH_IT_INCERR | FLASH_IT_OBKERR | FLASH_IT_OBKWERR | FLASH_IT_OPTCHANGEERR);
#else
		(*reg_cr) &= ~(FLASH_IT_EOP | FLASH_IT_WRPERR | FLASH_IT_PGSERR | FLASH_IT_STRBERR | FLASH_IT_INCERR | FLASH_IT_OPTCHANGEERR);
#endif /* FLASH_SR_OBKERR */
	}
}
