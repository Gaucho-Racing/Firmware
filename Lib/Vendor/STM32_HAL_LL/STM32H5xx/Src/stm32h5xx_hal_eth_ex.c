 ...
 * @{
 */
#define ETH_MACL4CR_MASK (ETH_MACL3L4CR_L4PEN | ETH_MACL3L4CR_L4SPM | ETH_MACL3L4CR_L4SPIM | ETH_MACL3L4CR_L4DPM | ETH_MACL3L4CR_L4DPIM)

#define ETH_MACL3CR_MASK (ETH_MACL3L4CR_L3PEN | ETH_MACL3L4CR_L3SAM | ETH_MACL3L4CR_L3SAIM | ETH_MACL3L4CR_L3DAM | ETH_MACL3L4CR_L3DAIM | ETH_MACL3L4CR_L3HSBM | ETH_MACL3L4CR_L3HDBM)

#define ETH_MACRXVLAN_MASK                                                                                                                                                                             \
	(ETH_MACVTR_EIVLRXS | ETH_MACVTR_EIVLS | ETH_MACVTR_ERIVLT | ETH_MACVTR_EDVLP | ETH_MACVTR_VTHM | ETH_MACVTR_EVLRXS | ETH_MACVTR_EVLS | ETH_MACVTR_DOVLTC | ETH_MACVTR_ERSVLM |                \
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_ETHEx_SetL4FilterConfig(ETH_HandleTypeDef *heth, uint32_t Filter, const ETH_L4FilterConfigTypeDef *pL4FilterConfig)
{
	...
	if (Filter == ETH_L4_FILTER_0) {
		...
		MODIFY_REG(heth->Instance->MACL3L4C0R, ETH_MACL4CR_MASK, (pL4FilterConfig->Protocol | pL4FilterConfig->SrcPortFilterMatch | pL4FilterConfig->DestPortFilterMatch));
	...
	} else /* Filter == ETH_L4_FILTER_1 */
	{
		...
		MODIFY_REG(heth->Instance->MACL3L4C1R, ETH_MACL4CR_MASK, (pL4FilterConfig->Protocol | pL4FilterConfig->SrcPortFilterMatch | pL4FilterConfig->DestPortFilterMatch));
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_ETHEx_GetL4FilterConfig(const ETH_HandleTypeDef *heth, uint32_t Filter, ETH_L4FilterConfigTypeDef *pL4FilterConfig)
{
	if (pL4FilterConfig == NULL) {
		return HAL_ERROR;
	}
	...
	if (Filter == ETH_L4_FILTER_0) {
		...
		pL4FilterConfig->DestPortFilterMatch = READ_BIT(heth->Instance->MACL3L4C0R, (ETH_MACL3L4CR_L4DPM | ETH_MACL3L4CR_L4DPIM));
		pL4FilterConfig->SrcPortFilterMatch = READ_BIT(heth->Instance->MACL3L4C0R, (ETH_MACL3L4CR_L4SPM | ETH_MACL3L4CR_L4SPIM));
	...
	} else /* Filter == ETH_L4_FILTER_1 */
	{
		...
		pL4FilterConfig->DestPortFilterMatch = READ_BIT(heth->Instance->MACL3L4C1R, (ETH_MACL3L4CR_L4DPM | ETH_MACL3L4CR_L4DPIM));
		pL4FilterConfig->SrcPortFilterMatch = READ_BIT(heth->Instance->MACL3L4C1R, (ETH_MACL3L4CR_L4SPM | ETH_MACL3L4CR_L4SPIM));
...
HAL_StatusTypeDef HAL_ETHEx_SetL3FilterConfig(ETH_HandleTypeDef *heth, uint32_t Filter, const ETH_L3FilterConfigTypeDef *pL3FilterConfig)
{
	...
	if (Filter == ETH_L3_FILTER_0) {
		...
		MODIFY_REG(heth->Instance->MACL3L4C0R, ETH_MACL3CR_MASK,
			   (pL3FilterConfig->Protocol | pL3FilterConfig->SrcAddrFilterMatch | pL3FilterConfig->DestAddrFilterMatch | (pL3FilterConfig->SrcAddrHigherBitsMatch << 6) |
...
HAL_StatusTypeDef HAL_ETHEx_SetRxVLANConfig(ETH_HandleTypeDef *heth, const ETH_RxVLANConfigTypeDef *pVlanConfig)
{
	...
	MODIFY_REG(heth->Instance->MACVTR, ETH_MACRXVLAN_MASK,
		   (((uint32_t)pVlanConfig->InnerVLANTagInStatus << 31) | pVlanConfig->StripInnerVLANTag | ((uint32_t)pVlanConfig->InnerVLANTag << 27) |
		    ((uint32_t)pVlanConfig->DoubleVLANProcessing << 26) | ((uint32_t)pVlanConfig->VLANTagHashTableMatch << 25) | ((uint32_t)pVlanConfig->VLANTagInStatus << 24) |
		    pVlanConfig->StripVLANTag | pVlanConfig->VLANTypeCheck | ((uint32_t)pVlanConfig->VLANTagInverceMatch << 17)));
 ...
 * @retval HAL Status.
 */
HAL_StatusTypeDef HAL_ETHEx_GetTxVLANConfig(const ETH_HandleTypeDef *heth, uint32_t VLANTag, ETH_TxVLANConfigTypeDef *pVlanConfig)
{
	if (pVlanConfig == NULL) {
		return HAL_ERROR;
	}
 ...
 * @retval HAL Status
 */
HAL_StatusTypeDef HAL_ETHEx_SetTxVLANConfig(ETH_HandleTypeDef *heth, uint32_t VLANTag, const ETH_TxVLANConfigTypeDef *pVlanConfig)
{
	if (VLANTag == ETH_INNER_TX_VLANTAG) {
		MODIFY_REG(heth->Instance->MACIVIR, ETH_MACTXVLAN_MASK, (((uint32_t)pVlanConfig->SourceTxDesc << 20) | ((uint32_t)pVlanConfig->SVLANType << 19) | pVlanConfig->VLANTagControl));
		/* Enable Double VLAN processing */
		SET_BIT(heth->Instance->MACVTR, ETH_MACVTR_EDVLP);
	} else {
		MODIFY_REG(heth->Instance->MACVIR, ETH_MACTXVLAN_MASK, (((uint32_t)pVlanConfig->SourceTxDesc << 20) | ((uint32_t)pVlanConfig->SVLANType << 19) | pVlanConfig->VLANTagControl));
	}
...
void HAL_ETHEx_EnterLPIMode(ETH_HandleTypeDef *heth, FunctionalState TxAutomate, FunctionalState TxClockStop)
{
	...
	MODIFY_REG(heth->Instance->MACLCSR, (ETH_MACLCSR_LPIEN | ETH_MACLCSR_LPITXA | ETH_MACLCSR_LPITCSE), (((uint32_t)TxAutomate << 19) | ((uint32_t)TxClockStop << 21) | ETH_MACLCSR_LPIEN));
}
