...
#if defined(FLASH_EDATAR_EDATA_EN)
	pFlashOperation->FlashArea = opsr_reg & (FLASH_OPSR_DATA_OP | FLASH_OPSR_BK_OP | FLASH_OPSR_SYSF_OP | FLASH_OPSR_OTP_OP);
#else
	...
	if (addr_reg != 0xFFFFFFFFU) {
		...
		switch (pData->Area) {
			...
			case FLASH_ECC_AREA_EDATA_BANK2:
				 ...
				 * In addition, the address returned by ECC registers represents 128-bit flash word (multiply by 4),
				 */
				pData->Address = (FLASH_EDATA_BASE + FLASH_EDATA_BANK_SIZE + ((addr_reg - FLASH_ADDRESS_OFFSET_EDATA) * 4U)) & 0xFFFFFFFFU;
				break;
#endif /* FLASH_EDATAR_EDATA_EN */
