 ...
 * @{
 */
#define RAM_PARAM_END(TAB, INDEX)                                                                                                                                                                      \
	do {                                                                                                                                                                                           \
		TAB[INDEX] = 0UL;                                                                                                                                                                      \
		TAB[INDEX + 1U] = 0UL;                                                                                                                                                                 \
	} while (0)

#define HAL_CCB_GET_FLAG(__HANDLE__, __FLAG__) (((((__HANDLE__)->Instance->SR) & (__FLAG__)) == (__FLAG__)) ? SET : RESET)

#define HAL_CCB_GET_SAES_FLAG(__FLAG__) (((__FLAG__) > 1U) ? (((SAES->SR & (__FLAG__)) == (__FLAG__)) ? SET : RESET) : (((SAES->ISR & (__FLAG__)) == (__FLAG__)) ? SET : RESET))

#define HAL_CCB_GET_PKA_FLAG(__FLAG__) ((((PKA->SR) & (__FLAG__)) == (__FLAG__)) ? SET : RESET)
...
static HAL_StatusTypeDef Protect_SAES_WaitFLAG(CCB_HandleTypeDef *hccb, uint32_t flag, FlagStatus Status, uint32_t Timeout);
...
static HAL_StatusTypeDef CCB_SetPram(CCB_HandleTypeDef *hccb, uint32_t modulusSizeByte, uint32_t dst_address, const uint8_t *src);
...
static HAL_StatusTypeDef WrappingKeyConfiguration(CCB_HandleTypeDef *hccb, uint32_t Operation, CCB_WrappingKeyTypeDef *pWrappingKey);
static HAL_StatusTypeDef Protect_SAES_UnwrapKey(CCB_HandleTypeDef *hccb, const CCB_WrappingKeyTypeDef *pWrappingKey, uint32_t Timeout);
...
static HAL_StatusTypeDef CCB_WrapSymmetricKey(CCB_HandleTypeDef *hccb, const uint32_t *pClearAESKey, uint32_t operation, CCB_WrappingKeyTypeDef *pWrappingKey);
...
static HAL_StatusTypeDef CCB_BlobCreation_FinalPhase(CCB_HandleTypeDef *hccb, uint32_t Operation, uint32_t *pTag, uint32_t sizeparam);
static HAL_StatusTypeDef CCB_BlobUse_FinalPhase(CCB_HandleTypeDef *hccb, uint32_t Operation, uint32_t sizeparam);
static HAL_StatusTypeDef CCB_ECDSA_SignBlobCreation(CCB_HandleTypeDef *hccb, CCB_ECDSACurveParamTypeDef *pCurveParam, const uint8_t *pClearPrivateKey, CCB_WrappingKeyTypeDef *pWrappingKey,
						    uint32_t *pIV, uint32_t *pTag, uint32_t *pWarappedKey, uint8_t CCB_Operation);
static HAL_StatusTypeDef CCB_ECC_ScalarMulBlobCreation(CCB_HandleTypeDef *hccb, CCB_ECCMulCurveParamTypeDef *pCurveParam, const uint8_t *pClearPrivateKey, CCB_WrappingKeyTypeDef *pWrappingKey,
						       uint32_t *pIV, uint32_t *pTag, uint32_t *pWarappedKey, uint8_t CCB_Operation);
static HAL_StatusTypeDef CCB_ECC_ComputeScalarMul(CCB_HandleTypeDef *hccb, CCB_ECCMulCurveParamTypeDef *pCurveParam, CCB_WrappingKeyTypeDef *pWrappingKey, uint32_t *pIV, uint32_t *pTag,
						  uint32_t *pWarappedKey, CCB_ECCMulPointTypeDef *pInputPoint, CCB_ECCMulPointTypeDef *pOutputPoint, const uint32_t *pPublicKey,
						  uint8_t VerifOperation);
static HAL_StatusTypeDef CCB_RSA_ExpBlobCreation(CCB_HandleTypeDef *hccb, CCB_RSAParamTypeDef *pParam, const CCB_RSAClearKeyTypeDef *pRSAClearPrivateKey, CCB_WrappingKeyTypeDef *pWrappingKey,
						 uint32_t *pIV, uint32_t *pTag, uint32_t *pWrappedExp, uint32_t *pWrappedPhi);
static HAL_StatusTypeDef CCB_RSA_ComputeModularExp(CCB_HandleTypeDef *hccb, CCB_RSAParamTypeDef *pParam, CCB_WrappingKeyTypeDef *pWrappingKey, uint32_t *pIV, uint32_t *pTag, uint32_t *pWrappedExp,
						   uint32_t *pWrappedPhi, const uint8_t *pOperand, uint8_t *pModularExp, const uint32_t *pReferenceModularExp, uint8_t VerifOperation);
static HAL_StatusTypeDef PKA_ECDSASign(CCB_ECDSACurveParamTypeDef *pCurveParam, const uint8_t *pClearPrivateKey, uint8_t *pInteger, uint8_t *pHash, CCB_ECDSASignTypeDef *pSignature);

static HAL_StatusTypeDef PKA_ECC_ComputeScalarMul(CCB_ECCMulCurveParamTypeDef *pCurveParam, const uint8_t *pClearPrivateKey, uint32_t PublicKey[2][20]);
static HAL_StatusTypeDef PKA_RSA_ComputeModularExp(CCB_RSAParamTypeDef *pParam, const CCB_RSAClearKeyTypeDef *pRSAClearPrivateKey, uint8_t *pOp1, uint32_t *pReferenceModularExp);
static HAL_StatusTypeDef PKA_ECDSAVerif(CCB_ECDSACurveParamTypeDef *pCurveParam, CCB_ECCMulPointTypeDef *pPublicKeyOut, const uint8_t *pHash, CCB_ECDSASignTypeDef *pSignature);
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_CCB_ECDSA_WrapPrivateKey(CCB_HandleTypeDef *hccb, CCB_ECDSACurveParamTypeDef *pCurveParam, const uint8_t *pClearPrivateKey, CCB_WrappingKeyTypeDef *pWrappingKey,
					       const CCB_ECDSAKeyBlobTypeDef *pWrappedPrivateKeyBlob)
{
	...
	if (hccb->State == HAL_CCB_STATE_READY) {
		...
		if (PKA_ECDSASign(pCurveParam, pClearPrivateKey, (uint8_t *)integer, (uint8_t *)hash, &signature) != HAL_OK) {
		...
		if (CCB_ECDSA_SignBlobCreation(hccb, pCurveParam, pClearPrivateKey, pWrappingKey, iv_temp, tag_temp, wrapped_key_temp, CCB_ECDSA_SIGN_CPU_BLOB_CREATION) != HAL_OK) {
		...
		if (CCB_ECC_ComputeScalarMul(hccb, pCurveParam, pWrappingKey, iv_temp, tag_temp, wrapped_key_temp, NULL, &publicKeyOut, NULL, CCB_COMPUTE_PUBLIC_KEY) != HAL_OK) {
			/* Set state, and intrusion error */
			hccb->State = HAL_CCB_STATE_ERROR;
			HAL_CCB_IntrusionCallback(hccb);
		}
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_CCB_ECDSA_GenerateWrapPrivateKey(CCB_HandleTypeDef *hccb, CCB_ECDSACurveParamTypeDef *pCurveParam, CCB_WrappingKeyTypeDef *pWrappingKey,
						       CCB_ECDSAKeyBlobTypeDef *pWrappedPrivateKeyBlob)
{
	...
	if (hccb->State == HAL_CCB_STATE_READY) {
		...
		/* Signature blob creation */
		if (CCB_ECDSA_SignBlobCreation(hccb, pCurveParam, NULL, pWrappingKey, iv_temp, tag_temp, wrapped_key_temp, CCB_ECDSA_SIGN_RNG_BLOB_CREATION) != HAL_OK) {
			/* Set state, error code and return error */
			hccb->State = HAL_CCB_STATE_ERROR;
			return HAL_ERROR;
		}
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_CCB_ECDSA_Sign(CCB_HandleTypeDef *hccb, CCB_ECDSACurveParamTypeDef *pCurveParam, CCB_WrappingKeyTypeDef *pWrappingKey, CCB_ECDSAKeyBlobTypeDef *pWrappedPrivateKeyBlob,
				     const uint8_t *pHash, CCB_ECDSASignTypeDef *pSignature)
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_CCB_ECDSA_ComputePublicKey(CCB_HandleTypeDef *hccb, CCB_ECDSACurveParamTypeDef *pCurveParam, CCB_WrappingKeyTypeDef *pWrappingKey,
						 CCB_ECDSAKeyBlobTypeDef *pWrappedPrivateKeyBlob, CCB_ECCMulPointTypeDef *pPublicKey)
{
	if (hccb->State == HAL_CCB_STATE_READY) {
		if (CCB_ECC_ComputeScalarMul(hccb, pCurveParam, pWrappingKey, pWrappedPrivateKeyBlob->pIV, pWrappedPrivateKeyBlob->pTag, pWrappedPrivateKeyBlob->pWrappedKey, NULL, pPublicKey, NULL,
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_CCB_ECC_WrapPrivateKey(CCB_HandleTypeDef *hccb, CCB_ECCMulCurveParamTypeDef *pCurveParam, const uint8_t *pClearPrivateKey, CCB_WrappingKeyTypeDef *pWrappingKey,
					     CCB_ECCMulKeyBlobTypeDef *pWrappedPrivateKeyBlob)
{
	...
	if (hccb->State == HAL_CCB_STATE_READY) {
		...
		if (CCB_ECC_ScalarMulBlobCreation(hccb, pCurveParam, pClearPrivateKey, pWrappingKey, iv_temp, tag_temp, wrapped_key_temp, CCB_ECC_SCALAR_MUL_CPU_BLOB_CREATION) != HAL_OK) {
			/* Set state, and intrusion error */
			hccb->State = HAL_CCB_STATE_ERROR;
			HAL_CCB_IntrusionCallback(hccb);
		}
		...
		if (CCB_ECC_ComputeScalarMul(hccb, pCurveParam, pWrappingKey, iv_temp, tag_temp, wrapped_key_temp, NULL, NULL, &public_key[0][0], CCB_VERIF_OPERATION_ENABLED) != HAL_OK)
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_CCB_ECC_GenerateWrapPrivateKey(CCB_HandleTypeDef *hccb, CCB_ECCMulCurveParamTypeDef *pCurveParam, CCB_WrappingKeyTypeDef *pWrappingKey,
						     CCB_ECCMulKeyBlobTypeDef *pWrappedPrivateKeyBlob)
{

	if (hccb->State == HAL_CCB_STATE_READY) {
		/* CCB ECC generate private wrap key */
		if (CCB_ECC_ScalarMulBlobCreation(hccb, pCurveParam, NULL, pWrappingKey, pWrappedPrivateKeyBlob->pIV, pWrappedPrivateKeyBlob->pTag, pWrappedPrivateKeyBlob->pWrappedKey,
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_CCB_ECC_ComputeScalarMul(CCB_HandleTypeDef *hccb, CCB_ECCMulCurveParamTypeDef *pCurveParam, CCB_WrappingKeyTypeDef *pWrappingKey,
					       CCB_ECCMulKeyBlobTypeDef *pWrappedPrivateKeyBlob, CCB_ECCMulPointTypeDef *pInputPoint, CCB_ECCMulPointTypeDef *pOutputPoint)
{
	if (hccb->State == HAL_CCB_STATE_READY) {
		if (CCB_ECC_ComputeScalarMul(hccb, pCurveParam, pWrappingKey, pWrappedPrivateKeyBlob->pIV, pWrappedPrivateKeyBlob->pTag, pWrappedPrivateKeyBlob->pWrappedKey, pInputPoint, pOutputPoint,
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_CCB_RSA_WrapPrivateKey(CCB_HandleTypeDef *hccb, CCB_RSAParamTypeDef *pParam, const CCB_RSAClearKeyTypeDef *pRSAClearPrivateKey, CCB_WrappingKeyTypeDef *pWrappingKey,
					     CCB_RSAKeyBlobTypeDef *pWrappedPrivateKeyBlob)
{
	...
	if (hccb->State == HAL_CCB_STATE_READY) {
		...
		if (CCB_RSA_ExpBlobCreation(hccb, pParam, pRSAClearPrivateKey, pWrappingKey, iv_temp, tag_temp, wrapped_exp, wrapped_phi) != HAL_OK) {
			/* Set state, and intrusion error */
			hccb->State = HAL_CCB_STATE_ERROR;
			HAL_CCB_IntrusionCallback(hccb);
		}

		if (CCB_RSA_ComputeModularExp(hccb, pParam, pWrappingKey, iv_temp, tag_temp, wrapped_exp, wrapped_phi, (uint8_t *)exp_base, NULL, modular_exp_ref, CCB_VERIF_OPERATION_ENABLED) !=
		    HAL_OK) {
			/* Set state, and intrusion error */
			hccb->State = HAL_CCB_STATE_ERROR;
			HAL_CCB_IntrusionCallback(hccb);
 ...
 * @retval HAL status.
 */
HAL_StatusTypeDef HAL_CCB_RSA_ComputeModularExp(CCB_HandleTypeDef *hccb, CCB_RSAParamTypeDef *pParam, CCB_WrappingKeyTypeDef *pWrappingKey, CCB_RSAKeyBlobTypeDef *pWrappedPrivateKeyBlob,
						const uint8_t *pOperand, uint8_t *pModularExp)
{
	if (hccb->State == HAL_CCB_STATE_READY) {
		if (CCB_RSA_ComputeModularExp(hccb, pParam, pWrappingKey, pWrappedPrivateKeyBlob->pIV, pWrappedPrivateKeyBlob->pTag, pWrappedPrivateKeyBlob->pWrappedExp,
					      pWrappedPrivateKeyBlob->pWrappedPhi, pOperand, pModularExp, NULL, CCB_VERIF_OPERATION_DISABLED) != HAL_OK) {
			return HAL_ERROR;
		}
	}
...
static HAL_StatusTypeDef CCB_ECDSASign_SetPram(CCB_HandleTypeDef *hccb, CCB_ECDSACurveParamTypeDef *in)
{
	...
	PKA->RAM[PKA_ECDSA_SIGN_IN_ORDER_NB_BITS] = GetOptBitSize_u8(in->primeOrderSizeByte, *(in->pPrimeOrder));
...
{
	/* Get the prime order n length */
	PKA->RAM[PKA_ECC_SCALAR_MUL_IN_EXP_NB_BITS] = GetOptBitSize_u8(in->primeOrderSizeByte, *(in->pPrimeOrder));
...
static HAL_StatusTypeDef CCB_SetPram(CCB_HandleTypeDef *hccb, uint32_t modulusSizeByte, uint32_t dst_address, const uint8_t *src)
{
	...
	if ((modulusSizeByte % 8UL) != 0UL) {
		for (offset = 0; offset < (operand_size - 4UL); offset += 2UL) {
			CCB_Memcpy_u8_to_u64(&PKA->RAM[dst_address + offset], &src[(modulusSizeByte) - ((offset * 4UL) + 1UL)]);
			if (Protect_SAES_WaitFLAG(hccb, AES_ISR_CCF, SET, HAL_CCB_TIMEOUT_DEFAULT_VALUE) != HAL_OK) {
				hccb->State = HAL_CCB_STATE_ERROR;
				return HAL_ERROR;
			}
		...
		CCB_Memcpy_Not_Align(&PKA->RAM[dst_address + offset], &src[(modulusSizeByte) - ((offset * 4UL) + 1UL)], modulusSizeByte % 8UL);
		...
		RAM_PARAM_END(PKA->RAM, dst_address + ((modulusSizeByte + 3UL) / 4UL));
		if (Protect_SAES_WaitFLAG(hccb, AES_ISR_CCF, SET, HAL_CCB_TIMEOUT_DEFAULT_VALUE) != HAL_OK) {
			hccb->State = HAL_CCB_STATE_ERROR;
			return HAL_ERROR;
		}
...
static HAL_StatusTypeDef Protect_PKA_Init(CCB_HandleTypeDef *hccb, uint32_t Operation)
{
	...
	if ((Operation == CCB_ECDSA_SIGN_CPU_BLOB_CREATION) || (Operation == CCB_ECDSA_SIGN_RNG_BLOB_CREATION) || (Operation == CCB_ECDSA_SIGN_BLOB_USE)) {
		MODIFY_REG(PKA->CR, PKA_CR_MODE | PKA_CR_PROCENDIE | PKA_CR_RAMERRIE | PKA_CR_ADDRERRIE | PKA_CR_OPERRIE, CCB_PKA_ECDSA_SIGNATURE_MODE << PKA_CR_MODE_Pos);
	}

	else if ((Operation == CCB_ECC_SCALAR_MUL_CPU_BLOB_CREATION) || (Operation == CCB_ECC_SCALAR_MUL_RNG_BLOB_CREATION) || (Operation == CCB_ECC_SCALAR_MUL_BLOB_USE)) {
		MODIFY_REG(PKA->CR, PKA_CR_MODE | PKA_CR_PROCENDIE | PKA_CR_RAMERRIE | PKA_CR_ADDRERRIE | PKA_CR_OPERRIE, CCB_PKA_ECC_MUL_MODE << PKA_CR_MODE_Pos);
	}

	else if ((Operation == CCB_MODULAR_EXP_CPU_BLOB_CREATION) || (Operation == CCB_MODULAR_EXP_BLOB_USE)) {
		MODIFY_REG(PKA->CR, PKA_CR_MODE | PKA_CR_PROCENDIE | PKA_CR_RAMERRIE | PKA_CR_ADDRERRIE | PKA_CR_OPERRIE, CCB_PKA_MODE_MODULAR_EXP_PROTECT << PKA_CR_MODE_Pos);
	} else {
		/* do nothing */
	}
...
static HAL_StatusTypeDef WrappingKeyConfiguration(CCB_HandleTypeDef *hccb, uint32_t Operation, CCB_WrappingKeyTypeDef *pWrappingKey)
{
	...
	switch (pWrappingKey->WrappingKeyType) {
		case HAL_CCB_USER_KEY_WRAPPED:
			...
			if ((Operation == CCB_ECDSA_SIGN_CPU_BLOB_CREATION) || (Operation == CCB_ECDSA_SIGN_RNG_BLOB_CREATION) || (Operation == CCB_ECC_SCALAR_MUL_CPU_BLOB_CREATION) ||
			    (Operation == CCB_ECC_SCALAR_MUL_RNG_BLOB_CREATION) || (Operation == CCB_MODULAR_EXP_CPU_BLOB_CREATION)) {
			...
			else if ((Operation == CCB_ECDSA_SIGN_BLOB_USE) || (Operation == CCB_ECC_SCALAR_MUL_BLOB_USE) || (Operation == CCB_MODULAR_EXP_BLOB_USE)) {
				/* SAES: GCMPH = 0x0 (initial phase) as event that trig OPSTEP 0x1 --> 0x12 */
				WRITE_REG(SAES->CR, AES_CR_KEYSIZE | AES_CR_CHMOD_0 | AES_CR_CHMOD_1 | AES_CR_MODE_1);
			}
		...
		case HAL_CCB_USER_KEY_HW:
			...
			if ((Operation == CCB_ECDSA_SIGN_CPU_BLOB_CREATION) || (Operation == CCB_ECDSA_SIGN_RNG_BLOB_CREATION) || (Operation == CCB_ECC_SCALAR_MUL_CPU_BLOB_CREATION) ||
			    (Operation == CCB_ECC_SCALAR_MUL_RNG_BLOB_CREATION) || (Operation == CCB_MODULAR_EXP_CPU_BLOB_CREATION)) {
				/* SAES: GCMPH = 0x0 (initial phase) as event that trig OPSTEP 0x1 --> 0x2 */
				WRITE_REG(SAES->CR, AES_CR_KEYSEL_0 | AES_CR_KEYSIZE | AES_CR_CHMOD_0 | AES_CR_CHMOD_1);
			}

			else if ((Operation == CCB_ECDSA_SIGN_BLOB_USE) || (Operation == CCB_ECC_SCALAR_MUL_BLOB_USE) || (Operation == CCB_MODULAR_EXP_BLOB_USE)) {
				/* SAES: GCMPH = 0x0 (initial phase) as event that trig OPSTEP 0x1 --> 0x12 */
				WRITE_REG(SAES->CR, AES_CR_KEYSEL_0 | AES_CR_KEYSIZE | AES_CR_CHMOD_0 | AES_CR_CHMOD_1 | AES_CR_MODE_1);
			}
	...
	if (dst != NULL) {
		if (src != NULL) {
			for (; index < (size / 4UL); index++) {
				...
				dst[index] = ((uint32_t)src[(size - (index * 4UL) - 1UL)]) | ((uint32_t)src[(size - (index * 4UL) - 2UL)] << 8UL) |
					     ((uint32_t)src[(size - (index * 4UL) - 3UL)] << 16UL) | ((uint32_t)src[(size - (index * 4UL) - 4UL)] << 24UL);
			}
			...
			if ((size % 4UL) == 1UL) {
				...
				dst[index] =
				    ((uint32_t)src[(size - (index * 4UL) - 1UL)]) | ((uint32_t)src[(size - (index * 4UL) - 2UL)] << 8UL) | ((uint32_t)src[(size - (index * 4UL) - 3UL)] << 16UL);
			} else {
				/* The last element is already handle in the loop */
			}
		}
...
static void CCB_Memcpy_u8_to_u64(__IO uint32_t dst[], const uint8_t src[])
{
	if ((dst != NULL) && (src != NULL)) {
		/* Apply the equivalent of __REV from uint8_t to uint32_t */
		dst[0] = ((uint32_t)src[0]) | ((uint32_t)src[-1] << 8UL) | ((uint32_t)src[-2] << 16UL) | ((uint32_t)src[-3] << 24UL);
		dst[1] = ((uint32_t)src[-4]) | ((uint32_t)src[-5] << 8UL) | ((uint32_t)src[-6] << 16UL) | ((uint32_t)src[-7] << 24UL);
	}
}
...
static HAL_StatusTypeDef CCB_BlobUse_InitialPhase(CCB_HandleTypeDef *hccb, const uint32_t *pIV, uint32_t *pTag)
{
	...
	MODIFY_REG(SAES->CR, AES_CR_GCMPH | AES_CR_EN, AES_CR_GCMPH_0 | AES_CR_EN);
						    ...
						    uint32_t *pIV, uint32_t *pTag, uint32_t *pWarappedKey, uint8_t CCB_Operation)
{
	...
	if (CCB_Operation == CCB_ECDSA_SIGN_CPU_BLOB_CREATION) {
		...
		if ((pCurveParam->modulusSizeByte % 8UL) != 0UL) {
			for (offset = 0UL; offset < (operand_size - 4UL); offset += 2UL) {
				CCB_Memcpy_u8_to_u64(&PKA->RAM[PKA_ECDSA_SIGN_IN_PRIVATE_KEY_D + offset], &pClearPrivateKey[(pCurveParam->modulusSizeByte) - ((offset * 4UL) + 1UL)]);
			}

			CCB_Memcpy_Not_Align(&PKA->RAM[PKA_ECDSA_SIGN_IN_PRIVATE_KEY_D + offset], &pClearPrivateKey[(pCurveParam->modulusSizeByte) - ((offset * 4UL) + 1UL)],
	...
	for (offset = 0; offset < cipherkey_size; offset++) {
		...
		if ((offset % 4UL) == 3UL) {
			...
			for (count = 0U; count < CCB_BLOCK_SIZE; count++) {
				pWarappedKey[cipherkey_size - (countBlock + count + 1UL)] = READ_REG(SAES->DOUTR);
			}
			countBlock += 4UL;
		}
	}
	...
	/* Final phase processing */
	if (CCB_BlobCreation_FinalPhase(hccb, CCB_Operation, pTag, pCurveParam->modulusSizeByte) != HAL_OK) {
		/* return error */
		return HAL_ERROR;
	}
						       ...
						       uint32_t *pIV, uint32_t *pTag, uint32_t *pWarappedKey, uint8_t CCB_Operation)
{
	...
	if (hccb->State == HAL_CCB_STATE_READY) {
		...
		if (CCB_Operation == CCB_ECC_SCALAR_MUL_CPU_BLOB_CREATION) {
			...
			if ((pCurveParam->modulusSizeByte % 8UL) != 0UL) {
				for (offset = 0UL; offset < (operand_size - 4UL); offset += 2UL) {
					CCB_Memcpy_u8_to_u64(&PKA->RAM[PKA_ECC_SCALAR_MUL_IN_K + offset], &pClearPrivateKey[(pCurveParam->modulusSizeByte) - ((offset * 4UL) + 1UL)]);
				}

				CCB_Memcpy_Not_Align(&PKA->RAM[PKA_ECC_SCALAR_MUL_IN_K + offset], &pClearPrivateKey[(pCurveParam->modulusSizeByte) - ((offset * 4UL) + 1UL)],
						     pCurveParam->modulusSizeByte % 8UL);
		...
		for (offset = 0; offset < cipherkey_size; offset++) {
			...
			if ((offset % 4UL) == 3UL) {
				...
				for (count = 0U; count < CCB_BLOCK_SIZE; count++) {
					pWarappedKey[cipherkey_size - (countBlock + count + 1U)] = READ_REG(SAES->DOUTR);
				}
				countBlock += 4UL;
			}
		}
		...
		if ((operand_size % 4UL) != 0UL) {
			RAM_PARAM_END(PKA->RAM, PKA_ECC_SCALAR_MUL_IN_K + cipherkey_size);
		}
 ...
 */
static HAL_StatusTypeDef CCB_ECC_ComputeScalarMul(CCB_HandleTypeDef *hccb, CCB_ECCMulCurveParamTypeDef *pCurveParam, CCB_WrappingKeyTypeDef *pWrappingKey, uint32_t *pIV, uint32_t *pTag,
						  uint32_t *pWarappedKey, CCB_ECCMulPointTypeDef *pInputPoint, CCB_ECCMulPointTypeDef *pOutputPoint, const uint32_t *pPublicKey, uint8_t VerifOperation)
{
	__IO uint16_t f_count;
	uint16_t random1 = 0;
	uint16_t random2 = 0;
	...
	if (VerifOperation == CCB_VERIF_OPERATION_DISABLED) {
		...
		CCB_Memcpy_u8_to_u32(&PKA->RAM[PKA_ECC_SCALAR_MUL_IN_INITIAL_POINT_X], pInputPoint->pPointX, pCurveParam->modulusSizeByte);
		RAM_PARAM_END(PKA->RAM, PKA_ECC_SCALAR_MUL_IN_INITIAL_POINT_X + ((pCurveParam->modulusSizeByte + 3UL) / 4UL));
		CCB_Memcpy_u8_to_u32(&PKA->RAM[PKA_ECC_SCALAR_MUL_IN_INITIAL_POINT_Y], pInputPoint->pPointY, pCurveParam->modulusSizeByte);
		...
		CCB_Memcpy_u8_to_u32(&PKA->RAM[PKA_ECC_SCALAR_MUL_IN_INITIAL_POINT_X], pCurveParam->pPointX, pCurveParam->modulusSizeByte);
		RAM_PARAM_END(PKA->RAM, PKA_ECC_SCALAR_MUL_IN_INITIAL_POINT_X + ((pCurveParam->modulusSizeByte + 3UL) / 4UL));
		CCB_Memcpy_u8_to_u32(&PKA->RAM[PKA_ECC_SCALAR_MUL_IN_INITIAL_POINT_Y], pCurveParam->pPointY, pCurveParam->modulusSizeByte);
		RAM_PARAM_END(PKA->RAM, PKA_ECC_SCALAR_MUL_IN_INITIAL_POINT_Y + ((pCurveParam->modulusSizeByte + 3UL) / 4UL));
	} else {
		/* nothing to do */
	}
	...
	if ((operand_size % 4UL) != 0UL) {
		RAM_PARAM_END(PKA->RAM, PKA_ECDSA_SIGN_IN_K + cipherkey_size);
	}
	...
	if (VerifOperation == CCB_VERIF_OPERATION_DISABLED) {
		/* P coordinate x */
		CCB_Memcpy_u32_to_u8(pOutputPoint->pPointX, &PKA->RAM[PKA_ECC_SCALAR_MUL_OUT_RESULT_X], pCurveParam->modulusSizeByte);

		/* P coordinate y */
		CCB_Memcpy_u32_to_u8(pOutputPoint->pPointY, &PKA->RAM[PKA_ECC_SCALAR_MUL_OUT_RESULT_Y], pCurveParam->modulusSizeByte);
	...
	} else /* (VerifOperation == CCB_VERIF_OPERATION_ENABLED) */
	{
		...
		for (offset = 0UL; offset < ((pCurveParam->modulusSizeByte + 3UL) / 4UL); offset++) {
			...
			if (((PKA->RAM[PKA_ECC_SCALAR_MUL_OUT_RESULT_X + offset] != pPublicKey[offset]) || (f_count != random1) || (f_count == 0U))) {
				/* Set state, and intrusion error */
				hccb->State = HAL_CCB_STATE_ERROR;
				HAL_CCB_IntrusionCallback(hccb);
			}
		...
		for (offset = 0UL; offset < ((pCurveParam->modulusSizeByte + 3UL) / 4UL); offset++) {
			...
			if (((PKA->RAM[PKA_ECC_SCALAR_MUL_OUT_RESULT_Y + offset] != pPublicKey[offset + 20U]) || (f_count != random1) || (f_count == 0U))) {
				/* Set state, and intrusion error */
				hccb->State = HAL_CCB_STATE_ERROR;
				HAL_CCB_IntrusionCallback(hccb);
			}
		...
		for (offset = 0UL; offset < ((pCurveParam->modulusSizeByte + 3UL) / 4UL); offset++) {
			/* Check  public key coordinate x and improve robustness against intrusion (intentional) */
			if (((PKA->RAM[PKA_ECC_SCALAR_MUL_OUT_RESULT_X + offset] != pPublicKey[offset]) || (f_count != random2) || (f_count == 0U))) {
				/* Set state, and intrusion error */
				hccb->State = HAL_CCB_STATE_ERROR;
				HAL_CCB_IntrusionCallback(hccb);
			}
		...
		for (offset = 0UL; offset < ((pCurveParam->modulusSizeByte + 3UL) / 4UL); offset++) {
			/* Check  public key coordinate y and improve robustness against intrusion (intentional) */
			if (((PKA->RAM[PKA_ECC_SCALAR_MUL_OUT_RESULT_Y + offset] != pPublicKey[offset + 20U]) || (f_count != random2) || (f_count == 0U))) {
				/* Set state, and intrusion error */
				hccb->State = HAL_CCB_STATE_ERROR;
				HAL_CCB_IntrusionCallback(hccb);
			}
		...
		for (offset = 0UL; offset < ((pCurveParam->modulusSizeByte + 3UL) / 4UL); offset++) {
			/* Check  public key coordinate x and improve robustness against intrusion (intentional) */
			if (((PKA->RAM[PKA_ECC_SCALAR_MUL_OUT_RESULT_X + offset] != pPublicKey[offset]) || (f_count != random3) || (f_count == 0U))) {
				/* Set state, and intrusion error */
				hccb->State = HAL_CCB_STATE_ERROR;
				HAL_CCB_IntrusionCallback(hccb);
			}
		...
		for (offset = 0UL; offset < ((pCurveParam->modulusSizeByte + 3UL) / 4UL); offset++) {
			/* Check  public key coordinate y and improve robustness against intrusion (intentional) */
			if (((PKA->RAM[PKA_ECC_SCALAR_MUL_OUT_RESULT_Y + offset] != pPublicKey[offset + 20U]) || (f_count != random3) || (f_count == 0U))) {
				/* Set state, and intrusion error */
				hccb->State = HAL_CCB_STATE_ERROR;
				HAL_CCB_IntrusionCallback(hccb);
			}
						 ...
						 uint32_t *pIV, uint32_t *pTag, uint32_t *pWrappedExp, uint32_t *pWrappedPhi)
{
	...
	if ((pParam->modulusSizeByte % 8UL) != 0UL) {
		for (offset = 0UL; offset < (operand_size - 4UL); offset += 2UL) {
			CCB_Memcpy_u8_to_u64(&PKA->RAM[PKA_MODULAR_EXP_PROTECT_IN_EXPONENT + offset], &pRSAClearPrivateKey->pExp[(pParam->modulusSizeByte) - ((offset * 4UL) + 1UL)]);
		}

		CCB_Memcpy_Not_Align(&PKA->RAM[PKA_MODULAR_EXP_PROTECT_IN_EXPONENT + offset], &pRSAClearPrivateKey->pExp[(pParam->modulusSizeByte) - ((offset * 4UL) + 1UL)],
		...
		RAM_PARAM_END(PKA->RAM, PKA_MODULAR_EXP_PROTECT_IN_EXPONENT + offset);
	}
	...
	if ((pParam->modulusSizeByte % 8UL) != 0UL) {
		for (offset = 0; offset < (operand_size - 4UL); offset += 2UL) {
			CCB_Memcpy_u8_to_u64(&PKA->RAM[PKA_MODULAR_EXP_PROTECT_IN_PHI + offset], &pRSAClearPrivateKey->pPhi[(pParam->modulusSizeByte) - ((offset * 4UL) + 1UL)]);
		}

		CCB_Memcpy_Not_Align(&PKA->RAM[PKA_MODULAR_EXP_PROTECT_IN_PHI + offset], &pRSAClearPrivateKey->pPhi[(pParam->modulusSizeByte) - ((offset * 4UL) + 1UL)], pParam->modulusSizeByte % 8UL);
	...
	for (offset = 0UL; offset < cipherkey_size; offset++) {
		...
		if ((offset % 4UL) == 3UL) {
			...
			for (count = 0U; count < CCB_BLOCK_SIZE; count++) {
				pWrappedExp[cipherkey_size - (countBlock + count + 1UL)] = READ_REG(SAES->DOUTR);
			}
			countBlock += 4UL;
		}
	}
	...
	for (offset = 0; offset < cipherkey_size; offset++) {
		...
		if ((offset % 4UL) == 3UL) {
			...
			/* Read ciphered phi */
			for (count = 0U; count < CCB_BLOCK_SIZE; count++) {
				pWrappedPhi[cipherkey_size - (countBlock + count + 1UL)] = READ_REG(SAES->DOUTR);
			}
			countBlock += 4UL;
		}
	}
	...
	/* Final phase processing */
	if (CCB_BlobCreation_FinalPhase(hccb, CCB_MODULAR_EXP_CPU_BLOB_CREATION, pTag, pParam->modulusSizeByte) != HAL_OK) {
		/* return error */
		return HAL_ERROR;
	}
						   ...
						   uint32_t *pWrappedPhi, const uint8_t *pOperand, uint8_t *pModularExp, const uint32_t *pReferenceModularExp, uint8_t VerifOperation)
{
	...
	} else /* CCB_VERIF_OPERATION_ENABLED */
	{
		...
		for (offset = 0UL; offset < ((pParam->modulusSizeByte + 3UL) / 4UL); offset++) {
			...
			if (((PKA->RAM[PKA_MODULAR_EXP_OUT_RESULT + offset] != pReferenceModularExp[offset]) || (f_count != random1) || (f_count == 0U))) {
				/* Set state, and intrusion error */
				hccb->State = HAL_CCB_STATE_ERROR;
				HAL_CCB_IntrusionCallback(hccb);
			}
		...
		for (offset = 0UL; offset < ((pParam->modulusSizeByte + 3UL) / 4UL); offset++) {
			/* Check Modular Exponentiation and improve robustness against intrusion (intentional) */
			if (((PKA->RAM[PKA_MODULAR_EXP_OUT_RESULT + offset] != pReferenceModularExp[offset]) || (f_count != random2) || (f_count == 0U))) {
				/* Set state, and intrusion error */
				hccb->State = HAL_CCB_STATE_ERROR;
				HAL_CCB_IntrusionCallback(hccb);
			}
		...
		for (offset = 0UL; offset < ((pParam->modulusSizeByte + 3UL) / 4UL); offset++) {
			/* Check Modular Exponentiation and improve robustness against intrusion (intentional) */
			if (((PKA->RAM[PKA_MODULAR_EXP_OUT_RESULT + offset] != pReferenceModularExp[offset]) || (f_count != random3) || (f_count == 0U))) {
				/* Set state, and intrusion error */
				hccb->State = HAL_CCB_STATE_ERROR;
				HAL_CCB_IntrusionCallback(hccb);
			}
...
static HAL_StatusTypeDef PKA_ECDSASign(CCB_ECDSACurveParamTypeDef *pCurveParam, const uint8_t *pClearPrivateKey, uint8_t *pInteger, uint8_t *pHash, CCB_ECDSASignTypeDef *pSignature)
{
	...
	/* Get the prime order n length */
	PKA->RAM[PKA_ECDSA_SIGN_IN_ORDER_NB_BITS] = GetOptBitSize_u8(pCurveParam->primeOrderSizeByte, *(pCurveParam->pPrimeOrder));
	...
	/* Move the input parameters base point G coordinate x to PKA RAM */
	CCB_Memcpy_u8_to_u32(&PKA->RAM[PKA_ECDSA_SIGN_IN_INITIAL_POINT_X], pCurveParam->pPointX, pCurveParam->modulusSizeByte);
	RAM_PARAM_END(PKA->RAM, PKA_ECDSA_SIGN_IN_INITIAL_POINT_X + ((pCurveParam->modulusSizeByte + 3UL) / 4UL));

	/* Move the input parameters base point G coordinate y to PKA RAM */
	CCB_Memcpy_u8_to_u32(&PKA->RAM[PKA_ECDSA_SIGN_IN_INITIAL_POINT_Y], pCurveParam->pPointY, pCurveParam->modulusSizeByte);
	...
	/* Move the input parameters prime order n to PKA RAM */
	CCB_Memcpy_u8_to_u32(&PKA->RAM[PKA_ECDSA_SIGN_IN_ORDER_N], pCurveParam->pPrimeOrder, pCurveParam->primeOrderSizeByte);
...
static HAL_StatusTypeDef PKA_ECC_ComputeScalarMul(CCB_ECCMulCurveParamTypeDef *pCurveParam, const uint8_t *pClearPrivateKey, uint32_t PublicKey[2][20])
{
	...
	/* Get the prime order n length */
	PKA->RAM[PKA_ECC_SCALAR_MUL_IN_EXP_NB_BITS] = GetOptBitSize_u8(pCurveParam->primeOrderSizeByte, *(pCurveParam->pPrimeOrder));

	/* Get the modulus length */
	PKA->RAM[PKA_ECC_SCALAR_MUL_IN_OP_NB_BITS] = GetOptBitSize_u8(pCurveParam->modulusSizeByte, *(pCurveParam->pModulus));
	...
	CCB_Memcpy_u8_to_u32(&PKA->RAM[PKA_ECC_SCALAR_MUL_IN_N_PRIME_ORDER], pCurveParam->pPrimeOrder, pCurveParam->modulusSizeByte);
	...
	CCB_Memcpy_u32_to_u32(PublicKey[0], &PKA->RAM[PKA_ECC_SCALAR_MUL_OUT_RESULT_X], ((pCurveParam->modulusSizeByte + 3UL) / 4UL));
	CCB_Memcpy_u32_to_u32(PublicKey[1], &PKA->RAM[PKA_ECC_SCALAR_MUL_OUT_RESULT_Y], ((pCurveParam->modulusSizeByte + 3UL) / 4UL));
...
static HAL_StatusTypeDef PKA_RSA_ComputeModularExp(CCB_RSAParamTypeDef *pParam, const CCB_RSAClearKeyTypeDef *pRSAClearPrivateKey, uint8_t *pOp1, uint32_t *pReferenceModularExp)
{
	...
	/* Move the result from appropriate location (with opprand size in 32bit word ) */
	CCB_Memcpy_u32_to_u32(pReferenceModularExp, &PKA->RAM[PKA_MODULAR_EXP_OUT_RESULT], ((pParam->modulusSizeByte + 3UL) / 4UL));
...
static HAL_StatusTypeDef PKA_ECDSAVerif(CCB_ECDSACurveParamTypeDef *pCurveParam, CCB_ECCMulPointTypeDef *pPublicKeyOut, const uint8_t *pHash, CCB_ECDSASignTypeDef *pSignature)
{
	...
	/* Get the prime order n length */
	PKA->RAM[PKA_ECDSA_VERIF_IN_ORDER_NB_BITS] = GetOptBitSize_u8(pCurveParam->primeOrderSizeByte, *(pCurveParam->pPrimeOrder));
	...
	/* Move the input parameters base point G coordinate x to PKA RAM */
	CCB_Memcpy_u8_to_u32(&PKA->RAM[PKA_ECDSA_VERIF_IN_INITIAL_POINT_X], pCurveParam->pPointX, pCurveParam->modulusSizeByte);
	RAM_PARAM_END(PKA->RAM, PKA_ECDSA_VERIF_IN_INITIAL_POINT_X + ((pCurveParam->modulusSizeByte + 3UL) / 4UL));

	/* Move the input parameters base point G coordinate y to PKA RAM */
	CCB_Memcpy_u8_to_u32(&PKA->RAM[PKA_ECDSA_VERIF_IN_INITIAL_POINT_Y], pCurveParam->pPointY, pCurveParam->modulusSizeByte);
	...
	CCB_Memcpy_u8_to_u32(&PKA->RAM[PKA_ECDSA_VERIF_IN_PUBLIC_KEY_POINT_X], pPublicKeyOut->pPointX, pCurveParam->modulusSizeByte);
	RAM_PARAM_END(PKA->RAM, PKA_ECDSA_VERIF_IN_PUBLIC_KEY_POINT_X + ((pCurveParam->modulusSizeByte + 3UL) / 4UL));

	/* Move the input parameters public-key curve point Q coordinate xQ to PKA RAM */
	CCB_Memcpy_u8_to_u32(&PKA->RAM[PKA_ECDSA_VERIF_IN_PUBLIC_KEY_POINT_Y], pPublicKeyOut->pPointY, pCurveParam->modulusSizeByte);
	...
	/* Move the input parameters curve prime order n to PKA RAM */
	CCB_Memcpy_u8_to_u32(&PKA->RAM[PKA_ECDSA_VERIF_IN_ORDER_N], pCurveParam->pPrimeOrder, pCurveParam->primeOrderSizeByte);
	RAM_PARAM_END(PKA->RAM, PKA_ECDSA_VERIF_IN_ORDER_N + ((pCurveParam->primeOrderSizeByte + 3UL) / 4UL));

	/* Set the mode and deactivate the interrupts */
	MODIFY_REG(PKA->CR, PKA_CR_MODE | PKA_CR_PROCENDIE | PKA_CR_RAMERRIE | PKA_CR_ADDRERRIE | PKA_CR_OPERRIE, CCB_PKA_MODE_ECDSA_VERIFICATION << PKA_CR_MODE_Pos);
