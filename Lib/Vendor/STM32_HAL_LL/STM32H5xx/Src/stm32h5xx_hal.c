...
#define __STM32H5XX_HAL_VERSION ((__STM32H5XX_HAL_VERSION_MAIN << 24U) | (__STM32H5XX_HAL_VERSION_SUB1 << 16U) | (__STM32H5XX_HAL_VERSION_SUB2 << 8U) | (__STM32H5XX_HAL_VERSION_RC))
...
void HAL_SBS_VDDCompensationCodeConfig(uint32_t SBS_PMOSCode, uint32_t SBS_NMOSCode)
{
	...
	MODIFY_REG(SBS->CCSWCR, SBS_CCSWCR_SW_ANSRC1 | SBS_CCSWCR_SW_APSRC1, (((uint32_t)(SBS_PMOSCode) << 4) | (uint32_t)(SBS_NMOSCode)));
}
...
void HAL_SBS_VDDIOCompensationCodeConfig(uint32_t SBS_PMOSCode, uint32_t SBS_NMOSCode)
{
	...
	assert_param(IS_SBS_CODE_CONFIG(SBS_PMOSCode));
	assert_param(IS_SBS_CODE_CONFIG(SBS_NMOSCode));
	MODIFY_REG(SBS->CCSWCR, SBS_CCSWCR_SW_ANSRC2 | SBS_CCSWCR_SW_APSRC2, (((uint32_t)(SBS_PMOSCode) << 12) | ((uint32_t)(SBS_NMOSCode) << 8)));
}
