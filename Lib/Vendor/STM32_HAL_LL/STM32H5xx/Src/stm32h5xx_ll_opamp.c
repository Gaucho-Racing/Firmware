...
#define IS_LL_OPAMP_POWER_MODE(__POWER_MODE__) (((__POWER_MODE__) == LL_OPAMP_POWERMODE_NORMAL) || ((__POWER_MODE__) == LL_OPAMP_POWERMODE_HIGHSPEED))
...
#define IS_LL_OPAMP_INPUT_NONINVERTING(__INPUT_NONINVERTING__)                                                                                                                                         \
	(((__INPUT_NONINVERTING__) == LL_OPAMP_INPUT_NONINVERT_IO0) || ((__INPUT_NONINVERTING__) == LL_OPAMP_INPUT_NONINVERT_IO1) || ((__INPUT_NONINVERTING__) == LL_OPAMP_INPUT_NONINVERT_DAC))
...
ErrorStatus LL_OPAMP_Init(OPAMP_TypeDef *OPAMPx, const LL_OPAMP_InitTypeDef *OPAMP_InitStruct)
{
	...
	if (OPAMP_InitStruct->FunctionalMode != LL_OPAMP_MODE_FOLLOWER) {
		MODIFY_REG(OPAMPx->CSR, OPAMP_CSR_OPAHSM | OPAMP_CSR_CALON | OPAMP_CSR_VMSEL | OPAMP_CSR_VPSEL | OPAMP_CSR_PGGAIN_2 | OPAMP_CSR_PGGAIN_1,
		...
		MODIFY_REG(OPAMPx->CSR, OPAMP_CSR_OPAHSM | OPAMP_CSR_CALON | OPAMP_CSR_VMSEL | OPAMP_CSR_VPSEL | OPAMP_CSR_PGGAIN_2 | OPAMP_CSR_PGGAIN_1,
			   (OPAMP_InitStruct->PowerMode & OPAMP_POWERMODE_CSR_BIT_MASK) | LL_OPAMP_MODE_FOLLOWER | OPAMP_InitStruct->InputNonInverting);
	}
