 ...
 * @{
 */
#define IS_LL_I3C_SDAHOLDTIME_VALUE(VALUE) (((VALUE) == LL_I3C_SDA_HOLD_TIME_0_5) || ((VALUE) == LL_I3C_SDA_HOLD_TIME_1_5))

#define IS_LL_I3C_WAITTIME_VALUE(VALUE)                                                                                                                                                                \
	(((VALUE) == LL_I3C_OWN_ACTIVITY_STATE_0) || ((VALUE) == LL_I3C_OWN_ACTIVITY_STATE_1) || ((VALUE) == LL_I3C_OWN_ACTIVITY_STATE_2) || ((VALUE) == LL_I3C_OWN_ACTIVITY_STATE_3))
/**
 * @}
 */
...
ErrorStatus LL_I3C_Init(I3C_TypeDef *I3Cx, LL_I3C_InitTypeDef *I3C_InitStruct, uint32_t Mode)
{
	...
	if (Mode == LL_I3C_MODE_CONTROLLER) {
		...
		waveform_value =
		    ((uint32_t)(I3C_InitStruct->CtrlBusCharacteristic.SCLPPLowDuration) | ((uint32_t)I3C_InitStruct->CtrlBusCharacteristic.SCLI3CHighDuration << I3C_TIMINGR0_SCLH_I3C_Pos) |
		...
		timing_value = ((uint32_t)(I3C_InitStruct->CtrlBusCharacteristic.SDAHoldTime) | (uint32_t)(I3C_InitStruct->CtrlBusCharacteristic.WaitTime) |
				((uint32_t)I3C_InitStruct->CtrlBusCharacteristic.BusFreeDuration << I3C_TIMINGR1_FREE_Pos) | (uint32_t)(I3C_InitStruct->CtrlBusCharacteristic.BusIdleDuration));
