...
void HAL_HASH_Suspend(HASH_HandleTypeDef *hhash, uint8_t *pMemBuffer)
{
	...
	*(uint32_t *)(mem_ptr) = READ_BIT(hhash->Instance->CR, HASH_CR_DMAE | HASH_CR_DATATYPE | HASH_CR_MODE | HASH_CR_ALGO | HASH_CR_LKEY | HASH_CR_MDMAT);
...
HAL_StatusTypeDef HAL_HASH_ProcessSuspend(HASH_HandleTypeDef *hhash)
{
	...
	if (__HAL_HASH_GET_FLAG(hhash, HASH_FLAG_DMAS) != RESET) {
		if (hhash->State == HAL_HASH_STATE_READY) {
			...
			remainingwords += ((((DMA_Channel_TypeDef *)hhash->hdmain->Instance)->CSR) & DMA_CSR_FIFOL) >> DMA_CSR_FIFOL_Pos;
	...
	if ((temp_state == HAL_HASH_STATE_READY) || (temp_state == HAL_HASH_STATE_SUSPENDED)) {
		...
		if ((hhash->hdmain->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
			if ((hhash->hdmain->LinkedListQueue != NULL) && (hhash->hdmain->LinkedListQueue->Head != NULL)) {
				...
				hhash->hdmain->LinkedListQueue->Head->LinkRegisters[NODE_CSAR_DEFAULT_OFFSET] = (uint32_t)(hhash->pHashInBuffPtr); /* Set DMA source address */
				hhash->hdmain->LinkedListQueue->Head->LinkRegisters[NODE_CDAR_DEFAULT_OFFSET] = (uint32_t)&hhash->Instance->DIN;   /* Set DMA destination address */
...
HAL_StatusTypeDef HAL_HASH_HMAC_Start(HASH_HandleTypeDef *hhash, const uint8_t *const pInBuffer, uint32_t Size, uint8_t *const pOutBuffer, uint32_t Timeout)
{
	...
	if (hhash->State == HAL_HASH_STATE_READY) {
		...
		if ((hhash->Init.Algorithm == HASH_ALGOSELECTION_SHA1) || (hhash->Init.Algorithm == HASH_ALGOSELECTION_SHA224) || (hhash->Init.Algorithm == HASH_ALGOSELECTION_SHA256)) {
			blocksize = BLOCK_64B;
		} else {
			blocksize = BLOCK_128B;
		}
		if (hhash->Init.KeySize > blocksize) {
			MODIFY_REG(hhash->Instance->CR, HASH_CR_LKEY | HASH_CR_MODE | HASH_CR_INIT, HASH_ALGOMODE_HMAC | HASH_LONGKEY | HASH_CR_INIT);
		} else {

			MODIFY_REG(hhash->Instance->CR, HASH_CR_LKEY | HASH_CR_MODE | HASH_CR_INIT, HASH_ALGOMODE_HMAC | HASH_CR_INIT);
		}
...
static void HASH_WriteData(HASH_HandleTypeDef *hhash, const uint8_t *pInBuffer, uint32_t Size)
{
	...
	if ((Size % 4U) != 0U) {
		...
		} else if ((hhash->Init.DataType == HASH_BYTE_SWAP) || (hhash->Init.DataType == HASH_BIT_SWAP)) /* byte swap or bit swap or */
		{
			/* Write remaining input data */
			if ((Size % 4U) == 1U) {
