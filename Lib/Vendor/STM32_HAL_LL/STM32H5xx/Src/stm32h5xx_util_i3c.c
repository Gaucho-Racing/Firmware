 ...
 */
ErrorStatus I3C_CtrlTimingComputation(const I3C_CtrlTimingTypeDef *pInputTiming, LL_I3C_CtrlBusConfTypeDef *pOutputConfig)
{
	...
	if (((pInputTiming->clockSrcFreq == 0U) || (pInputTiming->i3cPPFreq == 0U)) && (pInputTiming->busType == I3C_PURE_I3C_BUS)) {
		status = ERROR;
	}

	if (((pInputTiming->clockSrcFreq == 0U) || (pInputTiming->i3cPPFreq == 0U) || (pInputTiming->i2cODFreq == 0U)) && (pInputTiming->busType == I3C_MIXED_BUS)) {
		status = ERROR;
	}

	if (status == SUCCESS) {
		/* Period Clock source */
		ti3cclk = (uint32_t)((SEC210PSEC + ((uint64_t)pInputTiming->clockSrcFreq / (uint64_t)2)) / (uint64_t)pInputTiming->clockSrcFreq);
	...
	if ((status == SUCCESS) && (ti3cclk != 0U)) {
		/* I3C: Push pull period */
		ti3c_pp_min = (uint32_t)((SEC210PSEC + ((uint64_t)pInputTiming->i3cPPFreq / (uint64_t)2)) / (uint64_t)pInputTiming->i3cPPFreq);

		/* I2C: Open drain period */
		ti2c_od_min = (uint32_t)((SEC210PSEC + ((uint64_t)pInputTiming->i2cODFreq / (uint64_t)2)) / (uint64_t)pInputTiming->i2cODFreq);
	...
	if ((status == SUCCESS) && (ti3cclk != 0U)) {
		...
		/* I2C SCL high level (mixed bus with I2C) */
			...
			scllod = DIV_ROUND_CLOSEST(DIV_ROUND_CLOSEST(ti2c_od_min * (100U - pInputTiming->dutyCycle), ti3cclk), 100U) - 1U;
		...
		if ((scllpp > 0xFFU) || (sclhi3c > 0xFFU) || (scllod > 0xFFU) || (sclhi2c > 0xFFU) || (free > 0xFFU) || (oneus > 0xFFU)) {
