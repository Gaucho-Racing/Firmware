		...
		if (PreloadAccess == PRELOAD_ACCESS_POLLING) {
			...
			if ((hfmac->hdmaPreload->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
				if ((hfmac->hdmaPreload->LinkedListQueue != NULL) && (hfmac->hdmaPreload->LinkedListQueue->Head != NULL)) {
					...
					hfmac->hdmaPreload->LinkedListQueue->Head->LinkRegisters[NODE_CBR1_DEFAULT_OFFSET] = (uint32_t)(2UL * pConfig->CoeffBSize); /* Set DMA data size           */
					hfmac->hdmaPreload->LinkedListQueue->Head->LinkRegisters[NODE_CSAR_DEFAULT_OFFSET] = (uint32_t)pConfig->pCoeffB;	    /* Set DMA source address      */
	...
	if ((pInput != NULL) && (InputSize != 0U)) {
		...
		if (PreloadAccess == PRELOAD_ACCESS_POLLING) {
			...
			if ((hfmac->hdmaPreload->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
				if ((hfmac->hdmaPreload->LinkedListQueue != NULL) && (hfmac->hdmaPreload->LinkedListQueue->Head != NULL)) {
					/* Enable the DMA channel */
					hfmac->hdmaPreload->LinkedListQueue->Head->LinkRegisters[NODE_CBR1_DEFAULT_OFFSET] = (uint32_t)(2UL * InputSize);	/* Set DMA data size           */
					hfmac->hdmaPreload->LinkedListQueue->Head->LinkRegisters[NODE_CSAR_DEFAULT_OFFSET] = (uint32_t)pInput;			/* Set DMA source address      */
					hfmac->hdmaPreload->LinkedListQueue->Head->LinkRegisters[NODE_CDAR_DEFAULT_OFFSET] = (uint32_t)&hfmac->Instance->WDATA; /* Set DMA destination address */
	...
	if ((pOutput != NULL) && (OutputSize != 0U)) {
		...
		if (PreloadAccess == PRELOAD_ACCESS_POLLING) {
			...
			if ((hfmac->hdmaPreload->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
				if ((hfmac->hdmaPreload->LinkedListQueue != NULL) && (hfmac->hdmaPreload->LinkedListQueue->Head != NULL)) {
					/* Enable the DMA channel */
					hfmac->hdmaPreload->LinkedListQueue->Head->LinkRegisters[NODE_CBR1_DEFAULT_OFFSET] = (uint32_t)(2UL * OutputSize);	/* Set DMA data size           */
					hfmac->hdmaPreload->LinkedListQueue->Head->LinkRegisters[NODE_CSAR_DEFAULT_OFFSET] = (uint32_t)pOutput;			/* Set DMA source address      */
					hfmac->hdmaPreload->LinkedListQueue->Head->LinkRegisters[NODE_CDAR_DEFAULT_OFFSET] = (uint32_t)&hfmac->Instance->WDATA; /* Set DMA destination address */
	...
	if (hfmac->InputAccess == FMAC_BUFFER_ACCESS_DMA) {
		...
		if ((hfmac->hdmaIn->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
			if ((hfmac->hdmaIn->LinkedListQueue != NULL) && (hfmac->hdmaIn->LinkedListQueue->Head != NULL)) {
				/* Enable the DMA channel */
				hfmac->hdmaIn->LinkedListQueue->Head->LinkRegisters[NODE_CBR1_DEFAULT_OFFSET] = (uint32_t)(2UL * (*pInputSize));   /* Set DMA data size           */
	...
	/* Handle the pointer depending on the input access */
		...
		if ((hfmac->hdmaOut->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
			if ((hfmac->hdmaOut->LinkedListQueue != NULL) && (hfmac->hdmaOut->LinkedListQueue->Head != NULL)) {
				/* Enable the DMA channel */
				hfmac->hdmaOut->LinkedListQueue->Head->LinkRegisters[NODE_CBR1_DEFAULT_OFFSET] = (uint32_t)(4UL * (*pOutputSize));  /* Set DMA data size           */
				...
				hfmac->hdmaOut->LinkedListQueue->Head->LinkRegisters[NODE_CDAR_DEFAULT_OFFSET] = (uint32_t)pOutput;		    /* Set DMA destination address */
...
static void FMAC_DMAFilterConfig(DMA_HandleTypeDef *hdma)
{
	...
	if (hfmac->pInput != NULL) {
		...
		if ((hfmac->hdmaPreload->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
			if ((hfmac->hdmaPreload->LinkedListQueue != NULL) && (hfmac->hdmaPreload->LinkedListQueue->Head != NULL)) {
				/* Enable the DMA channel */
				hfmac->hdmaPreload->LinkedListQueue->Head->LinkRegisters[NODE_CBR1_DEFAULT_OFFSET] = (uint32_t)(2UL * hfmac->InputCurrentSize); /* Set DMA data size           */
