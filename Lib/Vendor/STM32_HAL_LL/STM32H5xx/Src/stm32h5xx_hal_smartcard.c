...
#define USART_CR3_FIELDS                                                                                                                                                                               \
	((uint32_t)(USART_CR3_ONEBIT | USART_CR3_NACK | USART_CR3_SCARCNT | USART_CR3_TXFTCFG | USART_CR3_RXFTCFG)) /*!< USART CR3 fields of parameters set by SMARTCARD_SetConfig API */
...
static HAL_StatusTypeDef SMARTCARD_SetConfig(SMARTCARD_HandleTypeDef *hsmartcard)
{
	...
	switch (clocksource) {
		...
		case SMARTCARD_CLOCKSOURCE_PLL2Q:
			...
			tmpreg = (uint32_t)(((pll2_clocks.PLL2_Q_Frequency / SMARTCARDPrescTable[hsmartcard->Init.ClockPrescaler]) + (hsmartcard->Init.BaudRate / 2U)) / hsmartcard->Init.BaudRate);
			break;
#if defined(RCC_CR_PLL3ON)
		case SMARTCARD_CLOCKSOURCE_PLL3Q:
			HAL_RCCEx_GetPLL3ClockFreq(&pll3_clocks);
			tmpreg = (uint32_t)(((pll3_clocks.PLL3_Q_Frequency / SMARTCARDPrescTable[hsmartcard->Init.ClockPrescaler]) + (hsmartcard->Init.BaudRate / 2U)) / hsmartcard->Init.BaudRate);
			break;
#endif /* RCC_CR_PLL3ON */
		...
		case SMARTCARD_CLOCKSOURCE_CSI:
			tmpreg = (uint32_t)(((CSI_VALUE / SMARTCARDPrescTable[hsmartcard->Init.ClockPrescaler]) + (hsmartcard->Init.BaudRate / 2U)) / hsmartcard->Init.BaudRate);
			break;
