 ...
 */
#define I3C_GET_ADVANCED_CAPABLE(__BCR__) (((((__BCR__) & I3C_BCR_BCR5_Msk) >> I3C_BCR_BCR5_Pos) == 1U) ? ENABLE : DISABLE)
 ...
 */
#define I3C_GET_OFFLINE_CAPABLE(__BCR__) (((((__BCR__) & I3C_BCR_BCR3_Msk) >> I3C_BCR_BCR3_Pos) == 1U) ? ENABLE : DISABLE)
 ...
 */
#define I3C_GET_MAX_DATA_SPEED_LIMIT(__BCR__) (((((__BCR__) & I3C_BCR_BCR0_Msk) >> I3C_BCR_BCR0_Pos) == 1U) ? ENABLE : DISABLE)
 ...
 */
#define I3C_BIG_TO_LITTLE_ENDIAN(__DATA__) ((uint32_t)((((__DATA__) & 0xff000000U) >> 24) | (((__DATA__) & 0x00ff0000U) >> 8) | (((__DATA__) & 0x0000ff00U) << 8) | (((__DATA__) & 0x000000ffU) << 24)))
...
static HAL_StatusTypeDef I3C_WaitOnFlagUntilTimeout(I3C_HandleTypeDef *hi3c, uint32_t flag, FlagStatus flagstatus, uint32_t timeout, uint32_t tickstart);
...
static uint32_t I3C_FillTxBuffer_CCC(I3C_HandleTypeDef *hi3c, uint32_t indexDesc, uint32_t txSize, uint32_t txCurrentIndex);
static uint32_t I3C_FillTxBuffer_Private(I3C_HandleTypeDef *hi3c, uint32_t indexDesc, uint32_t txSize, uint32_t txCurrentIndex);
static HAL_StatusTypeDef I3C_ControlBuffer_PriorPreparation(I3C_HandleTypeDef *hi3c, uint8_t counter, uint32_t option);
static HAL_StatusTypeDef I3C_Ctrl_IsDevice_Ready(I3C_HandleTypeDef *hi3c, const I3C_DeviceTypeDef *pDevice, uint32_t trials, uint32_t timeout);
...
HAL_StatusTypeDef HAL_I3C_Init(I3C_HandleTypeDef *hi3c)
{
	...
	if (hi3c == NULL) {
		...
		if (hi3c->Mode == HAL_I3C_MODE_CONTROLLER) {
			...
			waveform_value = ((uint32_t)hi3c->Init.CtrlBusCharacteristic.SCLPPLowDuration | ((uint32_t)hi3c->Init.CtrlBusCharacteristic.SCLI3CHighDuration << I3C_TIMINGR0_SCLH_I3C_Pos) |
			...
			timing_value = ((uint32_t)hi3c->Init.CtrlBusCharacteristic.SDAHoldTime | (uint32_t)hi3c->Init.CtrlBusCharacteristic.WaitTime |
					((uint32_t)hi3c->Init.CtrlBusCharacteristic.BusFreeDuration << I3C_TIMINGR1_FREE_Pos) | (uint32_t)hi3c->Init.CtrlBusCharacteristic.BusIdleDuration);
 ...
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_RegisterCallback(I3C_HandleTypeDef *hi3c, HAL_I3C_CallbackIDTypeDef callbackID, pI3C_CallbackTypeDef pCallback)
{
	HAL_StatusTypeDef status = HAL_OK;
 ...
 * @retval HAL Status          :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_RegisterTgtReqDynamicAddrCallback(I3C_HandleTypeDef *hi3c, pI3C_TgtReqDynamicAddrCallbackTypeDef pTgtReqAddrCallback)
{
	HAL_StatusTypeDef status = HAL_OK;
 ...
 * @retval HAL Status          :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_RegisterTgtHotJoinCallback(I3C_HandleTypeDef *hi3c, pI3C_TgtHotJoinCallbackTypeDef pTgtHotJoinCallback)
{
	HAL_StatusTypeDef status = HAL_OK;
 ...
 * @retval HAL Status    :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_ActivateNotification(I3C_HandleTypeDef *hi3c, I3C_XferTypeDef *pXferData, uint32_t interruptMask)
{
	HAL_StatusTypeDef status = HAL_OK;
	...
	if (hi3c == NULL) {
		...
		if ((hi3c->State == HAL_I3C_STATE_RESET) || ((hi3c->Mode != HAL_I3C_MODE_CONTROLLER) && (hi3c->Mode != HAL_I3C_MODE_TARGET))) {
			hi3c->ErrorCode = HAL_I3C_ERROR_NOT_ALLOWED;
			status = HAL_ERROR;
		}
		/* Check the I3C mode */
		else if ((hi3c->Mode == HAL_I3C_MODE_TARGET) && ((interruptMask & (HAL_I3C_IT_DEFIE | HAL_I3C_IT_GRPIE)) != 0U) && (pXferData == NULL)) {
			hi3c->ErrorCode = HAL_I3C_ERROR_INVALID_PARAM;
			status = HAL_ERROR;
		} else {
			/* Check the I3C mode */
...
void HAL_I3C_ER_IRQHandler(I3C_HandleTypeDef *hi3c)
{
	...
	if ((I3C_CHECK_FLAG(it_flag, HAL_I3C_FLAG_ERRF) != RESET) && (I3C_CHECK_IT_SOURCE(it_source, HAL_I3C_IT_ERRIE) != RESET)) {
 ...
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_BusCharacteristicConfig(I3C_HandleTypeDef *hi3c, const LL_I3C_CtrlBusConfTypeDef *pConfig)
{
	HAL_StatusTypeDef status = HAL_OK;
	uint32_t waveform_value;
	uint32_t timing_value;
	...
	if (hi3c == NULL) {
		...
		/* Check the I3C state and mode */
			...
			/*----------------- SCL signal waveform configuration : I3C timing register 0 (I3C_TIMINGR0) ------------------ */
			/* Set the controller SCL waveform */
			waveform_value = ((uint32_t)pConfig->SCLPPLowDuration | ((uint32_t)pConfig->SCLI3CHighDuration << I3C_TIMINGR0_SCLH_I3C_Pos) |
					  ((uint32_t)pConfig->SCLODLowDuration << I3C_TIMINGR0_SCLL_OD_Pos) | ((uint32_t)pConfig->SCLI2CHighDuration << I3C_TIMINGR0_SCLH_I2C_Pos));
			...
			/* Set SDA hold time, activity state, bus free duration and bus available duration */
			timing_value =
			    ((uint32_t)pConfig->SDAHoldTime | (uint32_t)pConfig->WaitTime | ((uint32_t)pConfig->BusFreeDuration << I3C_TIMINGR1_FREE_Pos) | (uint32_t)pConfig->BusIdleDuration);
...
HAL_StatusTypeDef HAL_I3C_Ctrl_Config(I3C_HandleTypeDef *hi3c, const I3C_CtrlConfTypeDef *pConfig)
{
	...
	if (hi3c == NULL) {
		...
		/* Check the I3C state and mode */
			...
			timing2_value = (((uint32_t)pConfig->StallTime << I3C_TIMINGR2_STALL_Pos) | ((uint32_t)pConfig->ACKStallState << I3C_TIMINGR2_STALLA_Pos) |
					 ((uint32_t)pConfig->CCCStallState << I3C_TIMINGR2_STALLC_Pos) | ((uint32_t)pConfig->TxStallState << I3C_TIMINGR2_STALLD_Pos) |
			...
			cfgr_value = (((uint32_t)pConfig->HighKeeperSDA << I3C_CFGR_HKSDAEN_Pos) | ((uint32_t)pConfig->HotJoinAllowed << I3C_CFGR_HJACK_Pos));
...
HAL_StatusTypeDef HAL_I3C_Tgt_Config(I3C_HandleTypeDef *hi3c, const I3C_TgtConfTypeDef *pConfig)
{
	...
	if (hi3c == NULL) {
		...
		/* Check the I3C state and mode */
			...
			getmxdsr_value = (pConfig->HandOffActivityState | pConfig->MaxDataSpeed | pConfig->DataTurnAroundDuration | ((uint32_t)pConfig->MaxReadTurnAround << I3C_GETMXDSR_RDTURN_Pos));
			...
			crccapr_value = (((uint32_t)pConfig->HandOffDelay << I3C_CRCAPR_CAPDHOFF_Pos) | ((uint32_t)pConfig->GroupAddrCapability << I3C_CRCAPR_CAPGRP_Pos));
			...
			bcr_value = (((uint32_t)pConfig->MaxSpeedLimitation << I3C_BCR_BCR0_Pos) | ((uint32_t)pConfig->IBIPayload << I3C_BCR_BCR2_Pos) |
			...
			/* Calculate value to be written in CFGR register */
			devr0_value = (((uint32_t)pConfig->IBIRequest << I3C_DEVR0_IBIEN_Pos) | ((uint32_t)pConfig->CtrlRoleRequest << I3C_DEVR0_CREN_Pos) |
 ...
 * @retval HAL Status :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_ConfigBusDevices(I3C_HandleTypeDef *hi3c, const I3C_DeviceConfTypeDef *pDesc, uint8_t nbDevice)
{
	HAL_StatusTypeDef status = HAL_OK;
	...
	if (hi3c == NULL) {
		...
		/* Check the I3C state and mode */
			...
			for (uint32_t index = 0U; index < nbDevice; index++) {
				...
				write_value = (((uint32_t)pDesc[index].TargetDynamicAddr << I3C_DEVRX_DA_Pos) | ((uint32_t)pDesc[index].IBIAck << I3C_DEVRX_IBIACK_Pos) |
					       ((uint32_t)pDesc[index].CtrlRoleReqAck << I3C_DEVRX_CRACK_Pos) | ((uint32_t)pDesc[index].CtrlStopTransfer << I3C_DEVRX_SUSP_Pos) |
 ...
 * @retval HAL Status    :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_AddDescToFrame(I3C_HandleTypeDef *hi3c, const I3C_CCCTypeDef *pCCCDesc, const I3C_PrivateTypeDef *pPrivateDesc, I3C_XferTypeDef *pXferData, uint8_t nbFrame, uint32_t option)
{
	...
	if (hi3c == NULL) {
		...
		if (((option & I3C_OPERATION_TYPE_MASK) == LL_I3C_CONTROLLER_MTYPE_CCC) || ((option & I3C_OPERATION_TYPE_MASK) == LL_I3C_CONTROLLER_MTYPE_DIRECT)) {
			/* Check on user parameters */
			if ((pCCCDesc == NULL) || (pXferData == NULL) || (nbFrame < 1U) ||
			    (((option & (I3C_OPERATION_TYPE_MASK | I3C_DEFINE_BYTE_MASK)) == (LL_I3C_CONTROLLER_MTYPE_DIRECT | I3C_DEFINE_BYTE_MASK)) && (pCCCDesc->CCCBuf.Size == 0U))) {
				hi3c->ErrorCode = HAL_I3C_ERROR_INVALID_PARAM;
				status = HAL_ERROR;
			}
		} else {
...
HAL_StatusTypeDef HAL_I3C_Ctrl_TransmitCCC_DMA(I3C_HandleTypeDef *hi3c, I3C_XferTypeDef *pXferData)
{
	...
	if (hi3c == NULL) {
		...
		/* check on the State */
			...
			control_dma_status = HAL_DMA_Start_IT(hi3c->hdmacr, (uint32_t)hi3c->pXferData->CtrlBuf.pBuffer, (uint32_t)&hi3c->Instance->CR, (hi3c->ControlXferCount * 4U));
			...
			if (hi3c->TxXferCount != 0U) {
				...
				if (LL_I3C_GetTxFIFOThreshold(hi3c->Instance) == LL_I3C_TXFIFO_THRESHOLD_1_4) {
					...
					tx_dma_status = HAL_DMA_Start_IT(hi3c->hdmatx, (uint32_t)hi3c->pXferData->TxBuf.pBuffer, (uint32_t)&hi3c->Instance->TDR, hi3c->pXferData->TxBuf.Size);
				} else {
					/* assert that DMA source and destination width are configured in word */
					...
					/* Enable the Tx data DMA channel */
					tx_dma_status = HAL_DMA_Start_IT(hi3c->hdmatx, (uint32_t)hi3c->pXferData->TxBuf.pBuffer, (uint32_t)&hi3c->Instance->TDWR, size_align_word);
				}
			}
...
HAL_StatusTypeDef HAL_I3C_Ctrl_ReceiveCCC(I3C_HandleTypeDef *hi3c, I3C_XferTypeDef *pXferData, uint32_t timeout)
{
	...
	if (hi3c == NULL) {
		...
		/* Check on user parameters */
		if ((pXferData == NULL) || (pXferData->RxBuf.pBuffer == NULL) || ((pXferData->TxBuf.pBuffer == NULL) && (hi3c->TxXferCount != 0U))) {
			hi3c->ErrorCode = HAL_I3C_ERROR_INVALID_PARAM;
			status = HAL_ERROR;
		}
		/* check on the Mode */
		...
		/* check on the State */
			...
			if (hi3c->RxXferCount != 0U) {
				...
				if (LL_I3C_GetRxFIFOThreshold(hi3c->Instance) == LL_I3C_RXFIFO_THRESHOLD_1_4) {
					...
					rx_dma_status = HAL_DMA_Start_IT(hi3c->hdmarx, (uint32_t)&hi3c->Instance->RDR, (uint32_t)hi3c->pXferData->RxBuf.pBuffer, hi3c->pXferData->RxBuf.Size);
				} else {
					/* assert that DMA source and destination width are configured in word */
					...
					/* Enable the Rx data DMA channel */
					rx_dma_status = HAL_DMA_Start_IT(hi3c->hdmarx, (uint32_t)&hi3c->Instance->RDWR, (uint32_t)hi3c->pXferData->RxBuf.pBuffer, size_align_word);
				}
			}
...
HAL_StatusTypeDef HAL_I3C_Ctrl_MultipleTransfer_IT(I3C_HandleTypeDef *hi3c, I3C_XferTypeDef *pXferData)
{
	...
	if (hi3c == NULL) {
		...
		/* Check on user parameters */
		if ((pXferData == NULL) || ((pXferData->RxBuf.pBuffer == NULL) && (hi3c->RxXferCount != 0U)) || ((pXferData->TxBuf.pBuffer == NULL) && (hi3c->TxXferCount != 0U))) {
			hi3c->ErrorCode = HAL_I3C_ERROR_INVALID_PARAM;
			status = HAL_ERROR;
		}
		/* check on the Mode */
 ...
 * @retval HAL Status     :          Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_DynAddrAssign(I3C_HandleTypeDef *hi3c, uint64_t *target_payload, uint32_t dynOption, uint32_t timeout)
{
	uint32_t tickstart;
	HAL_I3C_StateTypeDef handle_state;
	HAL_StatusTypeDef status = HAL_OK;
	...
	if (hi3c == NULL) {
		...
		/* check on the Mode */
			...
			if ((dynOption == I3C_RSTDAA_THEN_ENTDAA) && ((handle_state == HAL_I3C_STATE_READY) || (handle_state == HAL_I3C_STATE_LISTEN))) {
				/* Set handle transfer parameters */
				hi3c->ErrorCode = HAL_I3C_ERROR_NONE;
			...
			if (status == HAL_OK) {
				/* check on the State */
				if ((handle_state == HAL_I3C_STATE_READY) || (handle_state == HAL_I3C_STATE_LISTEN) || (handle_state == HAL_I3C_STATE_BUSY_DAA)) {
					/* Check on the state */
 ...
 * @retval HAL Status :      Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_IsDeviceI3C_Ready(I3C_HandleTypeDef *hi3c, uint8_t devAddress, uint32_t trials, uint32_t timeout)
{
 ...
 * @retval HAL Status :      Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Ctrl_IsDeviceI2C_Ready(I3C_HandleTypeDef *hi3c, uint8_t devAddress, uint32_t trials, uint32_t timeout)
{
	I3C_DeviceTypeDef device;
	HAL_StatusTypeDef status;
...
HAL_StatusTypeDef HAL_I3C_Tgt_Transmit(I3C_HandleTypeDef *hi3c, I3C_XferTypeDef *pXferData, uint32_t timeout)
{
	...
	if (hi3c == NULL) {
		...
		else if ((I3C_CHECK_IT_SOURCE(it_source, HAL_I3C_IT_DEFIE) != RESET) || (I3C_CHECK_IT_SOURCE(it_source, HAL_I3C_IT_GRPIE) != RESET)) {
			hi3c->ErrorCode = HAL_I3C_ERROR_NOT_ALLOWED;
			status = HAL_ERROR;
		}
 ...
 * @retval HAL Status  :       Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Tgt_IBIReq(I3C_HandleTypeDef *hi3c, const uint8_t *pPayload, uint8_t payloadSize, uint32_t timeout)
{
	uint32_t tickstart;
 ...
 * @retval HAL Status : Value from HAL_StatusTypeDef enumeration.
 */
HAL_StatusTypeDef HAL_I3C_Get_ENTDAA_Payload_Info(I3C_HandleTypeDef *hi3c, uint64_t ENTDAA_payload, I3C_ENTDAAPayloadTypeDef *pENTDAA_payload)
{
	HAL_StatusTypeDef status = HAL_OK;
	uint32_t BCR;
	uint64_t PID;
	...
	if (hi3c == NULL) {
		...
		if (pENTDAA_payload == NULL) {
			...
			PID = (uint64_t)((((uint64_t)I3C_BIG_TO_LITTLE_ENDIAN((uint32_t)PID) << 32) | ((uint64_t)I3C_BIG_TO_LITTLE_ENDIAN((uint32_t)(PID >> 32)))) >> 16);
	...
	for (uint32_t descr_index = 0U; descr_index < counter; descr_index++) {
		...
		if ((option & I3C_OPERATION_TYPE_MASK) == LL_I3C_CONTROLLER_MTYPE_DIRECT) {
			...
			if (((option & I3C_DEFINE_BYTE_MASK) != 0U) && (direction == HAL_I3C_DIRECTION_READ)) {
				...
				if ((global_tx_size > hi3c->pXferData->TxBuf.Size) || (current_tx_index > hi3c->pXferData->TxBuf.Size) || (hi3c->pXferData->TxBuf.pBuffer == NULL)) {
					hi3c->ErrorCode = HAL_I3C_ERROR_INVALID_PARAM;
				...
				if ((global_tx_size > hi3c->pXferData->TxBuf.Size) || (current_tx_index > hi3c->pXferData->TxBuf.Size) || (hi3c->pXferData->TxBuf.pBuffer == NULL)) {
					...
					/* Fill global Tx buffer with data and update the current index of the Tx buffer */
					current_tx_index = I3C_FillTxBuffer_CCC(hi3c, descr_index, hi3c->pCCCDesc[descr_index].CCCBuf.Size, current_tx_index);
				}
			}
		...
		/* Private */
			...
			if (direction == HAL_I3C_DIRECTION_WRITE) {
				...
				if ((global_tx_size > hi3c->pXferData->TxBuf.Size) || (current_tx_index > hi3c->pXferData->TxBuf.Size) || (hi3c->pXferData->TxBuf.pBuffer == NULL)) {
					...
					/* Fill global Tx buffer with data and update the current index of the Tx buffer */
					current_tx_index = I3C_FillTxBuffer_Private(hi3c, descr_index, hi3c->pPrivateDesc[descr_index].TxBuf.Size, current_tx_index);
				}
			} else {
		...
		if ((option & I3C_OPERATION_TYPE_MASK) == LL_I3C_CONTROLLER_MTYPE_CCC) {
			...
			if (hi3c->pXferData->CtrlBuf.Size < (uint32_t)counter) {
				...
				for (index = 0U; index < ((uint32_t)counter - 1U); index++) {
					...
					hi3c->pXferData->CtrlBuf.pBuffer[index] =
					    ((uint32_t)hi3c->pCCCDesc[index].CCCBuf.Size | ((uint32_t)hi3c->pCCCDesc[index].CCC << I3C_CR_CCC_Pos) | LL_I3C_CONTROLLER_MTYPE_CCC | stop_condition);
				}

				/* At the last device we should generate a stop condition */
				hi3c->pXferData->CtrlBuf.pBuffer[index] =
				    ((uint32_t)hi3c->pCCCDesc[index].CCCBuf.Size | ((uint32_t)hi3c->pCCCDesc[index].CCC << I3C_CR_CCC_Pos) | LL_I3C_CONTROLLER_MTYPE_CCC | LL_I3C_GENERATE_STOP);
			}
			...
			if (hi3c->pXferData->CtrlBuf.Size < ((uint32_t)counter * 2U)) {
				...
				for (index = 0U; index < (((uint32_t)counter * 2U) - 2U); index += 2U) {
					...
					hi3c->pXferData->CtrlBuf.pBuffer[index] =
					    (nb_define_bytes | ((uint32_t)hi3c->pCCCDesc[index / 2U].CCC << I3C_CR_CCC_Pos) | LL_I3C_CONTROLLER_MTYPE_CCC | LL_I3C_GENERATE_RESTART);

					/* Step 2 : update control buffer value for target address */
					hi3c->pXferData->CtrlBuf.pBuffer[index + 1U] =
					    (((uint32_t)hi3c->pCCCDesc[index / 2U].CCCBuf.Size - nb_define_bytes) | hi3c->pCCCDesc[index / 2U].Direction |
					     ((uint32_t)hi3c->pCCCDesc[index / 2U].TargetAddr << I3C_CR_ADD_Pos) | LL_I3C_CONTROLLER_MTYPE_DIRECT | stop_condition);
				}
				...
				hi3c->pXferData->CtrlBuf.pBuffer[index + 1U] =
				    (((uint32_t)hi3c->pCCCDesc[index / 2U].CCCBuf.Size - nb_define_bytes) | hi3c->pCCCDesc[index / 2U].Direction |
				     ((uint32_t)hi3c->pCCCDesc[index / 2U].TargetAddr << I3C_CR_ADD_Pos) | LL_I3C_CONTROLLER_MTYPE_DIRECT | LL_I3C_GENERATE_STOP);
			}
		} else {
			/*------------------------------------------ Private I3C/I2C ---------------------------------------------------*/
			/* Check on the control buffer size */
			if (hi3c->pXferData->CtrlBuf.Size < (uint32_t)counter) {
				...
				for (index = 0U; index < ((uint32_t)counter - 1U); index++) {
					...
					/* Update control buffer value */
					hi3c->pXferData->CtrlBuf.pBuffer[index] =
					    (nb_data_bytes | hi3c->pPrivateDesc[index].Direction | ((uint32_t)hi3c->pPrivateDesc[index].TargetAddr << I3C_CR_ADD_Pos) |
				...
				/* At the last device we should generate a stop condition */
				hi3c->pXferData->CtrlBuf.pBuffer[index] = (nb_data_bytes | hi3c->pPrivateDesc[index].Direction | ((uint32_t)hi3c->pPrivateDesc[index].TargetAddr << I3C_CR_ADD_Pos) |
...
static HAL_StatusTypeDef I3C_Ctrl_IsDevice_Ready(I3C_HandleTypeDef *hi3c, const I3C_DeviceTypeDef *pDevice, uint32_t trials, uint32_t timeout)
{
	...
	/* check on the State */
		...
		CR_tmp = (HAL_I3C_DIRECTION_WRITE | ((uint32_t)pDevice->Address << I3C_CR_ADD_Pos) | pDevice->MessageType | LL_I3C_GENERATE_STOP);
