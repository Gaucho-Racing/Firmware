	...
	if (LL_ADC_REG_IsConversionOngoing(hadc->Instance) != 0UL) {
		...
		if (tmp_hal_status == HAL_OK) {
			...
			if ((hadc->DMA_Handle->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
				...
				tmp_hal_status = HAL_DMA_Start_IT(hadc->DMA_Handle, (uint32_t)&tmpADC_Common->CDR, (uint32_t)pData, length_bytes);
			}
...
#if defined(ADC3)
		else if (((pConfigInjected->InjectedChannel == ADC_CHANNEL_VBAT) || (pConfigInjected->InjectedChannel == ADC_CHANNEL_VBAT_ADC3)) &&
	...
	if ((LL_ADC_REG_IsConversionOngoing(hadc->Instance) == 0UL) && (tmp_hadc_slave_conversion_on_going == 0UL)) {
		...
		if (pMultimode->Mode != ADC_MODE_INDEPENDENT) {
			...
			MODIFY_REG(tmpADC_Common->CCR, ADC_CCR_MDMA | ADC_CCR_DMACFG,
				   pMultimode->DMAAccessMode | ADC_CCR_MULTI_DMACONTREQ((hadc->Init.ConversionDataManagement & ADC_CFGR_DMACFG) >> ADC_CFGR_DMACFG_Pos));
#else
