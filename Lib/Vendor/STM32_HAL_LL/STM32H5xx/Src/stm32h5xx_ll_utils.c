...
#define IS_LL_UTILS_CHECK_VCO_RANGES(__RANGEIN__, __RANGEOUT__)                                                                                                                                        \
	((((__RANGEIN__) == LL_RCC_PLLINPUTRANGE_1_2) && ((__RANGEOUT__) == LL_RCC_PLLVCORANGE_MEDIUM)) || (((__RANGEIN__) != LL_RCC_PLLINPUTRANGE_1_2) && ((__RANGEOUT__) == LL_RCC_PLLVCORANGE_WIDE)))
...
#define IS_LL_UTILS_HSE_BYPASS(__STATE__) (((__STATE__) == LL_UTILS_HSEBYPASS_ON) || ((__STATE__) == LL_UTILS_HSEBYPASS_DIGITAL_ON) || ((__STATE__) == LL_UTILS_HSEBYPASS_OFF))
 ...
 */
ErrorStatus LL_PLL_ConfigSystemClock_CSI(LL_UTILS_PLLInitTypeDef *UTILS_PLLInitStruct, LL_UTILS_ClkInitTypeDef *UTILS_ClkInitStruct)
{
	ErrorStatus status;
#ifdef USE_FULL_ASSERT
...
#ifdef USE_FULL_ASSERT
	vcooutput_freq = LL_RCC_CalcPLLClockFreq(CSI_VALUE, UTILS_PLLInitStruct->PLLM, UTILS_PLLInitStruct->PLLN, UTILS_PLLInitStruct->FRACN, 1U);
	...
	if (UTILS_PLL_IsBusy() == SUCCESS) {
		...
		/* Configure PLL */
		LL_RCC_PLL1_ConfigDomain_SYS(LL_RCC_PLL1SOURCE_CSI, UTILS_PLLInitStruct->PLLM, UTILS_PLLInitStruct->PLLN, UTILS_PLLInitStruct->PLLP);
...
ErrorStatus LL_PLL_ConfigSystemClock_HSI(LL_UTILS_PLLInitTypeDef *UTILS_PLLInitStruct, LL_UTILS_ClkInitTypeDef *UTILS_ClkInitStruct)
{
	...
	if (UTILS_PLL_IsBusy() == SUCCESS) {
		...
		/* Configure PLL */
		LL_RCC_PLL1_ConfigDomain_SYS(LL_RCC_PLL1SOURCE_HSI, UTILS_PLLInitStruct->PLLM, UTILS_PLLInitStruct->PLLN, UTILS_PLLInitStruct->PLLP);
		LL_RCC_PLL1FRACN_Disable();
		LL_RCC_PLL1_SetFRACN(UTILS_PLLInitStruct->FRACN);
		LL_RCC_PLL1FRACN_Enable();
		LL_RCC_PLL1_SetVCOInputRange(UTILS_PLLInitStruct->VCO_Input);
...
ErrorStatus LL_PLL_ConfigSystemClock_HSE(uint32_t HSEFrequency, uint32_t HSEBypass, LL_UTILS_PLLInitTypeDef *UTILS_PLLInitStruct, LL_UTILS_ClkInitTypeDef *UTILS_ClkInitStruct)
{
	...
	if (UTILS_PLL_IsBusy() == SUCCESS) {
		...
		/* Configure PLL */
		LL_RCC_PLL1_ConfigDomain_SYS(LL_RCC_PLL1SOURCE_HSE, UTILS_PLLInitStruct->PLLM, UTILS_PLLInitStruct->PLLN, UTILS_PLLInitStruct->PLLP);
		LL_RCC_PLL1FRACN_Disable();
		LL_RCC_PLL1_SetFRACN(UTILS_PLLInitStruct->FRACN);
		LL_RCC_PLL1FRACN_Enable();
		LL_RCC_PLL1_SetVCOInputRange(UTILS_PLLInitStruct->VCO_Input);
