...
HAL_StatusTypeDef HAL_RCC_DeInit(void)
{
	...
	CLEAR_BIT(RCC->CR, RCC_CR_CSION | RCC_CR_CSIKERON | RCC_CR_HSECSSON | RCC_CR_HSIKERON | RCC_CR_HSI48ON | RCC_CR_HSEON);
...
HAL_StatusTypeDef HAL_RCC_OscConfig(const RCC_OscInitTypeDef *pOscInitStruct)
{
	...
	if (((pOscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_CSI) == RCC_OSCILLATORTYPE_CSI) {
		...
		if ((temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_CSI) || ((temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_PLLCLK) && (temp_pllckselr == RCC_PLL1_SOURCE_CSI))) {
	...
	if (((pOscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_HSE) == RCC_OSCILLATORTYPE_HSE) {
		...
		if ((temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_HSE) || ((temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_PLLCLK) && (temp_pllckselr == RCC_PLL1_SOURCE_HSE))) {
	...
	if (((pOscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_HSI) == RCC_OSCILLATORTYPE_HSI) {
		...
		if ((temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_HSI) || ((temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_PLLCLK) && (temp_pllckselr == RCC_PLL1_SOURCE_HSI))) {
	...
	if ((pOscInitStruct->PLL.PLLState) != RCC_PLL_NONE) {
		...
		if (temp_sysclksrc != RCC_SYSCLKSOURCE_STATUS_PLLCLK) {
			if ((pOscInitStruct->PLL.PLLState) == RCC_PLL_ON) {
				...
				__HAL_RCC_PLL1_CONFIG(pOscInitStruct->PLL.PLLSource, pOscInitStruct->PLL.PLLM, pOscInitStruct->PLL.PLLN, pOscInitStruct->PLL.PLLP, pOscInitStruct->PLL.PLLQ,
			...
			if (((pOscInitStruct->PLL.PLLState) == RCC_PLL_OFF) || (READ_BIT(temp1_pllckcfg, RCC_PLL1CFGR_PLL1SRC) != pOscInitStruct->PLL.PLLSource) ||
			    ((READ_BIT(temp1_pllckcfg, RCC_PLL1CFGR_PLL1M) >> RCC_PLL1CFGR_PLL1M_Pos) != (pOscInitStruct->PLL.PLLM)) ||
			    (READ_BIT(temp2_pllckcfg, RCC_PLL1DIVR_PLL1N) != (pOscInitStruct->PLL.PLLN - 1U)) ||
			    ((READ_BIT(temp2_pllckcfg, RCC_PLL1DIVR_PLL1P) >> RCC_PLL1DIVR_PLL1P_Pos) != (pOscInitStruct->PLL.PLLP - 1U)) ||
			    ((READ_BIT(temp2_pllckcfg, RCC_PLL1DIVR_PLL1Q) >> RCC_PLL1DIVR_PLL1Q_Pos) != (pOscInitStruct->PLL.PLLQ - 1U)) ||
			    ((READ_BIT(temp2_pllckcfg, RCC_PLL1DIVR_PLL1R) >> RCC_PLL1DIVR_PLL1R_Pos) != (pOscInitStruct->PLL.PLLR - 1U))) {
				return HAL_ERROR;
			}

			/* FRACN1 on-the-fly value update */
			if ((READ_BIT(RCC->PLL1FRACR, RCC_PLL1FRACR_PLL1FRACN) >> RCC_PLL1FRACR_PLL1FRACN_Pos) != (pOscInitStruct->PLL.PLLFRACN)) {
				assert_param(IS_RCC_PLL1_FRACN_VALUE(pOscInitStruct->PLL.PLLFRACN));
			...
			switch (pllsource) {
				case RCC_PLL1_SOURCE_HSI: /* HSI used as PLL1 clock source */
					...
					if (__HAL_RCC_GET_FLAG(RCC_FLAG_HSIDIVF) != 0U) {
						...
						pllvco = ((float_t)HSI_VALUE / (float_t)pllm) * ((float_t)(uint32_t)(RCC->PLL1DIVR & RCC_PLL1DIVR_PLL1N) + (fracn1 / (float_t)0x2000) + (float_t)1);
					}
...
uint32_t HAL_RCC_GetHCLKFreq(void)
{
	...
	SystemCoreClock = HAL_RCC_GetSysClockFreq() >> (AHBPrescTable[(RCC->CFGR2 & RCC_CFGR2_HPRE) >> RCC_CFGR2_HPRE_Pos] & 0x1FU);
...
void HAL_RCC_GetOscConfig(RCC_OscInitTypeDef *pOscInitStruct)
{
	...
	pOscInitStruct->OscillatorType = RCC_OSCILLATORTYPE_HSE | RCC_OSCILLATORTYPE_HSI | RCC_OSCILLATORTYPE_CSI | RCC_OSCILLATORTYPE_LSE | RCC_OSCILLATORTYPE_LSI | RCC_OSCILLATORTYPE_HSI48;
	...
	pOscInitStruct->HSICalibrationValue = (uint32_t)(READ_BIT(RCC->HSICFGR, RCC_HSICFGR_HSITRIM) >> RCC_HSICFGR_HSITRIM_Pos);
	...
	pOscInitStruct->PLL.PLLFRACN = (uint32_t)(((RCC->PLL1FRACR & RCC_PLL1FRACR_PLL1FRACN) >> RCC_PLL1FRACR_PLL1FRACN_Pos));
}
...
void HAL_RCC_GetClockConfig(RCC_ClkInitTypeDef *pClkInitStruct, uint32_t *pFLatency)
{
	...
	pClkInitStruct->ClockType = RCC_CLOCKTYPE_SYSCLK | RCC_CLOCKTYPE_HCLK | RCC_CLOCKTYPE_PCLK1 | RCC_CLOCKTYPE_PCLK2 | RCC_CLOCKTYPE_PCLK3;
