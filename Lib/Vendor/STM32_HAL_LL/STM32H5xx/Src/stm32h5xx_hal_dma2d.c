...
#define DMA2D_CL_LDM_MODIFY_REG(HANDLE, REG, MASK, VALUE)                                                                                                                                              \
	((HANDLE)->LDM_Reg_values[(REG)] = ((HANDLE)->LDM_Reg_values[(REG)] & ~(MASK)) | ((VALUE) & (MASK)), (HANDLE)->LDM_Instruction |= LDM_Decoder[(REG)])
#define DMA2D_CL_LDM_WRITE_REG(HANDLE, REG, VALUE) ((HANDLE)->LDM_Reg_values[(REG)] = (VALUE), (HANDLE)->LDM_Instruction |= LDM_Decoder[(REG)])
 ...
 */
#if (USE_DMA2D_COMMAND_LIST_MODE == 1)
static void DMA2D_CL_SetConfig(DMA2D_CL_HandleTypeDef *hdma2d, uint32_t pdata, uint32_t DstAddress, uint32_t Width, uint32_t Height);
#endif /* USE_DMA2D_COMMAND_LIST_MODE == 1 */
...
HAL_StatusTypeDef HAL_DMA2D_Init(DMA2D_HandleTypeDef *hdma2d)
{
	...
	MODIFY_REG(hdma2d->Instance->OPFCCR, (DMA2D_OPFCCR_AI | DMA2D_OPFCCR_RBS | DMA2D_OPFCCR_APOS),
		   ((hdma2d->Init.AlphaInverted << DMA2D_OPFCCR_AI_Pos) | (hdma2d->Init.RedBlueSwap << DMA2D_OPFCCR_RBS_Pos) | (hdma2d->Init.AlphaPosition << DMA2D_OPFCCR_APOS_Pos)));
...
HAL_StatusTypeDef HAL_DMA2D_CLUTStartLoad(DMA2D_HandleTypeDef *hdma2d, const DMA2D_CLUTCfgTypeDef *CLUTCfg, uint32_t LayerIdx)
{
	...
	if (LayerIdx == DMA2D_BACKGROUND_LAYER) {
		...
		MODIFY_REG(hdma2d->Instance->BGPFCCR, (DMA2D_BGPFCCR_CS | DMA2D_BGPFCCR_CM), ((CLUTCfg->Size << DMA2D_BGPFCCR_CS_Pos) | (CLUTCfg->CLUTColorMode << DMA2D_BGPFCCR_CM_Pos)));
	...
	/* Configure the CLUT of the foreground DMA2D layer */
		...
		MODIFY_REG(hdma2d->Instance->FGPFCCR, (DMA2D_FGPFCCR_CS | DMA2D_FGPFCCR_CM), ((CLUTCfg->Size << DMA2D_FGPFCCR_CS_Pos) | (CLUTCfg->CLUTColorMode << DMA2D_FGPFCCR_CM_Pos)));
...
HAL_StatusTypeDef HAL_DMA2D_ConfigLayer(DMA2D_HandleTypeDef *hdma2d, uint32_t LayerIdx)
{
	...
	regValue = pLayerCfg->InputColorMode | (pLayerCfg->AlphaMode << DMA2D_BGPFCCR_AM_Pos) | (pLayerCfg->AlphaInverted << DMA2D_BGPFCCR_AI_Pos) | (pLayerCfg->RedBlueSwap << DMA2D_BGPFCCR_RBS_Pos) |
		   (pLayerCfg->AlphaPosition << DMA2D_BGPFCCR_APOS_Pos);
	regMask = (DMA2D_BGPFCCR_CM | DMA2D_BGPFCCR_AM | DMA2D_BGPFCCR_ALPHA | DMA2D_BGPFCCR_AI | DMA2D_BGPFCCR_RBS | DMA2D_BGPFCCR_APOS);
...
HAL_StatusTypeDef HAL_DMA2D_ConfigDownscaling(DMA2D_HandleTypeDef *hdma2d, uint32_t Source, DMA2D_DownscalingCfgTypeDef *pDownscalingCfg)
{
	...
	MODIFY_REG(hdma2d->Instance->SNLR, (DMA2D_SNLR_NL | DMA2D_SNLR_PL), (pDownscalingCfg->NumberOfLines | (pDownscalingCfg->PixelPerLines << DMA2D_SNLR_PL_Pos)));

	/* Config scaling HSEP and VSTEP */
	MODIFY_REG(hdma2d->Instance->SSR, (DMA2D_SSR_HSTEP | DMA2D_SSR_VSTEP), (hstep | (((uint32_t)vstep) << DMA2D_SSR_VSTEP_Pos)));

	/* Config scaling HPHASE and VPHASE */
	MODIFY_REG(hdma2d->Instance->SPR, (DMA2D_SPR_HPHASE | DMA2D_SPR_VPHASE), (pDownscalingCfg->HPhase | (((uint32_t)pDownscalingCfg->VPhase) << DMA2D_SPR_VPHASE_Pos)));
...
HAL_StatusTypeDef HAL_DMA2D_ConfigStencil(DMA2D_HandleTypeDef *hdma2d, uint32_t Source, DMA2D_StencilCfgTypeDef *pStencilCfg)
{
	...
	MODIFY_REG(hdma2d->Instance->SBCR, DMA2D_SBCR_SRC | DMA2D_SBCR_MODE | DMA2D_SBCR_AI, Source | (pStencilCfg->Mode << DMA2D_SBCR_MODE_Pos) | (pStencilCfg->AlphaInverted << DMA2D_SBCR_AI_Pos));
	...
	MODIFY_REG(hdma2d->Instance->SBMSR, (DMA2D_SBMSR_HPRE | DMA2D_SBMSR_HTRAIL), (pStencilCfg->HPre | (pStencilCfg->HTrail << DMA2D_SBMSR_HTRAIL_Pos)));
...
HAL_StatusTypeDef HAL_DMA2D_CL_Init(DMA2D_CL_HandleTypeDef *hdma2d)
{
	...
	/* DMA2D CR register configuration -------------------------------------------*/
	DMA2D_CL_LDM_MODIFY_REG(hdma2d, DMA2D_CL_CR_REG, DMA2D_CR_MODE | DMA2D_CR_LOM, hdma2d->Init.Mode | hdma2d->Init.LineOffsetMode);
	/* DMA2D OPFCCR register configuration ---------------------------------------*/
	DMA2D_CL_LDM_MODIFY_REG(hdma2d, DMA2D_CL_OPFCCR_REG, DMA2D_OPFCCR_CM | DMA2D_OPFCCR_SB, hdma2d->Init.ColorMode | hdma2d->Init.BytesSwap);
	/* DMA2D OOR register configuration ------------------------------------------*/
...
HAL_StatusTypeDef HAL_DMA2D_CL_AddConfigLayerCMD(DMA2D_CL_HandleTypeDef *hdma2d, uint32_t LayerIdx, DMA2D_LayerCfgTypeDef *pLayerCfg)
{
	...
	if ((pLayerCfg->InputColorMode == DMA2D_INPUT_A8) || (pLayerCfg->InputColorMode == DMA2D_INPUT_A2) || (pLayerCfg->InputColorMode == DMA2D_INPUT_A2) ||
	    (pLayerCfg->InputColorMode == DMA2D_INPUT_A1)) {
		regValue |= (pLayerCfg->InputAlpha & DMA2D_BGPFCCR_ALPHA);
	} else {
		regValue |= (pLayerCfg->InputAlpha << DMA2D_BGPFCCR_ALPHA_Pos);
	}
	...
	if (LayerIdx == DMA2D_BACKGROUND_LAYER) {
		...
		if ((pLayerCfg->InputColorMode == DMA2D_INPUT_A8) || (pLayerCfg->InputColorMode == DMA2D_INPUT_A2) || (pLayerCfg->InputColorMode == DMA2D_INPUT_A2) ||
		    (pLayerCfg->InputColorMode == DMA2D_INPUT_A1)) {
			DMA2D_CL_LDM_WRITE_REG(hdma2d, DMA2D_CL_BGCOLR_REG, pLayerCfg->InputAlpha & (DMA2D_BGCOLR_BLUE | DMA2D_BGCOLR_GREEN | DMA2D_BGCOLR_RED));
		}
	...
	/* Configure the foreground DMA2D layer */
		...
		if ((pLayerCfg->InputColorMode == DMA2D_INPUT_A8) || (pLayerCfg->InputColorMode == DMA2D_INPUT_A2) || (pLayerCfg->InputColorMode == DMA2D_INPUT_A2) ||
		    (pLayerCfg->InputColorMode == DMA2D_INPUT_A1)) {
			DMA2D_CL_LDM_WRITE_REG(hdma2d, DMA2D_CL_FGCOLR_REG, pLayerCfg->InputAlpha & (DMA2D_FGCOLR_BLUE | DMA2D_FGCOLR_GREEN | DMA2D_FGCOLR_RED));
		}
	}
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_DMA2D_CL_AddConfigDownscalingCMD(DMA2D_CL_HandleTypeDef *const hdma2d, uint32_t Source, DMA2D_DownscalingCfgTypeDef *pDownscalingCfg)
{
	...
	/* Config scaling Height and Width */
	DMA2D_CL_LDM_WRITE_REG(hdma2d, DMA2D_CL_SNLR_REG, (pDownscalingCfg->NumberOfLines | (pDownscalingCfg->PixelPerLines << DMA2D_SNLR_PL_Pos)));

	/* Config scaling HSEP and VSTEP */
	DMA2D_CL_LDM_WRITE_REG(hdma2d, DMA2D_CL_SSR_REG, (hstep | (vstep << DMA2D_SSR_VSTEP_Pos)));

	/* Config scaling HPHASE and VPHASE */
	DMA2D_CL_LDM_WRITE_REG(hdma2d, DMA2D_CL_SPR_REG, (pDownscalingCfg->HPhase | (pDownscalingCfg->VPhase << DMA2D_SPR_VPHASE_Pos)));
...
HAL_StatusTypeDef HAL_DMA2D_CL_AddConfigStencilCMD(DMA2D_CL_HandleTypeDef *hdma2d, uint32_t Source, DMA2D_StencilCfgTypeDef *pStencilCfg)
{
	...
	/* Set stencil buffer memory skip: Horizontal pre/trail pixels */
	DMA2D_CL_LDM_MODIFY_REG(hdma2d, DMA2D_CL_SBMSR_REG, (DMA2D_SBMSR_HPRE | DMA2D_SBMSR_HTRAIL), (pStencilCfg->HPre | (pStencilCfg->HTrail << DMA2D_SBMSR_HTRAIL_Pos)));
...
HAL_StatusTypeDef HAL_DMA2D_CL_InsertCommandList(DMA2D_CL_HandleTypeDef *hdma2d, DMA2D_CL_CommandListTypeDef *pCommandList, uint32_t gpflag, uint32_t post_flag_config, uint32_t pre_flag_config)
{
	...
	uint32_t ring_buffer_size = RBS_TO_RING_BUFFER_SIZE(((hdma2d->Instance->CLCR & DMA2D_CLCR_RBS_Msk) >> DMA2D_CLCR_RBS_Pos));
	...
	/* fill the descriptor */
	*descriptor = ((pCommandList->Index - 1) | (post_flag_config << (24 + 2 * gpflag)) | (pre_flag_config << (16 + 2 * gpflag)));
	*descriptor <<= 32;
...
HAL_StatusTypeDef HAL_DMA2D_CL_Start(DMA2D_CL_HandleTypeDef *hdma2d)
{
	...
	__HAL_DMA2D_ENABLE_IT(hdma2d, DMA2D_IT_CE | DMA2D_IT_TW | DMA2D_IT_TC | DMA2D_IT_TE | DMA2D_IT_CLE | DMA2D_IT_CLS | DMA2D_IT_RBE | DMA2D_IT_RBC);
...
static void DMA2D_CL_SetConfig(DMA2D_CL_HandleTypeDef *hdma2d, uint32_t pdata, uint32_t DstAddress, uint32_t Width, uint32_t Height)
{
	...
	/* Swap Width and Height when X/Y Swap is enabled */
	if ((hdma2d->Instance->TBCR & DMA2D_TBCR_XYSEN) == DMA2D_TBCR_XYSEN) {
		DMA2D_CL_LDM_MODIFY_REG(hdma2d, DMA2D_CL_NLR_REG, (DMA2D_NLR_NL | DMA2D_NLR_PL), (Width | (Height << DMA2D_NLR_PL_Pos)));
	} else {
		DMA2D_CL_LDM_MODIFY_REG(hdma2d, DMA2D_CL_NLR_REG, (DMA2D_NLR_NL | DMA2D_NLR_PL), (Height | (Width << DMA2D_NLR_PL_Pos)));
	}
