...
#define LL_OPAMP_MODE_PGA_IO0_IO1_BIAS                                                                                                                                                                 \
	(OPAMP_CSR_PGGAIN_3 | OPAMP_CSR_PGGAIN_2 | OPAMP_CSR_VMSEL_1) /*!< In PGA mode, the inverting input is                                                                                         \
									   connected to VINM0.                                                                                                         \
									   - Input signal on VINM0, bias on VINPx: negative gain                                                                       \
...
#define LL_OPAMP_TRIMMING_NMOS_VREF_90PC_VDDA                                                                                                                                                          \
	(OPAMP_OTR_TRIMOFFSETN | ((OPAMP_CSR_CALSEL_1 | OPAMP_CSR_CALSEL_0) << 4)) /*!< OPAMP trimming of transistors                                                                                  \
...
__STATIC_INLINE uint32_t LL_OPAMP_GetCalibrationSelection(const OPAMP_TypeDef *OPAMPx)
{
	...
	return (uint32_t)((CalibrationSelection << 4) | (((CalibrationSelection & OPAMP_CSR_CALSEL_1) == 0UL) ? OPAMP_OTR_TRIMOFFSETP : OPAMP_OTR_TRIMOFFSETN));
}
