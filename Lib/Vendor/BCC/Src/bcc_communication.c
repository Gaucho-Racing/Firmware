...
#define BCC_GET_MSG_DATA(msg)                                                                                          \
	(((uint16_t) * ((msg) + BCC_MSG_IDX_DATA_H) << 8U) | (uint16_t) * ((msg) + BCC_MSG_IDX_DATA_L))
 ...
 */
static void BCC_PackFrame(const uint16_t data, const uint8_t addr, const bcc_cid_t cid, const uint8_t cmdCnt,
			  uint8_t *const frame);
...
static bcc_status_t BCC_CheckEchoFrame(const uint8_t *const txBuf, const uint8_t *const resp)
{
	...
	if ((txBuf[BCC_MSG_IDX_DATA_H] == resp[BCC_MSG_IDX_DATA_H]) &&
	    ...
	    (txBuf[BCC_MSG_IDX_ADDR] == resp[BCC_MSG_IDX_ADDR]) && (txBuf[BCC_MSG_IDX_CID] == resp[BCC_MSG_IDX_CID]) &&
...
bcc_status_t BCC_Reg_ReadTpl(bcc_drv_config_t *const drvConfig, const bcc_cid_t cid, const uint8_t regAddr,
			     const uint8_t regCnt, uint16_t *regVal)
{
	...
	if (((uint8_t)cid > drvConfig->devicesCnt) || (regAddr > BCC_MAX_REG_ADDR) || (regCnt == 0U) ||
	    ((regAddr + regCnt - 1U) > BCC_MAX_REG_ADDR)) {
 ...
 *
 *END**************************************************************************/
bcc_status_t BCC_Reg_ReadSpi(bcc_drv_config_t *const drvConfig, const bcc_cid_t cid, uint8_t regAddr,
			     const uint8_t regCnt, uint16_t *regVal)
{
	uint8_t txBuf[BCC_MSG_SIZE]; /* Transmission buffer. */
 ...
 *
 *END**************************************************************************/
bcc_status_t BCC_Reg_WriteTpl(bcc_drv_config_t *const drvConfig, const bcc_cid_t cid, const uint8_t regAddr,
 ...
 *
 *END**************************************************************************/
bcc_status_t BCC_Reg_WriteSpi(bcc_drv_config_t *const drvConfig, const bcc_cid_t cid, const uint8_t regAddr,
			      const uint16_t regVal)
{
	uint8_t txBuf[BCC_MSG_SIZE]; /* Transmission buffer. */
	uint8_t rxBuf[BCC_MSG_SIZE]; /* Buffer for receiving. */
