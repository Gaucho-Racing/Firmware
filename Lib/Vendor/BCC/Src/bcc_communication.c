...
static void BCC_PackFrame(const uint16_t data, const uint8_t addr,
			  const bcc_cid_t cid, const uint8_t cmdCnt,
			  uint8_t *const frame);
				     ...
				     const uint8_t *const resp)
{
	...
	msgCntRcv = (resp[BCC_MSG_IDX_CNT_CMD] & BCC_MSG_MSG_CNT_MASK) >>
		    BCC_MSG_MSG_CNT_SHIFT;
	...
	if ((cid != BCC_CID_UNASSIG) &&
	    (msgCntRcv != BCC_INC_MSG_CNTR(msgCntPrev))) {
			     ...
			     const uint8_t regCnt, uint16_t *regVal)
{
	...
	if (((uint8_t)cid > drvConfig->devicesCnt) ||
	    (regAddr > BCC_MAX_REG_ADDR) || (regCnt == 0U) ||
	...
	status = BCC_MCU_TransferTpl(drvConfig->drvInstance, txBuf, rxBuf,
				     regCnt + 1);
	...
	for (regIdx = 0U; regIdx < regCnt; regIdx++) {
		...
		/* Check the Message counter value. */
		if ((status = BCC_CheckMsgCntr(drvConfig, cid, rxBuf)) !=
		    BCC_STATUS_SUCCESS) {
			return status;
		}
			     ...
			     const uint8_t regCnt, uint16_t *regVal)
{
	...
	for (regIdx = 0U; regIdx < regCnt; regIdx++) {
		 ...
		 * following transfer. */
		status =
		    BCC_MCU_TransferSpi(drvConfig->drvInstance, txBuf, rxBuf);
		if (status != BCC_STATUS_SUCCESS) {
			return status;
		}
			      ...
			      const uint16_t regVal)
{
	...
	status = BCC_MCU_TransferTpl(drvConfig->drvInstance, txBuf,
				     drvConfig->drvData.rxBuf, 1);
	if (status != BCC_STATUS_SUCCESS) {
		return status;
	}
				    ...
				    const uint16_t regVal)
{
	...
	/* Create frame for writing. */
	BCC_PackFrame(regVal, regAddr, BCC_CID_UNASSIG, BCC_CMD_GLOB_WRITE,
		      txBuf);
