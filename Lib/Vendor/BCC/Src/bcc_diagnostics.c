 ...
 */
static bcc_status_t BCC_SetNumbOdd(bcc_drv_config_t *const drvConfig, const bcc_cid_t cid);
 ...
 * @return bcc_status_t Error code.
 */
static bcc_status_t BCC_EnterDiagnostics(bcc_drv_config_t *const drvConfig, const bcc_cid_t cid);
 ...
 */
static bcc_status_t
BCC_CommandSwitches(bcc_drv_config_t *const drvConfig, const bcc_cid_t cid, const bcc_diag_switch_sel_t switches, const bcc_diag_switch_pos_t odd, const bcc_diag_switch_pos_t even);
...
static bcc_status_t BCC_DiagOvuvPart(bcc_drv_config_t *const drvConfig,
				     ...
				     const uint32_t sm01twait,
				     uint16_t *const fltOvrv,
				     uint16_t *const fltUndv);
...
static bcc_status_t BCC_DiagCtxopenPart(
    bcc_drv_config_t *const drvConfig, const bcc_cid_t cid, const bcc_diag_switch_pos_t odd, const bcc_diag_switch_pos_t even, const bcc_diag_const_t *const diagTimeConst, uint32_t *const measVal);
    ...
    bcc_drv_config_t *const drvConfig, const bcc_cid_t cid, const bcc_diag_switch_pos_t odd, const bcc_diag_switch_pos_t even, const bcc_diag_const_t *const diagTimeConst, uint32_t *const measVal)
{
	...
	/* 8. Open all switches. */
	status = BCC_CommandSwitches(drvConfig, cid, BCC_SWITCH_SEL_CT, BCC_SWITCH_POS_OPEN, BCC_SWITCH_POS_OPEN);
	if (status != BCC_STATUS_SUCCESS) {
		return status;
	}
...
bcc_status_t BCC_Diag_OvUvVer(bcc_drv_config_t *const drvConfig, const bcc_cid_t cid, const bcc_diag_const_t *const diagConst, bcc_diag_ov_uv_ver_res_t *const results)
{
	...
	status = BCC_Reg_Update(drvConfig,
				cid,
...
bcc_status_t BCC_Diag_OvUvDet(bcc_drv_config_t *const drvConfig, const bcc_cid_t cid, const bcc_diag_const_t *const diagConst, bcc_diag_ov_uv_det_res_t *const results)
{
	...
	status = BCC_Reg_Read(drvConfig, cid, MC33771C_CB1_CFG_OFFSET, BCC_MAX_CELLS_DEV(deviceType), cbxCfgRegs);
	if (status != BCC_STATUS_SUCCESS) {
		return status;
	}
...
bcc_status_t BCC_Diag_CellVolt(bcc_drv_config_t *const drvConfig, const bcc_cid_t cid, bcc_diag_cell_volt_res_t *const results)
{
	...
	if (deviceType == BCC_DEVICE_MC33771C) {
		...
		if (drvConfig->cellCnt[(uint8_t)cid - 1] == 3U) {
			 ...
			 */
			results->vErrX[1] = ((int32_t)(measVoltUv[5])) - ((int32_t)(measVoltUv[0] + measVoltUv[1])) / 2;
		} else {
...
bcc_status_t BCC_Diag_ConnResistance(bcc_drv_config_t *const drvConfig, const bcc_cid_t cid, bcc_diag_conn_res_res_t *const result)
{
	...
	status = BCC_Reg_Read(drvConfig, cid, firstMeasRegAddr, deviceCellsMax, &(measRaw[0][0]));
	if (status != BCC_STATUS_SUCCESS) {
		return status;
	}
	...
	status = BCC_Reg_Read(drvConfig, cid, firstMeasRegAddr + 1, deviceCellsMax - 1, &(measRaw[1][1]));
	if (status != BCC_STATUS_SUCCESS) {
		return status;
	}
	...
	status = BCC_Reg_Read(drvConfig, cid, firstMeasRegAddr, deviceCellsMax - 1, &(measRaw[2][0]));
	if (status != BCC_STATUS_SUCCESS) {
		return status;
	}
...
bcc_status_t BCC_Diag_CTxLeak(bcc_drv_config_t *const drvConfig, const bcc_cid_t cid, bcc_diag_ctx_leak_res_t *const result)
{
	...
	for (i = 0U; i <= deviceCellsMax; i++) {
		 ...
		 */
		result->vLeakX[i] = (measUv[0][i] > measUv[1][i]) ? measUv[0][i] : measUv[1][i];
...
bcc_status_t BCC_Diag_CBxOpen(bcc_drv_config_t *const drvConfig, const bcc_cid_t cid, bcc_diag_cbx_open_res_t *const results)
{
	 ...
	 * configuration. */
	status = BCC_CommandSwitches(drvConfig, cid, BCC_SWITCH_SEL_CB, BCC_SWITCH_POS_CLOSED, BCC_SWITCH_POS_OPEN);
	if (status != BCC_STATUS_SUCCESS) {
		return status;
	}
	 ...
	 * configuration. */
	status = BCC_CommandSwitches(drvConfig, cid, BCC_SWITCH_SEL_CB, BCC_SWITCH_POS_OPEN, BCC_SWITCH_POS_CLOSED);
	if (status != BCC_STATUS_SUCCESS) {
		return status;
	}
	 ...
	 * configuration. */
	status = BCC_CommandSwitches(drvConfig, cid, BCC_SWITCH_SEL_CB, BCC_SWITCH_POS_OPEN, BCC_SWITCH_POS_OPEN);
	if (status != BCC_STATUS_SUCCESS) {
		return status;
	}
