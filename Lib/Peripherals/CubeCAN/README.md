...
Within `main` after `MX_FDCAN#_Init()` and Logomatic are setup (see [Logomatic](/Lib/Utils/Logomatic/README.md)), call `CubeCAN_Entrance()` saving the handle for global use later (one entrance and handle per peripheral).
