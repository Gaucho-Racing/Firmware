...
int can_internal_test(void)
{
	...
	if (get_cfg(FDCAN1, can_test_rx_callback1, &cfg1, FDCAN_MODE_INTERNAL_LOOPBACK,0,0)) {
		LOGOMATIC("Could not get config for FDCAN1\n");
		return ERROR;
	}
	if (get_cfg(FDCAN2, can_test_rx_callback2, &cfg2, FDCAN_MODE_INTERNAL_LOOPBACK,0,0)) {
	...
	if ((rc = can_release(primary_can))) {
		LOGOMATIC("FAIL: can_internal_test; could not release primary_can\n");
	}
	error |= rc;
	if ((rc = can_release(data_can))) {
		LOGOMATIC("FAIL: can_internal_test; could not release data_can\n");
	}
	error |= rc;
	...
	LOGOMATIC("can_internal_test: SUCCESS\n");
