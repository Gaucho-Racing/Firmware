...
function regenerateStaticCircles() {
    const w = Math.max(window.innerWidth, document.documentElement.scrollWidth, document.body.scrollWidth);
...
function drawStaticSquaresBg() {
    ...
    const pageW = Math.max(window.innerWidth, document.documentElement.scrollWidth, document.body.scrollWidth);
    const pageH = Math.max(window.innerHeight, document.documentElement.scrollHeight, document.body.scrollHeight);
    if (!staticCircles || staticCircles.w !== pageW || staticCircles.h !== pageH) {
    ...
    for (let i = 0; i < circles.length; ++i) {
        ...
        for (let j = 0; j < circ.squares.length; ++j) {
            ...
            if (sx + SQUARE_SIZE < 0 || sx > viewW || sy + SQUARE_SIZE < 0 || sy > viewH) continue;
