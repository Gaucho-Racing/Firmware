...
async function fetchCando(ref) {
	try {
		const res = await fetch(`${GITHUB_API}/contents/${CANDO_PATH}?ref=${encodeURIComponent(ref)}`);
		if (res.status === 403) {
			return { content: '[GitHub API rate limited]', notFound: false };
		}
		if (res.status === 404) {
			return { content: '[Unable to load GRCAN.CANdo for this reference]', notFound: true };
		}
...
async function onRefInputChange() {
	...
	if (!ref) {
		candoBox.innerHTML = '<span class="placeholder">Please enter a branch, tag, or commit SHA</span>';
		return;
	}
...
async function init() {
	...
	if (branches === 'RATE_LIMITED' || tags === 'RATE_LIMITED') {
		...
		candoBox.innerHTML = '<span class="placeholder">GitHub API unavailable or rate limited. Please try again in a few minutes.</span>';
		return;
	}
	refList.innerHTML = '';
	let mainFound = false;
