...
void setSoftwareLatch(bool state, CCU_StateData *state_data)
{
	...
	if (state && !LL_GPIO_IsInputPinSet(SOFTWARE_OK_CONTROL_GPIO_Port, SOFTWARE_OK_CONTROL_Pin)) {
		...
		state_data->ACU_S2_SOFTWARE_LATCH = true;
		...
		state_data->ACU_S2_SOFTWARE_LATCH = false;
...
void CheckDebuggerPrint(const CCU_StateData *state_data)
{
	...
	LOGOMATIC("\n--- ACU_STATUS_2 ---\n");
	LOGOMATIC("20V: %u\n", state_data->ACU_S2_20Volt);
	LOGOMATIC("12V: %u\n", state_data->ACU_S2_12Volt);
	LOGOMATIC("SDC Volt: %u\n", state_data->ACU_S2_SDC_Volt);
	LOGOMATIC("Min Cell Volt: %u\n", state_data->ACU_S2_MIN_CELL_Volt);
	LOGOMATIC("Max Cell Temp: %u\n", state_data->ACU_S2_MAX_CELL_TEMP);

	LOGOMATIC("\n--- Errors ---\n");
	LOGOMATIC("OVERTEMP: %d\n", state_data->ACU_S2_OVERTEMP_ERROR);
	LOGOMATIC("OVERVOLT: %d\n", state_data->ACU_S2_OVERVOLT_ERROR);
	LOGOMATIC("UNDERVOLT: %d\n", state_data->ACU_S2_UNDERVOLT_ERROR);
	LOGOMATIC("OVERCURR: %d\n", state_data->ACU_S2_OVERCURR_ERROR);
	LOGOMATIC("UNDERCURR: %d\n", state_data->ACU_S2_UNDERCURR_ERROR);

	LOGOMATIC("\n--- Warnings ---\n");
	LOGOMATIC("UNDER20V: %d\n", state_data->ACU_S2_UNDER20v_WARNING);
	LOGOMATIC("UNDER12V: %d\n", state_data->ACU_S2_UNDER12v_WARNING);
	LOGOMATIC("UNDERVOLT SDC: %d\n", state_data->ACU_S2_UNDERVOLTSDC_WARNING);

	LOGOMATIC("\n--- State Bits ---\n");
	LOGOMATIC("SOFTWARE LATCH: %d\n", state_data->ACU_S2_SOFTWARE_LATCH);
