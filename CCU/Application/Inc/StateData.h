...
//IDK if this is any good, if so, prolly move to move, just wanted to get something down
...
typedef struct{ //Stores Charger DATA CAN (TX, output)
    ...
    ccu_fault_t fault; //probably need a better representation of this since it needs  0000, 0101, etc.
...
typedef struct{
    ...
    // uint32_t last_bcu_message; (original intent unneeded but heard something similar might be implemented, some ping thing)
...
void ccu_tick(ccu_t *ccu, uint32_t now_ms, uint32_t delta_ms); // now_ms = curr time, delta_ms = time since last