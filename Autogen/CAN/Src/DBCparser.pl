...
use strict;
use warnings;
use autodie qw(open close print); # Fixes: Unchecked return values (Errors #1 & #5)

# Bit size mapping (Constant - Unchanged)
my %TYPE_BITS = (
    'b' => 1, 'u4' => 4, 's4' => 4, 'u8' => 8, 's8' => 8, 'i8' => 8,
    'u16' => 16, 's16' => 16, 'i16' => 16, 'u32' => 32, 's32' => 32,
    'i32' => 32, 'u64' => 64, 's64' => 64, 'i64' => 64, 'double' => 64, 's' => 64
);
...
sub main {
    ...
    my $data_ref = parse_input($input_file);

    open( my $out, '>', $output_file );

    # Header Info
    print $out "VERSION \"\"\n\nNS_ :\n\nBS_:\n\nBU_: ";
    print $out join( ' ', sort grep { $_ ne 'ALL' } map { normalize($_) } keys %{ $data_ref->{grid} } ) . " ALL\n\n";

    foreach my $route ( @{ $data_ref->{routing} } ) {
        write_dbc_message($out, $route, $data_ref);
    }

    close($out);
    print "Successfully generated $output_file\n";
    return 0; # Fixes: Explicit return (Error #3)
}

# New Subroutine to reduce Complexity (Fixes Error #2)
sub write_dbc_message {
    my ($out, $r, $d) = @_;
    my $m_name = $r->{msg};
    ...
    # ID Calculation Logic (Identical to original)
    my $can_id;
    if ( $r->{override} ) {
    ...
    foreach my $s ( sort { ( $a->{start} // 0 ) <=> ( $b->{start} // 0 ) } @sigs ) {
        ...
        my ( $factor, $offset ) = parse_eq( $s->{'map equation'} );
        my $raw_type = $s->{'data type'} // 'u8';
        my $bits = ($raw_type =~ /^(\w+)\[(\d+)\]$/) ? ($TYPE_BITS{$1} // 8) * $2 : ($TYPE_BITS{$raw_type} // 8);

        my $unit = ( $s->{units} && $s->{units} ne '-' && $s->{units} ne 'Enum' ) ? $s->{units} : "";
        $unit =~ s/'//g;

        printf $out " SG_ %s : %d|%d\@1%s (%g,%g) [%s|%s] \"%s\" %s\n",
          normalize( $s->{name} ), $s->{start}, $bits,
          ( $raw_type =~ /^[is]/ ? '-' : '+' ), $factor, $offset,
          ( $s->{'scaled min'} // 0 ), ( $s->{'scaled max'} // 0 ),
          $unit, normalize( $r->{target} );
    }
    print $out "\n";
    return; # Fixes: Explicit return (Error #3)
}
...
sub parse_input {
    ...
    open my $fh, '<', $file; # autodie handles failure
