...
// ADC Data
uint16_t adc_data[ADC_SIZE]; // 0: ts_current, 1: ts_voltage, 2: sdc_volt_w, 3:
			     // sdc_volt_v, 4:volt_12v, 5: water_sense
...
// SHARED BUFFER: ACCESSIBLE BY INTERRUPT HANDLER AND DEV (ME!) (ensure mods are
// atomic)
...
int main(void)
{
	...
	HAL_FDCAN_ActivateNotification(&hfdcan2, FDCAN_IT_RX_FIFO0_NEW_MESSAGE,
				       0);
	HAL_FDCAN_ActivateNotification(&hfdcan3, FDCAN_IT_RX_FIFO0_NEW_MESSAGE,
				       0);
	...
	HAL_FDCAN_ActivateNotification(&hfdcan1, FDCAN_IT_RX_FIFO0_MESSAGE_LOST,
				       0);
	HAL_FDCAN_ActivateNotification(&hfdcan2, FDCAN_IT_RX_FIFO0_MESSAGE_LOST,
				       0);
	HAL_FDCAN_ActivateNotification(&hfdcan3, FDCAN_IT_RX_FIFO0_MESSAGE_LOST,
				       0);
	...
	RxHeader.RxTimestamp =
	    0; /* Specifies the timestamp counter value captured on start of
		  frame reception. Between 0 and 0xFFFF  */
...
void SystemClock_Config(void)
{
	...
	LL_RCC_PLL_ConfigDomain_SYS(LL_RCC_PLLSOURCE_HSI, LL_RCC_PLLM_DIV_1, 16,
				    LL_RCC_PLLR_DIV_2);
...
{
	// do we need to check (!CoreDebug->DEMCR |=
	// CoreDebug_DEMCR_TRCENA_Msk)? don't know but it works so I'm not
	// touching this
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
	...
	if (RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) {
		...
		if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &RxHeader,
					   CAN_RxData) == HAL_OK) {
			CAN_RxBuffer[CAN_RxBufferTop].identifier =
			    RxHeader.Identifier;
			CAN_RxBuffer[CAN_RxBufferTop].length =
			    RxHeader.DataLength;
			CAN_RxBuffer[CAN_RxBufferTop].instance =
			    hfdcan->Instance;

#if SPAMPRINT == 0
			if (CAN_RxBuffer[CAN_RxBufferTop].identifier ==
			    Precharge_ACU) {
				uint8_t headerBuff[64], dataBuff[8];
				sprintf(headerBuff,
					"Received CAN message: ID=0x%lX, "
					"DLC=%ld, Data=\n",
					RxHeader.Identifier,
					RxHeader.DataLength);
				print_lpuart(headerBuff);
				for (uint32_t i = 0; i < RxHeader.DataLength;
				     i++) {
					sprintf(dataBuff, " 0x%02X, ",
						CAN_RxData[i]);
			...
			memcpy(
			    (void *restrict)CAN_RxBuffer[CAN_RxBufferTop].data,
			    CAN_RxData, RxHeader.DataLength);
			CAN_RxBufferTop++; // increment and mod the pointers in
					   // the buffer
			CAN_RxBufferLevel++;
		}
	}
