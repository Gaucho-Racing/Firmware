...
typedef struct {
	...
	uint8_t status_device_activity; // 0 - initialization, 1 - Normal
					// Operation, 2 - Self Test
...
typedef struct {
	...
	uint8_t
	    chgr_status; /* Map:    Bit 0: Hardware Failure: 0-Normal, 1-Error
				    Bit 1: OverTemp: 0-Normal, 1-Error
				    Bit 2: Input Voltage: 0-Normal, 1-Wrong
			    input voltage Bit 3: Starting State: 0-Correct,
			    1-Wrong polarity or NC Bit 4: Communication State:
			    0-Normal, 1-Timeout
			*/
...
typedef struct {
	...
	uint8_t chg_ctrl;  // 0 : Start Charging, 1: Stop Charging => to be sent
			   // from ACU to others via Tx
	float acu_current; // current output of ACU

	// ADC Stuff
	// 0:ts_current, 1:ts_voltage, 2:sdc_volt_w, 3:sdc_volt_v,
	// 4:voltage_12v,5:water_sense
	...
	uint16_t acu_err_warns; // [ 0:OT, OV, UV, OC, UC, UV_20v, UV_GLV,
				// 7:UV_SDC, 8:Precharge, 0, 0, 0, 0, 0, 0, 0]
