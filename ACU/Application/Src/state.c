...
uint8_t get_state() { return (uint8_t)(state); }

/// @brief sets the state in functions that can't access the state
void set_state(uint8_t value) { state = value; }
...
void shitdown()
{
	...
	if (acu.ts_voltage < SAFE_V_TO_TURN_OFF &&
	    pass) { // safe to turn off if TS voltage < 60V
...
/// @brief do nothing, in initial state wait for VDM to send start command,
/// maybe poll CAN
void standby()
{
	write_prechg(false);
...
void precharge()
{
	...
	if ((fabsf(acu.sdc_volt_w - acu.sdc_volt_v) < GLV_SDC_LOW) &&
	    acu.sdc_volt_w < SDC_HIGH) {
	...
	while (acu.ts_voltage < get_total_voltage(&acu) * PRECHARGE_THRESHOLD) {
		...
		// LL_mDelay(100);
		if (!state_system_check(true, false)) {
			print_lpuart("¯\\_(ツ)_/¯ PreCharge "
				     "state_system_check() => Shutdown\n");
#if DEBUGG == 0
			state = state == INIT ? INIT : SHITDOWN;
			return;
#endif
		...
		if (fabsf(acu.voltage_12v - acu.sdc_volt_w) > GLV_SDC_LOW) {
			print_lpuart(
			    "¯\\_(ツ)_/¯ PreCharge (250) SDC volt dropped\n");
			acu.acu_err_warns |= ACU_PRECHARGE;
...
// enqueue(ACU_Status_2,FDCAN1);
	...
	while (HAL_GetTick() - start_time < 2000) {
		...
		if (acu.ts_voltage <
		    get_total_voltage(&acu) * PRECHARGE_THRESHOLD) {
			print_lpuart(
			    "( ˶°ㅁ°) !! TS Voltage went down in Precharge\n");
			acu.acu_err_warns |= ACU_PRECHARGE;
#if DEBUGG == 0
			state = SHITDOWN;
			return;
...
void charge()
{
	...
	if (!acu_check(&acu, false)) {
		...
		// voltage in tractive system must be low voltage in 5 seconds
		// or less turn off charger, disabled until manually reset
	...
	if (HAL_GetTick() - last_charge_time >= 2000) {
		...
		reset_discharge(&battery, false);
		if (!state_system_check(true, false)) {
			print_lpuart("( ˶°ㅁ°) !! Failed system check inside "
				     "of charge\n");
#if DEBUGG == 0
			state = state == INIT ? INIT : SHITDOWN;
			return;
	...
	if (HAL_GetTick() - last_send_time > 990) {
		...
		if (battery.max_cell_volt > battery.max_volt_thresh) {
			battery.max_chg_current = map(
			    battery.max_cell_volt, CELL_MAX_VOLT - 0.06,
			    CELL_MAX_VOLT - 0.01, acu.target_chg_current, 0);
			battery.max_chg_current =
			    constrain(battery.max_chg_current, 0.0f,
	...
	if (HAL_GetTick() - acu.lastChrgRecieveTime > 5000U) {
		print_lpuart(
		    "( ˶°ㅁ°) !! CHARGE: Charger CAN timeout, shutting down\n");
#if DEBUGG == 0
		state = SHITDOWN;
		return;
#endif
...
void normal()
{
	...
	if (!state_system_check(false, false)) {
		print_lpuart(
		    "( ˶°ㅁ°) !! SystemCheck failed in NORMAL state\n");
#if DEBUGG == 0
		state = state == INIT ? INIT : SHITDOWN;
		return;
#endif
