...
extern volatile uint8_t
    TPL_RxBuffer[256]; // Array to store received SPI data => Replace with
		       // struct holding CAN RxBuffer
...
const uint8_t thermistor_map[28] = {0,	1,  2,	3,  8,	9,  10, 11, 12, 13,
...
{
	// bcc_error = BCC_Fault_GetStatus(&(bty->drvConfig), cid,
	// fault_status);
}
...
void clear_faults(bcc_drv_config_t *drvConfig)
{
	for (uint8_t ic = 1; ic <= NUM_CELL_IC; ic++) {
		...
		bcc_error =
		    BCC_Fault_ClearStatus(drvConfig, cid, BCC_FS_CELL_OV);
		bcc_error =
		    BCC_Fault_ClearStatus(drvConfig, cid, BCC_FS_CELL_UV);
		bcc_error =
		    BCC_Fault_ClearStatus(drvConfig, cid, BCC_FS_CB_OPEN);
		bcc_error =
		    BCC_Fault_ClearStatus(drvConfig, cid, BCC_FS_CB_SHORT);
		bcc_error =
		    BCC_Fault_ClearStatus(drvConfig, cid, BCC_FS_GPIO_STATUS);
		bcc_error =
		    BCC_Fault_ClearStatus(drvConfig, cid, BCC_FS_AN_OT_UT);
		bcc_error =
		    BCC_Fault_ClearStatus(drvConfig, cid, BCC_FS_GPIO_SHORT);
		bcc_error = BCC_Fault_ClearStatus(drvConfig, cid, BCC_FS_COMM);
		bcc_error =
		    BCC_Fault_ClearStatus(drvConfig, cid, BCC_FS_FAULT1);
		bcc_error =
...
void reset_discharge(Battery *bty, bool on)
{
	for (uint8_t i = 0; i < NUM_TOTAL_IC; i++) {
		for (uint8_t j = 0; j < NUM_CELL_IC; j++) {
			if ((bcc_error = set_cell_balance(bty, (bcc_cid_t)(i),
							  j, 0, on)) !=
			    BCC_STATUS_SUCCESS) { // resets after default = 30
						  // seconds
...
float max_volt = -1000.0f, max_temp = -1000.0f, min_volt = 1000.0f,
      min_temp = 1000.0f;
				      ...
				      uint8_t read_temp)
{
	...
	for (uint8_t i = 0; i < NUM_TOTAL_IC; i++) {
		bcc_error = BCC_CB_Pause(&(bty->drvConfig), (bcc_cid_t)(i + 1),
					 true); // pause b4 read
		if (bcc_error != BCC_STATUS_SUCCESS) {
			print_bcc_status(bcc_error);
			bcc_error = BCC_CB_Pause(&(bty->drvConfig),
						 (bcc_cid_t)(i + 1), true);
		}
		if (read_volt) { // CELL VOLTAGES
			bcc_error = BCC_Meas_StartAndWait(
			    &(bty->drvConfig), (bcc_cid_t)(i + 1), BCC_AVG_1);
			if (bcc_error != BCC_STATUS_SUCCESS) {
				print_bcc_status(bcc_error);
			}
			bcc_error = BCC_Meas_GetCellVoltages(&(bty->drvConfig),
							     (bcc_cid_t)(i + 1),
							     measurements);
			...
			for (uint8_t j = 0; j < NUM_CELL_IC; j++) {
				bty->cell_volt[GET_INDEX(i, j)] =
				    measurements[j] * 1e-6f;
				battery_total_volt_temp +=
				    (measurements[j] * 1e-6f);
				temp_stack_volt[i] += measurements[j] * 1e-6f;

				// measurements
				max_volt =
				    fmaxf(max_volt, measurements[j] * 1e-6f);
				min_volt =
				    fminf(min_volt, measurements[j] * 1e-6f);
			}

			bzero(measurements, sizeof(measurements));
			bcc_error = BCC_Meas_GetStackVoltage(&(bty->drvConfig),
							     (bcc_cid_t)(i + 1),
							     measurements);
			bty->stack_voltage[i] =
			    *measurements *
			    1e-6f; // theres ~ .3 difference from manual calcs
		...
		if (read_temp && cycle == i) {
			temp_measures = 0;
			bcc_error = BCC_Meas_GetIcTemperature(
			    &(bty->drvConfig), (bcc_cid_t)(i + 1),
			    BCC_TEMP_CELSIUS, &temp_measures);
			bty->icTemp[i] = (float)
			    temp_measures; // TRIAGE: check this in charging
		...
		if (read_temp && cycle == i) {
			for (uint8_t j = 0; j < 28; j++) {
				uint8_t readByte = 0;
				bcc_error = BCC_EEPROM_Read(
				    &(bty->drvConfig), (bcc_cid_t)(i + 1),
				    thermistor_map[j] + 1, &readByte);
				if (bcc_error == BCC_STATUS_SUCCESS) {
					bty->cell_temp[GET_INDEX(i * 2, j)] =
					    V2T(readByte * 0.01953125f,
						4250.0f);
					max_temp = fmaxf(
					    max_temp,
					    bty->cell_temp[GET_INDEX(i, j)]);
					min_temp = fminf(
					    min_temp,
					    bty->cell_temp[GET_INDEX(i, j)]);
				}
	...
	for (uint8_t i = 0; i < NUM_TOTAL_IC; i++) {
		...
		if (to_discharge == 1) {
			for (uint8_t j = 0; j < NUM_CELL_IC; j++) {
				...
				if ((bcc_error = set_cell_balance(
					 bty, (bcc_cid_t)(i), j, 0, 1)) !=
				    BCC_STATUS_SUCCESS) {
					print_bcc_status(bcc_error);
					return false;
				}
	...
	if (all) {
		for (uint8_t i = 0; i < NUM_TOTAL_IC; i++) {
			for (uint8_t j = 0; j < NUM_CELL_IC; j++) {
				...
				bcc_status_t errors;
				if ((errors = BCC_CB_SetIndividual(
					 &(bty->drvConfig), (bcc_cid_t)(i + 1),
					 j, true, 0)) != BCC_STATUS_SUCCESS) {
					bzero(print_buffer,
					      sizeof(print_buffer));
				...
				bty->cell_balancing[GET_INDEX(i, j)] =
				    enable == true ? 1 : 0;
			}
		}
		return bcc_error;
		...
		// set individuals
		if ((bcc_error = BCC_CB_SetIndividual(
			 &(bty->drvConfig), (bcc_cid_t)(cid + 1), cellIndex,
			 enable, 0)) != BCC_STATUS_SUCCESS) {
			bzero(print_buffer, sizeof(print_buffer));
			sprintf(print_buffer,
				"Error IC %d, Cell %d: ", (int)(cid + 1),
				(int)(cellIndex));
			print_lpuart(print_buffer);
			return bcc_error;
		}
		bty->cell_balancing[(uint8_t)cid * NUM_TOTAL_IC + cellIndex] =
		    enable == true ? 1 : 0;
	}
	return bcc_error;
}
...
bool init_cell_balancing(Battery *bty)
{
	for (uint8_t i = 0; i < NUM_TOTAL_IC; i++) {
		bcc_error =
		    BCC_CB_Enable(&(bty->drvConfig), (bcc_cid_t)(i + 1), true);
		if (bcc_error != BCC_STATUS_SUCCESS) {
			for (uint8_t j = 0; j < NUM_CELL_IC; j++) {
...
bool disable_cell_balancing(Battery *bty)
{
	for (uint8_t i = 0; i < NUM_TOTAL_IC; i++) {
		bcc_error =
		    BCC_CB_Enable(&(bty->drvConfig), (bcc_cid_t)(i + 1), false);
		if (bcc_error != BCC_STATUS_SUCCESS) {
			for (uint8_t j = 0; j < NUM_CELL_IC; j++) {
				bty->cell_balancing[GET_INDEX(i, j)] = 2;
			}
...
bool check_temp(Battery *bty)
{
	...
	for (uint8_t i = 0; i < NUM_TOTAL_IC; i++) {
		for (uint8_t j = 0; j < (NUM_CELL_IC); j++) {
			// overtemp check
			if (bty->cell_temp[i * NUM_CELL_IC + j] >
			    bty->max_temp_thresh) {
				bty->cell_temp_errors++;
				bty->cell_temp_err_cnt[i * NUM_CELL_IC + j]++;
				if (bty->cell_temp_err_cnt[i * NUM_CELL_IC +
							   j] > ERRMG_BMS_ERR) {
					bty->cell_temp_err_cnt[i * NUM_CELL_IC +
							       j] =
					    ERRMG_BMS_ERR;
					bty->battery_check_faults |=
					    BATTERY_FAULT_CELL_OT; // probably
								   // not the
								   // correct
								   // one
					success = 0;
...
bool check_volt(Battery *bty)
{
	...
	for (uint8_t i = 0; i < NUM_TOTAL_IC; i++) {
		for (uint8_t j = 0; j < NUM_CELL_IC; j++) {
			...
			// max_volt check
			if (bty->cell_volt[i * NUM_CELL_IC + j] >
			    bty->max_volt_thresh) {
				bty->cell_volt_errors++;
				bty->cell_volt_err_cnt[i * NUM_CELL_IC + j]++;
				if (bty->cell_volt_err_cnt[i * NUM_CELL_IC +
							   j] > ERRMG_BMS_ERR) {
					bty->cell_volt_err_cnt[i * NUM_CELL_IC +
							       j] =
					    ERRMG_BMS_ERR;
					bty->battery_check_faults |=
					    BATTERY_FAULT_CELL_OV; // probably
								   // not the
								   // correct
								   // one
					success = 0;
			...
			if (bty->cell_volt[i * NUM_CELL_IC + j] <
				...
				if (bty->cell_volt_err_cnt[i * NUM_CELL_IC +
					...
					bty->cell_volt_err_cnt[i * NUM_CELL_IC +
					    ...
					    ERRMG_BMS_ERR;
					bty->battery_check_faults |=
					    BATTERY_FAULT_CELL_UV; // probably
								   // not the
								   // correct
								   // one
					success = 0;
		...
		if (fabsf(bty->stack_voltage[i] -
			  bty->calculated_stack_voltage[i]) > STACK_VOLT_DIFF) {
			bty->cell_volt_errors++;
			print_lpuart("stack voltage vs manual sum of cell "
				     "volts is > 0.5!\n");
			success = 0;
		}
	}
...
{
	// note, reading temp may or may not be different bc Vamsi changed
	// thermisters, now data is 12 bit resolution instead of 10
	bcc_error = BCC_STATUS_SUCCESS;
...
void print_temperature(Battery *bty)
{
	...
	for (size_t i = 0; i < NUM_TOTAL_IC; i++) {
		...
		for (size_t j = 0; j < NUM_CELL_IC; j++) {
			...
			const float curr_temp = fmaxf(
			    bty->cell_temp[(i * 2 * NUM_CELL_IC) + j * 2],
			    bty->cell_temp[(i * 2 * NUM_CELL_IC) + j * 2 + 1]);
			if (curr_temp < -40.0f) {
				sprintf(print_buffer, "C%u: NC |", j);
				print_lpuart(print_buffer);
			} else {
				sprintf(print_buffer, "C%u: %.3f |", j,
					curr_temp);
				print_lpuart(print_buffer);
			}
		}
...
void print_voltage(Battery *bty)
{
	...
	for (uint8_t i = 0; i < NUM_TOTAL_IC; i++) { // change this later
		...
		sprintf(print_buffer,
			...
			bty->stack_voltage[i],
			bty->calculated_stack_voltage[i]);
		print_lpuart(print_buffer);
	}
