...
#define INITARRAYFIELD(FIELD, FORMAT, SRCFORMAT, DSTFORMAT)                    \
	if (FIELD) {                                                           \
		...
		if (FIELD##c) {                                                \
			...
			self->instance->FIELD =                                \
			    PyMem_Malloc(sizeof(DSTFORMAT) * n);               \
...
#define GETARGUMENT(FIELD, FORMAT, SRCFORMAT, DSTFORMAT)                       \
	...
	if (FIELD) {                                                           \
		...
		if (FIELD##c) {                                                \
			...
			FIELD##_converted = PyMem_Malloc(sizeof(DSTFORMAT) *   \
							 arraySize##FIELD);    \
			MEMCPY(FIELD##_converted, f, arraySize##FIELD,         \
			       DSTFORMAT);                                     \
			Py_DECREF(FIELD##c);                                   \
		}                                                              \
	}
...
#define NUMPYARRAYFROMMATRIX(EXT, NUMPYTYPE_FROMC)                             \
	...
	{                                                                      \
		...
		PyObject *OBJ = PyArray_SimpleNewFromData(                     \
		    ND, dims, NUMPYTYPE_FROMC, mat->pData);                    \
		return (OBJ);                                                  \
	}
