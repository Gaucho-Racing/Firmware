			    ...
			    q31_t *realResult, q31_t *imagResult)
{
	...
	for (i = 0; i < numSamples * 2; i += 2) {
		sumr += (q31_t)pSrcA[i] * pSrcB[i] -
			(q31_t)pSrcA[i + 1] * pSrcB[i + 1];
		sumi += (q31_t)pSrcA[i] * pSrcB[i + 1] +
			(q31_t)pSrcA[i + 1] * pSrcB[i];
	}

	// shifting down 6 at the end here because there are already 32 guard
	// bits available, this method is more accurate
