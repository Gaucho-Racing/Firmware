			...
			uint32_t blockSize)
{
	...
	int32_t *pTapDelay =
	    S->pTapDelay; /* Pointer to the array containing offset of the
			     non-zero tap values. */
	...
	/* StateIndex points to the starting position to write in the state
	 * buffer */
	...
	while (tapCnt > 0U) {
		 ...
		 * increment the coefficient buffer for the next set of state
		 * values */
		coeff = *pCoeffs++;

		/* Read Index, from where the state buffer should be read, is
		 * calculated. */
		readIndex = ((int32_t)S->stateIndex - (int32_t)blockSize) -
			    *pTapDelay++;
			...
			uint32_t blockSize)
{
	...
	/* StateIndex points to the starting position to write in the state
	 * buffer */
	arm_circularWrite_q15(py, delaySize, &S->stateIndex, 1, pIn, 1,
			      blockSize);
	...
	while (tapCnt > 0U) {
		...
		/* blockSize samples are read from the state buffer */
		arm_circularRead_q15(py, delaySize, &readIndex, 1, pb, pb,
		       ...
		       q7_t *pScratchIn, q31_t *pScratchOut, uint32_t blockSize)
{
	...
	q31_t *pScr2 = pScratchOut; /* Working pointer for scratch buffer of
				       output values */
	q31_t in;
	...
	while (tapCnt > 0U) {
		...
		/* blockSize samples are read from the state buffer */
		arm_circularRead_q7(py, (int32_t)delaySize, &readIndex, 1, pb,
				    pb, (int32_t)blockSize, 1, blockSize);
