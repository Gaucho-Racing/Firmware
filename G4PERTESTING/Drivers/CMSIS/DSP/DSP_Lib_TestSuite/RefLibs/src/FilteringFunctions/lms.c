		 ...
		 uint32_t blockSize)
{
	...
	while (blkCnt > 0U) {
		...
		*pStateCurnt++ = *pSrc++;
		...
		for (i = 0; i < numTaps;
		     i++) { /* Perform the multiply-accumulate */
		      ...
		      uint32_t blockSize)
{
	...
	/* S->pState points to buffer which contains previous frame (numTaps -
	 * 1) samples */
	/* pStateCurnt points to the location where the new input data should be
	 * written */
	pStateCurnt = &(S->pState[numTaps - 1U]);
	...
	for (blkCnt = blockSize; blkCnt > 0U; blkCnt--) {
		...
		*pStateCurnt++ = *pSrc;
		...
		/* The result in the accumulator is stored in the destination
		 * buffer. */
		*pOut++ = sum;
		 ...
		 q31_t *pOut, q31_t *pErr, uint32_t blockSize)
{
	...
	for (blkCnt = blockSize; blkCnt > 0U; blkCnt--) {
		...
		*pStateCurnt++ = *pSrc++;
		...
		/* Store the result from accumulator into the destination
		 * buffer. */
		      ...
		      q31_t *pOut, q31_t *pErr, uint32_t blockSize)
{
	...
	for (blkCnt = blockSize; blkCnt > 0U; blkCnt--) {
		...
		*pStateCurnt++ = *pSrc;
		...
		/* Update the energy calculation */
		energy =
		    (q31_t)((((q63_t)energy << 32) - (((q63_t)x0 * x0) << 1)) >>
			    32) &
		    0xffffffff;
		energy =
		    (q31_t)(((((q63_t)in * in) << 1) + ((q63_t)energy << 32)) >>
			    32) &
		    0xffffffff;
		      ...
		      q15_t *pOut, q15_t *pErr, uint32_t blockSize)
{
	...
	q15_t errorXmu, oneByEnergy; /* Temporary variables to store error and
					mu product and reciprocal of energy */
	// q31_t errorXmu;                   				 /*
	// Temporary variables to store error and mu product and reciprocal of
	// energy */
