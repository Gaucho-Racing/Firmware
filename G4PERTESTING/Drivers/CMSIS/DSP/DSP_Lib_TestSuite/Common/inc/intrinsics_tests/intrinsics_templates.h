...
#define INTRINSICS_SNR_COMPARE_INTERFACE(block_size, output_type)              \
	do {                                                                   \
		TEST_CONVERT_AND_ASSERT_SNR(                                   \
		    ...
		    (output_type##_t *)intrinsics_output_fut, block_size,      \
	...
	{                                                                      \
		...
		JTEST_COUNT_CYCLES(for (i = 0; i < INTRINSICS_MAX_LEN; i++) {  \
			*((dataType##_t *)intrinsics_output_fut + i) =         \
			    functionName(                                      \
				(dataType##_t)                                 \
				    intrinsics_##dataType##_inputs[i]);        \
		});                                                            \
                                                                               \
			...
			*((dataType##_t *)intrinsics_output_ref + i) =         \
				...
				(dataType##_t)                                 \
				    intrinsics_##dataType##_inputs[i]);        \
                                                                               ...
                                                                               \
		INTRINSICS_SNR_COMPARE_INTERFACE(INTRINSICS_MAX_LEN,           \
						 dataType);                    \
                                                                               \
		...
		JTEST_COUNT_CYCLES(for (i = 0; i < INTRINSICS_MAX_LEN; i++) {  \
			*((dataType2##_t *)intrinsics_output_fut + i) =        \
				...
				(dataType##_t)                                 \
				    intrinsics_##dataType##_inputs[i],         \
				(dataType2##_t)                                \
				    intrinsics_##dataType2##_inputs[i]);       \
		});                                                            \
                                                                               \
		for (i = 0; i < INTRINSICS_MAX_LEN; i++) {                     \
