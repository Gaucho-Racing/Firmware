...
#define TEST_CONVERT_AND_ASSERT_SNR(ref_dst_ptr, ref_src_ptr, tst_dst_ptr,     \
				    ...
				    threshold)                                 \
	do {                                                                   \
		TEST_CONVERT_TO_FLOAT(ref_src_ptr, ref_dst_ptr, block_size,    \
				      tst_src_type);                           \
		TEST_CONVERT_TO_FLOAT(tst_src_ptr, tst_dst_ptr, block_size,    \
				      tst_src_type);                           \
		TEST_ASSERT_SNR(ref_dst_ptr, tst_dst_ptr, block_size,          \
				threshold);                                    \
	} while (0)
	...
	do {                                                                   \
		TEMPLATE_DO_ARR_DESC(                                          \
			...
			TEST_DO_VALID_BLOCKSIZE(                               \
				...
				ref,                                           \
				ref_arg_interface(input_data_ptr,              \
						  block_size));                \
                                                                               \
...
#define TEST_TEMPLATE_BUF1_ELT1(arr_desc_inputs, arr_desc_elts, input_type,    \
	...
	do {                                                                   \
		TEMPLATE_DO_ARR_DESC(                                          \
			...
			void *input_data_ptr = input_ptr->data_ptr;            \
			TEST_CALL_FUT_AND_REF(                                 \
			    fut, fut_arg_interface(input_data_ptr, elt), ref,  \
		...
		TEMPLATE_DO_ARR_DESC(                                          \
			...
			TEMPLATE_DO_ARR_DESC(                                  \
				...
				TEST_DO_VALID_BLOCKSIZE(                       \
				    ...
				    TEST_CALL_FUT_AND_REF(                     \
					fut,                                   \
					fut_arg_interface(input_data_ptr,      \
							  elt1, elt2,          \
							  block_size),         \
					ref,                                   \
					ref_arg_interface(input_data_ptr,      \
...
#define TEST_TEMPLATE_BUF2_BLK(arr_desc_inputs_a, arr_desc_inputs_b,           \
	...
	do {                                                                   \
		...
		TEMPLATE_DO_ARR_DESC(                                          \
			...
			void *input_b_ptr =                                    \
			    ARR_DESC_ELT(ARR_DESC_t *, input_idx,              \
					 &(arr_desc_inputs_b))                 \
				->data_ptr;                                    \
                                                                               \
			TEST_DO_VALID_BLOCKSIZE(                               \
			    ...
			    TEST_CALL_FUT_AND_REF(                             \
				fut,                                           \
				fut_arg_interface(input_a_ptr, input_b_ptr,    \
						  block_size),                 \
				ref,                                           \
				ref_arg_interface(input_a_ptr, input_b_ptr,    \
						  block_size));                \
                                                                               \
			    compare_interface(block_size, output_type))));     \
		return JTEST_TEST_PASSED;                                      \
...
#define TEST_TEMPLATE_ELT1(arr_desc_elts, elt_type, output_type, fut,                                                                                               \
	...
	do {                                                                                                                                                        \
		TEMPLATE_DO_ARR_DESC(                                                                                                                               \
		    ...
		    TEST_CALL_FUT_AND_REF(                                                                                                                          \
			fut,                                                                                                                                        \
			fut_arg_interface(elt), ref, ref_arg_interface(elt)); /* Comparison interfaces typically accept */ /* a block_size. Pass a dummy value 1.*/ \
...
#define TEST_TEMPLATE_ELT2(arr_desc_elts_a, arr_desc_elts_b, elt_a_type,                                                                                         \
			   ...
			   ref, ref_arg_interface, compare_interface)                                                                                            \
	do {                                                                                                                                                     \
		TEMPLATE_DO_ARR_DESC(elt_a_idx, elt_a_type, elt_a,                                                                                               \
				     arr_desc_elts_a,                                                                                                            \
				     elt_b_type *elt_b =                                                                                                         \
					 ARR_DESC_ELT(elt_b_type, elt_a_idx,                                                                                     \
						      arr_desc_elts_b);                                                                                          \
                                                                                                                                                                 \
				     TEST_CALL_FUT_AND_REF(                                                                                                      \
					 fut, fut_arg_interface(elt_a, elt_b),                                                                                   \
					 ref,                                                                                                                    \
					 ref_arg_interface(elt_a, elt_b)); /* Comparison interfaces typically accept */ /* a block_size. Pass a dummy value 1.*/ \
				     compare_interface(1, output_type));                                                                                         \
		return JTEST_TEST_PASSED;                                                                                                                        \
	} while (0)
...
#define TEST_TEMPLATE_ELT1_BLK(arr_desc_elts, arr_desc_block_sizes, elt_type,  \
	...
	do {                                                                   \
		TEMPLATE_DO_ARR_DESC(                                          \
			...
			TEST_CALL_FUT_AND_REF(                                 \
			    fut, fut_arg_interface(elt, block_size), ref,      \
