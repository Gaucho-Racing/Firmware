...
#define CORRELATE_DEFINE_RAND_INPUT_ARR_DESCS(type_suffix)                     \
	ARR_DESC_DEFINE_USING_ARR(TYPE_FROM_ABBREV(type_suffix),               \
				  ...
				  CORRELATE_##type_suffix##_INPUTS, 0,         \
...
#define CORRELATE_TEST_TEMPLATE(fut, fut_arg_interface, ref,                   \
	...
	{                                                                      \
		TEMPLATE_DO_ARR_DESC(                                          \
		    input_idx, ARR_DESC_t *, input_ptr,                        \
			...
			ARR_DESC_ELT(ARR_DESC_t *, input_idx,                  \
			    ...
			    ->data_ptr;                                        \
                                                                               \
		    TEMPLATE_DO_ARR_DESC(                                      \
			correlate_len_idx, uint32_t, correlate_len_a,          \
			correlate_lens_a,                                      \
			...
			TEST_CALL_FUT_AND_REF(                                 \
			    fut,                                               \
			    fut_arg_interface(input_a_ptr, correlate_len_a,    \
					      input_b_ptr, correlate_len_b),   \
			    ref,                                               \
			    ref_arg_interface(input_a_ptr, correlate_len_a,    \
					      input_b_ptr, correlate_len_b));  \
                                                                               \
...
#define CORRELATE_DEFINE_TEST(fn_name, suffix, output_type, test_template)     \
	test_template(arm_##fn_name##_##suffix,                                \
		      ...
		      CORRELATE_ref_##fn_name##_INPUT_INTERFACE, suffix,       \
