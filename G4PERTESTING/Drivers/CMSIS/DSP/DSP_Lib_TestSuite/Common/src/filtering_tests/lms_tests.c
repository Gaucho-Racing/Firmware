		...
		TEMPLATE_DO_ARR_DESC(                                          \
			...
			arm_fir_init_##suffix(                                 \
			    ...
			    (void *)filtering_pState, blockSize);              \
                                                                               \
			ref_fir_##suffix(                                      \
			    &fir_inst, (void *)filtering_##suffix##_inputs,    \
			...
			for (i = 0; i < blockSize; i++) {                      \
				...
				*((output_type *)filtering_input_lms + i) =    \
				    *((output_type *)filtering_input_lms +     \
				      i) /                                     \
				    200.0f;                                    \
                                                                               \
				*((output_type *)filtering_output_f32_fut +    \
				  i) = *((output_type *)                       \
					     filtering_##suffix##_inputs +     \
					 i) /                                  \
				       200.0f;                                 \
			}                                                      \
                                                                               \
	...
	{                                                                      \
		...
		TEMPLATE_DO_ARR_DESC(                                          \
			...
			TEMPLATE_DO_ARR_DESC(                                  \
			    ...
			    filtering_postshifts, /* Initialize the FIR        \
						     Instances */              \
			    arm_fir_init_##suffix(                             \
				&fir_inst, numTaps,                            \
				(output_type *)filtering_coeffs_##suffix,      \
				...
				&lms_inst_fut, numTaps,                        \
				(output_type *)filtering_coeffs_lms,           \
				(void *)filtering_pState, mu_##suffix,         \
				blockSize, postShift);                         \
                                                                               \
			    JTEST_COUNT_CYCLES(                                \
				arm_lms##config_suffix##_##suffix(             \
