...
#define CONV_TEST_VALID_PARTIAL_PARAMS(input_a_len, input_b_len, first_index,  \
				       num_points)                             \
	(((((input_a_len) + (input_b_len) - 1)) >=                             \
	  num_points + first_index) &&                                         \
	 (num_points > 0))
...
#define CONV_DEFINE_ALL_INPUTS(type_suffix)                                    \
	...
	ARR_DESC_DEFINE(                                                       \
	    ...
	    CURLY(&conv_input_zeros, &conv_input_rand1_##type_suffix,          \
		  &conv_input_zeros,                                           \
		  &conv_input_rand2_##type_suffix)) /* Note the lacking        \
						       semicolon */
	...
	{                                                                      \
		TEMPLATE_DO_ARR_DESC(                                          \
			...
			FILTERING_SNR_COMPARE_INTERFACE(                       \
			    conv_len_a + conv_len_b - 1, output_type)));       \
                                                                               \
		...
		TEMPLATE_DO_ARR_DESC(                                           \
			...
			TEMPLATE_DO_ARR_DESC(                                   \
				...
				if (CONV_TEST_VALID_PARTIAL_PARAMS(             \
					conv_len_a, conv_len_b, first_index,    \
					num_points)) {                          \
...
#define CONV_DEFINE_TEST(fn_name, suffix, output_type, test_template)          \
	test_template(                                                         \
	    ...
	    ref_##fn_name##_##suffix, CONV_ref_##fn_name##_INPUT_INTERFACE,    \
