...
#define BIQUAD_DEFINE_TEST(suffix, instance_name, config_suffix, output_type)  \
	JTEST_DEFINE_TEST(                                                     \
	    arm_biquad_cascade_##config_suffix##_##suffix##_test,              \
	...
	{                                                                      \
		...
		TEMPLATE_DO_ARR_DESC(                                          \
			...
			filtering_numstages, /* Initialize the BIQUAD          \
						Instances */                   \
...
#define BIQUAD_WITH_POSTSHIFT_DEFINE_TEST(suffix, config_suffix, speed,               \
	...
	{                                                                             \
		arm_biquad_casd_##config_suffix##_inst_##suffix                       \
		    biquad_inst_fut = {0};                                            \
		arm_biquad_casd_##config_suffix##_inst_##suffix                       \
		    biquad_inst_ref = {0};                                            \
                                                                                      \
		TEMPLATE_DO_ARR_DESC(                                                 \
			...
			TEMPLATE_DO_ARR_DESC(                                         \
			    ...
			    filtering_postshifts, /* Display test parameter           \
						     values */                        \
			    JTEST_DUMP_STRF("Block Size: %d\n"                        \
					    "Number of Stages: %d\n"                  \
					    "Post Shift: %d\n",                       \
...
JTEST_DEFINE_TEST(arm_biquad_cascade_df2T_f64_test, arm_biquad_cascade_df2T_f64)
{
	...
	TEMPLATE_DO_ARR_DESC(
		...
		ref_biquad_cascade_df2T_f64(
		    &biquad_inst_ref, (void *)filtering_f64_inputs,
		    (void *)filtering_output_ref, blockSize);
