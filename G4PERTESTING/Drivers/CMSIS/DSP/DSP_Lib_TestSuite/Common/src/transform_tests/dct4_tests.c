...
#define DCT4_DEFINE_TEST(suffix, input_type)                                                       \
	...
	{                                                                                          \
		...
		TEMPLATE_DO_ARR_DESC(                                                              \
		    ...
		    TEST_CONVERT_FLOAT_TO(&normalize_f32, &normalize, 1,                           \
					  input_type);                                             \
                                                                                                   \
		    /* Initialize the DCT4, RFFT, and CFFT instances */                            \
		    arm_dct4_init_##suffix(&dct4_inst_fut, &rfft_inst_fut,                         \
					   &cfft_inst_fut, fftlen, fftlen / 2,                     \
					   normalize);                                             \
                                                                                                   \
		    arm_dct4_init_##suffix(&dct4_inst_ref, &rfft_inst_ref,                         \
					   &cfft_inst_ref, fftlen, fftlen / 2,                     \
					   normalize);                                             \
                                                                                                   \
		    ...
		    JTEST_COUNT_CYCLES(arm_dct4_##suffix(                                          \
			&dct4_inst_fut, (void *)transform_fft_input_fut,                           \
		    ...
		    ref_dct4_##suffix(                                                             \
			&dct4_inst_ref, (void *)transform_fft_input_ref,                           \
...
#define DCT4_FIXED_POINT_DEFINE_TEST(suffix, input_type, prefix)                                   \
	...
	{                                                                                          \
		...
		TEMPLATE_DO_ARR_DESC(                                                              \
		    ...
		    for (i = 0; i < fftlen; i++) {                                                 \
			    ((input_type *)                                                        \
				 transform_fft_inplace_input_fut)[i] =                             \
				prefix##transform_fft_##suffix##_inputs[i] >>                      \
				1;                                                                 \
			    ((input_type *)                                                        \
				 transform_fft_inplace_input_ref)[i] =                             \
				prefix##transform_fft_##suffix##_inputs[i] >>                      \
				1;                                                                 \
		    }                                                                              \
                                                                                                   \
