...
#define RFFT_DEFINE_TEST(suffix, config_suffix, ifft_flag, input_type,         \
	...
	{                                                                      \
		...
		TEMPLATE_DO_ARR_DESC(                                          \
		    ...
		    arm_rfft_init_##suffix(&rfft_inst_fut, (uint32_t)fftlen,   \
					   ifft_flag, 1U);                     \
                                                                               \
		    arm_rfft_init_##suffix(&rfft_inst_ref, (uint32_t)fftlen,   \
					   ifft_flag, 1U);                     \
                                                                               \
		    if (ifft_flag) {                                           \
			    TRANSFORM_PREPARE_INVERSE_INPUTS(                  \
				transform_fft_##suffix##_inputs, fftlen,       \
				input_type, fftlen * sizeof(input_type));      \
		    } else {                                                   \
		    ...
		    JTEST_COUNT_CYCLES(arm_rfft_##suffix(                      \
			&rfft_inst_fut, (void *)transform_fft_input_fut,       \
