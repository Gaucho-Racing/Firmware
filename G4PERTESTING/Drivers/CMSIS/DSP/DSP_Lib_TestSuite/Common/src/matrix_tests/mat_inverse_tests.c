...
JTEST_DEFINE_TEST(arm_mat_inverse_f32_test, arm_mat_inverse_f32)
{
	TEMPLATE_DO_ARR_DESC(
	    mat_idx, arm_matrix_instance_f32 *, mat_ptr,
	    ...
	    if (MATRIX_TEST_VALID_SQUARE_DIMENSIONS(arm_matrix_instance_f32 *,
			...
			arm_matrix_instance_f32 *, mat_ptr);

		    /* arm_mat_inverse_f32() modifies its source input. Use the
		     * scratch buffer to store a copy of the intended input. */
		    {
			    float32_t *original_pdata_ptr = mat_ptr->pData;

			    memcpy(matrix_output_scratch, mat_ptr->pData,
				   mat_ptr->numRows * mat_ptr->numCols *
				       sizeof(float32_t));
			    mat_ptr->pData = (void *)&matrix_output_scratch;

			    JTEST_COUNT_CYCLES(arm_mat_inverse_f32(
				mat_ptr, &matrix_output_fut));
...
{
	TEMPLATE_DO_ARR_DESC(
	    mat_idx, arm_matrix_instance_f64 *, mat_ptr,
	    ...
	    if (MATRIX_TEST_VALID_SQUARE_DIMENSIONS(arm_matrix_instance_f64 *,
						    mat_ptr)) {
		    MATRIX_TEST_CONFIG_SAMESIZE_OUTPUT(
			arm_matrix_instance_f64 *, mat_ptr);

		    /* arm_mat_inverse_f64() modifies its source input. Use the
		     * scratch buffer to store a copy of the intended input. */
		    {
			    ...
			    JTEST_COUNT_CYCLES(arm_mat_inverse_f64(
				mat_ptr, &matrix_output_fut64));
			    mat_ptr->pData = original_pdata_ptr;
		    }
