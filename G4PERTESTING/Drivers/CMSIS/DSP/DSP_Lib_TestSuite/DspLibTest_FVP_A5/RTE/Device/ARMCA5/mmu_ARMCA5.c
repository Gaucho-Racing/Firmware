...
void MMU_CreateTranslationTable(void)
{
	...
	// all DRAM executable, rw, cacheable - applications may choose to
	// divide memory into ro executable
	MMU_TTSection(&Image$$TTB$$ZI$$Base, (uint32_t)&Image$$TTB$$ZI$$Base,
		      2043, Sect_Normal);
	...
	MMU_TTSection(&Image$$TTB$$ZI$$Base, VE_A5_MP_FLASH_BASE1, 64,
		      Sect_Device_RO);
	MMU_TTSection(&Image$$TTB$$ZI$$Base, VE_A5_MP_SRAM_BASE, 64,
		      Sect_Device_RW);
	MMU_TTSection(&Image$$TTB$$ZI$$Base, VE_A5_MP_VRAM_BASE, 32,
		      Sect_Device_RW);
	MMU_TTSection(&Image$$TTB$$ZI$$Base, VE_A5_MP_ETHERNET_BASE, 16,
		      Sect_Device_RW);
	MMU_TTSection(&Image$$TTB$$ZI$$Base, VE_A5_MP_USB_BASE, 16,
		      Sect_Device_RW);

	// Create (16 * 64k)=1MB faulting entries to cover peripheral range
	// 0x1C000000-0x1C00FFFF
	MMU_TTPage64k(&Image$$TTB$$ZI$$Base, PERIPHERAL_A_FAULT, 16,
	...
	MMU_TTPage64k(&Image$$TTB$$ZI$$Base, VE_A5_MP_DAP_BASE, 1, Page_L1_64k,
		      ...
		      Page_64k_Device_RW);
	MMU_TTPage64k(&Image$$TTB$$ZI$$Base, VE_A5_MP_SYSTEM_REG_BASE, 1,
		      Page_L1_64k, (uint32_t *)PERIPHERAL_A_TABLE_L2_BASE_64k,
		      Page_64k_Device_RW);
	MMU_TTPage64k(&Image$$TTB$$ZI$$Base, VE_A5_MP_SERIAL_BASE, 1,
		      Page_L1_64k, (uint32_t *)PERIPHERAL_A_TABLE_L2_BASE_64k,
		      Page_64k_Device_RW);
	...
	MMU_TTPage64k(&Image$$TTB$$ZI$$Base, PERIPHERAL_B_FAULT, 16,
		      Page_L1_64k, (uint32_t *)PERIPHERAL_B_TABLE_L2_BASE_64k,
		      DESCRIPTOR_FAULT);
	// Define peripheral range 0x1C100000-0x1C10FFFF
	MMU_TTPage64k(&Image$$TTB$$ZI$$Base, VE_A5_MP_TIMER_BASE, 2,
		      Page_L1_64k, (uint32_t *)PERIPHERAL_B_TABLE_L2_BASE_64k,
		      Page_64k_Device_RW);
	...
	MMU_TTPage64k(&Image$$TTB$$ZI$$Base, VE_A5_MP_RTC_BASE, 1, Page_L1_64k,
		      (uint32_t *)PERIPHERAL_B_TABLE_L2_BASE_64k,
		      Page_64k_Device_RW);
	MMU_TTPage64k(&Image$$TTB$$ZI$$Base, VE_A5_MP_UART4_BASE, 1,
		      Page_L1_64k, (uint32_t *)PERIPHERAL_B_TABLE_L2_BASE_64k,
		      Page_64k_Device_RW);
	...
	// Create (256 * 4k)=1MB faulting entries to cover private address
	// space. Needs to be marked as Device memory
	...
	//    Page_L1_4k, (uint32_t *)PRIVATE_TABLE_L2_BASE_4k,
	//    Page_4k_Device_RW);

	// Create (256 * 4k)=1MB faulting entries to synchronization space
	// (Useful if some non-cacheable DMA agent is present in the SoC)
	...
	; 31:14 - Translation table base addr (31:14-TTBCR.N, TTBCR.N is 0 out
	of reset) ; 13:7  - 0x0 ; 6     - IRGN[0] 0x1  (Inner WB WA) ; 5     -
	NOS     0x0  (Non-shared) ; 4:3   - RGN     0x01 (Outer WB WA) ; 2     -
	IMP     0x0  (Implementation Defined) ; 1     - S       0x0 (Non-shared)
