...
{
	uint32_t FS =
	    (DFSR & (1 << 10)) >> 6 | (DFSR & 0x0f); // Store Fault Status
	...
	switch (FS) {
		...
		// Your code here. Value in DFAR is invalid for some fault
		// statuses.
...
{
	uint32_t FS =
	    (IFSR & (1 << 10)) >> 6 | (IFSR & 0x0f); // Store Fault Status
	...
	switch (FS) {
		...
		// Your code here. Value in IFAR is invalid for some fault
		// statuses.
		case FSR_SYNC_EXT_TTB_WALK_FIRST:
		case FSR_SYNC_EXT_TTB_WALK_SECOND:
		case FSR_TRANSLATION_FAULT_FIRST:
...
uint32_t CUndefHandler(uint32_t opcode, uint32_t state, uint32_t LR)
{
	...
	// (ARM ARM section A7.9) VFP/NEON 64-bit register data transfer
	// instruction?
