...
typedef struct {
	...
	const q7_t *pCoeffs; /**< points to the coefficient array. The array is
				of length numTaps.*/
} arm_fir_instance_q7;
...
typedef struct {
	...
	q15_t *pState; /**< points to the state variable array. The array is of
			  length numTaps+blockSize-1. */
	const q15_t *pCoeffs; /**< points to the coefficient array. The array is
				 of length numTaps.*/
...
typedef struct {
	...
	q31_t *pState; /**< points to the state variable array. The array is of
			  length numTaps+blockSize-1. */
	const q31_t *pCoeffs; /**< points to the coefficient array. The array is
				 of length numTaps. */
...
typedef struct {
	...
	const float32_t *pCoeffs; /**< points to the coefficient array. The
...
typedef struct {
	...
	const q15_t *pCoeffs; /**< Points to the array of coefficients.  The
...
typedef struct {
	...
	q31_t *pState;	    /**< Points to the array of state coefficients.  The
			       array is of length 4*numStages. */
	const q31_t *pCoeffs; /**< Points to the array of coefficients.  The
				 array is of length 5*numStages. */
...
arm_linear_interp_f32(arm_linear_interp_instance_f32 *S, float32_t x)
{
	...
	if (i < 0) {
		/* Iniatilize output for below specified range as least output
		 * value of table */
		y = pYData[0];
	} else if ((uint32_t)i >= S->nValues) {
		/* Iniatilize output for above specified range as last output
		 * value of table */
						 ...
						 uint32_t nValues)
{
	...
	if (index >= (int32_t)(nValues - 1)) {
		...
		/* Read two nearest output values from the index in 1.31(q31)
		 * format */
		y0 = pYData[index];
		y1 = pYData[index + 1];
		...
		/* Calculation of y0 * (1-fract) + y1 *fract and y is in 2.30
		 * format */
						 ...
						 uint32_t nValues)
{
	...
	if (index >= (int32_t)(nValues - 1)) {
		...
		/* Calculation of (y0 * (1-fract) + y1 * fract) and y is
		 * in 13.35 format */
		y += ((q63_t)y1 * (fract));
					       ...
					       uint32_t nValues)
{
	...
	if (index >= (nValues - 1)) {
		...
		/* Read two nearest output values from the index and are
		 * in 1.7(q7) format */
		y0 = pYData[index];
		y1 = pYData[index + 1];
		...
		/* Calculation of y1 * fract + y0 * (1-fract) and y is
		 * in 13.27(q27) format */
		y += (y1 * fract);
		      ...
		      uint32_t blockSize)
{
	...
	while (i > 0U) {
		...
		/* Circularly update wOffset.  Watch out for positive and
		 * negative value */
		wOffset += bufferInc;
		if (wOffset >= L) {
			wOffset -= L;
		     ...
		     int32_t dst_length, int32_t dstInc, uint32_t blockSize)
{
	...
	while (i > 0U) {
		...
		*dst = circBuffer[rOffset];
		...
		/* Circularly update rOffset.  Watch out for positive and
		 * negative value  */
		rOffset += bufferInc;
