		 ...
		 float32_t *pDst, uint32_t blockSize)
{
	...
	float32x4_t accv0, accv1, samples0, samples1, x0, x1, x2, xa, xb, x, b,
	    accv;
		 ...
		 float32_t *pDst, uint32_t blockSize)
{
	 ...
	 * The variables acc0 ... acc7 hold output values that are being
	 * computed:
	 *
	...
	while (blkCnt > 0U) {
		...
		*pStateCurnt++ = *pSrc++;
		...
		/* Advance the state pointer by 8 to process the next group of 8
		 * samples */
		pState = pState + 8;

		/* The results in the 8 accumulators, store in the destination
		 * buffer. */
		*pDst++ = acc0;
		*pDst++ = acc1;
		*pDst++ = acc2;
