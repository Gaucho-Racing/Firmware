		  ...
		  uint32_t srcBLen, q15_t *pDst)
{
	...
	while (blockSize1 > 0U) {
		...
		/* For the next MAC operations, the pointer py is used without
		 * SIMD So, py is incremented by 1 */
		py = py + 1U;
	...
	/* In this stage the MAC operations are decreased by 1 for every
	   iteration. The blockSize3 variable holds the number of MAC operations
	   performed */
	...
	while ((j > 0U) && (blockSize3 > 0U)) {
		...
		while (k > 0U) {
			 ...
			 * are multiplied with y[srcBLen - 1], y[srcBLen - 2]
			 * respectively */
			sum = __SMLALDX(read_q15x2_ia((q15_t **)&px),
					read_q15x2_da((q15_t **)&py), sum);
			/* x[srcALen - srcBLen + 3], x[srcALen - srcBLen + 4]
			 * are multiplied with y[srcBLen - 3], y[srcBLen - 4]
			 * respectively */
			sum = __SMLALDX(read_q15x2_ia((q15_t **)&px),
					read_q15x2_da((q15_t **)&py), sum);
		...
		/* If the blockSize3 is not a multiple of 4, compute any
		 *remaining MACs here.
		 ** No loop unrolling is used. */
		k = blockSize3 % 0x4U;
