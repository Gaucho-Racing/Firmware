			 ...
			 const q31_t *pSrc, q31_t *pDst, uint32_t blockSize)
{
	...
	/* Processing is complete. Now copy last S->numStages samples to start
	   of the buffer for the preperation of next frame process */
	...
	while (blkCnt > 0U) {
		...
		*pDst++ = (q31_t)(acc >> 31U);
		...
		/* Advance the state pointer by 1 to process the next group of
		 * samples */
		pState = pState + 1U;
