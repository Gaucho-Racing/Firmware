			       ...
			       uint32_t blockSize)
{
	...
	while (blkCnt > 0U) {
		/* Copy 4 * decimation factor number of new input samples into
		 * the state buffer */
		i = S->M * 4;
		...
		while (tapCnt > 0U) {
			...
			acc0 =
			    (q31_t)((((q63_t)acc0 << 32) + ((q63_t)x0 * c0)) >>
				    32);
			acc1 =
			    (q31_t)((((q63_t)acc1 << 32) + ((q63_t)x1 * c0)) >>
				    32);
			acc2 =
			    (q31_t)((((q63_t)acc2 << 32) + ((q63_t)x2 * c0)) >>
				    32);
			acc3 =
			    (q31_t)((((q63_t)acc3 << 32) + ((q63_t)x3 * c0)) >>
				    32);
	...
	while (blkCnt > 0U) {
		/* Copy decimation factor number of new input samples into the
		 * state buffer */
		i = S->M;
	...
	/* Processing is complete.
	   Now copy the last numTaps - 1 samples to the satrt of the state
