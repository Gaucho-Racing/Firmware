				...
				uint32_t blockSize)
{
	...
	do {
		...
		while (sample > 0U) {
			 ...
			 * y[n-1] + a2 * y[n-2] */
			acc = ((q63_t)b0 * Xn) + ((q63_t)b1 * Xn1) +
			      ((q63_t)b2 * Xn2) + ((q63_t)a1 * Yn1) +
			      ((q63_t)a2 * Yn2);

			/* The result is converted to 1.31 , Yn2 variable is
			 * reused */
			acc_l = (acc) & 0xffffffff; /* Calc lower part of acc */
			acc_h = (acc >> 32) &
				0xffffffff; /* Calc upper part of acc */
			...
			*pOut++ = Yn2;
			...
			/* acc =  b0 * x[n] + b1 * x[n-1] + b2 * x[n-2] + a1 *
			 * y[n-1] + a2 * y[n-2] */
			acc = ((q63_t)b0 * Xn2) + ((q63_t)b1 * Xn) +
			      ((q63_t)b2 * Xn1) + ((q63_t)a1 * Yn2) +
			      ((q63_t)a2 * Yn1);

			/* The result is converted to 1.31, Yn1 variable is
			 * reused  */
			acc_l = (acc) & 0xffffffff; /* Calc lower part of acc */
			acc_h = (acc >> 32) &
				0xffffffff; /* Calc upper part of acc */
			...
			*pOut++ = Yn1;
			...
			/* acc =  b0 * x[n] + b1 * x[n-1] + b2 * x[n-2] + a1 *
			 * y[n-1] + a2 * y[n-2] */
			acc = ((q63_t)b0 * Xn1) + ((q63_t)b1 * Xn2) +
			      ((q63_t)b2 * Xn) + ((q63_t)a1 * Yn1) +
			      ((q63_t)a2 * Yn2);
