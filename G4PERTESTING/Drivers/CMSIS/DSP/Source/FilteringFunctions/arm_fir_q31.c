		 ...
		 uint32_t blockSize)
{
	...
	while (blkCnt > 0U) {
		...
		*pStateCurnt++ = *pSrc++;
		...
		/* Advance the state pointer by 3 to process the next group of 3
		 * samples */
		pState = pState + 3;

		/* The result is in 2.30 format. Convert to 1.31 and store in
		 * destination buffer. */
		*pDst++ = (q31_t)(acc0 >> 31U);
		*pDst++ = (q31_t)(acc1 >> 31U);
		*pDst++ = (q31_t)(acc2 >> 31U);
	...
	while (blkCnt > 0U) {
		...
		*pStateCurnt++ = *pSrc++;
		...
		/* Result is in 2.62 format. Convert to 1.31 and store in
		 * destination buffer. */
		*pDst++ = (q31_t)(acc0 >> 31U);
