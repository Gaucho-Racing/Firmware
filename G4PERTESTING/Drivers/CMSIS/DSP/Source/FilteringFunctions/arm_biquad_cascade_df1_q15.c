				...
				uint32_t blockSize)
{
	...
	do {
		...
		/* Read the input state values from the state buffer:  x[n-1],
		 * x[n-2] */
		state_in = read_q15x2_ia(&pState);

		/* Read the output state values from the state buffer:  y[n-1],
		 * y[n-2] */
		state_out = read_q15x2_da(&pState);

		/* Apply loop unrolling and compute 2 output values
		 * simultaneously. */
		/*      The variable acc hold output values that are being
		 * computed:
		 *
		 *    acc =  b0 * x[n] + b1 * x[n-1] + b2 * x[n-2] + a1 * y[n-1]
		 * + a2 * y[n-2] acc =  b0 * x[n] + b1 * x[n-1] + b2 * x[n-2] +
		 * a1 * y[n-1] + a2 * y[n-2]
		 */
		sample = blockSize >> 1U;

		/* First part of the processing with loop unrolling.  Compute 2
		 *outputs at a time.
		...
		while (sample > 0U) {
			...
			/* The result is converted from 3.29 to 1.31 if
			 * postShift = 1, and then saturation is applied */
			...
			/* Apply shift for lower part of acc and upper part of
			 * acc */
			out = (uint32_t)acc_l >> lShift | acc_h << uShift;

			out = __SSAT(out, 16);

			/* Every time after the output is computed state should
			 * be updated. */
			...
			/* Yn2 = Yn1 */
			/* Yn1 = acc */
			/* x[n-N], x[n-N-1] are packed together to make state_in
			 * of type q31 */
			/* y[n-N], y[n-N-1] are packed together to make
			 * state_out of type q31 */
		...
		/* If the blockSize is not a multiple of 2, compute any
	...
	do {
		...
		while (sample > 0U) {
			...
			/* acc =  b0 * x[n] + b1 * x[n-1] + b2 * x[n-2] + a1 *
			 * y[n-1] + a2 * y[n-2] */
			/* acc =  b0 * x[n] */
			acc = (q31_t)b0 * Xn;
		...
		/*  The first stage goes from the input buffer to the output
		 * buffer. */
