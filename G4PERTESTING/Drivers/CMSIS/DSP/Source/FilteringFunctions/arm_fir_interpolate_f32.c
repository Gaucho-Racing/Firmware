			     ...
			     uint32_t blockSize)
{
	...
	while (blkCnt > 0U) {
		...
		while (i > 0U) {
			...
			/* Loop over the polyPhase length. Unroll by a factor
			 *of 4.
			 ** Repeat until we've computed numTaps-(4*S->L)
			 *coefficients. */
			tapCnt = phaseLen >> 2U;
			...
			while (tapCnt > 0U) {
				...
				/* Read the coefficients, inputs and perform
				 * multiply-accumulate */
				c1 = *(ptr2 + S->L);
				...
				/* Upsampling is done by stuffing L-1 zeros
				 * between each sample. So instead of
				 * multiplying zeros with coefficients,
				 * Increment the coefficient pointer by
				 * interpolation factor times. */
				ptr2 += 4 * S->L;
				ptr1 += 4;
				x0v = x1v;
				x1v = x2v;
			...
			/* If the polyPhase length is not a multiple of 4,
			 * compute the remaining filter taps */
			...
			/* The result is in the accumulator, store in the
			 * destination buffer. */
			*pDst = accV0[0];
			*(pDst + S->L) = accV0[1];
			*(pDst + 2 * S->L) = accV0[2];
			...
			*(pDst + 7 * S->L) = accV1[3];
			...
			/* Increment the address modifier index of coefficient
			 * buffer */
			j++;
		 ...
		 * to process the next group of interpolation factor number
		 * samples */
		pState = pState + 8;
	...
	while (blkCntN4 > 0U) {
		...
		*pStateCurnt++ = *pSrc++;
		...
		while (i > 0U) {
			...
			tempV = vpadd_f32(vget_low_f32(sum0v),
					  vget_high_f32(sum0v));
	 ...
	 ** Now copy the last phaseLen - 1 samples to the satrt of the state
	 *buffer.
			     ...
			     uint32_t blockSize)
{
	...
	/* S->pState buffer contains previous frame (phaseLen - 1) samples */
	/* pStateCur points to the location where the new input data should be
	 * written */
	...
	while (blkCnt > 0U) {
		...
		*pStateCur++ = *pSrc++;
		...
		while (i > 0U) {
			...
			/* Loop over the polyPhase length. Unroll by a factor
			   of 4. Repeat until we've computed numTaps-(4*S->L)
			   coefficients. */
			tapCnt = phaseLen >> 2U;
	...
	/* Processing is complete.
	 ** Now copy the last phaseLen - 1 samples to the start of the state
	 *buffer.
	 ** This prepares the state buffer for the next function call. */
