				  ...
				  q31_t *pSrc, q31_t *pDst, uint32_t blockSize)
{
	...
	do {
		 ...
		 * simultaneously. */
		/* Variable acc hold output value that is being computed and
		 * stored in destination buffer acc =  b0 * x[n] + b1 * x[n-1] +
		...
		while (sample > 0U) {
			...
			/* The result is converted to 1.63 , Yn2 variable is
			 * reused */
			Yn2 = acc << shift;
