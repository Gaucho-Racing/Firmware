				    ...
				    q15_t *pScratch2)
{
	...
	if ((firstIndex + numPoints) > ((srcALen + (srcBLen - 1U)))) {
		...
		/* Copy smaller length input sequence in reverse order into
		...
		while (k > 0U) {
			/* copy second buffer in reversal manner for remaining
			 * samples */
			*pScr2-- = *px++;
		...
		/* Copy bigger length sequence(srcALen) samples in scratch1
		 * buffer */
		...
		while (blkCnt > 0) {
			...
			/* Update scratch pointer for remaining samples of
			 * smaller length sequence */
			pScr1 -= 4U;

			/* apply same above for remaining samples of smaller
			 * length sequence */
			tapCnt = (srcBLen) & 3U;
		...
		/* Calculate convolution for remaining samples of Bigger length
		 * sequence */
		while (blkCnt > 0) {
			/* Initialze temporary scratch pointer as scratch1 */
			pScr1 = pScratch1;
