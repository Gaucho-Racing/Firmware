		       ...
		       uint32_t srcBLen, q31_t *pDst)
{
	...
	q31_t x0, x1, x2, c0;	/* Temporary variables for holding input and
				   coefficient values */
#endif
	...
	if (srcBLen >= 4U) {
		...
		while (blkCnt > 0U) {
			...
			/* If the srcBLen is not a multiple of 3, compute any
			 *remaining MACs here.
	 ...
	 * calcualted using correlation but with the shorter signal time
	 * shifted. */
	...
	for (i = 0U; i <= tot; i++) {
		...
		/* Loop to perform MAC operations according to correlation
		 * equation */
		for (j = 0U; j <= i; j++) {
