		       ...
		       float32_t *pDst)
{
	...
	float32_t x0, x1, x2, x3, c0; /* temporary variables for holding input
	 ...
	 * we assume zeroes in the output instead of zero padding either of the
	 * the inputs*/
	/* If srcALen > srcBLen,
	...
	if (srcBLen >= 4U) {
		...
		while (blkCnt > 0U) {
			...
			accum =
			    vpadd_f32(vget_low_f32(res), vget_high_f32(res));
			sum += accum[0] + accum[1];
#else
			/* First part of the processing with loop unrolling.
			 *Compute 4 MACs at a time.
	 ...
	 * two inputs of same length. But to improve the performance, we assume
