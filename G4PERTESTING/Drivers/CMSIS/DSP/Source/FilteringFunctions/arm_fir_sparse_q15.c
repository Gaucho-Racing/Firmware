			...
			uint32_t blockSize)
{
	 ...
	 * buffer */
	arm_circularWrite_q15(py, (int32_t)delaySize, &S->stateIndex, 1, pSrc,
			      1, blockSize);
	...
	while (tapCnt > 0U) {
		...
		/* blockSize samples are read from the state buffer */
		arm_circularRead_q15(py, (int32_t)delaySize, &readIndex, 1, pb,
	...
	while (blkCnt > 0U) {
		...
		write_q15x2_ia(&pOut,
			       __PKHBT((q15_t)__SSAT(in1 >> 15, 16),
				       (q15_t)__SSAT(in2 >> 15, 16), 16));
#else
		write_q15x2_ia(&pOut,
			       __PKHBT((q15_t)__SSAT(in2 >> 15, 16),
