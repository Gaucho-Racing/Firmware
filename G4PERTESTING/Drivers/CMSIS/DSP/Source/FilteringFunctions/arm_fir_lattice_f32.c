			 ...
			 uint32_t blockSize)
{
	...
	float32_t fcurr1, fnext1, gnext1; /* Temporary variables for second
					     sample in loop unrolling */
	float32_t fcurr2, fnext2,
	    gnext2; /* Temporary variables for third sample in loop unrolling */
	float32_t fcurr3, fnext3, gnext3; /* Temporary variables for fourth
					     sample in loop unrolling */
#endif
		...
		*px++ = fcurr3;
		...
		while (stageCnt > 0U) {
			...
			*px++ = gnext3;
			...
			/* Update of f values for next coefficient set
			 * processing */
			fcurr0 = fnext0;
			fcurr1 = fnext1;
			fcurr2 = fnext2;
		...
		/* The results in the 4 accumulators, store in the destination
		 * buffer. */
		/* y(n) = fN(n) */
		*pDst++ = fcurr0;
		*pDst++ = fcurr1;
