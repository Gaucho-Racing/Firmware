				     ...
				     float32_t *pState, uint32_t blockSize)
{
	...
	/* The size of the input block must be a multiple of the decimation
	 * factor */
	if ((blockSize % M) != 0U) {
		...
		/* Clear the state buffer. The size is always (blockSize +
		 * numTaps - 1) */
		memset(pState, 0,
		       (numTaps + (blockSize - 1U)) * sizeof(float32_t));
