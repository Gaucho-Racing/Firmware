		 ...
		 uint32_t blockSize)
{
	 ...
	 * The variables acc0 ... acc3 hold output values that are being
	 * computed:
	 *
	 *    acc0 =  b[numTaps-1] * x[n-numTaps-1] + b[numTaps-2] *
	 * x[n-numTaps-2] + b[numTaps-3] * x[n-numTaps-3] +...+ b[0] * x[0] acc1
	 * =  b[numTaps-1] * x[n-numTaps]   + b[numTaps-2] * x[n-numTaps-1] +
	 * b[numTaps-3] * x[n-numTaps-2] +...+ b[0] * x[1] acc2 =  b[numTaps-1]
	 * * x[n-numTaps+1] + b[numTaps-2] * x[n-numTaps]   + b[numTaps-3] *
	 * x[n-numTaps-1] +...+ b[0] * x[2] acc3 =  b[numTaps-1] *
	 * x[n-numTaps+2] + b[numTaps-2] * x[n-numTaps+1] + b[numTaps-3] *
	 * x[n-numTaps]   +...+ b[0] * x[3]
	 */
	blkCnt = blockSize >> 2U;
	...
	while (blkCnt > 0U) {
		...
		*pStateCurnt++ = *pSrc++;
		...
		/* Typecast q15_t pointer to q31_t pointer for state reading in
		 * q31_t */
		px = pState;

		/* Typecast q15_t pointer to q31_t pointer for coefficient
		 * reading in q31_t */
		pb = pCoeffs;

		/* Read the first two samples from the state buffer:  x[n-N],
		 * x[n-N-1] */
		x0 = read_q15x2_ia(&px);

		/* Read the third and forth samples from the state buffer:
		 * x[n-N-2], x[n-N-3] */
		x2 = read_q15x2_ia(&px);
		...
		while (tapCnt > 0U) {
			/* Read the first two coefficients using SIMD:  b[N] and
			 * b[N-1] coefficients */
		...
		/* If the filter length is not a multiple of 4, compute the
		   remaining filter taps. This is always be 2 taps since the
		...
		/* The results in the 4 accumulators are in 2.30 format. Convert
		   to 1.15 with saturation. Then store the 4 outputs in the
		   destination buffer. */
#ifndef ARM_MATH_BIG_ENDIAN
		write_q15x2_ia(&pDst, __PKHBT(__SSAT((acc0 >> 15), 16),
					      __SSAT((acc1 >> 15), 16), 16));
		write_q15x2_ia(&pDst, __PKHBT(__SSAT((acc2 >> 15), 16),
					      __SSAT((acc3 >> 15), 16), 16));
#else
		write_q15x2_ia(&pDst, __PKHBT(__SSAT((acc1 >> 15), 16),
					      __SSAT((acc0 >> 15), 16), 16));
		write_q15x2_ia(&pDst, __PKHBT(__SSAT((acc3 >> 15), 16),
					      __SSAT((acc2 >> 15), 16), 16));
#endif /* #ifndef ARM_MATH_BIG_ENDIAN */

		/* Advance the state pointer by 4 to process the next group of 4
		 * samples */
		pState = pState + 4U;
	...
	while (blkCnt > 0U) {
		...
		*pStateCurnt++ = *pSrc++;
		...
		/* The result is in 2.30 format. Convert to 1.15 with
		   saturation. Then store the output in the destination buffer.
		 */
	...
	/* Processing is complete.
	   ...
	   buffer. This prepares the state buffer for the next function call. */
