		 ...
		 uint32_t srcBLen, q7_t *pDst)
{
	...
	if (srcBLen >= 4U) {
		...
		while (blkCnt > 0U) {
			...
			do {
				...
				/* y[srcBLen - 1]   and y[srcBLen - 2] are
				 * packed */
				in1 = (q15_t)c0;
				in2 = (q15_t)c1;
	...
	while (blockSize3 > 0U) {
		...
		while (k > 0U) {
			/* Reading two inputs, x[srcALen - srcBLen + 1] and
			 * x[srcALen - srcBLen + 2] of SrcA buffer and packing
			 */
			in1 = (q15_t)*px++;
			in2 = (q15_t)*px++;
			input1 =
			...
			/* Reading two inputs, y[srcBLen - 1] and y[srcBLen - 2]
			 * of SrcB buffer and packing */
			in1 = (q15_t)*py--;
			in2 = (q15_t)*py--;
			input2 =
			    ((q31_t)in1 & 0x0000FFFF) | ((q31_t)in2 << 16U);
			...
			/* Reading two inputs, x[srcALen - srcBLen + 3] and
			 * x[srcALen - srcBLen + 4] of SrcA buffer and packing
			 */
			in1 = (q15_t)*px++;
			in2 = (q15_t)*px++;
			input1 =
			...
			/* Reading two inputs, y[srcBLen - 3] and y[srcBLen - 4]
			 * of SrcB buffer and packing */
			in1 = (q15_t)*py--;
			in2 = (q15_t)*py--;
			input2 =
