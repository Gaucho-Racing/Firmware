				...
				uint32_t blockSize)
{
	...
	while (stage > 0U) {
		...
		/* Apply loop unrolling and compute 4 output values
		 * simultaneously. */
		 ...
		 * a1 * y[n-1] + a2 * y[n-2] acc =  b0 * x[n] + b1 * x[n-1] + b2
		 * * x[n-2] + a1 * y[n-1] + a2 * y[n-2] acc =  b0 * x[n] + b1 *
		 * x[n-1] + b2 * x[n-2] + a1 * y[n-1] + a2 * y[n-2]
		 */
		...
		*/
		...
		/* If the block size is not a multiple of 4, compute any
		...
		vst1q_f32(pState, vcombine_f32(vrev64_f32(vget_high_f32(Xn)),
					       vrev64_f32(Yn)));
		pState += 4;
				...
				uint32_t blockSize)
{
	...
	do {
		 ...
		 * * y[n-1] + a2 * y[n-2] acc =  b0 * x[n] + b1 * x[n-1] + b2 *
		 * x[n-2] + a1 * y[n-1] + a2 * y[n-2] acc =  b0 * x[n] + b1 *
		 * x[n-1] + b2 * x[n-2] + a1 * y[n-1] + a2 * y[n-2]
		 */
		...
		while (sample > 0U) {
			...
			/* acc =  b0 * x[n] + b1 * x[n-1] + b2 * x[n-2] + a1 *
			 * y[n-1] + a2 * y[n-2] */
			Yn2 = (b0 * Xn) + (b1 * Xn1) + (b2 * Xn2) + (a1 * Yn1) +
			      (a2 * Yn2);
			...
			*pOut++ = Yn2;
			...
			/* acc =  b0 * x[n] + b1 * x[n-1] + b2 * x[n-2] + a1 *
			 * y[n-1] + a2 * y[n-2] */
			Yn1 = (b0 * Xn2) + (b1 * Xn) + (b2 * Xn1) + (a1 * Yn2) +
			      (a2 * Yn1);
			...
			*pOut++ = Yn1;
			...
			/* acc =  b0 * x[n] + b1 * x[n-1] + b2 * x[n-2] + a1 *
			 * y[n-1] + a2 * y[n-2] */
			Yn2 = (b0 * Xn1) + (b1 * Xn2) + (b2 * Xn) + (a1 * Yn1) +
			      (a2 * Yn2);
			...
			*pOut++ = Yn2;
			...
			/* acc =  b0 * x[n] + b1 * x[n-1] + b2 * x[n-2] + a1 *
			 * y[n-1] + a2 * y[n-2] */
			Yn1 = (b0 * Xn) + (b1 * Xn1) + (b2 * Xn2) + (a1 * Yn2) +
			      (a2 * Yn1);
		...
		while (sample > 0U) {
			...
			/* acc =  b0 * x[n] + b1 * x[n-1] + b2 * x[n-2] + a1 *
			 * y[n-1] + a2 * y[n-2] */
			acc = (b0 * Xn) + (b1 * Xn1) + (b2 * Xn2) + (a1 * Yn1) +
			      (a2 * Yn2);
