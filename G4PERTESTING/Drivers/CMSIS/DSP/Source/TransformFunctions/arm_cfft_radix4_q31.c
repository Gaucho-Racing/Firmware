...
void arm_cfft_radix4_q31(const arm_cfft_radix4_instance_q31 *S, q31_t *pSrc)
{
	...
	if (S->bitReverseFlag == 1U) {
		/*  Bit Reversal */
		arm_bitreversal_q31(pSrc, S->fftLen, S->bitRevFactor,
				    S->pBitRevTable);
	}
}
			      ...
			      uint32_t twidCoefModifier)
{
	...
	do {
		...
		/* input is in 1.31(q31) format and provide 4 guard bits for the
		 * input */
		...
		/* ya + yc */
		s1 =
		    (pSrc[(2U * i0) + 1U] >> 4U) + (pSrc[(2U * i2) + 1U] >> 4U);
		/* ya - yc */
		s2 =
		    (pSrc[(2U * i0) + 1U] >> 4U) - (pSrc[(2U * i2) + 1U] >> 4U);
		...
		/* yb + yd */
		t2 =
		    (pSrc[(2U * i1) + 1U] >> 4U) + (pSrc[(2U * i3) + 1U] >> 4U);
	...
	for (k = fftLen / 4U; k > 4U; k >>= 2U) {
		...
		for (j = 0U; j <= (n2 - 1U); j++) {
			...
			for (i0 = j; i0 < fftLen; i0 += n1) {
				/*  index calculation for the input as, */
				/*  pSrc[i0 + 0], pSrc[i0 + fftLen/4], pSrc[i0 +
				 * fftLen/2U], pSrc[i0 + 3fftLen/4] */
				i1 = i0 + n2;
				i2 = i1 + n2;
				i3 = i2 + n2;
	 ...
	 * stages */
	/* data is in 7.25(q25) format for the 64 point as there are 1 middle
	 * stage */
