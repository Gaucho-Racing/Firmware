...
void arm_cfft_radix4_q15(const arm_cfft_radix4_instance_q15 *S, q15_t *pSrc)
{
	...
	if (S->bitReverseFlag == 1U) {
		/*  Bit Reversal */
		arm_bitreversal_q15(pSrc, S->fftLen, S->bitRevFactor,
				    S->pBitRevTable);
	}
}
			      ...
			      const q15_t *pCoef16, uint32_t twidCoefModifier)
{
	...
	do {
		...
		T = read_q15x2(pSi0);
		T = __SHADD16(
		    T, 0); /* this is just a SIMD arithmetic shift right by 1 */
		T = __SHADD16(T,
			      0); /* it turns out doing this twice is 2 cycles,
		...
		write_q15x2_ia(&pSi1, (q31_t)((out2) & 0xFFFF0000) |
					  (out1 & 0x0000FFFF));
	...
	for (k = fftLen / 4U; k > 4U; k >>= 2U) {
		...
		for (j = 0U; j <= (n2 - 1U); j++) {
			...
			for (i0 = j; i0 < fftLen; i0 += n1) {
				 ...
				 * fftLen/4 sample */
				/* xc' = (xa-xb+xc-xd)* co2 + (ya-yb+yc-yd)*
				 * (si2) */
				/* yc' = (ya-yb+yc-yd)* co2 - (xa-xb+xc-xd)*
				 * (si2) */
				write_q15x2(pSi1, ((out2) & 0xFFFF0000) |
						      (out1 & 0x0000FFFF));
				pSi1 += 2 * n1;
...
#ifndef ARM_MATH_BIG_ENDIAN
				/* R = packed((ya-yc) + (xb- xd) , (xa-xc) -
				 * (yb-yd)) */
				R = __SHASX(S, T);

				/* S = packed((ya-yc) - (xb- xd),  (xa-xc) +
				 * (yb-yd)) */
				S = __SHSAX(S, T);
				...
				/* xb' = (xa+yb-xc-yd)* co1 + (ya-xb-yc+xd)*
				 * (si1) */
				/* yb' = (ya-xb-yc+xd)* co1 - (xa+yb-xc-yd)*
				 * (si1) */
				write_q15x2(pSi2, ((out2) & 0xFFFF0000) |
						      (out1 & 0x0000FFFF));
				pSi2 += 2 * n1;

				/*  Butterfly process for the i0+3fftLen/4
				 * sample */
				...
				/* xd' = (xa-yb-xc+yd)* co3 + (ya+xb-yc-xd)*
				 * (si3) */
				/* yd' = (ya+xb-yc-xd)* co3 - (xa-yb-xc+yd)*
				 * (si3) */
				write_q15x2(pSi3, ((out2) & 0xFFFF0000) |
						      (out1 & 0x0000FFFF));
				pSi3 += 2 * n1;
			}
		}
	...
	for (k = fftLen / 4U; k > 4U; k >>= 2U) {
		...
		for (j = 0U; j <= (n2 - 1U); j++) {
			...
			for (i0 = j; i0 < fftLen; i0 += n1) {
				...
				/* R0 = (ya-yc) + (xb- xd), R1 = (xa-xc) -
				 * (yb-yd)) */
				R0 = (S0 >> 1U) - (T1 >> 1U);
				R1 = (S1 >> 1U) + (T0 >> 1U);

				/* S0 = (ya-yc) - (xb- xd), S1 = (xa-xc) +
				 * (yb-yd)) */
				S0 = (S0 >> 1U) + (T1 >> 1U);
				S1 = (S1 >> 1U) - (T0 >> 1U);
