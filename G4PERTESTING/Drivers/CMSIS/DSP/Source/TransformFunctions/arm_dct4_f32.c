		  ...
		  float32_t *pInlineBuffer)
{
	 ...
	 *                  Step1: Re-ordering of even and odd elements of
	 * input. Step2: Calculating FFT of the re-ordered input. Step3: Taking
	 * the real part of the product of FFT output and weights. (c)
	 * Post-processing - DCT4 can be obtained from DCT2 output using the
	 * following equation: Y4(k) = Y2(k) - Y4(k-1) and Y4(-1) = Y4(0) where,
	...
	/* pS2 initialized to pState+N-1, so that it points to the end of the
	 * state buffer */
	pS2 = pState + (S->N - 1U);
	 ...
	 * ---------------------------------------------------------- */
	/* pInlineBuffer is real input of length N , pState is the complex
	 * output of length 2N */
	...
	/* input buffer acts as inplace, so output values are stored in the
	 * input itself. */
	*pbuff++ = in;
	 ...
	 ** a second loop below computes the remaining 1 to 3 samples. */
	do {
		/* Calculating Y4(1) to Y4(N-1) from Y2 using equation Y4(k) =
		 * Y2(k) - Y4(k-1) */
		/* pState pointer (pS1) is incremented twice as the real values
		 * are located alternatively in the array */
		in = *pS1++ - in;
		*pbuff++ = in;
	...
	/*------------ Normalizing the output by multiplying with the
	 * normalizing factor ----------*/
	...
	/* pbuff initialized to the pInlineBuffer(now contains the output
	 * values) */
	pbuff = pInlineBuffer;
	 ...
	 * Compute 4 outputs at a time */
	do {
		/* Multiplying pInlineBuffer with the normalizing factor
		 * sqrt(2/N) */
		in = *pbuff;
