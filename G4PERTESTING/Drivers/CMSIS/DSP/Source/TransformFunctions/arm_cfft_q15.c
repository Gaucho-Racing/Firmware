		  ...
		  uint8_t bitReverseFlag)
{
	...
	if (ifftFlag == 1U) {
		switch (L) {
			...
			case 512:
			case 2048:
				arm_cfft_radix4by2_inverse_q15(p1, L,
							       S->pTwiddle);
				break;
		}
	} else {
	...
	if (bitReverseFlag) {
		arm_bitreversal_16((uint16_t *)p1, S->bitRevLength,
				   S->pBitRevTable);
	}
}
...
void arm_cfft_radix4by2_q15(q15_t *pSrc, uint32_t fftLen, const q15_t *pCoef)
{
	...
	for (i = n2; i > 0; i--) {
		...
		S = read_q15x2(pSl);
		S = __SHADD16(
		    S, 0); /* this is just a SIMD arithmetic shift right by 1 */
		...
		write_q15x2_ia(&pSl, (q31_t)((out2) & 0xFFFF0000) |
					 (out1 & 0x0000FFFF));
	}
