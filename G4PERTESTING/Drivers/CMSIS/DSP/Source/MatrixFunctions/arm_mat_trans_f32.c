			     ...
			     arm_matrix_instance_f32 *pDst)
{
...

	{
		...
		while (rowCnt > 0U) {
			...
			/* The pointer px is set to starting address of the
			...
			while (blkCnt > 0U) {
				/* Read and store the input element in the
				 * destination */
				*px++ = *pIn;
				*px++ = *(pIn + 1 * nColumns);
				*px++ = *(pIn + 2 * nColumns);
		...
		while (rowCnt > 0U) {
			...
			while (blkCnt > 0U) {
				...
				*px = *pIn++;
				...
				/* Update the pointer px to point to the next
				 * row of the transposed matrix */
				px += nRows;
			     ...
			     arm_matrix_instance_f32 *pDst)
{
...

	{
		...
		/* row loop */
		do {
			/* Pointer px is set to starting address of column being
			 * processed */
			px = pOut + i;
