...

	{
...
#if defined(ARM_MATH_DSP)
		/* Process two rows from matrix A at a time and output two rows
		 * at a time */
		row = row >> 1U;
		px2 = px + numColsB;
#endif
		...
		while (row > 0U) {
...
#if defined(ARM_MATH_DSP)
			/* Process two (transposed) columns from matrix B at a
			 * time */
			col = col >> 1U;
			j = 0;
#endif
			...
			while (col > 0U) {
				...
				/* Read in two elements at once - alows dual MAC
				 * instruction */
				colCnt = numColsA >> 1U;
#else
				colCnt = numColsA >> 2U;
