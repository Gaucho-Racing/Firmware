...

	{
		...
		while (row > 0U) {
			...
			while (col > 0U) {
...
#else
					sum1 = (q31_t)((((q63_t)sum1 << 32) +
							((q63_t)inA1 * inB1)) >>
						       32);
					sum2 = (q31_t)((((q63_t)sum2 << 32) +
							((q63_t)inA1 * inB2)) >>
						       32);
					sum3 = (q31_t)((((q63_t)sum3 << 32) +
							((q63_t)inA2 * inB1)) >>
						       32);
					sum4 = (q31_t)((((q63_t)sum4 << 32) +
							((q63_t)inA2 * inB2)) >>
						       32);
#endif
				...
				/* Convert the result from 2.30 to 1.31 format
				 * and store in destination buffer */
				*px++ = sum1 << 1;
				*px++ = sum2 << 1;
				*px2++ = sum3 << 1;
				*px2++ = sum4 << 1;
		...
		if (numColsB & 1U) {
			...
			while (row > 0) {
				...
				/* Set variable sum1, that acts as accumulator,
				 * to zero */
				sum1 = 0;
		...
		if (numRowsA & 1U) {
			...
			while (col > 0) {
				...
				/* Saturate and store the result in the
				 * destination buffer */
				*px++ = sum1 << 1;
				i++;
