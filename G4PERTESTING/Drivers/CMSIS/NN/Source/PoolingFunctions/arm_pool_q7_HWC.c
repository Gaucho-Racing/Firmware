			...
			const uint16_t dim_im_out, q7_t *bufferA, q7_t *Im_out)
{
	...
	for (i_y = 0; i_y < dim_im_in; i_y++) {
		...
		for (i_x = 0; i_x < dim_im_out; i_x++) {
			...
			q7_t *target =
			    Im_in + (i_y * dim_im_in + i_x) * ch_im_in;
			q7_t *win_start;
			q7_t *win_stop;
			...
			if (i_x * stride - padding + dim_kernel >= dim_im_in) {
				win_stop =
				    Im_in +
				    (i_y * dim_im_in + dim_im_in) * ch_im_in;
			} else {
				win_stop =
				    Im_in + (i_y * dim_im_in + i_x * stride -
					     padding + dim_kernel) *
						ch_im_in;
			}
	...
	for (i_y = 0; i_y < dim_im_out; i_y++) {
		...
		if (i_y * stride - padding < 0) {
			...
			row_start = Im_in + (i_y * stride - padding) *
						dim_im_in * ch_im_in;
		}
		...
		if (i_y * stride - padding + dim_kernel >= dim_im_in) {
			...
			row_end =
			    Im_in + (i_y * stride - padding + dim_kernel) *
					dim_im_in * ch_im_in;
		}
			...
			const uint16_t dim_im_out, q7_t *bufferA, q7_t *Im_out)
{
	...
	for (i_y = 0; i_y < dim_im_in; i_y++) {
		...
		for (i_x = 0; i_x < dim_im_out; i_x++) {
			...
			win_start += ch_im_in;
			for (; win_start < win_stop; win_start += ch_im_in) {
				accumulate_q7_to_q15(buffer, win_start,
						     ch_im_in);
				count++;
			}
			buffer_scale_back_q15_to_q7(buffer, target, ch_im_in,
						    count);
		}
	}
	...
	for (i_y = 0; i_y < dim_im_out; i_y++) {
		...
		/* copy over the first row */
		arm_q7_to_q15_no_shift(row_start, buffer,
				       dim_im_out * ch_im_in);
		count = 1;
		...
		for (; row_start < row_end; row_start += dim_im_in * ch_im_in) {
			accumulate_q7_to_q15(buffer, row_start,
					     dim_im_out * ch_im_in);
			count++;
		}
