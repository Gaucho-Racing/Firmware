    ...
    const uint16_t dim_im_out, q15_t *bufferA, q7_t *bufferB)
{
	 ...
	 * padding situation Top: i_out_y from 0 to padding - 1 Middle: i_out_y
	 * from padding to dim_im_out-padding-1 Bottom: i_out_y from
	 * dim_im_out-padding to dim_im_out-1
	 */
	...
	for (i_out_y = 0; i_out_y < padding; i_out_y++) {
		for (i_out_x = 0; i_out_x < dim_im_out; i_out_x++) {
			...
			if (pBuffer ==
			    bufferA + 2 * ch_im_in * dim_kernel * dim_kernel) {
	...
	if (pBuffer != bufferA) {
		...
		for (i = 0; i < ch_im_out; i++) {
			...
			uint16_t colCnt =
			    ch_im_in * dim_kernel * dim_kernel >> 2;
			...
			while (colCnt) {
				...
				pA = (q7_t *)read_and_pad_reordered(
				    (void *)pA, &inA1, &inA2);
