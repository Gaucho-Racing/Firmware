		...
		for (i_out_x = 0; i_out_x < dim_im_out; i_out_x++) {
			for (i_ker_y = i_out_y * stride - padding;
				...
				for (i_ker_x = i_out_x * stride - padding;
				     i_ker_x <
				     i_out_x * stride - padding + dim_kernel;
				     i_ker_x++) {
					if (i_ker_y < 0 ||
					    i_ker_y >= dim_im_in ||
					    i_ker_x < 0 ||
					    i_ker_x >= dim_im_in) {
						/* Equivalent to arm_fill_q15(0,
						 * pBuffer, ch_im_in) with
						...
						*(pBuffer + 2) = 0;
						 ...
						 * (q7_t*)Im_in+(i_ker_y*dim_im_in+i_ker_x)*3,
						 * pBuffer, 3);
						 */
						...
						bottom.word =
						    __SXTB16(__ROR(buf, 8));

#ifndef ARM_MATH_BIG_ENDIAN
						/*
						 *  little-endian, | omit | 3rd
						 * | 2nd  | 1st  | MSB LSB top |
						 * 3rd | 1st |; bottom | omit |
						 * 2nd |
						 *
						 *  version 1, need to swap 2nd
						 * and 3rd weight
						 * *__SIMD32(pBuffer) =
						 * top.word;
						 * *(pBuffer+2) =
						...
						*__SIMD32(pBuffer) = __PKHBT(
						    bottom.word, top.word, 0);
#else
						/*
						 *  big-endian,    | 1st  | 2nd
						 * | 3rd  | omit | MSB LSB top |
						 * 2nd | omit |; bottom | 1st |
						 * 3rd |
						 *
						 *  version 1, need to swap 2nd
						...
						*pBuffer++ =
						    bottom.half_words[0];
						*__SIMD32(pBuffer) = __PKHTB(
						    top.word, bottom.word, 0);
#endif
						pBuffer += 2;
					}
				}
			...
			if (pBuffer ==
			    bufferA + 2 * 3 * dim_kernel * dim_kernel) {
				pOut = arm_nn_mat_mult_kernel_q7_q15(
				    wt, bufferA, ch_im_out,
				    3 * dim_kernel * dim_kernel, bias_shift,
				    out_shift, bias, pOut);
	...
	if (pBuffer != bufferA) {
		...
		for (i = 0; i < ch_im_out; i++) {
			...
			while (colCnt) {
				...
				pA = (q7_t *)read_and_pad((void *)pA, &inA1,
							  &inA2);
