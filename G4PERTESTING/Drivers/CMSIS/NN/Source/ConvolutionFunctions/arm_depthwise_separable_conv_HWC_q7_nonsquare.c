    ...
    const uint16_t dim_im_out_y, q15_t *bufferA, q7_t *bufferB)
{
	 ...
	 * accumulator Mid   loop: loop over different output channel Outer
	 * loop: loop over different output (x, y)
	 *
	 */
	...
	for (i_out_y = 0; i_out_y < dim_im_out_y; i_out_y++) {
		for (i_out_x = 0; i_out_x < dim_im_out_x; i_out_x++) {
			...
			for (i_ker_y = i_out_y * stride_y - padding_y;
			     i_ker_y <
			     i_out_y * stride_y - padding_y + dim_kernel_y;
			     i_ker_y++) {
				for (i_ker_x = i_out_x * stride_x - padding_x;
					...
					if (i_ker_y < 0 ||
					    ...
					    i_ker_x < 0 ||
					    i_ker_x >= dim_im_in_x) {
						/* arm_fill_q7(0, pBuffer,
						 * ch_im_in); */
						memset(pBuffer, 0, ch_im_in);
					} else {
						/* arm_copy_q7((q7_t *) Im_in +
						 * (i_ker_y * dim_im_in_x +
						 * i_ker_x) * ch_im_in, pBuffer,
						 * ch_im_in); */
						memcpy(
						    pBuffer,
						    (q7_t *)Im_in +
			...
			while (rowCnt) {
				...
				asm volatile(
				    ...
				    : [sum] "+r"(sum), [sum2] "+r"(sum2),
				      [sum3] "+r"(sum3), [sum4] "+r"(sum4),
				      [pB] "+r"(pB), [pA] "+r"(pA)
				    : [colCnt] "r"(colCnt), [ch_im_in] "r"(
								ch_im_in)
				    : "r0", "r1", "r2", "r3", "r4", "r5");
#else
				//  r0    r1    r2    r3    r4   r5
	...
	for (i_out_y = 0; i_out_y < dim_im_out_y; i_out_y++) {
		for (i_out_x = 0; i_out_x < dim_im_out_x; i_out_x++) {
			for (i_ch_out = 0; i_ch_out < ch_im_out; i_ch_out++) {
				// for each output
				int conv_out =
				    ((q31_t)(bias[i_ch_out]) << bias_shift) +
				    NN_ROUND(out_shift);
				for (i_ker_y = 0; i_ker_y < dim_kernel_y;
				     i_ker_y++) {
					for (i_ker_x = 0;
					     i_ker_x < dim_kernel_x;
					     i_ker_x++) {
						int in_row =
						    stride_y * i_out_y +
						    i_ker_y - padding_y;
				...
				Im_out[(i_out_y * dim_im_out_x + i_out_x) *
					   ch_im_out +
				       i_ch_out] =
