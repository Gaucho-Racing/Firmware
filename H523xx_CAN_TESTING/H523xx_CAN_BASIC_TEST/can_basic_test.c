...
CAN_STATUS defaultSTM32H523xx_CANCfg(FDCAN_GlobalTypeDef *instance, CAN_RXCallback callback, CANConfig *out_cfg, uint32_t Mode, uint32_t numStdFilters, uint32_t numExtFilters)
{
	CANConfig canCfg;
	...
	canCfg.rx_interrupt_priority = 14;
	canCfg.tx_interrupt_priority = 14;
...
int can_basic_test(void)
{
	...
	if (defaultSTM32H523xx_CANCfg(FDCAN1, can_test_rx_callback1, &cfg1, LOOPBACK_MODE, 0, 0)) {
...
#ifdef DATA_CAN
	if (defaultSTM32H523xx_CANCfg(FDCAN2, can_test_rx_callback2, &cfg2, LOOPBACK_MODE, 0, 0)) {
